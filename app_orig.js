...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Repères visuels, fiches pratiques et carnet technique, disponibles hors ligne.</div></div><span class="badge autosave">v14.6.21.80</span></div>
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.6.21.80</span></div>
...
function renderVisits() {
  ...
  app.innerHTML = `
    <div class="section-title"><div><h2>Visites</h2><div class="muted">Toutes les visites enregistrées restent accessibles, y compris les plus anciennes.</div></div><span class="badge autosave">v14.6.21.80</span></div>
...
function renderFarmerQuestionnaires(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📨 Questionnaires éleveur</h2><div class="muted">Préparer la visite puis récupérer uniquement les réponses encore manquantes.</div></div><span class="badge autosave">v14.6.21.80</span></div>${activeVisitBanner(visit)}
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.6.21.80</span></div>
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.6.21.80</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
  visit.feeding=visit.feeding&&typeof visit.feeding==='object'?visit.feeding:{rations:[],settings:{},history:[]};
  visit.feeding.nutrition=visit.feeding.nutrition&&typeof visit.feeding.nutrition==='object'?visit.feeding.nutrition:{};
...
function renderFollowup(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Historique & évolution</h2><div class="muted">Comparer deux visites d’une même exploitation : reproduction, mortalité, mesures, alimentation, bâtiment et plans d’action.</div></div><span class="badge autosave">v14.6.21.80</span></div>
...
function annualHistoryTablesHtml(rows,{forPrint=false}={}){
  ...
  const tr=(label,fn)=>`<tr><th>${label}</th>${rows.map(r=>`<td>${fn(r)}</td>`).join('')}</tr>`;
  return `<div class="annual-history-block ${forPrint?'print-annual-history':''}"><h4>Évolution sur les 10 dernières années</h4><p class="muted small-text">Années civiles. Pour l’année en cours, données arrêtées à la date de visite. Mortalité : veaux nés dans l’année et morts avant 6 mois ; la dernière année peut être incomplète pour les veaux n’ayant pas encore 6 mois de recul.</p><h5>Reproduction</h5><div class="table-wrap"><table class="annual-history-table"><thead><tr><th>Indicateur</th>${years.map(y=>`<th>${y}</th>`).join('')}</tr></thead><tbody>${tr('Vaches ayant vêlé',r=>cell(r.cowsCalved))}${tr('IVV calculables',r=>cell(r.ivvCount))}${tr('IVV moyen',r=>cell(r.ivvMean,' j'))}${tr('IVV ≤400 j',r=>cell(r.ivvLe400))}${tr('IVV 401–450 j',r=>cell(r.ivv401to450))}${tr('IVV 451–500 j',r=>cell(r.ivv451to500))}${tr('IVV >500 j',r=>cell(r.ivvOver500))}${tr('Âge moyen 1er vêlage',r=>cell(r.firstCalvingMean,' m'))}</tbody></table></div><h5>Mortalité veaux</h5><div class="table-wrap"><table class="annual-history-table"><thead><tr><th>Indicateur</th>${years.map(y=>`<th>${y}</th>`).join('')}</tr></thead><tbody>${tr('Veaux nés / suivis',r=>cell(r.births))}${tr('Morts <6 mois',r=>cell(r.dead6))}${tr('Mortalité <6 mois',r=>cell(r.mortalityRate,' %'))}</tbody></table></div></div>`;
}
...
// v14.6.21.80 — estimation économique pré-visite, calculée uniquement sur les données déjà disponibles.
...
// v14.6.21.80 — check-up, suivi d'étude, économie et conservation globale du défilement horizontal
...
// v14.6.21.80 — suivi du temps par intervenant + temps GDS préparation/terrain/compte rendu/suivi

// v14.6.21.80 — questionnaires persistants, repères de saisie, bilan sanitaire/éco partenaire, supports papier, check-up responsive.

window.addEventListener('DOMContentLoaded',()=>{initGlobalSearch();initAccordionMemory();});
// v14.6.21.80 — cohérence, imprimables préparation, animaux présents, recherche globale.
