...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Quelques repères visuels utiles, intégrés à l’application et disponibles hors ligne.</div></div><span class="badge autosave">v14.5.6</span></div>
...
window.addEventListener('audit-bovin-cloud-merged',event=>{
  ...
  if(!editing && ['dashboard','visits'].includes(currentView)) render();
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.5.6</span></div>
...
function renderVisits() {
  ...
  const farmFilter=localStorage.getItem('audit-bovin-visits-farm-filter')||'';
  const yearFilter=localStorage.getItem('audit-bovin-visits-year-filter')||'';
  const searchFilter=localStorage.getItem('audit-bovin-visits-search')||'';
  const allYears=[...new Set(db.visits.map(v=>String(v.date||'').slice(0,4)).filter(Boolean))].sort().reverse();
  const filteredVisits=db.visits.slice().sort((a,b)=>(b.date||'').localeCompare(a.date||'')).filter(v=>{
    if(farmFilter&&v.farmId!==farmFilter)return false;
    if(yearFilter&&!String(v.date||'').startsWith(yearFilter))return false;
    const q=normalizeSearchText(searchFilter);if(q&&!normalizeSearchText([farmName(v.farmId),v.date,v.type,v.technician,v.status].join(' ')).includes(q))return false;
    return true;
  });
  app.innerHTML = `
    <div class="section-title"><div><h2>Visites</h2><div class="muted">Historique complet : ${db.visits.length} visite(s) enregistrée(s), sans limitation.</div></div><span class="badge autosave">Sauvegarde automatique active</span></div>
    <section class="grid cols-2">
      ${visitFormHtml(editVisit || {})}
      <section class="card visits-history-card">
        <div class="section-title"><div><h3>Historique des visites</h3><div class="muted">${filteredVisits.length} visite(s) affichée(s) sur ${db.visits.length}</div></div><button class="btn small secondary" id="reset-visits-filters">Réinitialiser</button></div>
        <div class="grid cols-3 visits-history-filters"><div class="field"><label>Exploitation</label><select id="visits-farm-filter"><option value="">Toutes</option>${db.farms.map(f=>`<option value="${f.id}" ${f.id===farmFilter?'selected':''}>${escapeHtml(f.name)}</option>`).join('')}</select></div><div class="field"><label>Année</label><select id="visits-year-filter"><option value="">Toutes</option>${allYears.map(y=>`<option ${y===yearFilter?'selected':''}>${y}</option>`).join('')}</select></div><div class="field"><label>Recherche</label><input id="visits-search" value="${escapeHtml(searchFilter)}" placeholder="Type, technicien, exploitation…"></div></div>
        ${filteredVisits.length ? `<div class="table-wrap visits-history-table"><table><thead><tr><th>Exploitation</th><th>Date</th><th>Type</th><th>Sujets</th><th>Statut</th><th>Actions</th></tr></thead><tbody>${filteredVisits.map(v => `<tr><td><strong>${escapeHtml(farmName(v.farmId))}</strong><br><span class="muted">${escapeHtml(v.technician || '')}</span></td><td>${formatDate(v.date)}</td><td>${escapeHtml(v.type || '—')}</td><td>${v.subjects?.length || 0}</td><td><span class="badge ${v.status==='complete'?'complete':'in-progress'}">${v.status==='complete'?'Terminée':'En cours'}</span></td><td><div class="actions"><button class="btn small" data-edit-visit="${v.id}">Ouvrir</button><button class="btn small" data-open-animals="${v.id}">Animaux</button><button class="btn small" data-share-visit="${v.id}">Partager</button><button class="btn small secondary" data-export-visit="${v.id}">JSON</button><button class="btn small danger" data-delete-visit="${v.id}">Supprimer</button></div></td></tr>`).join('')}</tbody></table></div>` : '<div class="empty">Aucune visite correspondant aux filtres.</div>'}
      </section>
    </section>
  ...
  form.addEventListener('submit', event => {
    ...
    if (editVisit) {
      ...
      addJournal(visit, 'Visite créée.');db.visits.push(visit);ensurePreviousVisitReview(visit);setActiveVisit(visit.id);editingVisitId=visit.id;
  ...
  document.getElementById('visits-farm-filter')?.addEventListener('change',e=>{localStorage.setItem('audit-bovin-visits-farm-filter',e.target.value);renderVisits();});
  document.getElementById('visits-year-filter')?.addEventListener('change',e=>{localStorage.setItem('audit-bovin-visits-year-filter',e.target.value);renderVisits();});
  document.getElementById('visits-search')?.addEventListener('input',e=>{localStorage.setItem('audit-bovin-visits-search',e.target.value);clearTimeout(window.__visitSearchTimer);window.__visitSearchTimer=setTimeout(renderVisits,180);});
  document.getElementById('reset-visits-filters')?.addEventListener('click',()=>{['audit-bovin-visits-farm-filter','audit-bovin-visits-year-filter','audit-bovin-visits-search'].forEach(k=>localStorage.removeItem(k));renderVisits();});
  ...
  app.querySelectorAll('[data-open-animals]').forEach(button => button.onclick = () => {setActiveVisit(button.dataset.openAnimals);setView('animals');});
  app.querySelectorAll('[data-share-visit]').forEach(button => button.onclick = () => shareVisitFile(db.visits.find(v => v.id === button.dataset.shareVisit)));
  app.querySelectorAll('[data-export-visit]').forEach(button => button.onclick = () => {const visit=db.visits.find(v=>v.id===button.dataset.exportVisit);downloadJson(`${slugify(farmName(visit.farmId))}-${visit.date||'visite'}.json`,{schemaVersion:2,farm:db.farms.find(f=>f.id===visit.farmId),visit});});
  app.querySelectorAll('[data-delete-visit]').forEach(button => button.onclick = () => {if(confirm('Supprimer cette visite et tous ses sujets ?')){db.visits=db.visits.filter(v=>v.id!==button.dataset.deleteVisit);if(activeVisitId===button.dataset.deleteVisit)setActiveVisit('');saveDatabase(db);renderVisits();}});
...
function renderAnimals() {
  ...
  quickForm?.addEventListener('submit', event => {
    ...
    const farm = db.farms.find(f=>f.id===visit.farmId);
    const match = resolveRegistryAnimal(farm, tag);
    if (match?.ambiguous) {
      const choices = match.items.map((a,i)=>`${i+1}. ${a.id} · ${a.birthDate ? formatDate(a.birthDate) : 'date inconnue'} · race ${a.breed||'—'} · ${a.sex||'—'}`).join('\n');
      const choice = Number(prompt(`Plusieurs bovins correspondent à « ${tag} ». Choisissez le numéro :\n${choices}`, '1'));
      if (!choice || !match.items[choice-1]) return showToast('Ajout annulé : bovin non sélectionné.');
      tag = match.items[choice-1].id;
    } else if (match?.animal) tag = match.animal.id;
    ...
    const registryAnimal = resolveRegistryAnimal(farm, tag)?.animal || null;
    const repro = registryAnimal ? reproductionForCow(farm, registryAnimal.id) : null;
    const subject = {
      id: uid('subject'), tag, location: data.location.trim(), name: registryAnimal?.name || '', category: 'Non classé', stage: 'Non renseigné',
      gestationMonths: '', lactationDays: '', age: registryAnimal?.birthDate ? ageLabelAt(registryAnimal.birthDate, visit.date) : '', rank: '', lot: '', notes: '', measurements: {},
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.5.6</span></div>
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.5.6</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
...
function followupVisitTechnicalIndicators(visit){
  const imported=visit.auditGlobal?.importedHerdData?.summary||visit.importedHerdData?.summary||{};
  const farm=db.farms.find(f=>f.id===visit.farmId);
  const registry=Array.isArray(visit.reproductionRegistry)?visit.reproductionRegistry:(farm?.herdRegistry||[]);
  const rf=registry.length?{...(farm||{}),herdRegistry:registry}:null;
  const end=visit.date||new Date().toISOString().slice(0,10);const d=new Date(end+'T12:00:00');d.setFullYear(d.getFullYear()-1);const start=d.toISOString().slice(0,10);
  const rp=rf?reproductionPeriodIndicators(rf,start,end):null;
  return {
    'Effectif total':numericValue(imported.totalHerd),
    'Naissances':numericValue(imported.births),
    'Achats':numericValue(imported.purchases),
    'Sorties':numericValue(imported.totalOutputs),
    'Mortalité totale':numericValue(imported.mortalityTotal),
    'Mortalité jeunes (%)':numericValue(imported.mortalityYoungRate),
    'IVV moyen (j)':rf?(()=>{const x=currentReproductionCows(rf).map(r=>r.meanIVV).filter(v=>v!=null);return x.length?Math.round(x.reduce((a,b)=>a+b,0)/x.length):null})():null,
    'Taux de vêlage (%)':rp?.calvingRate??null,
    'Primipares (%)':rp?.primiparousRate??null,
    'IVV 1–2 (j)':rp?.ivv12Mean??null,
    'Veaux issus d’IA (%)':rp?.iaRate??null,
    'Vaches sans mise bas':rp?.withoutCalving?.length??null
  };
}
function followupTechnicalRows(visits){
  const sets=visits.map(followupVisitTechnicalIndicators),labels=[...new Set(sets.flatMap(x=>Object.keys(x)))];
  return labels.map(label=>{const vals=sets.map(x=>x[label]??null);if(!vals.some(v=>v!==null))return '';const present=vals.filter(v=>v!==null),t=followupTrend(present[0]??null,present.at(-1)??null);return `<tr><td><strong>${escapeHtml(label)}</strong></td>${vals.map(v=>`<td>${v==null?'—':Number(v).toLocaleString('fr-FR',{maximumFractionDigits:1})}</td>`).join('')}<td>${sparklineSvg(vals)}</td><td><span class="trend-badge ${t.cls}">${t.icon} ${t.label}</span></td></tr>`;}).join('');
}
function renderFollowup(){
  ...
  const technicalRows=followupTechnicalRows(selected);
  ...
  <section class="card"><div class="section-title"><div><h3>Résultats technico-économiques et Reproduction</h3><div class="muted">Effectifs, mouvements, mortalité et indicateurs reproduction calculés pour chaque visite.</div></div></div><div class="table-wrap"><table class="followup-table"><thead><tr><th>Indicateur</th>${selected.map(v=>`<th>${formatDate(v.date)}</th>`).join('')}<th>Évolution</th><th>Tendance</th></tr></thead><tbody>${technicalRows||'<tr><td colspan="99">Aucune donnée technico-économique ou reproduction comparable.</td></tr>'}</tbody></table></div></section>
...
function renderHerdData(){
  ...
  const currentImport=db.herdImports.find(x=>x.id===currentSourceId)||audit.importedHerdData?.snapshot||null;
...
function resolveRegistryAnimal(farm,query){
  const list=farm?.herdRegistry||[],q=normalizeAnimalId(query),digits=animalDigits(query);if(!q)return null;
  let items=list.filter(a=>normalizeAnimalId(a.id)===q||normalizeAnimalId(a.workNumber)===q);
  if(!items.length&&digits)items=list.filter(a=>animalDigits(a.id).endsWith(digits)||String(a.workNumber||'').replace(/\D/g,'')===digits);
  if(items.length===1)return {animal:items[0]}; if(items.length>1)return {ambiguous:true,items}; return null;
}
...
function currentReproductionCows(farm){
  return (farm?.herdRegistry||[])
    .filter(a=>a.sex==='F'&&isRegistryAnimalPresent(a))
...
function reproductionSourceForVisit(visit,farm){
  ...
  return {registry:farm?.herdRegistry||[],meta:null,scope:'legacy'};
}
function dateInPeriod(date,start,end){return !!date&&(!start||date>=start)&&(!end||date<=end);}
function registryAnimalWasPresent(animal,start,end){if(!animal)return false;return (!animal.entryDate||!end||animal.entryDate<=end)&&(!animal.exitDate||!start||animal.exitDate>=start);}
function reproductionPeriodIndicators(farm,start,end){
  const registry=farm?.herdRegistry||[];
  const allCows=registry.filter(a=>a.sex==='F').map(c=>reproductionForCow(farm,c.id)).filter(r=>r.calves.length>=1);
  const eligible=allCows.filter(r=>registryAnimalWasPresent(r.cow,start,end));
  const calves=registry.filter(c=>c.birthDate&&c.motherId&&dateInPeriod(c.birthDate,start,end));
  const motherIds=[...new Set(calves.map(c=>normalizeAnimalId(c.motherId)).filter(Boolean))];
  const calved=eligible.filter(r=>motherIds.includes(normalizeAnimalId(r.cow.id)));
  const withoutCalving=eligible.filter(r=>!motherIds.includes(normalizeAnimalId(r.cow.id)));
  const primiparous=calved.filter(r=>dateInPeriod(r.firstCalvingDate,start,end));
  const ivv12=[];eligible.forEach(r=>{if(r.calvingDates.length>=2&&dateInPeriod(r.calvingDates[1],start,end)){const d=daysBetweenDates(r.calvingDates[0],r.calvingDates[1]);if(d!=null)ivv12.push(d);}});
  const maleRows=registry.filter(a=>{const sex=normalizeSearchText(a.sex);return sex==='m'||sex==='male'||sex==='taureau'||sex.startsWith('mâle')||sex.startsWith('male');});
  const maleIds=new Set();maleRows.forEach(a=>{[a.id,a.workNumber].forEach(x=>{const n=normalizeAnimalId(x);if(n)maleIds.add(n);const d=animalDigits(x);if(d)maleIds.add(d);});});
  let ia=0,natural=0,unknown=0;const iaDetails=[];calves.forEach(c=>{const raw=c.sireId||'',sire=normalizeAnimalId(raw),digits=animalDigits(raw);let mode='unknown';if(!sire)unknown++;else if(maleIds.has(sire)||(digits&&maleIds.has(digits))){natural++;mode='natural';}else{ia++;mode='ia';}iaDetails.push({calf:c,sireId:raw,mode});});
  const known=ia+natural;return {eligible,calves,calved,withoutCalving,primiparous,ivv12,ia,natural,unknown,iaDetails,calvingRate:eligible.length?Math.round(calved.length/eligible.length*1000)/10:null,primiparousRate:calved.length?Math.round(primiparous.length/calved.length*1000)/10:null,ivv12Mean:ivv12.length?Math.round(ivv12.reduce((a,b)=>a+b,0)/ivv12.length):null,iaRate:known?Math.round(ia/known*1000)/10:null};
}
function renderReproduction(){
  const visit=activeVisit();
  const selectedFarmId=visit?.farmId||localStorage.getItem('audit-bovin-repro-farm')||db.farms[0]?.id||'';
  const farm=db.farms.find(f=>f.id===selectedFarmId);
  const source=reproductionSourceForVisit(visit,farm);
  const registry=source.registry;
  const reproFarm=farm?{...farm,herdRegistry:registry}:farm;
  const allFemales=registry.filter(a=>a.sex==='F');
  const presentFemales=allFemales.filter(a=>isRegistryAnimalPresent(a));
  const currentCows=currentReproductionCows(reproFarm);
  const currentCowIds=new Set(currentCows.map(r=>normalizeAnimalId(r.cow.id)));
  const presentHeifers=presentFemales.filter(a=>!currentCowIds.has(normalizeAnimalId(a.id))).sort((a,b)=>(a.birthDate||'9999').localeCompare(b.birthDate||'9999'));
  let rows=[...currentCows];
  const today=new Date().toISOString().slice(0,10);
  const periodEnd=localStorage.getItem('audit-bovin-repro-period-end')||today;
  const defaultStart=new Date(new Date(periodEnd+'T12:00:00').setFullYear(new Date(periodEnd+'T12:00:00').getFullYear()-1)).toISOString().slice(0,10);
  const periodStart=localStorage.getItem('audit-bovin-repro-period-start')||defaultStart;
  const periodStats=reproductionPeriodIndicators(reproFarm,periodStart,periodEnd);
  const firstAges=currentCows.map(r=>r.firstCalvingAgeMonths).filter(x=>x!=null),ivvs=currentCows.flatMap(r=>r.intervals);
  const stats={firstMean:firstAges.length?Math.round(firstAges.reduce((a,b)=>a+b,0)/firstAges.length*10)/10:null,firstMin:firstAges.length?Math.min(...firstAges):null,firstMax:firstAges.length?Math.max(...firstAges):null,ivvMean:ivvs.length?Math.round(ivvs.reduce((a,b)=>a+b,0)/ivvs.length):null,ivvMin:ivvs.length?Math.min(...ivvs):null,ivvMax:ivvs.length?Math.max(...ivvs):null};
  if(reproductionFilter==='400')rows=rows.filter(r=>r.daysSinceLast>400);if(reproductionFilter==='ivvLe400')rows=rows.filter(r=>r.meanIVV!=null&&r.meanIVV<=400);if(reproductionFilter==='ivv401to450')rows=rows.filter(r=>r.meanIVV>400&&r.meanIVV<=450);if(reproductionFilter==='ivv451to500')rows=rows.filter(r=>r.meanIVV>450&&r.meanIVV<=500);if(reproductionFilter==='ivvOver500')rows=rows.filter(r=>r.meanIVV>500);if(reproductionFilter==='dead')rows=rows.filter(r=>r.deadBefore6.length>=2);if(reproductionFilter==='under28')rows=rows.filter(r=>r.firstCalvingAgeMonths<28);if(reproductionFilter==='28to36')rows=rows.filter(r=>r.firstCalvingAgeMonths>=28&&r.firstCalvingAgeMonths<=36);if(reproductionFilter==='over36')rows=rows.filter(r=>r.firstCalvingAgeMonths>36);
  const sorters={lastCalvingOld:(a,b)=>(a.lastCalvingDate||'9999').localeCompare(b.lastCalvingDate||'9999'),ivvHigh:(a,b)=>(b.meanIVV||-1)-(a.meanIVV||-1),firstAgeHigh:(a,b)=>(b.firstCalvingAgeMonths||-1)-(a.firstCalvingAgeMonths||-1),deadHigh:(a,b)=>b.deadBefore6.length-a.deadBefore6.length,scoreLow:(a,b)=>reproductionScore(a)-reproductionScore(b)}; rows.sort(sorters[reproductionSort]||sorters.lastCalvingOld);
  const meta=source.meta,period=meta?.period||reproductionRegistryPeriod(registry);
  const sourceHtml=meta?`<section class="notice imported-data-notice"><strong>📄 Registre bovins utilisé pour cette visite</strong><div class="grid cols-3" style="margin-top:10px"><div><span class="muted">Fichier</span><br><strong>${escapeHtml(meta.fileName||'CSV reproduction')}</strong></div><div><span class="muted">Importé le</span><br><strong>${formatDateTime(meta.importedAt)}</strong></div><div><span class="muted">Lignes reconnues</span><br><strong>${meta.rowCount??registry.length}</strong></div></div><div class="muted" style="margin-top:8px">${period?.from&&period?.to?`Période des données détectée : ${formatDate(period.from)} → ${formatDate(period.to)} · `:''}🟢 Ce fichier est associé uniquement à la visite du ${formatDate(visit?.date)}.</div></section>`:registry.length?`<section class="notice warning"><strong>⚠️ Registre antérieur détecté pour l’exploitation</strong><br>Le nom du fichier d’origine n’était pas enregistré dans les anciennes versions. Réimportez le bon CSV depuis cette visite pour l’identifier et le verrouiller à cette visite.</section>`:`<section class="notice warning"><strong>🟠 Aucun registre bovins associé à cette visite.</strong><br>Importez le CSV Reproduction correspondant à cette visite.</section>`;
  app.innerHTML=`<div class="section-title"><div><h2>🐄 Reproduction</h2><div class="muted">Registre historique pour les fiches individuelles ; indicateurs calculés uniquement sur les vaches présentes ayant déjà eu au moins un veau.</div></div><span class="badge autosave">v14.5.6</span></div>${visit?activeVisitBanner(visit):''}
  ${sourceHtml}
  <section class="card"><div class="row"><div class="field"><label>Exploitation</label><select id="repro-farm" ${visit?'disabled':''}>${db.farms.map(f=>`<option value="${f.id}" ${f.id===selectedFarmId?'selected':''}>${escapeHtml(f.name)}${f.farmNumber?` · ${escapeHtml(f.farmNumber)}`:''}</option>`).join('')}</select></div><div class="field"><label>${meta?'Remplacer le registre bovins CSV':'Importer le registre bovins CSV'}</label><input id="repro-file" type="file" accept=".csv,text/csv"></div></div><div class="muted">${registry.length?`${registry.length} bovin(s) historiques · ${presentFemales.length} femelle(s) présente(s) · ${currentCows.length} vache(s) avec au moins un veau${meta?.importedAt?` · mise à jour ${formatDateTime(meta.importedAt)}`:''}`:'Aucun registre importé pour cette visite.'}</div></section>
  ${!registry.length?'<section class="empty" style="margin-top:16px">Importez le fichier CSV Reproduction contenant tous les bovins de cette exploitation.</section>':`
  <section class="card repro-dashboard-compact" style="margin-top:16px"><div class="section-title"><div><h3>Indicateurs sur une période</h3><div class="muted">Calculés sur la période choisie.</div></div></div><div class="row"><div class="field"><label>Du</label><input id="repro-period-start" type="date" value="${periodStart}"></div><div class="field"><label>Au</label><input id="repro-period-end" type="date" value="${periodEnd}"></div></div><div class="grid cols-4 repro-metrics-grid"><article class="metric"><strong>${periodStats.calvingRate??'—'}${periodStats.calvingRate!=null?' %':''}</strong><span>Taux de vêlage vaches</span><small>${periodStats.calved.length}/${periodStats.eligible.length} vaches</small></article><article class="metric"><strong>${periodStats.primiparous.length}</strong><span>Primipares</span><small>${periodStats.primiparousRate??'—'}${periodStats.primiparousRate!=null?' %':''}</small></article><article class="metric"><strong>${periodStats.ivv12Mean??'—'}</strong><span>IVV 1–2 moyen</span><small>${periodStats.ivv12.length} calculable(s)</small></article><article class="metric"><strong>${periodStats.iaRate??'—'}${periodStats.iaRate!=null?' %':''}</strong><span>Veaux issus d’IA</span><small>${periodStats.ia} IA · ${periodStats.natural} monte · ${periodStats.unknown} inconnu</small></article></div><details style="margin-top:12px"><summary><strong>Vaches sans mise bas : ${periodStats.withoutCalving.length}</strong></summary><div class="table-wrap"><table><thead><tr><th>Vache</th><th>Dernier vêlage</th><th>Âge</th></tr></thead><tbody>${periodStats.withoutCalving.map(r=>`<tr><td>${escapeHtml(r.cow.workNumber||r.cow.id)}</td><td>${r.lastCalvingDate?formatDate(r.lastCalvingDate):'—'}</td><td>${ageLabelAt(r.cow.birthDate,periodEnd)||'—'}</td></tr>`).join('')||'<tr><td colspan="3">Aucune.</td></tr>'}</tbody></table></div></details><details class="ia-quality-details" style="margin-top:12px"><summary><strong>Contrôle du calcul IA</strong> · ${periodStats.ia} IA, ${periodStats.natural} monte naturelle, ${periodStats.unknown} père non renseigné</summary><div class="table-wrap"><table><thead><tr><th>Veau</th><th>Date</th><th>Père</th><th>Classement</th></tr></thead><tbody>${periodStats.iaDetails.map(x=>`<tr><td>${escapeHtml(x.calf.workNumber||x.calf.id)}</td><td>${formatDate(x.calf.birthDate)}</td><td>${escapeHtml(x.sireId||'—')}</td><td><span class="badge ${x.mode==='ia'?'in-progress':x.mode==='natural'?'complete':''}">${x.mode==='ia'?'IA':x.mode==='natural'?'Monte naturelle':'Inconnu'}</span></td></tr>`).join('')||'<tr><td colspan="4">Aucun veau sur la période.</td></tr>'}</tbody></table></div><p class="muted small-text">Monte naturelle si le numéro du père correspond à l’identifiant ou au numéro de travail d’un mâle présent dans le registre ; sinon IA. Les pères non renseignés restent inconnus.</p></details></section>
  <section class="grid cols-4" style="margin-top:16px"><article class="card metric"><strong>${stats.firstMean??'—'}</strong><span>Âge moyen 1er vêlage (mois)</span><small>${stats.firstMin??'—'} à ${stats.firstMax??'—'}</small></article><article class="card metric"><strong>${stats.ivvMean??'—'}</strong><span>IVV moyen (jours)</span><small>${stats.ivvMin??'—'} à ${stats.ivvMax??'—'}</small></article><article class="card metric"><strong>${currentCows.filter(r=>r.daysSinceLast>400).length}</strong><span>Vaches &gt; 400 jours</span></article><article class="card metric"><strong>${currentCows.filter(r=>r.deadBefore6.length>=2).length}</strong><span>Mères avec ≥2 morts &lt;6 mois</span></article></section>
  <section class="card" style="margin-top:16px"><div class="row"><div class="field"><label>Filtre</label><select id="repro-filter"><option value="all">Toutes les vaches présentes</option><option value="400">Sans vêlage depuis plus de 400 j</option><option value="ivvLe400">IVV moyen ≤ 400 j</option><option value="ivv401to450">IVV moyen de 401 à 450 j</option><option value="ivv451to500">IVV moyen de 451 à 500 j</option><option value="ivvOver500">IVV moyen &gt; 500 j</option><option value="dead">Au moins 2 veaux morts avant 6 mois</option><option value="under28">1er vêlage avant 28 mois</option><option value="28to36">1er vêlage entre 28 et 36 mois</option><option value="over36">1er vêlage après 36 mois</option></select></div><div class="field"><label>Classement</label><select id="repro-sort"><option value="lastCalvingOld">Dernier vêlage le plus ancien</option><option value="ivvHigh">IVV moyen le plus élevé</option><option value="firstAgeHigh">1er vêlage le plus tardif</option><option value="deadHigh">Mortalité veaux la plus élevée</option><option value="scoreLow">Score le plus faible</option></select></div></div>
  <div class="table-wrap"><table><thead><tr><th>Vache</th><th>Race / âge</th><th>1er vêlage</th><th>Dernier vêlage</th><th>IVV</th><th>Veaux</th><th>Score</th><th>Détail</th></tr></thead><tbody>${rows.map(r=>`<tr><td><strong>${escapeHtml(r.cow.workNumber||r.cow.id)}</strong><br><small>${escapeHtml(r.cow.id)}</small></td><td>Race ${escapeHtml(r.cow.breed||'—')}<br><small>${ageLabelAt(r.cow.birthDate,today)||'—'}</small></td><td>${r.firstCalvingDate?formatDate(r.firstCalvingDate):'—'}<br><small>${r.firstCalvingAgeMonths!=null?`${Math.round(r.firstCalvingAgeMonths*10)/10} mois`:'—'}</small></td><td>${r.lastCalvingDate?formatDate(r.lastCalvingDate):'—'}<br><small>${r.daysSinceLast!=null?`${r.daysSinceLast} j`:'—'}</small></td><td>Dernier ${r.lastIVV??'—'} j<br><small>Moy. ${r.meanIVV??'—'} · mini ${r.minIVV??'—'} · maxi ${r.maxIVV??'—'}</small></td><td>${r.calves.length}<br><small class="${r.deadBefore6.length>=2?'danger-text':''}">${r.deadBefore6.length} mort(s) &lt;6 mois</small></td><td><button type="button" class="badge ${reproductionScore(r)>=75?'complete':reproductionScore(r)>=60?'in-progress':'danger'}" data-repro-score="${escapeHtml(r.cow.id)}" title="Voir le détail du calcul">${reproductionScore(r)}/100</button></td><td><button class="btn small" data-repro-detail="${escapeHtml(r.cow.id)}">Voir</button></td></tr>`).join('')||'<tr><td colspan="8">Aucun animal correspondant au filtre.</td></tr>'}</tbody></table></div></section>
  <section class="card" style="margin-top:16px"><div class="section-title"><div><h3>Génisses présentes sans vêlage</h3><div class="muted">Ces femelles sont exclues de la liste des vaches et du calcul des IVV.</div></div><span class="badge">${presentHeifers.length}</span></div>${presentHeifers.length?`<div class="table-wrap"><table><thead><tr><th>Génisse</th><th>Date de naissance</th><th>Âge</th><th>Race</th></tr></thead><tbody>${presentHeifers.map(g=>`<tr><td><strong>${escapeHtml(g.workNumber||g.id)}</strong><br><small>${escapeHtml(g.id)}</small></td><td>${g.birthDate?formatDate(g.birthDate):'—'}</td><td>${ageLabelAt(g.birthDate,today)||'—'}</td><td>${escapeHtml(g.breed||'—')}</td></tr>`).join('')}</tbody></table></div>`:'<div class="empty">Aucune génisse présente sans vêlage.</div>'}</section>
  <section id="repro-detail"></section>`}`;
  ...
  const fi=document.getElementById('repro-file');if(fi)fi.onchange=async()=>{const file=fi.files?.[0];if(!file||!farm)return;try{const text=await file.text();const animals=importHerdRegistryRows(parseSemicolonCsv(text));const importedAt=new Date().toISOString(),detectedPeriod=reproductionRegistryPeriod(animals);if(visit&&visit.farmId===farm.id){visit.reproductionRegistry=JSON.parse(JSON.stringify(animals));visit.reproductionRegistrySource={fileName:file.name,importedAt,rowCount:animals.length,period:detectedPeriod};}farm.herdRegistry=JSON.parse(JSON.stringify(animals));farm.herdRegistryImportedAt=importedAt;farm.herdRegistrySource={fileName:file.name,importedAt,rowCount:animals.length,period:detectedPeriod};const ede=[...new Set(animals.map(a=>a.farmNumber).filter(Boolean))];if(!farm.farmNumber&&ede.length===1)farm.farmNumber=ede[0];saveDatabase(db);showToast(`${animals.length} bovin(s) importé(s) et associés à cette visite.`);renderReproduction();}catch(e){console.error(e);showToast(`Import impossible : ${e.message}`);}};
  const ps=document.getElementById('repro-period-start'),pe=document.getElementById('repro-period-end');[ps,pe].forEach(el=>{if(el)el.onchange=()=>{if(ps?.value)localStorage.setItem('audit-bovin-repro-period-start',ps.value);if(pe?.value)localStorage.setItem('audit-bovin-repro-period-end',pe.value);renderReproduction();};});
  const filter=document.getElementById('repro-filter');if(filter){filter.value=reproductionFilter;filter.onchange=()=>{reproductionFilter=filter.value;localStorage.setItem('audit-bovin-repro-filter',reproductionFilter);renderReproduction();};}
  const sort=document.getElementById('repro-sort');if(sort){sort.value=reproductionSort;sort.onchange=()=>{reproductionSort=sort.value;localStorage.setItem('audit-bovin-repro-sort',reproductionSort);renderReproduction();};}
  app.querySelectorAll('[data-repro-score]').forEach(b=>b.onclick=()=>{const r=reproductionForCow(reproFarm,b.dataset.reproScore),d=reproductionScoreDetails(r),box=document.getElementById('repro-detail');box.innerHTML=`<section class="card" style="margin-top:16px"><div class="section-title"><div><h3>Détail du score reproduction · ${escapeHtml(r.cow.workNumber||r.cow.id)}</h3><div class="muted">Le score est un repère de tri et ne remplace pas l’interprétation du technicien.</div></div><span class="badge ${d.score>=75?'complete':d.score>=60?'in-progress':'danger'}">${d.score}/100</span></div><div class="table-wrap"><table><thead><tr><th>Élément</th><th>Valeur</th><th>Impact</th></tr></thead><tbody>${d.lines.map(x=>`<tr><td>${escapeHtml(x.label)}</td><td>${escapeHtml(x.value)}</td><td>${x.delta?`${x.delta} point(s)`:'0'}</td></tr>`).join('')}</tbody></table></div></section>`;box.scrollIntoView({behavior:'smooth',block:'start'});});
  app.querySelectorAll('[data-repro-detail]').forEach(b=>b.onclick=()=>{const r=reproductionForCow(reproFarm,b.dataset.reproDetail),box=document.getElementById('repro-detail');box.innerHTML=`<section class="card" style="margin-top:16px"><div class="section-title"><div><h3>${escapeHtml(r.cow.workNumber||r.cow.id)} · ${escapeHtml(r.cow.id)}</h3><div class="muted">Race ${escapeHtml(r.cow.breed||'—')} · née le ${r.cow.birthDate?formatDate(r.cow.birthDate):'—'} · mère ${escapeHtml(r.cow.motherId||'—')}</div></div><span class="badge complete">Score ${reproductionScore(r)}/100</span></div><div class="grid cols-3"><div class="notice"><strong>1er vêlage</strong><br>${r.firstCalvingDate?formatDate(r.firstCalvingDate):'—'}<br>${r.firstCalvingAgeMonths!=null?`${Math.round(r.firstCalvingAgeMonths*10)/10} mois`:''}</div><div class="notice"><strong>Dernier vêlage</strong><br>${r.lastCalvingDate?formatDate(r.lastCalvingDate):'—'}<br>${r.daysSinceLast!=null?`${r.daysSinceLast} jours`:''}</div><div class="notice"><strong>IVV</strong><br>dernier ${r.lastIVV??'—'} j<br>moyen ${r.meanIVV??'—'} j</div></div><h4>Historique des veaux</h4><div class="table-wrap"><table><thead><tr><th>Date</th><th>Veau</th><th>Sexe</th><th>Race</th><th>Sortie</th><th>IVV précédent</th></tr></thead><tbody>${r.calves.map((c,i)=>`<tr><td>${formatDate(c.birthDate)}</td><td>${escapeHtml(c.workNumber||c.id)}<br><small>${escapeHtml(c.id)}</small></td><td>${escapeHtml(c.sex||'—')}</td><td>${escapeHtml(c.breed||'—')}</td><td>${escapeHtml(c.exitCause||'Présent')}${c.exitDate?` · ${formatDate(c.exitDate)}`:''}</td><td>${i?r.intervals[i-1]+' j':'—'}</td></tr>`).join('')}</tbody></table></div></section>`;box.scrollIntoView({behavior:'smooth',block:'start'});});
}
