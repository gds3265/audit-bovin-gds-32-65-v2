...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Repères visuels, fiches pratiques et carnet technique, disponibles hors ligne.</div></div><span class="badge autosave">v14.6.21.110</span></div>
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.6.21.110</span></div>
...
function renderVisits() {
  ...
  app.innerHTML = `
    <div class="section-title"><div><h2>Visites</h2><div class="muted">Toutes les visites enregistrées restent accessibles, y compris les plus anciennes.</div></div><span class="badge autosave">v14.6.21.110</span></div>
...
function renderFarmerQuestionnaires(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📨 Questionnaires éleveur</h2><div class="muted">Préparer la visite puis récupérer uniquement les réponses encore manquantes.</div></div><span class="badge autosave">v14.6.21.110</span></div>${activeVisitBanner(visit)}
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.6.21.110</span></div>
...
function feedProductCardHtml(visit){
  ...
  ${products.length?products.map(p=>{const c=feedProductDailyContribution(p);return `<article class="card compact" data-feed-product="${p.id}"><div class="section-title"><h4>${escapeHtml(p.name||'Aliment')}</h4><div class="actions"><button type="button" class="btn small secondary" data-save-feed-product="${p.id}">💾 Enregistrer pour les prochaines visites</button><button type="button" class="btn small danger" data-delete-feed-product="${p.id}">Supprimer</button></div></div>
    ...
    <h4>Oligo-éléments (mg/kg de produit)</h4><div class="grid cols-4">${[['cu','Cuivre'],['zn','Zinc'],['mn','Manganèse'],['i','Iode'],['co','Cobalt'],['se','Sélénium']].map(([k,l])=>`<div class="field"><label>${l} (mg/kg)</label><input inputmode="decimal" data-feed-product-field="${k}" data-id="${p.id}" value="${escapeHtml(p[k]||'')}"></div>`).join('')}</div>
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.6.21.110</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
  visit.feeding=visit.feeding&&typeof visit.feeding==='object'?visit.feeding:{rations:[],settings:{},history:[]};
  visit.feeding.nutrition=visit.feeding.nutrition&&typeof visit.feeding.nutrition==='object'?visit.feeding.nutrition:{};
...
function renderQuickFieldNotes(){
  ...
  const cats=['Alimentation / ration','Eau / bâtiment','Reproduction','Mortalité / sanitaire','Animaux / observations','Organisation / économie','À classer'];
  const SpeechRecognition=window.SpeechRecognition||window.webkitSpeechRecognition;
  app.innerHTML=`<div class="section-title"><div><h2>📝 Notes terrain</h2><div class="muted">Dictez ou écrivez librement pendant l’échange. Les notes brutes restent toujours conservées.</div></div><span class="badge autosave">Sauvegarde automatique</span></div>
  ${activeVisitBanner(visit)}
  <section class="card quick-field-notes">
    <div class="notice"><strong>Mode terrain :</strong> utilisez le micro pour dicter au fil de la visite. Ensuite « Classer par thèmes » regroupe les informations et vous laisse valider avant de compléter l’audit.</div>
    <div class="field"><label>Notes prises pendant la visite</label><textarea id="quick-field-notes-raw" rows="24" style="min-height:520px;resize:vertical" placeholder="Dictez ou saisissez vos notes ici…">${escapeHtml(n.raw||'')}</textarea></div>
    <div class="actions quick-notes-actions">
      <button class="btn primary" id="quick-note-dictate" ${SpeechRecognition?'':'disabled'}>🎙️ Démarrer la dictée</button>
      <button class="btn" id="quick-note-time">+ Heure</button>
      <button class="btn secondary" id="quick-notes-export">⬇️ Exporter les notes</button>
      <button class="btn primary" id="quick-notes-process">✨ Classer par thèmes</button>
    </div>
    <div id="quick-note-dictation-status" class="muted small-text">${SpeechRecognition?'La dictée utilise la reconnaissance vocale disponible dans ce navigateur.':'Dictée vocale non disponible dans ce navigateur. Vous pouvez utiliser le micro du clavier ou saisir les notes.'}</div>
  </section>
  <section class="card"><div class="section-title"><div><h3>Classement proposé</h3><div class="muted">Une fiche par grand thème. Relisez/corrigez si besoin. « Compléter l’audit » ajoute les blocs aux rubriques correspondantes sans effacer les notes brutes.</div></div>${n.items.length?'<button class="btn primary" id="quick-notes-integrate">✅ Compléter l’audit avec les thèmes cochés</button>':''}</div>
  <div id="quick-notes-items">${n.items.length?n.items.map((x,i)=>`<article class="quick-note-item"><div class="field"><label>Destination</label><select data-qn-cat="${i}">${cats.map(c=>`<option ${x.category===c?'selected':''}>${c}</option>`).join('')}</select></div><div class="field field-wide"><label>Notes regroupées</label><textarea rows="7" data-qn-text="${i}">${escapeHtml(x.text||'')}</textarea></div><label class="check"><input type="checkbox" data-qn-use="${i}" ${x.use!==false?'checked':''}> Utiliser pour compléter l’audit</label></article>`).join(''):'<div class="empty">Aucun classement pour l’instant.</div>'}</div></section>`;
  ...
  document.getElementById('quick-note-time').onclick=()=>{const d=new Date();raw.value+=(raw.value&&!raw.value.endsWith('\n')?'\n':'')+`[${d.toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit'})}] `;raw.focus();saveRaw();};
  ...
  const mic=document.getElementById('quick-note-dictate'),status=document.getElementById('quick-note-dictation-status');
  if(status&&!document.getElementById('quick-note-live-preview'))status.insertAdjacentHTML('afterend','<div id="quick-note-live-preview" class="notice" style="display:none;margin-top:8px"><strong>Texte entendu :</strong> <span></span><div class="muted small-text">Le texte provisoire est visible en direct. Seules les phrases validées sont ajoutées aux notes.</div></div>');
  const liveBox=document.getElementById('quick-note-live-preview'),liveSpan=liveBox?.querySelector('span');
  if(SpeechRecognition&&mic){
    let wanted=false,rec=null,restartTimer=null,lastAccepted='',lastAcceptedAt=0;
    const clean=x=>String(x||'').replace(/\s+/g,' ').trim();
    const norm=x=>normalizeSearchText(clean(x)).replace(/[^a-z0-9à-ÿ ]/gi,'').replace(/\s+/g,' ').trim();
    const showLive=t=>{if(liveSpan)liveSpan.textContent=clean(t)||'…';if(liveBox)liveBox.style.display=wanted?'block':'none';};
    const setUi=on=>{mic.textContent=on?'⏹️ Arrêter la dictée':'🎙️ Démarrer la dictée';mic.classList.toggle('danger',on);};
    const appendFinal=t=>{
      const piece=clean(t),np=norm(piece);if(!np)return;
      const now=Date.now();if(np===lastAccepted&&now-lastAcceptedAt<20000)return;
      const tail=norm((raw.value||'').slice(-1200));if(np.length>10&&(tail===np||tail.endsWith(np))){lastAccepted=np;lastAcceptedAt=now;return;}
      const sep=raw.value&&!/[\s\n]$/.test(raw.value)?' ':'';raw.value=(raw.value||'')+sep+piece;
      lastAccepted=np;lastAcceptedAt=now;saveRaw();raw.scrollTop=raw.scrollHeight;
    };
    const stop=()=>{wanted=false;if(restartTimer){clearTimeout(restartTimer);restartTimer=null;}try{rec?.abort();}catch(_){}rec=null;__fieldNotesRecognition=null;setUi(false);if(liveBox)liveBox.style.display='none';status.textContent='Dictée arrêtée. Les phrases validées sont enregistrées.';};
    const startCycle=()=>{
      if(!wanted)return;
      try{
        rec=new SpeechRecognition();__fieldNotesRecognition=rec;rec.lang='fr-FR';rec.continuous=false;rec.interimResults=true;rec.maxAlternatives=1;
        rec.onstart=()=>{setUi(true);status.textContent='🎙️ Dictée en cours — le texte entendu s’affiche en direct.';showLive('…');};
        rec.onresult=e=>{let interim='';for(let i=e.resultIndex;i<e.results.length;i++){const r=e.results[i],t=clean(r?.[0]?.transcript||'');if(!t)continue;if(r.isFinal)appendFinal(t);else interim=t;}showLive(interim||'✓ phrase enregistrée');};
        rec.onerror=e=>{const err=e?.error||'erreur';if(['not-allowed','service-not-allowed','audio-capture'].includes(err)){wanted=false;setUi(false);if(liveBox)liveBox.style.display='none';status.textContent=`Dictée impossible (${err}). Vérifiez l’autorisation du micro.`;}else if(wanted)status.textContent=`Dictée interrompue (${err}), reprise automatique…`;};
        rec.onend=()=>{rec=null;__fieldNotesRecognition=null;if(wanted){restartTimer=setTimeout(()=>{restartTimer=null;startCycle();},700);}else{setUi(false);if(liveBox)liveBox.style.display='none';}};
        rec.start();
      }catch(err){console.warn('Dictée',err);if(wanted)restartTimer=setTimeout(()=>{restartTimer=null;startCycle();},1200);}
    };
    mic.onclick=()=>{if(wanted){stop();return;}wanted=true;lastAccepted='';lastAcceptedAt=0;startCycle();};
  }
  document.getElementById('quick-notes-process').onclick=()=>{
    saveRaw(); rememberQuickFieldNotesUi(visit.id,raw);
    n.items=groupQuickNotes(raw.value);
    saveDatabase(db);renderQuickFieldNotes();
    showToast(`${n.items.length} thème(s) proposé(s) à partir des notes.`);
  };
  app.querySelectorAll('[data-qn-cat]').forEach(el=>el.onchange=()=>{n.items[Number(el.dataset.qnCat)].category=el.value;saveDatabase(db);});
  app.querySelectorAll('[data-qn-text]').forEach(el=>el.oninput=()=>{n.items[Number(el.dataset.qnText)].text=el.value;saveDatabase(db);});
  app.querySelectorAll('[data-qn-use]').forEach(el=>el.onchange=()=>{n.items[Number(el.dataset.qnUse)].use=el.checked;saveDatabase(db);});
  document.getElementById('quick-notes-integrate')?.addEventListener('click',()=>{
    const chosen=n.items.filter(x=>x.use!==false&&x.text.trim()&&x.category!=='À classer');
    chosen.forEach(x=>appendSectionNote(x.category,x.text.trim()));
    n.integratedAt=new Date().toISOString();visit.updatedAt=n.integratedAt;saveDatabase(db);saveAuditDraftLocal(visit,ensureAuditGlobal(visit));
    showToast(`${chosen.length} thème(s) ajoutés aux rubriques correspondantes de l’audit. Les notes brutes sont conservées.`);
  });
}
...
function renderFollowup(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Historique & évolution</h2><div class="muted">Comparer deux visites d’une même exploitation : reproduction, mortalité, mesures, alimentation, bâtiment et plans d’action.</div></div><span class="badge autosave">v14.6.21.110</span></div>
...
if ('serviceWorker' in navigator){
  ...
  const AUDIT_SW=location.origin+AUDIT_PATH+'sw.js?v=14.6.21.110';
