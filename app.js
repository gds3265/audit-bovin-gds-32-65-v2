...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Repères visuels, fiches pratiques et carnet technique, disponibles hors ligne.</div></div><span class="badge autosave">v14.6.21.118</span></div>
...
window.addEventListener('audit-bovin-cloud-merged',async event=>{
  ...
  const noHotReload=new Set(['audit','visits','documents','building','analysis','animals','feeding','quicknotes','questionnaires','herddata','metabolic','parasitism','waterlab']);
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.6.21.118</span></div>
...
function renderVisits() {
  ...
  app.innerHTML = `
    <div class="section-title"><div><h2>Visites</h2><div class="muted">Toutes les visites enregistrées restent accessibles, y compris les plus anciennes.</div></div><span class="badge autosave">v14.6.21.118</span></div>
...
function renderFarmerQuestionnaires(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📨 Questionnaires éleveur</h2><div class="muted">Préparer la visite puis récupérer uniquement les réponses encore manquantes.</div></div><span class="badge autosave">v14.6.21.118</span></div>${activeVisitBanner(visit)}
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.6.21.118</span></div>
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.6.21.118</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
  visit.feeding=visit.feeding&&typeof visit.feeding==='object'?visit.feeding:{rations:[],settings:{},history:[]};
  visit.feeding.nutrition=visit.feeding.nutrition&&typeof visit.feeding.nutrition==='object'?visit.feeding.nutrition:{};
...
function renderFollowup(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Historique & évolution</h2><div class="muted">Comparer deux visites d’une même exploitation : reproduction, mortalité, mesures, alimentation, bâtiment et plans d’action.</div></div><span class="badge autosave">v14.6.21.118</span></div>
...
function importChatgptRows(visit,rows,fileName){
  ...
  visit.feeding=visit.feeding&&typeof visit.feeding==='object'?visit.feeding:{rations:[],settings:{},history:[]};
  visit.feeding.nutrition=visit.feeding.nutrition&&typeof visit.feeding.nutrition==='object'?visit.feeding.nutrition:{};
  const nutrition=visit.feeding.nutrition;nutrition.forageAnalyses=Array.isArray(nutrition.forageAnalyses)?nutrition.forageAnalyses:[];
...
function chatgptImportCardHtml(visit){const items=(visit.chatgptImports||[]).slice(0,5);return `<section class="card import-type-card"><div class="section-title"><div><h3>3 · Import ChatGPT — analyses & étiquettes</h3><p class="muted">Après m’avoir envoyé une photo/PDF, importez ici le CSV que je vous fournis. Il peut remplir fourrages, eau, parasitisme, sang/profil métabolique et compositions d’aliments.</p></div><a class="btn secondary" href="./MODELE_IMPORT_CHATGPT.csv" download>⬇️ Modèle CSV</a></div><div class="notice"><strong>Colonnes :</strong> TYPE ; ELEMENT ; PARAMETRE ; VALEUR ; UNITE ; DATE ; SOURCE ; COMMENTAIRE.<br><strong>TYPE :</strong> FOURRAGE, EAU, PARASITISME, SANG ou ALIMENT. L’import remplace automatiquement un précédent import portant le même nom de fichier.</div><div class="field"><label>Fichier CSV préparé par ChatGPT</label><input id="chatgpt-analysis-import" type="file" accept=".csv,text/csv"></div>${items.length?`<div class="small-text muted">Derniers imports : ${items.map(x=>`${escapeHtml(x.fileName)} (${x.count} valeur(s))`).join(' · ')}</div>`:''}</section>`}
function renderHerdData(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Données technico-économiques</h2><span class="muted">Visite du ${formatDate(visit.date)} — ${escapeHtml(farm?.name||'Exploitation')}</span></div><span class="badge autosave">v14.6.21.118</span></div><section class="notice"><strong>Ordre conseillé :</strong> créez d’abord la visite, importez les données préparatoires, puis définissez ici l’orientation de l’audit. Le mode ciblé automatique utilise les éléments calculés ci-dessous et le motif éventuellement signalé par l’éleveur.</section>${targetedAuditHtml(visit)}${chatgptImportCardHtml(visit)}${registryImportHtml}${reproPreparationHtml(visit,farm)}${prepEconomicHtml(visit,farm)}${currentHtml}<div class="actions"><button class="btn secondary" id="toggle-herd-library">${hasCurrentCsv?'Changer de CSV':'Choisir un CSV déjà importé'}</button></div>${libraryHtml}${importHtml}`;
  bindTargetedAudit(visit);
...
if ('serviceWorker' in navigator){
  ...
  const AUDIT_SW=location.origin+AUDIT_PATH+'sw.js?v=14.6.21.118';
