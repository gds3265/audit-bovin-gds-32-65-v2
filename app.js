...
// Cette fonction manquait dans la v14.6.21.89 et provoquait l'erreur splitUsefulLines is not defined.
...
// v14.6.21.89 — Audits ciblés + motif libre + orientation syndromique veaux
...
function targetedAuditHtml(visit,{forPrint=false}={}){
  ...
  return `<section class="card targeted-audit-card"><div class="section-title"><div><h3>🎯 Audit adapté au motif et à la pré-visite</h3><div class="muted">Le logiciel priorise les questions sans masquer le reste de l’audit. Les hypothèses pathologiques sont des pistes à explorer, jamais un diagnostic.</div></div><span class="badge">${escapeHtml(modeLabel)}</span></div>
...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Repères visuels, fiches pratiques et carnet technique, disponibles hors ligne.</div></div><span class="badge autosave">v14.6.21.89</span></div>
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.6.21.89</span></div>
...
function visitFormHtml(visit = {}) {
  ...
  return `<form id="visit-form" class="card">
    ...
    <div class="field"><label>Mode d’audit</label><select name="auditMode">${targetedAuditModes.map(([v,l])=>`<option value="${v}" ${(data.auditMode||data.auditTargeting?.mode||'automatic')===v?'selected':''}>${escapeHtml(l)}</option>`).join('')}</select></div>
    <div class="field"><label>Objectif / attentes</label><textarea name="objective">${escapeHtml(data.objective || '')}</textarea></div>
    <div class="field"><label>Motif / problèmes rencontrés</label><textarea name="problemNarrative" placeholder="Ex. diarrhée sur veaux de 3 semaines ; mortalité adulte ; état corporel insuffisant…">${escapeHtml(data.problemNarrative || data.auditTargeting?.problemNarrative || '')}</textarea><small class="muted">Ce texte sert à prioriser automatiquement les questions de l’audit.</small></div>
...
function renderVisits() {
  ...
  app.innerHTML = `
    <div class="section-title"><div><h2>Visites</h2><div class="muted">Toutes les visites enregistrées restent accessibles, y compris les plus anciennes.</div></div><span class="badge autosave">v14.6.21.89</span></div>
  ...
  form.addEventListener('submit',event=>{event.preventDefault();const data=Object.fromEntries(new FormData(form));if(!data.farmId||!data.date)return showToast('Exploitation et date obligatoires.');if(editVisit){Object.assign(editVisit,data,{updatedAt:new Date().toISOString()});const tt=ensureAuditTargeting(editVisit);tt.mode=data.auditMode||tt.mode;tt.problemNarrative=data.problemNarrative||'';addJournal(editVisit,'Informations générales mises à jour.');showToast('Visite mise à jour.');}else{const visit={id:uid('visit'),...data,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),journal:[],subjects:[]};const tt=ensureAuditTargeting(visit);tt.mode=data.auditMode||'automatic';tt.problemNarrative=data.problemNarrative||'';addJournal(visit,'Visite créée.');db.visits.push(visit);ensurePreviousVisitReview(visit);setActiveVisit(visit.id);editingVisitId=null;showToast(visit.previousVisitReview?.previousVisitId?'Visite créée : commencez par contrôler les actions précédentes.':'Visite créée.');}saveDatabase(db);clearDraft();editingVisitId=null;renderVisits();});
...
function renderFarmerQuestionnaires(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📨 Questionnaires éleveur</h2><div class="muted">Préparer la visite puis récupérer uniquement les réponses encore manquantes.</div></div><span class="badge autosave">v14.6.21.89</span></div>${activeVisitBanner(visit)}
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.6.21.89</span></div>
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.6.21.89</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
  visit.feeding=visit.feeding&&typeof visit.feeding==='object'?visit.feeding:{rations:[],settings:{},history:[]};
  visit.feeding.nutrition=visit.feeding.nutrition&&typeof visit.feeding.nutrition==='object'?visit.feeding.nutrition:{};
...
function renderFollowup(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Historique & évolution</h2><div class="muted">Comparer deux visites d’une même exploitation : reproduction, mortalité, mesures, alimentation, bâtiment et plans d’action.</div></div><span class="badge autosave">v14.6.21.89</span></div>
...
function reproRolling12Stats(visit,farm,x=null){
  ...
  return {start,end,births,dams:dams.size,intervals,ivvMean:mean(intervals),ivvLe400:intervals.filter(d=>d<=400).length,ivv401to450:intervals.filter(d=>d>=401&&d<=450).length,ivv451to500:intervals.filter(d=>d>=451&&d<=500).length,ivvOver500:intervals.filter(d=>d>500).length,firstCalvingCount:firstAges.length,firstCalvingMean:mean1(firstAges),dead6,mortalityRate:births.length?Math.round(dead6.length/births.length*1000)/10:null};
}
...
// v14.6.21.89 — aide à la décision transversale : priorités, tendances et références internes.
...
// v14.6.21.89 — cohérence pré-visite, tableau global mouvements/renouvellement et âges lisibles.
...
// v14.6.21.89 — « fabrique du troupeau » : naissance femelles → conservation jusqu’à la reproduction → 1er vêlage → réforme.
...
function prepRenewalFactoryHtml(data,{forPrint=false}={}){
  ...
  const L=data.last||{},summary=[
    ['Femelles nées',L.femaleBirths??'—','cohorte la plus récente'],
    [`Conservées jusqu’à ${data.reproAgeMonths} mois`,L.reachedRepro??'—',L.reachedReproRate!=null?`${L.reachedReproRate}% des femelles évaluables`:'cohorte encore trop jeune'],
    ['Ayant atteint le 1er vêlage',L.firstCalved??'—',L.firstCalvingRate!=null?`${L.firstCalvingRate}% des femelles avec recul suffisant`:'recul insuffisant'],
    ['Réformées après 1er vêlage',L.reformed??'—',L.reformAgeMean!=null?`âge moyen ${prepAgeYearsMonths(L.reformAgeMean)}`:'']
  ];
  return `<div class="renewal-factory ${forPrint?'for-print':''}"><h4>🐄 Fabrique du troupeau — naissance → reproduction → 1er vêlage → réforme</h4><p class="muted small-text">Le seuil de « conservation jusqu’à la reproduction » est actuellement fixé à <strong>${data.reproAgeMonths} mois</strong> (modifiable dans Structure du troupeau et renouvellement). Les cohortes récentes sont volontairement marquées par des valeurs « — » lorsqu’elles n’ont pas encore assez de recul.</p><div class="renewal-funnel">${summary.map(([l,v,n])=>`<article><strong>${escapeHtml(String(v))}</strong><span>${escapeHtml(l)}</span><small>${escapeHtml(n||'')}</small></article>`).join('')}</div><div class="table-wrap"><table class="annual-history-table"><thead><tr><th>Indicateur / cohorte de naissance</th>${years.map(y=>`<th>${y}</th>`).join('')}</tr></thead><tbody>${tr('Femelles nées','femaleBirths')}${tr('Cohorte évaluable à l’âge repro','evaluable')}${tr(`Conservées jusqu’à ${data.reproAgeMonths} mois`,'reachedRepro')}${tr('Pertes / sorties avant mise à la repro','lostBeforeRepro')}${tr('Taux conservées jusqu’à la repro','reachedReproRate',' %')}${tr('Femelles avec recul suffisant pour 1er vêlage','calvingEligible')}${tr('Ayant atteint le 1er vêlage','firstCalved')}${tr('Taux atteignant le 1er vêlage','firstCalvingRate',' %')}${tr('Âge moyen au 1er vêlage','firstCalvingAgeMean',' mois')}${tr('Réformées / sorties après 1er vêlage*','reformed')}${tr('Âge moyen à la réforme / sortie*','reformAgeMean',' mois')}</tbody></table></div><p class="muted small-text">* Réforme approchée par les sorties de femelles après leur premier vêlage, hors mortalité. À confirmer avec le motif réel de sortie lorsqu’il est disponible.</p></div>`;
}
...
function prepMovementRenewalGlobalHtml(movements,renewalFactory,{forPrint=false}={}){
  ...
  const years=movements.map(r=>r.year),fByYear=new Map(renewalFactory.rows.map(r=>[r.year,r])),L=renewalFactory.last||{};
...
function prepExtendedDecisionHtml(visit,farm,x,hist,rolling,d,{forPrint=false}={}){
  ...
  const strengths=d.themes.filter(t=>Number.isFinite(t.score)&&t.score>=75).sort((a,b)=>b.score-a.score).slice(0,5),risks=d.alerts.slice(0,5),priorities=risks.slice(0,3);
  ...
  return `<div class="decision-extended ${forPrint?'for-print':''}"><div class="decision-onepage"><h3>Lecture en 30 secondes</h3><div class="grid cols-2"><div><h4>Points forts</h4>${strengths.length?`<ul>${strengths.map(t=>`<li><strong>${escapeHtml(t.label)}</strong> — ${escapeHtml(t.why)}</li>`).join('')}</ul>`:'<p class="muted">Pas assez de données pour dégager 5 points forts.</p>'}</div><div><h4>Points à risque</h4>${risks.length?`<ul>${risks.map(a=>`<li><strong>${escapeHtml(a.theme)}</strong> — ${escapeHtml(a.title)}</li>`).join('')}</ul>`:'<p>Aucun risque prioritaire fort détecté.</p>'}</div></div><h4>3 priorités de visite</h4>${priorities.length?`<ol>${priorities.map(a=>`<li>${escapeHtml(a.title)} — <span class="muted">${escapeHtml(a.detail)}</span></li>`).join('')}</ol>`:'<p class="muted">À définir après revue des données.</p>'}</div>
...
function prepDecisionSupportData(visit,farm,x,hist,rolling){
  ...
  if(rolling?.firstCalvingMean!=null&&rolling.firstCalvingMean>36)add(3,'Renouvellement',`1er vêlage tardif : ${rolling.firstCalvingMean} mois`,'Moyenne sur les 12 derniers mois','problems',rolling.firstCalvingAges?.length||null);
  else if(rolling?.firstCalvingMean!=null&&rolling.firstCalvingMean>30)add(2,'Renouvellement',`Âge au 1er vêlage à surveiller : ${rolling.firstCalvingMean} mois`,'Moyenne sur les 12 derniers mois','problems',rolling.firstCalvingAges?.length||null);
...
function prepDecisionSupportHtml(visit,farm,x,hist,rolling,{forPrint=false}={}){
  ...
  const alertHtml=d.alerts.length?d.alerts.map(a=>`<div class="decision-alert sev-${a.severity}"><div><strong>${escapeHtml(a.theme)} — ${escapeHtml(a.title)}</strong><small>${escapeHtml(a.detail)}</small></div>${!forPrint&&a.key?`<button type="button" class="btn small secondary" data-prep-repro-list="${a.key}">Voir les animaux</button>`:''}</div>`).join(''):'<div class="notice success">Aucune alerte prioritaire forte détectée avec les données disponibles.</div>';
  return `<section class="decision-support ${forPrint?'for-print':''}"><h2>${forPrint?'0. Synthèse prioritaire avant visite':'🎯 Ce qui mérite d’être creusé'}</h2><p class="muted">Lecture d’aide à la décision, non diagnostique : elle croise les données disponibles et indique où commencer la visite.</p><div class="decision-theme-grid">${d.themes.map(t=>`<article class="decision-theme ${status(t.score)}"><strong>${escapeHtml(t.label)}</strong><span>${t.score==null?'À évaluer pendant la visite':`${t.score}/100 · ${label(t.score)}`}</span><small>${escapeHtml(t.why)}</small></article>`).join('')}</div><h3>Alertes prioritaires</h3><div class="decision-alert-list">${alertHtml}</div>${prepAnalysisComparisonHtml(hist,{forPrint})}${prepExtendedDecisionHtml(visit,farm,x,hist,rolling,d,{forPrint})}</section>`;
}
...
// v14.6.21.89 — estimation économique pré-visite, calculée uniquement sur les données déjà disponibles.
...
// v14.6.21.89 — check-up, suivi d'étude, économie et conservation globale du défilement horizontal
...
// v14.6.21.89 — suivi du temps par intervenant + temps GDS préparation/terrain/compte rendu/suivi

// v14.6.21.89 — questionnaires persistants, repères de saisie, bilan sanitaire/éco partenaire, supports papier, check-up responsive.

window.addEventListener('DOMContentLoaded',()=>{initGlobalSearch();initAccordionMemory();});
// v14.6.21.89 — cohérence, imprimables préparation, animaux présents, recherche globale.
