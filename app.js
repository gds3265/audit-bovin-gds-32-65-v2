...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Quelques repères visuels utiles, intégrés à l’application et disponibles hors ligne.</div></div><span class="badge autosave">v14.6.1</span></div>
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.6.1</span></div>
...
function renderVisits() {
  ...
  app.innerHTML = `
    <div class="section-title"><div><h2>Visites</h2><div class="muted">Toutes les visites enregistrées restent accessibles, y compris les plus anciennes.</div></div><span class="badge autosave">v14.6.1</span></div>
  ...
  app.querySelectorAll('[data-delete-visit]').forEach(b=>b.onclick=()=>{if(confirm('Supprimer cette visite et tous ses sujets ?')){db.visits=db.visits.filter(v=>v.id!==b.dataset.deleteVisit);if(activeVisitId===b.dataset.deleteVisit)setActiveVisit('');saveDatabase(db);renderVisits();}});
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.6.1</span></div>
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.6.1</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
...
function visitReproductionSnapshot(visit){
  ...
  const cows=currentReproductionCows(farm),present=(farm.herdRegistry||[]).filter(a=>a.sex==='F'&&isRegistryAnimalPresent(a)),heifers=present.filter(a=>!cows.some(r=>normalizeAnimalId(r.cow.id)===normalizeAnimalId(a.id)));
  const allIvvs=cows.flatMap(r=>r.intervals),calves=cows.flatMap(r=>r.calves),dead=calves.filter(c=>c.exitCause==='M'&&c.exitDate&&daysBetweenDates(c.birthDate,c.exitDate)<183);
  const malesPresent=new Set((farm.herdRegistry||[]).filter(a=>a.sex==='M'&&isRegistryAnimalPresent(a)).map(a=>normalizeAnimalId(a.id)));
  const knownFather=calves.filter(c=>c.fatherId);const probableIA=knownFather.filter(c=>!malesPresent.has(normalizeAnimalId(c.fatherId))).length;
  const a=ensureAuditGlobal(visit),mortTotal=Object.values(a.mortality||{}).reduce((n,x)=>n+(Number(x?.count)||0),0);
  const rr=renewalRate(a.renewal),rf=reformRate(a.renewal);
  farm.herdRegistry=original;
  return {ivvMean:allIvvs.length?Math.round(allIvvs.reduce((x,y)=>x+y,0)/allIvvs.length):null,ivvLe400:allIvvs.filter(x=>x<=400).length,ivv401450:allIvvs.filter(x=>x>400&&x<=450).length,ivv451500:allIvvs.filter(x=>x>450&&x<=500).length,ivvOver500:allIvvs.filter(x=>x>500).length,cows:cows.length,withoutCalving:heifers.length,calves:calves.length,deadCalves:dead.length,calfMortalityRate:calves.length?Math.round(dead.length/calves.length*1000)/10:null,probableIA:knownFather.length?probableIA:null,probableIARate:knownFather.length?Math.round(probableIA/knownFather.length*1000)/10:null,renewalRate:rr,reformRate:rf,mortalityTotal:mortTotal};
}
function compareScalarRows(selected){
 const defs=[['Reproduction','IVV moyen','ivvMean','j'],['Reproduction','IVV ≤ 400 j','ivvLe400',''],['Reproduction','IVV 401–450 j','ivv401450',''],['Reproduction','IVV 451–500 j','ivv451500',''],['Reproduction','IVV > 500 j','ivvOver500',''],['Reproduction','Femelles présentes sans vêlage','withoutCalving',''],['Reproduction','Veaux probablement issus d’IA','probableIA',''],['Reproduction','Part probable IA','probableIARate','%'],['Reproduction','Mortalité veaux < 6 mois','calfMortalityRate','%'],['Troupeau','Taux de renouvellement','renewalRate','%'],['Troupeau','Taux de réforme','reformRate','%'],['Mortalité','Mortalité totale saisie','mortalityTotal','']];
...
function renderFollowup(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Historique & évolution</h2><div class="muted">Comparer deux visites d’une même exploitation : reproduction, mortalité, mesures, alimentation, bâtiment et plans d’action.</div></div><span class="badge autosave">v14.6.1</span></div>
...
function renderReproduction(){
  ...
  const reproFarm={...farm,herdRegistry:registry},today=new Date().toISOString().slice(0,10),presentFemales=registry.filter(a=>a.sex==='F'&&isRegistryAnimalPresent(a,today)),currentCows=currentReproductionCows(reproFarm),cowIds=new Set(currentCows.map(r=>normalizeAnimalId(r.cow.id))),presentHeifers=presentFemales.filter(a=>!cowIds.has(normalizeAnimalId(a.id)));
  const allIvvs=currentCows.flatMap(r=>r.intervals),allCalves=currentCows.flatMap(r=>r.calves),deadCalves=allCalves.filter(c=>c.exitCause==='M'&&c.exitDate&&daysBetweenDates(c.birthDate,c.exitDate)<183),malesPresent=new Set(registry.filter(a=>a.sex==='M'&&isRegistryAnimalPresent(a,today)).map(a=>normalizeAnimalId(a.id))),knownFather=allCalves.filter(c=>c.fatherId),probableIA=knownFather.filter(c=>!malesPresent.has(normalizeAnimalId(c.fatherId)));
  const stats={ivvMean:allIvvs.length?Math.round(allIvvs.reduce((a,b)=>a+b,0)/allIvvs.length):null,firstMean:(()=>{const v=currentCows.map(r=>r.firstCalvingAgeMonths).filter(x=>x!=null);return v.length?Math.round(v.reduce((a,b)=>a+b,0)/v.length*10)/10:null})()};
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Reproduction</h2><div class="muted">IVV, femelles sans vêlage, mortalité des veaux et origine probable IA.</div></div><span class="badge autosave">v14.6.1</span></div>${activeVisitBanner(visit)}<section class="card"><div class="row"><div class="field"><label>Exploitation</label><select id="repro-farm" ${visit?'disabled':''}>${db.farms.map(f=>`<option value="${f.id}" ${f.id===selectedFarmId?'selected':''}>${escapeHtml(f.name)}</option>`).join('')}</select></div><div class="field"><label>${meta?'Remplacer le registre bovins CSV':'Importer le registre bovins CSV'}</label><input id="repro-file" type="file" accept=".csv,text/csv"></div></div><div class="muted">${registry.length?`${registry.length} bovin(s) historiques · ${presentFemales.length} femelle(s) présente(s) · ${currentCows.length} vache(s) avec vêlage`:'Aucun registre importé pour cette visite.'}</div></section>${!registry.length?'<section class="empty" style="margin-top:16px">Importez le fichier CSV Reproduction.</section>':`<section class="repro-kpi-grid"><article class="card metric"><strong>${stats.ivvMean??'—'}</strong><span>IVV moyen</span><small>jours</small></article><article class="card metric"><strong>${stats.firstMean??'—'}</strong><span>Âge moyen au 1er vêlage</span><small>mois</small></article><article class="card metric"><strong>${presentHeifers.length}</strong><span>Femelles présentes sans vêlage</span></article><article class="card metric"><strong>${allCalves.length?Math.round(deadCalves.length/allCalves.length*1000)/10:'—'}%</strong><span>Mortalité veaux &lt; 6 mois</span><small>${deadCalves.length}/${allCalves.length}</small></article><article class="card metric"><strong>${knownFather.length?probableIA.length:'—'}</strong><span>Veaux probablement issus d’IA</span><small>${knownFather.length?`${Math.round(probableIA.length/knownFather.length*1000)/10}% des pères renseignés`:'Colonne père absente ou vide'}</small></article></section><section class="card"><h3>Répartition de tous les IVV calculés</h3><div class="ivv-band-grid">${ivvBands.map(([l,n])=>`<div><strong>${n}</strong><span>${l}</span></div>`).join('')}</div></section><section class="card"><div class="row"><div class="field"><label>Filtre</label><select id="repro-filter"><option value="all">Toutes les vaches présentes</option><option value="400">Sans vêlage depuis plus de 400 j</option><option value="ivvLe400">IVV moyen ≤ 400 j</option><option value="ivv401to450">IVV moyen 401–450 j</option><option value="ivv451to500">IVV moyen 451–500 j</option><option value="ivvOver500">IVV moyen &gt; 500 j</option><option value="dead">Au moins 2 veaux morts avant 6 mois</option><option value="under28">1er vêlage avant 28 mois</option><option value="28to36">1er vêlage 28–36 mois</option><option value="over36">1er vêlage après 36 mois</option></select></div><div class="field"><label>Classement</label><select id="repro-sort"><option value="lastCalvingOld">Dernier vêlage le plus ancien</option><option value="ivvHigh">IVV moyen le plus élevé</option><option value="firstAgeHigh">1er vêlage le plus tardif</option><option value="deadHigh">Mortalité veaux la plus élevée</option><option value="scoreLow">Score le plus faible</option></select></div></div><div class="table-wrap"><table><thead><tr><th>Vache</th><th>Âge / race</th><th>1er vêlage</th><th>Dernier vêlage</th><th>IVV complet</th><th>Veaux</th><th>Score</th><th></th></tr></thead><tbody>${rows.map(r=>`<tr><td><strong>${escapeHtml(r.cow.workNumber||r.cow.id)}</strong><br><small>${escapeHtml(r.cow.id)}</small></td><td>${ageLabelAt(r.cow.birthDate,today)||'—'}<br><small>Race ${escapeHtml(r.cow.breed||'—')}</small></td><td>${r.firstCalvingDate?formatDate(r.firstCalvingDate):'—'}<br><small>${r.firstCalvingAgeMonths!=null?`${r.firstCalvingAgeMonths} mois`:'—'}</small></td><td>${r.lastCalvingDate?formatDate(r.lastCalvingDate):'—'}<br><small>${r.daysSinceLast??'—'} j</small></td><td>Moy. ${r.meanIVV??'—'} · dernier ${r.lastIVV??'—'}<br><small>${r.intervals.length?r.intervals.join(' / ')+' j':'Un seul vêlage'}</small></td><td>${r.calves.length}<br><small>${r.deadBefore6.length} mort(s) &lt;6 mois</small></td><td><button class="badge ${reproductionScore(r)>=75?'complete':reproductionScore(r)>=60?'in-progress':'danger'}" data-repro-score="${escapeHtml(r.cow.id)}">${reproductionScore(r)}/100</button></td><td><button class="btn small" data-repro-detail="${escapeHtml(r.cow.id)}">Voir</button></td></tr>`).join('')||'<tr><td colspan="8">Aucun animal correspondant.</td></tr>'}</tbody></table></div></section><section class="card"><div class="section-title"><div><h3>Femelles présentes sans vêlage</h3><div class="muted">Exclues du calcul des IVV.</div></div><span class="badge">${presentHeifers.length}</span></div>${presentHeifers.length?`<div class="table-wrap"><table><thead><tr><th>Femelle</th><th>Date de naissance</th><th>Âge</th><th>Race</th><th>Alerte</th></tr></thead><tbody>${presentHeifers.map(g=>{const age=monthsBetweenDates(g.birthDate,today);return `<tr><td><strong>${escapeHtml(g.workNumber||g.id)}</strong><br><small>${escapeHtml(g.id)}</small></td><td>${g.birthDate?formatDate(g.birthDate):'—'}</td><td>${ageLabelAt(g.birthDate,today)||'—'}</td><td>${escapeHtml(g.breed||'—')}</td><td>${age!=null&&age>36?'<span class="badge danger">Plus de 36 mois</span>':age!=null&&age>28?'<span class="badge in-progress">À surveiller</span>':'—'}</td></tr>`}).join('')}</tbody></table></div>`:'<div class="empty">Aucune femelle présente sans vêlage.</div>'}</section><section id="repro-detail"></section>`}`;
  ...
  const fi=document.getElementById('repro-file');if(fi)fi.onchange=async()=>{const file=fi.files?.[0];if(!file)return;try{const animals=importHerdRegistryRows(parseSemicolonCsv(await file.text())),importedAt=new Date().toISOString(),period=reproductionRegistryPeriod(animals);if(visit){visit.reproductionRegistry=JSON.parse(JSON.stringify(animals));visit.reproductionRegistrySource={fileName:file.name,importedAt,rowCount:animals.length,period};}farm.herdRegistry=JSON.parse(JSON.stringify(animals));farm.herdRegistryImportedAt=importedAt;farm.herdRegistrySource={fileName:file.name,importedAt,rowCount:animals.length,period};saveDatabase(db);showToast(`${animals.length} bovin(s) importé(s).`);renderReproduction()}catch(e){showToast(`Import impossible : ${e.message}`)}};
...
initWorkMode();
initGlobalSearch();
if ('serviceWorker' in navigator) navigator.serviceWorker.register('./sw.js?v=14.6.1',{updateViaCache:'none'}).then(r=>r.update()).catch(console.error);
render();
