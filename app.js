...
// v14.6.21.88 — Audits ciblés + motif libre + orientation syndromique veaux
...
function targetedAuditHtml(visit,{forPrint=false}={}){
  ...
  if(forPrint){return `<section class="targeted-audit-print"><h2>Priorités de visite — audit adapté</h2><p><b>Mode :</b> ${escapeHtml(modeLabel)}${t.problemNarrative?`<br><b>Problèmes signalés :</b> ${escapeHtml(t.problemNarrative)}`:''}</p>${p.priorities.length?`<ol>${p.priorities.map(x=>`<li><b>${escapeHtml(x.title)}</b> — ${escapeHtml(x.reason)}<br><small>${x.questions.map(q=>escapeHtml(q)).join(' · ')}</small></li>`).join('')}</ol>`:''}${syn?`<h3>Orientation syndromique veau — à confirmer sur le terrain</h3><p><b>Âge au début :</b> ≈ ${syn.ageDays} jours. <b>Ce bloc ne constitue pas un diagnostic.</b></p><p><b>Causes compatibles à explorer :</b> ${syn.causes.map(escapeHtml).join(' · ')}</p><p><b>Facteurs à vérifier :</b> ${syn.risk.map(escapeHtml).join(' · ')}</p>`:''}</section>`;}
  ...
  <div class="target-priorities">${p.priorities.map((x,i)=>`<article><span>${i+1}</span><div><strong>${escapeHtml(x.title)}</strong><small>${escapeHtml(x.reason)}</small><ul>${x.questions.map(q=>`<li>${escapeHtml(q)}</li>`).join('')}</ul>${x.section?`<button type="button" class="btn small secondary" data-target-open-section="${escapeHtml(x.section)}">Ouvrir cette rubrique</button>`:''}</div></article>`).join('')}</div>
...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Repères visuels, fiches pratiques et carnet technique, disponibles hors ligne.</div></div><span class="badge autosave">v14.6.21.88</span></div>
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.6.21.88</span></div>
...
function renderVisits() {
  ...
  app.innerHTML = `
    <div class="section-title"><div><h2>Visites</h2><div class="muted">Toutes les visites enregistrées restent accessibles, y compris les plus anciennes.</div></div><span class="badge autosave">v14.6.21.88</span></div>
...
function renderFarmerQuestionnaires(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📨 Questionnaires éleveur</h2><div class="muted">Préparer la visite puis récupérer uniquement les réponses encore manquantes.</div></div><span class="badge autosave">v14.6.21.88</span></div>${activeVisitBanner(visit)}
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.6.21.88</span></div>
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.6.21.88</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
  visit.feeding=visit.feeding&&typeof visit.feeding==='object'?visit.feeding:{rations:[],settings:{},history:[]};
  visit.feeding.nutrition=visit.feeding.nutrition&&typeof visit.feeding.nutrition==='object'?visit.feeding.nutrition:{};
...
function renderFollowup(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Historique & évolution</h2><div class="muted">Comparer deux visites d’une même exploitation : reproduction, mortalité, mesures, alimentation, bâtiment et plans d’action.</div></div><span class="badge autosave">v14.6.21.88</span></div>
...
// v14.6.21.88 — aide à la décision transversale : priorités, tendances et références internes.
...
// v14.6.21.88 — cohérence pré-visite, tableau global mouvements/renouvellement et âges lisibles.
...
// v14.6.21.88 — « fabrique du troupeau » : naissance femelles → conservation jusqu’à la reproduction → 1er vêlage → réforme.
...
// v14.6.21.88 — estimation économique pré-visite, calculée uniquement sur les données déjà disponibles.
...
// v14.6.21.88 — check-up, suivi d'étude, économie et conservation globale du défilement horizontal
...
// v14.6.21.88 — suivi du temps par intervenant + temps GDS préparation/terrain/compte rendu/suivi

// v14.6.21.88 — questionnaires persistants, repères de saisie, bilan sanitaire/éco partenaire, supports papier, check-up responsive.

window.addEventListener('DOMContentLoaded',()=>{initGlobalSearch();initAccordionMemory();});
// v14.6.21.88 — cohérence, imprimables préparation, animaux présents, recherche globale.
