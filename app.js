...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Repères visuels, fiches pratiques et carnet technique, disponibles hors ligne.</div></div><span class="badge autosave">v14.6.21.39</span></div>
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.6.21.39</span></div>
...
function renderVisits() {
  ...
  app.innerHTML = `
    <div class="section-title"><div><h2>Visites</h2><div class="muted">Toutes les visites enregistrées restent accessibles, y compris les plus anciennes.</div></div><span class="badge autosave">v14.6.21.39</span></div>
  ...
  app.querySelectorAll('[data-select-visit]').forEach(b=>b.onclick=()=>{setActiveVisit(b.dataset.selectVisit);app.querySelectorAll('.visit-history-table tbody tr').forEach(r=>r.classList.remove('active-history-row'));b.closest('tr')?.classList.add('active-history-row');showToast(`Visite du ${formatDate(db.visits.find(v=>v.id===b.dataset.selectVisit)?.date||'')} sélectionnée.`);});app.querySelectorAll('[data-edit-visit]').forEach(b=>b.onclick=()=>{setActiveVisit(b.dataset.editVisit);editingVisitId=b.dataset.editVisit;clearDraft();renderVisits();document.getElementById('visit-form')?.scrollIntoView({block:'start'});});
...
function renderFarmerQuestionnaires(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📨 Questionnaires éleveur</h2><div class="muted">Préparer la visite puis récupérer uniquement les réponses encore manquantes.</div></div><span class="badge autosave">v14.6.21.39</span></div>${activeVisitBanner(visit)}
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.6.21.39</span></div>
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.6.21.39</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
...
const saveA=(q,f,v)=>{a.answers[q]=a.answers[q]||{};a.answers[q][f]=v;saveAuditGlobal(visit)};
const bindAuditQuestionControls=(root)=>{
  ...
  root.querySelectorAll('[data-audit-answer]').forEach(e=>{const fn=()=>saveA(e.dataset.auditAnswer,'answer',e.value);e.addEventListener('input',fn);e.addEventListener('change',fn)});
  root.querySelectorAll('[data-audit-multi]').forEach(e=>e.onchange=()=>{const q=e.dataset.auditMulti;a.answers[q]=a.answers[q]||{};a.answers[q].values=[...root.querySelectorAll(`[data-audit-multi="${CSS.escape(q)}"]:checked`)].map(x=>x.value);saveAuditGlobal(visit)});
  root.querySelectorAll('[data-audit-comment]').forEach(e=>e.oninput=()=>saveA(e.dataset.auditComment,'comment',e.value));
  root.querySelectorAll('[data-summary]').forEach(e=>e.oninput=()=>{a.chapterSummaries[e.dataset.summary]=a.chapterSummaries[e.dataset.summary]||{};a.chapterSummaries[e.dataset.summary][e.dataset.summaryField]=e.value;saveAuditGlobal(visit)});
};
...
function renderFollowup(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Historique & évolution</h2><div class="muted">Comparer deux visites d’une même exploitation : reproduction, mortalité, mesures, alimentation, bâtiment et plans d’action.</div></div><span class="badge autosave">v14.6.21.39</span></div>
...
// v14.6.21.39 — navigation par workflow
...
// v14.6.21.39 — check-up, suivi d'étude, économie et conservation globale du défilement horizontal
...
// v14.6.21.39 — suivi du temps par intervenant + temps GDS préparation/terrain/compte rendu/suivi

// v14.6.21.39 — questionnaires persistants, repères de saisie, bilan sanitaire/éco partenaire, supports papier, check-up responsive.

window.addEventListener('DOMContentLoaded',()=>{initGlobalSearch();initAccordionMemory();});
// v14.6.21.39 — cohérence, imprimables préparation, animaux présents, recherche globale.
