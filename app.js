...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Repères visuels, fiches pratiques et carnet technique, disponibles hors ligne.</div></div><span class="badge autosave">v14.6.21.86</span></div>
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.6.21.86</span></div>
...
function renderVisits() {
  ...
  app.innerHTML = `
    <div class="section-title"><div><h2>Visites</h2><div class="muted">Toutes les visites enregistrées restent accessibles, y compris les plus anciennes.</div></div><span class="badge autosave">v14.6.21.86</span></div>
...
function renderFarmerQuestionnaires(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📨 Questionnaires éleveur</h2><div class="muted">Préparer la visite puis récupérer uniquement les réponses encore manquantes.</div></div><span class="badge autosave">v14.6.21.86</span></div>${activeVisitBanner(visit)}
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.6.21.86</span></div>
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.6.21.86</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
  visit.feeding=visit.feeding&&typeof visit.feeding==='object'?visit.feeding:{rations:[],settings:{},history:[]};
  visit.feeding.nutrition=visit.feeding.nutrition&&typeof visit.feeding.nutrition==='object'?visit.feeding.nutrition:{};
...
function renderFollowup(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Historique & évolution</h2><div class="muted">Comparer deux visites d’une même exploitation : reproduction, mortalité, mesures, alimentation, bâtiment et plans d’action.</div></div><span class="badge autosave">v14.6.21.86</span></div>
...
// v14.6.21.86 — aide à la décision transversale : priorités, tendances et références internes.
...
// v14.6.21.86 — analyses transversales complémentaires.
...
// v14.6.21.86 — « fabrique du troupeau » : naissance femelles → conservation jusqu’à la reproduction → 1er vêlage → réforme.
...
function prepRenewalFactoryHtml(data,{forPrint=false}={}){
  ...
  const L=data.last||{},summary=[
    ...
    ['Réformées après 1er vêlage',L.reformed??'—',L.reformAgeMean!=null?`âge moyen ${L.reformAgeMean} mois`:'']
  ];
...
function prepCrossHypotheses(visit,farm,x,hist,rolling,decision){
  ...
  if((intro?.entries||0)>0&&((domains['Santé / analyses']??100)<70||auditAttentionCount(visit,'sanitaire')>0))add('Introductions + sanitaire : vérifier la chronologie',[`${intro.entries} introduction(s) sur N`,`signal sanitaire/audit présent`],['origine et statut sanitaire','quarantaine et délai avant mélange','date d’apparition des problèmes par rapport aux entrées']);
  if((domains['Bâtiment']??100)<60&&auditAttentionCount(visit,'sanitaire')>0)add('Bâtiment / ambiance + pathologies : rechercher une cause environnementale',[`indice bâtiment ${domains['Bâtiment']}/100`,'points sanitaires à surveiller'],['ventilation et ammoniac','litière et zones humides','densité/couchage','lien avec respiratoire, mammites ou boiteries']);
  if((domains['Eau']??100)<60)add('Abreuvement : impact transversal possible',[`indice eau ${domains['Eau']}/100`],['débit réel aux heures de pointe','nombre de points d’eau et concurrence','propreté/accessibilité','impact sur ingestion, production et chaleur']);
  return h.slice(0,6);
}
...
function prepExtendedDecisionHtml(visit,farm,x,hist,rolling,d,{forPrint=false}={}){
  ...
  const strengths=d.themes.filter(t=>t.score>=75).sort((a,b)=>b.score-a.score).slice(0,5),risks=d.alerts.slice(0,5),priorities=risks.slice(0,3);
  ...
  <div class="decision-topic-grid"><article><h4>🥣 Alimentation — ration à risque</h4><p>${escapeHtml(feedText)}</p><small>Les champs énergie/protéines ne sont interprétés que s’ils sont renseignés.</small></article><article><h4>🩺 Sanitaire</h4><p>${escapeHtml(san.status)}</p><small>${san.cur} point(s) d’attention dans l’audit sanitaire${san.prevCount!=null?` · précédente visite : ${san.prevCount}`:''}.</small></article></div>
  ...
  ${prepMovementHistoryHtml(movements,{forPrint})}${prepRenewalFactoryHtml(renewalFactory,{forPrint})}${followHtml}</div>`;
}
...
function prepDecisionSupportData(visit,farm,x,hist,rolling){
  ...
  if(waterScore<60)add(2,'Eau / abreuvement',`Abreuvement à approfondir (indice ${waterScore}/100)`,'Les données existent déjà : vérifier surtout débit, accessibilité, propreté et concurrence aux points d’eau.');
  if(buildingScore<60)add(2,'Logement / litière',`Bâtiment à approfondir (indice ${buildingScore}/100)`,'Les données existent déjà : cibler ventilation, litière, densité, zones humides, couchage et circulation.');
  ...
  if(bioIssues)add(bioIssues>=2?3:2,'Biosécurité',`${bioIssues} point(s) de biosécurité à revoir`,'Introductions, quarantaine, isolement et maîtrise des accès doivent être recroisés avec les mouvements et le sanitaire.');
  ...
  const themes=[
    ...
    {label:'Veaux / colostrum',score:calfScore,why:youngAttention?`${youngAttention} point(s) d’attention dans la conduite des veaux`:'Pas de point majeur signalé dans le questionnaire'},
    ...
    {label:'Sanitaire',score:sanitaryThemeScore,why:sanitaryAttention?`${sanitaryAttention} point(s) à approfondir`:'Pas de point majeur signalé'},
    {label:'Biosécurité',score:biosafetyScore,why:bioIssues?`${bioIssues} point(s) à revoir`:'Introductions / isolement / accès à confirmer'},
    {label:'Alimentation',score:feedScore,why:feed.hasData?(feed.low.length?`${feed.low.length} déficit(s) potentiel(s) calculé(s)`:'Ration renseignée / couverture calculable'):'Ration à renseigner'},
    {label:'Eau / abreuvement',score:waterScore,why:'Débit, accès, propreté et disponibilité'},
    {label:'Logement / litière',score:buildingScore,why:'Ventilation, litière, densité et ambiance'}
  ];
...
function prepDecisionSupportHtml(visit,farm,x,hist,rolling,{forPrint=false}={}){
  const d=prepDecisionSupportData(visit,farm,x,hist,rolling),status=s=>s>=75?'good':s>=55?'warn':'bad',label=s=>s>=75?'Satisfaisant':s>=55?'À surveiller':'À creuser';
  ...
  return `<section class="decision-support ${forPrint?'for-print':''}"><h2>${forPrint?'0. Synthèse prioritaire avant visite':'🎯 Ce qui mérite d’être creusé'}</h2><p class="muted">Lecture d’aide à la décision, non diagnostique : elle croise les données disponibles et indique où commencer la visite.</p><div class="decision-theme-grid">${d.themes.map(t=>`<article class="decision-theme ${status(t.score)}"><strong>${escapeHtml(t.label)}</strong><span>${t.score}/100 · ${label(t.score)}</span><small>${escapeHtml(t.why)}</small></article>`).join('')}</div><h3>Alertes prioritaires</h3><div class="decision-alert-list">${alertHtml}</div>${prepAnalysisComparisonHtml(hist,{forPrint})}${prepExtendedDecisionHtml(visit,farm,x,hist,rolling,d,{forPrint})}</section>`;
}
...
// v14.6.21.86 — estimation économique pré-visite, calculée uniquement sur les données déjà disponibles.
...
// v14.6.21.86 — check-up, suivi d'étude, économie et conservation globale du défilement horizontal
...
// v14.6.21.86 — suivi du temps par intervenant + temps GDS préparation/terrain/compte rendu/suivi

// v14.6.21.86 — questionnaires persistants, repères de saisie, bilan sanitaire/éco partenaire, supports papier, check-up responsive.

window.addEventListener('DOMContentLoaded',()=>{initGlobalSearch();initAccordionMemory();});
// v14.6.21.86 — cohérence, imprimables préparation, animaux présents, recherche globale.
