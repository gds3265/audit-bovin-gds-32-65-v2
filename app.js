...
const libraryPdfCatalog = [
  {groups:['protocoles'],kind:'Fiche thématique',title:'Approche globale & protocole',file:'theme-approche-globale.pdf',thumb:'theme-approche-globale.jpg',note:'Repères + visuels/mémo terrain.'},
  {groups:['animaux'],kind:'Fiche thématique',title:'Observation du troupeau & hydratation',file:'theme-observation-troupeau.pdf',thumb:'theme-observation-troupeau.jpg',note:'Observation adultes/veaux, températures et signes pratiques de déshydratation.'},
  {groups:['animaux','eau'],kind:'Fiche thématique',title:'Urines & pilotage de l’hydratation',file:'theme-urines.pdf',thumb:'theme-urines.jpg',note:'pH, densité, Brix, Redox, échantillonnage et suivi de l’hydratation.'},
  {groups:['sang'],kind:'Fiche thématique',title:'Sang',file:'theme-sang.pdf',thumb:'theme-sang.jpg',note:'Glycémie, BOH et interprétation rapide.'},
  {groups:['fourrages'],kind:'Fiche thématique',title:'Fèces & digestion',file:'theme-feces-digestion.pdf',thumb:'theme-feces-digestion.jpg',note:'Observation, pH/Redox et tamisage.'},
  {groups:['eau','courants','animaux'],kind:'Fiche thématique',title:'Eau, abreuvement & courants parasites',file:'theme-eau-abreuvement.pdf',thumb:'theme-eau-abreuvement.jpg',note:'Besoins en eau, accessibilité, types d’abreuvoirs, qualité de l’eau et tensions parasites.'},
  {groups:['eau','animaux','courants','protocoles'],kind:'Fiche enrichie',title:'Hydratation des ruminants en bâtiment',file:'theme-hydratation-ruminants.pdf',note:'Synthèse GDS 32-65 : signes de déshydratation, urines, besoins, accès à l’eau, qualité et courants parasites.'},
  ...
  {groups:['colostrum'],kind:'Fiche thématique',title:'Lait, colostrum & veaux',file:'theme-lait-colostrum-veaux.pdf',thumb:'theme-lait-colostrum-veaux.jpg',note:'Colostrum, transfert et conduite des veaux.'},
  {groups:['sol-plantes'],kind:'Fiche thématique',title:'Sol',file:'theme-sol.pdf',thumb:'theme-sol.jpg',note:'Repères et mesures du sol.'},
  {groups:['sol-plantes'],kind:'Fiche thématique',title:'Plantes, pâturage & herbe',file:'theme-plantes-paturage.pdf',thumb:'theme-plantes-paturage.jpg',note:'Repères plante/herbe et contexte de prélèvement.'},
  {groups:['fourrages','nutrition'],kind:'Fiche thématique',title:'Fourrages & alimentation',file:'theme-fourrages-alimentation.pdf',thumb:'theme-fourrages-alimentation.jpg',note:'Fourrages, ration et interprétation terrain.'},
  {groups:['protocoles','appareils'],kind:'Fiche thématique',title:'Outils, étalonnage & prélèvements',file:'theme-outils-prelevements.pdf',thumb:'theme-outils-prelevements.jpg',note:'Bonnes pratiques de mesure et prélèvement.'},
  {groups:['environnement'],kind:'Fiche thématique',title:'Litière & environnement',file:'theme-litiere-environnement.pdf',thumb:'theme-litiere-environnement.jpg',note:'Litière, ambiance et environnement.'},
...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Repères visuels, fiches pratiques et carnet technique, disponibles hors ligne.</div></div><span class="badge autosave">v14.6.21.5</span></div>
...
function render() {
  const renderers = { dashboard: renderDashboard, farms: renderFarms, journal: renderJournalSuivi, documents: renderFarmDocuments, visits: renderVisits, questionnaires: renderFarmerQuestionnaires, animals: renderAnimals, analysis: renderAnalysis, assistant: renderAssistantGDS, feeding: renderFeeding, nutrition: renderNutritionAnalysis, reproduction: renderReproduction, building: renderBuilding, audit: renderAuditGlobal, planches: renderPlanches, photos: renderPhotos, herddata: renderHerdData, followup: renderFollowup, pilotage: renderPilotageActions, reports: renderReports, backup: renderBackup };
  app.innerHTML = '';
  try {
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.6.21.5</span></div>
...
function renderVisits() {
  ...
  app.innerHTML = `
    <div class="section-title"><div><h2>Visites</h2><div class="muted">Toutes les visites enregistrées restent accessibles, y compris les plus anciennes.</div></div><span class="badge autosave">v14.6.21.5</span></div>
...
// v14.6.21.5 — Pré-questionnaire éleveur / questions restantes
...
const farmerQuestionnaireSections=[
  ...
  {id:'economics',title:'💶 Données technico-économiques non importées',default:false}
];
...
function answerAlreadyKnown(visit,q){
  ...
  if(t.kind==='audit'){const x=a.answers[t.question]||{};return !!(x.answer||(Array.isArray(x.values)&&x.values.length)||x.comment)}
  if(t.kind==='building'){
...
function buildFarmerQuestionnaireSchema(visit,type,selectedIds){
  ...
  return {version:1,type,farmName:farm.name||'',farmer:farm.farmer||farm.manager||'',visitDate:visit.date||'',visitId:visit.id,farmId:visit.farmId,title:type==='remaining'?'Questions restantes après la visite':'Pré-questionnaire de préparation de visite',intro:type==='remaining'?'Merci de compléter uniquement les éléments encore manquants.':'Les effectifs, mouvements, mortalités et données de reproduction sont récupérés par import lorsqu’ils sont disponibles : ils ne sont donc pas redemandés ici.',sections};
}
...
async function createFarmerQuestionnaire(visit,type,selectedIds){
  const token=secureQuestionnaireToken(),schema=buildFarmerQuestionnaireSchema(visit,type,selectedIds);if(!schema.sections.some(s=>s.questions.length))throw new Error('Aucune question à envoyer avec cette sélection.');
...
function renderFarmerQuestionnaires(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📨 Questionnaires éleveur</h2><div class="muted">Préparer la visite puis récupérer uniquement les réponses encore manquantes.</div></div><span class="badge autosave">v14.6.21.5</span></div>${activeVisitBanner(visit)}
  ...
  <section class="grid cols-2"><article class="card"><h3>📤 Pré-questionnaire</h3><p>À envoyer avant la visite. Les analyses et documents disponibles sont proposés en premier.</p><div class="questionnaire-section-selector">${farmerQuestionnaireSections.map(x=>`<label><input type="checkbox" data-fq-section value="${x.id}" ${x.default?'checked':''}><span>${x.title}</span></label>`).join('')}</div><button class="btn primary" id="create-pre-questionnaire">Créer et partager</button></article><article class="card"><h3>📤 Questions restantes</h3><p>Génère uniquement les questions encore non renseignées dans l’audit, le bâtiment et les données complémentaires.</p><button class="btn primary" id="create-remaining-questionnaire">Créer les questions restantes</button></article></section>
  ...
  document.getElementById('create-pre-questionnaire').onclick=async()=>{const ids=[...app.querySelectorAll('[data-fq-section]:checked')].map(x=>x.value);try{const q=await createFarmerQuestionnaire(visit,'pre',ids);showToast('Pré-questionnaire créé.');await shareQuestionnaire(q);renderFarmerQuestionnaires()}catch(e){showToast(`Création impossible : ${e.message}`)}};
  document.getElementById('create-remaining-questionnaire').onclick=async()=>{try{const q=await createFarmerQuestionnaire(visit,'remaining',farmerQuestionnaireSections.map(x=>x.id));showToast('Questionnaire des éléments manquants créé.');await shareQuestionnaire(q);renderFarmerQuestionnaires()}catch(e){showToast(`Création impossible : ${e.message}`)}};
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.6.21.5</span></div>
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.6.21.5</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
...
function renderBuildingQuestionnaire(panel,{visit,audit}){
  ...
  panel.querySelectorAll('[data-open-building-group]').forEach(b=>b.onclick=()=>{const d=panel.querySelector(`[data-building-group="${b.dataset.openBuildingGroup}"]`);if(d){panel.querySelectorAll('.question-group').forEach(other=>{if(other!==d)other.open=false;});d.open=true;scrollBuildingGroupTop(d);}});
...
function renderFollowup(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Historique & évolution</h2><div class="muted">Comparer deux visites d’une même exploitation : reproduction, mortalité, mesures, alimentation, bâtiment et plans d’action.</div></div><span class="badge autosave">v14.6.21.5</span></div>
...
function reportReasoningHtml(visit){
  ...
  return cards.map(h=>`<article class="report-reason"><h3>${escapeHtml(h.title)} <small>— ${escapeHtml(h.category)}</small></h3><p><strong>Confiance :</strong> ${escapeHtml(h.confidence.label)} · ${h.sourceCount} source(s) — ${(h.evidence||[]).length} élément(s) en faveur, ${(h.nuance||[]).length} prudence/contradiction(s), ${(h.missing||[]).length} donnée(s) manquante(s).</p><p>${escapeHtml(h.summary)}</p>${h.mechanism?`<p><strong>Ce que cette piste peut traduire :</strong> ${escapeHtml(h.mechanism)}</p>`:''}<div class="report-columns"><div><h4>Éléments en faveur</h4>${reportList((h.evidence||[]).join('\n'))}</div><div><h4>Prudence / contradictions</h4>${reportList((h.nuance||[]).join('\n'))}</div><div><h4>Facteurs à examiner</h4>${reportList((h.causes||[]).join('\n'))}</div><div><h4>Données manquantes</h4>${reportList((h.missing||[]).join('\n'))}</div></div>${h.state.note?`<p><strong>Commentaire du technicien :</strong> ${escapeHtml(h.state.note)}</p>`:''}</article>`).join('');
}
...
initWorkMode();
initGlobalSearch();
if ('serviceWorker' in navigator) navigator.serviceWorker.register('./sw.js?v=14.6.17',{updateViaCache:'none'}).then(r=>r.update()).catch(console.error);
render();
