...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Repères visuels, fiches pratiques et carnet technique, disponibles hors ligne.</div></div><span class="badge autosave">v14.6.21.36</span></div>
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.6.21.36</span></div>
...
function renderVisits() {
  ...
  app.innerHTML = `
    <div class="section-title"><div><h2>Visites</h2><div class="muted">Toutes les visites enregistrées restent accessibles, y compris les plus anciennes.</div></div><span class="badge autosave">v14.6.21.36</span></div>
...
function derivedAuditValue(visit,label){
  try{
    const snap=visitReproductionSnapshot(visit)||{};
    const map={
      'Intervalle vêlage-vêlage':snap.ivvMean,'Âge moyen au premier vêlage':snap.firstCalvingAgeMean,
      'Mortalité veaux (%)':snap.calfMortalityRate,'Nombre moyen de vaches sur exercice':snap.cows,
      'Brix colostrum':partnerAverageMeasure(visit,'colostrumBrix')
    };
    if(map[label]!==undefined&&map[label]!==null)return map[label];
    const item=linkedHerdImportForVisit(visit),mort=item?.years?.N?.mortality||{},mv=item?.current?.movements||{},rep=item?.years?.N?.reproduction||{};
    if(label==='Avortements (nombre/an)'&&rep.abortions!==undefined)return rep.abortions;
    if(label==='Mortalité adultes (%)'){const cows=snap.cows||0,ad=partnerExcelNumber(mort.over24)||0;if(cows)return Math.round(ad/cows*1000)/10;}
    return null;
  }catch(e){return null}
}
...
function renderFarmerQuestionnaires(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📨 Questionnaires éleveur</h2><div class="muted">Préparer la visite puis récupérer uniquement les réponses encore manquantes.</div></div><span class="badge autosave">v14.6.21.36</span></div>${activeVisitBanner(visit)}
...
function preparedAuditBody(visit,blank=false){
 const a=ensureAuditGlobal(visit);return `<h1>${blank?'Modèle vierge':'Support de visite préparé'}</h1>${activeFarmNameForPrint(visit)}${auditGlobalSections.map(sec=>`<h2>${sec.icon} ${escapeHtml(sec.title)}</h2>${sec.questions.map(q=>{const m=auditQuestionSource(q),v=blank?'':(a.answers?.[q]?.answer||preparedQuestionValue(visit,q));return `<div class="q"><span class="source ${m.key}">${m.label}</span><b>${escapeHtml(q)}</b>${v!==''?`<div class="known">Déjà disponible : ${escapeHtml(v)}</div>`:'<div class="line"></div>'}</div>`}).join('')}`).join('')}`;
}
...
function printPreparedAudit(blank=false){const v=activeVisit();if(!v)return showToast('Ouvrez une visite.');paperWindow(blank?'Audit bovin — modèle vierge':'Audit bovin — support préparé',preparedAuditBody(v,blank));}
function printReproMortalityPrep(){const v=activeVisit();if(!v)return;const f=db.farms.find(x=>x.id===v.farmId),src=reproductionSourceForVisit(v,f),reg=src.registry||[],rf={...f,herdRegistry:reg},date=v.date||new Date().toISOString().slice(0,10),cows=currentReproductionCows(rf,date),present=reg.filter(a=>isRegistryAnimalPresent(a,date));const calves=cows.flatMap(r=>r.calves||[]),dead=calves.filter(c=>c.exitCause==='M'&&c.exitDate&&daysBetweenDates(c.birthDate,c.exitDate)<183),problems=cows.filter(r=>(r.daysSinceLast||0)>400||(r.meanIVV||0)>450||(r.deadBefore6||[]).length>=2||(r.firstCalvingAgeMonths||0)>36);const body=`<h1>Bilan préparatoire — reproduction, mortalité et vaches à problèmes</h1>${activeFarmNameForPrint(v)}<p><b>Animaux présents dans le registre :</b> ${present.length} &nbsp; <b>Vaches avec vêlage :</b> ${cows.length} &nbsp; <b>Mortalité veaux &lt; 6 mois :</b> ${calves.length?Math.round(dead.length/calves.length*1000)/10:'—'} % (${dead.length}/${calves.length})</p><h2>Vaches à problèmes</h2><table><thead><tr><th>Animal</th><th>Âge</th><th>Dernier vêlage</th><th>IVV moyen</th><th>1er vêlage</th><th>Veaux morts &lt;6m</th></tr></thead><tbody>${problems.map(r=>`<tr><td>${escapeHtml(r.cow.workNumber||r.cow.id)}${r.cow.name?` · ${escapeHtml(r.cow.name)}`:''}</td><td>${escapeHtml(ageLabelAt(r.cow.birthDate,date)||'—')}</td><td>${r.lastCalvingDate?formatDate(r.lastCalvingDate):'—'}</td><td>${r.meanIVV??'—'}</td><td>${r.firstCalvingAgeMonths??'—'} mois</td><td>${(r.deadBefore6||[]).length}</td></tr>`).join('')||'<tr><td colspan="6">Aucune alerte selon les critères actuels.</td></tr>'}</tbody></table>`;paperWindow('Bilan reproduction et mortalité',body)}
function printPresentAnimals(){const v=activeVisit();if(!v)return;const f=db.farms.find(x=>x.id===v.farmId),src=reproductionSourceForVisit(v,f),reg=(src.registry||[]).filter(a=>isRegistryAnimalPresent(a,v.date)),rf={...f,herdRegistry:src.registry||[]};const rows=reg.map(a=>{const r=reproductionForCow(rf,a.id);return `<tr><td>${escapeHtml(a.workNumber||'')}</td><td>${escapeHtml(a.id)}</td><td>${escapeHtml(a.name||'')}</td><td>${escapeHtml(ageLabelAt(a.birthDate,v.date)||'—')}</td><td>${escapeHtml(a.breed||'—')}</td><td>${r?.lastCalvingDate?formatDate(r.lastCalvingDate):'—'}</td><td>${r?.meanIVV??'—'}</td><td>${r?.calves?.length??0}</td></tr>`}).join('');paperWindow('Animaux présents',`<h1>Listing des animaux présents</h1>${activeFarmNameForPrint(v)}<table><thead><tr><th>N° travail</th><th>Boucle</th><th>Nom</th><th>Âge</th><th>Race</th><th>Dernier vêlage</th><th>IVV moyen</th><th>Rang / veaux</th></tr></thead><tbody>${rows||'<tr><td colspan="8">Aucun registre présent disponible.</td></tr>'}</tbody></table>`)}
function renderPreparationPrints(){const v=activeVisit();if(!v){renderNoActiveVisit('Documents imprimables');return}app.innerHTML=`<div class="section-title"><div><h2>🖨️ Documents imprimables — préparation de visite</h2><div class="muted">À imprimer avant de partir. Le modèle vierge ne contient aucune donnée ; les autres documents reprennent automatiquement ce qui est déjà connu.</div></div><span class="badge autosave">v14.6.21.36</span></div>${activeVisitBanner(v)}<section class="print-prep-grid"><article class="card print-prep-card"><h3>📄 Modèle vierge</h3><p>Questions uniquement, sans données préremplies.</p><button class="btn" id="pp-blank">Ouvrir / imprimer</button></article><article class="card print-prep-card"><h3>📝 Support préparé</h3><p>Toutes les questions, avec les réponses et données déjà disponibles mises en évidence.</p><button class="btn" id="pp-prepared">Ouvrir / imprimer</button></article><article class="card print-prep-card"><h3>🐄 Repro + mortalité + alertes</h3><p>Indicateurs calculés à partir du fichier lié et vaches à problèmes.</p><button class="btn" id="pp-repro">Ouvrir / imprimer</button></article><article class="card print-prep-card"><h3>📋 Animaux présents</h3><p>Listing des animaux présents uniquement, avec les principales données de fiche.</p><button class="btn" id="pp-animals">Ouvrir / imprimer</button></article></section><section class="card"><h3>📷 Import d’un audit papier</h3><div class="paper-import-zone"><p>Photographiez ou joignez le formulaire papier. Le document est conservé avec la visite et peut être utilisé pour une saisie de contrôle.</p><input id="paper-audit-file" type="file" accept="image/*,.pdf,application/pdf" capture="environment"><div id="paper-audit-info" class="muted" style="margin-top:8px"></div><p class="muted small-text">La reconnaissance automatique de l’écriture manuscrite dépend d’un moteur OCR compatible. Cette version tente l’OCR natif du navigateur lorsqu’il existe, mais demande toujours une validation avant d’intégrer une valeur.</p></div></section>`;document.getElementById('pp-blank').onclick=()=>printPreparedAudit(true);document.getElementById('pp-prepared').onclick=()=>printPreparedAudit(false);document.getElementById('pp-repro').onclick=printReproMortalityPrep;document.getElementById('pp-animals').onclick=printPresentAnimals;document.getElementById('paper-audit-file').onchange=async(e)=>{const file=e.target.files?.[0];if(!file)return;v.paperAudits=Array.isArray(v.paperAudits)?v.paperAudits:[];const rec={id:uid('paper'),name:file.name,type:file.type,size:file.size,createdAt:new Date().toISOString(),ocrText:'',dataUrl:''};if(file.size<=4*1024*1024){try{rec.dataUrl=await new Promise((res,rej)=>{const fr=new FileReader();fr.onload=()=>res(fr.result);fr.onerror=rej;fr.readAsDataURL(file)})}catch{}}if(file.type.startsWith('image/')&&'TextDetector' in window){try{const bmp=await createImageBitmap(file),det=new TextDetector(),blocks=await det.detect(bmp);rec.ocrText=blocks.map(x=>x.rawValue||'').filter(Boolean).join('\n')}catch{}}v.paperAudits.unshift(rec);saveDatabase(db);document.getElementById('paper-audit-info').innerHTML=`<strong>✓ ${escapeHtml(file.name)}</strong> enregistré pour cette visite.${rec.ocrText?'<br>Texte détecté automatiquement : à vérifier avant toute intégration.':'<br>Aucun OCR natif disponible sur ce navigateur : le scan reste joint à la visite.'}`;};}
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.6.21.36</span></div>
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.6.21.36</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
...
function renderBuildingQuestionnaire(panel,{visit,audit}){
  ...
  panel.querySelectorAll('.question-group').forEach(d=>d.addEventListener('toggle',()=>{if(!d.open)return;panel.querySelectorAll('.question-group').forEach(other=>{if(other!==d)other.open=false;});scrollBuildingGroupTop(d);}));
...
const auditQuestionConfigs = {
  ...
  'Mortalité veaux (%)':['number','%'],'Mortalité adultes (%)':['number','%'],'Diarrhées néonatales (% veaux)':['number','%'],'Pathologies respiratoires / pneumonies (% animaux/an)':['number','%'],'Mammites cliniques (% vaches/an)':['number','%'],'Boiteries (% animaux/an)':['number','%'],'Omphalites / arthrites (% veaux)':['number','%'],'Troubles de reproduction (% femelles)':['number','%'],'Avortements (nombre/an)':['number','cas/an'],'Vêlages difficiles avec intervention (%)':['number','%'],'Réformes suite au vêlage (%)':['number','%'],'Usage antiparasitaires (traitements/an)':['number','trait./an'],'Usage antibiotiques (traitements/UGB/an)':['number','trait./UGB/an'],
...
function auditQuestionSource(q){
  ...
  const calculated=['intervalle velage','age moyen au premier velage','mortalite veaux','mortalite adultes','taux','kg viande','chargement','concentres/kg','nombre moyen de vaches'];
  const later=['ebe','revenu disponible','endettement','aides pac','frais veterinaires','charge aliments','charge mineraux','fertilisation','semences','travaux par tiers','autres charges sfp','prix moyen kg carcasse','total kg carcasse','prix du lait','lait produit','marge brute'];
  const measure=['poids veaux','gmq','poids carcasse','consommation d eau'];
  if(later.some(x=>text.includes(x)))return {key:'later',label:'📎 À récupérer / compléter après'};
  if(measure.some(x=>text.includes(x)))return {key:'measure',label:'📏 À mesurer / relever'};
...
function auditQuestionRow(q,a){const i=a.answers[q]||{},m=auditQuestionSource(q),dv=derivedAuditValue(activeVisit(),q);return `<div class="audit-question-row audit-question-smart source-${m.key}"><div class="audit-question-title"><strong>${escapeHtml(q)}</strong><span class="source-badge ${m.key}">${m.label}</span>${dv!==null&&dv!==undefined&&dv!==''?`<span class="derived-value">Auto : ${escapeHtml(String(dv).replace('.',','))}</span>`:''}</div><div class="audit-question-control">${auditInputHtml(q,i)}</div><textarea data-audit-comment="${escapeHtml(q)}" placeholder="Commentaire facultatif">${escapeHtml(i.comment||'')}</textarea></div>`}
...
function renderFollowup(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Historique & évolution</h2><div class="muted">Comparer deux visites d’une même exploitation : reproduction, mortalité, mesures, alimentation, bâtiment et plans d’action.</div></div><span class="badge autosave">v14.6.21.36</span></div>
...
async function exportPartnerWorkbook(visit,phase='auto'){
  ...
  set(4,`${col}3`,cows);set(4,`${col}4`,renewal);set(4,`${col}7`,partnerExcelNumber(rep.ivv)||partnerExcelNumber(partnerAuditAnswer(visit,'Intervalle vêlage-vêlage')));set(4,`${col}10`,partnerExcelNumber(rep.firstCalvingAge)||partnerExcelNumber(partnerAuditAnswer(visit,'Âge moyen au premier vêlage')));set(4,`${col}21`,partnerAverageMeasure(visit,'colostrumBrix'));set(4,`${col}22`,births?Math.round(calfDeaths/births*1000)/10:null);set(4,`${col}23`,cows?Math.round(adultDeaths/cows*1000)/10:null);set(4,`${col}24`,partnerExcelNumber(rep.abortions));
  ...
  set(4,`${col}25`,partnerExcelNumber(partnerAuditAnswer(visit,'Diarrhées néonatales (% veaux)')));set(4,`${col}26`,partnerExcelNumber(partnerAuditAnswer(visit,'Pathologies respiratoires / pneumonies (% animaux/an)')));set(4,`${col}27`,partnerExcelNumber(partnerAuditAnswer(visit,'Troubles de reproduction (% femelles)')));set(4,`${col}29`,partnerExcelNumber(partnerAuditAnswer(visit,'Pathologies respiratoires / pneumonies (% animaux/an)')));set(4,`${col}30`,partnerExcelNumber(partnerAuditAnswer(visit,'Omphalites / arthrites (% veaux)')));set(4,`${col}31`,partnerExcelNumber(partnerAuditAnswer(visit,'Usage antiparasitaires (traitements/an)'))||partnerAuditAnswer(visit,'Gestion du parasitisme et recours aux coprologies'));set(4,`${col}32`,partnerExcelNumber(partnerAuditAnswer(visit,'Usage antibiotiques (traitements/UGB/an)')));set(4,`${col}33`,partnerAuditAnswer(visit,'Gestion du parasitisme et recours aux coprologies'));set(4,`${col}37`,partnerExcelNumber(partnerAuditAnswer(visit,'Consommation d’eau (L/animal/jour)'))||partnerExcelNumber(partnerAuditAnswer(visit,'Consommation d’eau')));
  ...
  const teCol=col==='C'?'C':col==='D'?'E':'G';Object.entries(teMap).forEach(([row,label])=>set(5,`${teCol}${row}`,partnerExcelNumber(partnerAuditAnswer(visit,label))));
...
function universalSearchResults(query){
  ...
  const navigation=[
    ...
    {view:'metabolic',icon:'🧬',title:'Profil métabolique / oligo',keywords:'sang oligo cuivre zinc selenium iode cobalt manganese magnesium vitamine d3 25ohd3'},
    {view:'parasitism',icon:'🦠',title:'Parasitisme',keywords:'copro coproscopie strongles douve fasciola pepsinogene serologie'},
    {view:'building',icon:'🏠',title:'Bâtiment / eau / électricité',keywords:'batiment eau abreuvoir redox ph conductivite electricite litiere ambiance'},
    ...
    {view:'audit',icon:'📋',title:'Questions terrain / sanitaire',keywords:'mortalite diarrhee diarrhees mammite mammites boiterie boiteries omphalite omphalites avortement avortements pneumonie pneumonies sanitaire'},
    ...
    {view:'prepprint',icon:'🖨️',title:'Documents imprimables',keywords:'imprimer papier support visite liste animaux mortalite repro vaches problemes'},
    {view:'study',icon:'📁',title:'Suivi de l’étude',keywords:'intervenant veterinaire cda chambre analyses factures justificatifs temps'},
  ...
  navigation.forEach(n=>{if(matches(`${n.title} ${n.keywords}`))results.push({kind:'Rubrique',...n});});
  db.farms.forEach(f=>{
...
function openUniversalSearch(){
  ...
  overlay.innerHTML=`<section class="global-search-panel"><div class="global-search-head"><div><strong>🔎 Recherche dans l’application</strong><small>Recherche aussi les rubriques : sang, urine, Brix, redox, IVV, minéral…</small></div><button type="button" aria-label="Fermer">×</button></div><div class="search-quick"><button data-q="sang">Sang</button><button data-q="urine">Urines</button><button data-q="brix">Brix</button><button data-q="redox">Redox</button><button data-q="mortalité">Mortalité</button></div><input id="global-search-input" type="search" autocomplete="off" inputmode="search" placeholder="Rechercher une rubrique, un animal, une exploitation…"><div id="global-search-results" class="global-search-results"><div class="empty compact">Touchez un raccourci ou saisissez un mot.</div></div></section>`;
  document.body.appendChild(overlay);
  ...
  overlay.querySelectorAll('[data-q]').forEach(b=>b.onclick=()=>{input.value=b.dataset.q;renderResults();});input.oninput=renderResults;
}
...
function renderReproduction(){
  ...
  app.querySelectorAll('[data-repro-score]').forEach(b=>b.onclick=()=>showReproScoreModal(reproFarm,b.dataset.reproScore,'score'));
  app.querySelectorAll('[data-repro-detail]').forEach(b=>b.onclick=()=>showReproScoreModal(reproFarm,b.dataset.reproDetail,'history'));
}
...
if ('serviceWorker' in navigator){
  ...
  const AUDIT_SW=location.origin+AUDIT_PATH+'sw.js?v=14.6.21.32';
...
// v14.6.21.36 — navigation par workflow
(function initWorkflowPhases(){const map={dashboard:'prepare',farms:'prepare',visits:'prepare',documents:'prepare',herddata:'prepare',questionnaires:'prepare',followup:'prepare',animals:'terrain',analysis:'terrain',feeding:'terrain',building:'terrain',audit:'terrain',journal:'terrain',photos:'terrain',checkout:'terrain',nutrition:'report',reproduction:'report',metabolic:'report',parasitism:'report',waterlab:'report',assistant:'report',pilotage:'report',reports:'report',review:'report',planches:'report',references:'report',backup:'report',study:'follow',economy:'follow'};function apply(phase){localStorage.setItem('audit-bovin-phase',phase);document.querySelectorAll('.phase-btn').forEach(b=>b.classList.toggle('active',b.dataset.phase===phase));document.querySelectorAll('.nav-btn[data-phase-item]').forEach(b=>b.classList.toggle('phase-hidden',b.dataset.phaseItem!==phase));}document.querySelectorAll('.phase-btn').forEach(b=>b.addEventListener('click',()=>apply(b.dataset.phase)));const orig=window.setView;const observer=new MutationObserver(()=>{});apply(localStorage.getItem('audit-bovin-phase')||map[currentView]||'prepare');document.querySelector('.top-nav')?.addEventListener('click',e=>{const b=e.target.closest('[data-view]');if(b)apply(map[b.dataset.view]||'report')},true);})();


// v14.6.21.36 — check-up, suivi d'étude, économie et conservation globale du défilement horizontal
...
// v14.6.21.36 — suivi du temps par intervenant + temps GDS préparation/terrain/compte rendu/suivi

// v14.6.21.36 — questionnaires persistants, repères de saisie, bilan sanitaire/éco partenaire, supports papier, check-up responsive.

window.addEventListener('DOMContentLoaded',()=>{initGlobalSearch();initAccordionMemory();});
// v14.6.21.36 — cohérence, imprimables préparation, animaux présents, recherche globale.
