...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Quelques repères visuels utiles, intégrés à l’application et disponibles hors ligne.</div></div><span class="badge autosave">v14.4.7</span></div>
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.4.7</span></div>
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.4.7</span></div>
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.4.7</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
...
function repairHerdImportFarmLinks(){repairLegacyFarmNumbers();return{linked:0,applied:0};}
...
function linkedHerdImportForVisit(visit){const link=ensureAuditGlobal(visit).importedHerdData;if(!link)return null;if(link.snapshot){const snap=repairHerdImport(link.snapshot);return snap&&snap.farmId===visit.farmId?snap:null;}const sourceId=link.sourceId;const item=sourceId?(db.herdImports?.find(x=>x.id===sourceId)||null):null;return item&&item.farmId===visit.farmId?item:null;}
...
function removeHerdImportFromVisit(visit,sourceId=''){
  const a=ensureAuditGlobal(visit),oldId=sourceId||a.importedHerdData?.sourceId||'',item=(db.herdImports||[]).find(x=>x.id===oldId);
...
function renderReproduction(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🐄 Reproduction</h2><div class="muted">Registre historique pour les fiches individuelles ; indicateurs calculés uniquement sur les vaches présentes ayant déjà eu au moins un veau.</div></div><span class="badge autosave">v14.4.7</span></div>
