...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Repères visuels, fiches pratiques et carnet technique, disponibles hors ligne.</div></div><span class="badge autosave">v14.6.21.84</span></div>
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.6.21.84</span></div>
...
function renderVisits() {
  ...
  app.innerHTML = `
    <div class="section-title"><div><h2>Visites</h2><div class="muted">Toutes les visites enregistrées restent accessibles, y compris les plus anciennes.</div></div><span class="badge autosave">v14.6.21.84</span></div>
...
function renderFarmerQuestionnaires(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📨 Questionnaires éleveur</h2><div class="muted">Préparer la visite puis récupérer uniquement les réponses encore manquantes.</div></div><span class="badge autosave">v14.6.21.84</span></div>${activeVisitBanner(visit)}
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.6.21.84</span></div>
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.6.21.84</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
  visit.feeding=visit.feeding&&typeof visit.feeding==='object'?visit.feeding:{rations:[],settings:{},history:[]};
  visit.feeding.nutrition=visit.feeding.nutrition&&typeof visit.feeding.nutrition==='object'?visit.feeding.nutrition:{};
...
function renderFollowup(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Historique & évolution</h2><div class="muted">Comparer deux visites d’une même exploitation : reproduction, mortalité, mesures, alimentation, bâtiment et plans d’action.</div></div><span class="badge autosave">v14.6.21.84</span></div>
...
// v14.6.21.84 — aide à la décision transversale : priorités, tendances et références internes.
...
function prepAnalysisComparisonRows(hist=[]){
  ...
  return defs.map(([label,key,suffix,direction])=>{
    ...
    let trend='—',cls='';
    if(c!=null&&pr!=null){if(Number(c)===Number(pr)){trend='→ stable';cls='neutral';}else{const good=direction==='down'?Number(c)<Number(pr):Number(c)>Number(pr);trend=good?'↗ amélioration':'↘ dégradation';cls=good?'good':'bad';}}
    return {label,key,suffix,direction,current:c,previous:pr,avg5:avg,best10:best,trend,cls};
  });
}
...
function prepDecisionSupportData(visit,farm,x,hist,rolling){
  ...
  const currentPurch=Number(item?.years?.N?.purchases);if(Number.isFinite(currentPurch)&&currentPurch>0){const base=Math.max(1,x?.present?.length||0),pct=Math.round(currentPurch/base*100);if(pct>=15)add(2,'Mouvements / biosécurité',`${currentPurch} introduction(s) / achat(s) sur N`,`≈ ${pct}% de l’effectif présent : vérifier origine, quarantaine et statuts sanitaires`,'',currentPurch);}
  if((domains['Eau']??100)<60)add(2,'Eau',`Points d’abreuvement à approfondir (indice ${domains['Eau']}/100)`,'Vérifier débit, accessibilité, propreté et nombre d’animaux par point d’eau');
  if((domains['Bâtiment']??100)<60)add(2,'Bâtiment',`Bâtiment à approfondir (indice ${domains['Bâtiment']}/100)`,'Reprendre ventilation, litière, densité et zones à corriger');
  if((domains['Alimentation']??100)<60)add(2,'Alimentation',`Ration / alimentation à compléter (indice ${domains['Alimentation']}/100)`,'Vérifier ration réelle, aliments/concentrés, minéraux/bolus et couverture des besoins');
  ...
  const renewalRaw=renewalRate(a.renewal),renewalN=Number(renewalRaw),renewalScore=Number.isFinite(renewalN)?(renewalN>=15&&renewalN<=30?85:renewalN>=10&&renewalN<=35?65:45):45;
  const movementScore=Number.isFinite(currentPurch)?(currentPurch===0?85:(x?.present?.length&&currentPurch/x.present.length>.15?50:70)):55;
  const themes=[
    ...
    {label:'Mouvements / biosécurité',score:movementScore,why:Number.isFinite(currentPurch)?`${currentPurch} achat(s)/introduction(s) sur N`:'Historique mouvements à compléter'},
    {label:'Alimentation',score:domains['Alimentation']??40,why:(visit.feeding?.rations||[]).length?'Ration renseignée':'Ration à renseigner'},
    {label:'Eau',score:domains['Eau']??40,why:'Débit, accès et disponibilité'},
    {label:'Bâtiment',score:domains['Bâtiment']??40,why:'Ambiance, litière et questionnaire'}
  ];
  return {alerts:alerts.slice(0,8),themes};
}
...
// v14.6.21.84 — estimation économique pré-visite, calculée uniquement sur les données déjà disponibles.
...
// v14.6.21.84 — check-up, suivi d'étude, économie et conservation globale du défilement horizontal
...
// v14.6.21.84 — suivi du temps par intervenant + temps GDS préparation/terrain/compte rendu/suivi

// v14.6.21.84 — questionnaires persistants, repères de saisie, bilan sanitaire/éco partenaire, supports papier, check-up responsive.

window.addEventListener('DOMContentLoaded',()=>{initGlobalSearch();initAccordionMemory();});
// v14.6.21.84 — cohérence, imprimables préparation, animaux présents, recherche globale.
