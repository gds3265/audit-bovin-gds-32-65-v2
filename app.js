...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Repères visuels, fiches pratiques et carnet technique, disponibles hors ligne.</div></div><span class="badge autosave">v14.6.21.78</span></div>
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.6.21.78</span></div>
...
function renderVisits() {
  ...
  app.innerHTML = `
    <div class="section-title"><div><h2>Visites</h2><div class="muted">Toutes les visites enregistrées restent accessibles, y compris les plus anciennes.</div></div><span class="badge autosave">v14.6.21.78</span></div>
...
function renderFarmerQuestionnaires(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📨 Questionnaires éleveur</h2><div class="muted">Préparer la visite puis récupérer uniquement les réponses encore manquantes.</div></div><span class="badge autosave">v14.6.21.78</span></div>${activeVisitBanner(visit)}
...
function printReproMortalityPrep(){
  ...
  const repro=`<h2>3. Reproduction — indicateurs calculés</h2><div class="print-metrics">${metric('Vaches avec vêlage',x.cows.length)}${metric('Vaches avec IVV calculable',x.cows.filter(r=>r.meanIVV!=null).length)}${metric('IVV moyen',meanIVV==null?'—':meanIVV+' j')}${metric('IVV ≤400 j',x.ivvLe400.length)}${metric('IVV 401–450 j',x.ivv401to450.length)}${metric('IVV 451–500 j',x.ivv451to500.length)}${metric('IVV >500 j',x.ivvOver500.length)}${metric('Âge moyen au 1er vêlage',firstMean==null?'—':firstMean+' mois')}${metric('Vaches à problème',x.cowProblems.length)}</div>`;
  const mortality=`<h2>4. Mortalité</h2><div class="print-metrics">${metric('Veaux suivis',x.calves.length)}${metric('Veaux morts <6 mois',x.dead6.length)}${metric('Mortalité veaux <6 mois',pct(x.dead6.length,x.calves.length)==null?'—':pct(x.dead6.length,x.calves.length)+' %')}</div>`;
  ...
  const problems=`<h2>7. Vaches à problèmes</h2><table><thead><tr><th>N° travail / animal</th><th>Score</th><th>Pourquoi ce score ?</th><th>Âge</th><th>Dernier vêlage</th><th>Jours depuis vêlage</th><th>IVV moyen</th><th>IVV complet</th><th>1er vêlage</th><th>Veaux morts &lt;6m</th></tr></thead><tbody>${x.cowProblems.map(r=>{const sd=reproductionScoreDetails(r),why=sd.lines.filter(z=>z.delta<0).map(z=>`<strong>${escapeHtml(z.label)} (${z.delta} pt${z.delta<-1?'s':''})</strong>`).join(' · ')||'Aucune pénalité calculée',flags=reproductionPenaltyFlags(r);return `<tr><td><b>${escapeHtml(r.cow.workNumber||r.cow.id)}</b>${r.cow.name?` · ${escapeHtml(r.cow.name)}`:''}<br><small>${escapeHtml(r.cow.id||'')}</small></td><td><b>${sd.score}/100</b></td><td>${why}</td><td>${escapeHtml(ageLabelAt(r.cow.birthDate,x.date)||'—')}</td><td>${r.lastCalvingDate?formatDate(r.lastCalvingDate):'—'}</td><td>${r.daysSinceLast??'—'}</td><td class="${flags.meanIVVWarn?'report-penalty-hit':''} ${flags.meanIVVSevere?'severe':''}">${r.meanIVV??'—'}</td><td class="${flags.maxIVVWarn?'report-penalty-hit':''} ${flags.maxIVVSevere?'severe':''}">${r.intervals?.length?r.intervals.join(' / ')+' j':'—'}</td><td class="${flags.firstCalvingWarn?'report-penalty-hit':''} ${flags.firstCalvingSevere?'severe':''}">${r.firstCalvingAgeMonths??'—'}${r.firstCalvingAgeMonths!=null?' mois':''}</td><td class="${flags.deadWarn?'report-penalty-hit':''} ${flags.deadSevere?'severe':''}">${(r.deadBefore6||[]).length}</td></tr>`}).join('')||'<tr><td colspan="10">Aucune alerte selon les critères actuels.</td></tr>'}</tbody></table><p class="muted"><i>Les cases colorées correspondent aux critères qui font baisser le score.</i></p>`;
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.6.21.78</span></div>
...
function feedProductCardHtml(visit){
  ...
  return `<section class="card" style="margin-top:16px"><div class="section-title"><div><h3>🥣 Aliments / concentrés</h3><div class="muted">Saisissez l’étiquette et la quantité réellement distribuée. Les minéraux et oligo-éléments sont ajoutés automatiquement au bilan de couverture.</div></div><button type="button" class="btn primary" id="add-feed-product">+ Ajouter un aliment</button></div>
  ${products.length?products.map(p=>{const c=feedProductDailyContribution(p);return `<article class="card compact" data-feed-product="${p.id}"><div class="section-title"><h4>${escapeHtml(p.name||'Aliment')}</h4><button type="button" class="btn small danger" data-delete-feed-product="${p.id}">Supprimer</button></div>
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.6.21.78</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
  visit.feeding=visit.feeding&&typeof visit.feeding==='object'?visit.feeding:{rations:[],settings:{},history:[]};
  visit.feeding.nutrition=visit.feeding.nutrition&&typeof visit.feeding.nutrition==='object'?visit.feeding.nutrition:{};
...
function renderFollowup(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Historique & évolution</h2><div class="muted">Comparer deux visites d’une même exploitation : reproduction, mortalité, mesures, alimentation, bâtiment et plans d’action.</div></div><span class="badge autosave">v14.6.21.78</span></div>
...
// v14.6.21.78 — estimation économique pré-visite, calculée uniquement sur les données déjà disponibles.
...
function reproAgeDonutSvg(bucketRows,total){
  ...
  return `<svg class="repro-age-donut-svg" viewBox="0 0 320 240" role="img" aria-label="Répartition des femelles par tranche d'âge"><g transform="rotate(-90 ${cx} ${cy})">${base}${segments}</g><circle cx="${cx}" cy="${cy}" r="41" fill="var(--card,#fff)"></circle><text x="${cx}" y="${cy-4}" text-anchor="middle" class="repro-age-donut-total">${total}</text><text x="${cx}" y="${cy+18}" text-anchor="middle" class="repro-age-donut-sub">femelles</text>${labels}</svg>`;
}
...
// v14.6.21.78 — check-up, suivi d'étude, économie et conservation globale du défilement horizontal
...
// v14.6.21.78 — suivi du temps par intervenant + temps GDS préparation/terrain/compte rendu/suivi

// v14.6.21.78 — questionnaires persistants, repères de saisie, bilan sanitaire/éco partenaire, supports papier, check-up responsive.

window.addEventListener('DOMContentLoaded',()=>{initGlobalSearch();initAccordionMemory();});
// v14.6.21.78 — cohérence, imprimables préparation, animaux présents, recherche globale.
