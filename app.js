...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Quelques repères visuels utiles, intégrés à l’application et disponibles hors ligne.</div></div><span class="badge autosave">v14.6.8</span></div>
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.6.8</span></div>
...
function renderVisits() {
  ...
  app.innerHTML = `
    <div class="section-title"><div><h2>Visites</h2><div class="muted">Toutes les visites enregistrées restent accessibles, y compris les plus anciennes.</div></div><span class="badge autosave">v14.6.8</span></div>
...
function enrichVisitSubjectsFromReproduction(visit){
  ...
  const before=JSON.stringify((visit.subjects||[]).map(s=>({id:s.id,registryAnimalId:s.registryAnimalId,registryLinked:s.registryLinked,registryLinkStatus:s.registryLinkStatus,registryCoherenceIssues:s.registryCoherenceIssues,name:s.name,birthDate:s.birthDate,age:s.age,breedCode:s.breedCode,sex:s.sex,motherNumber:s.motherNumber,lastCalvingDate:s.lastCalvingDate,lastIVV:s.lastIVV,meanIVV:s.meanIVV})));
  (visit.subjects||[]).forEach(subject=>linkSubjectToReproduction(subject,visit,{fillBlanks:true}));
  const after=JSON.stringify((visit.subjects||[]).map(s=>({id:s.id,registryAnimalId:s.registryAnimalId,registryLinked:s.registryLinked,registryLinkStatus:s.registryLinkStatus,registryCoherenceIssues:s.registryCoherenceIssues,name:s.name,birthDate:s.birthDate,age:s.age,breedCode:s.breedCode,sex:s.sex,motherNumber:s.motherNumber,lastCalvingDate:s.lastCalvingDate,lastIVV:s.lastIVV,meanIVV:s.meanIVV})));
    ...
    ${!visit ? `<section class="empty" style="margin-top:16px">Créez ou sélectionnez une visite avant d’ajouter des sujets.</section>` : `
      <section class="grid cols-2 animal-workspace" style="margin-top:16px">
        <form id="quick-subject-form" class="card quick-subject-form">
          ...
          <div class="field"><label for="subject-tag">Numéro de boucle / n° de travail *</label><input id="subject-tag" name="tag" required autocomplete="off" placeholder="Ex. 6248 ou FR6501234567" /></div><div id="subject-registry-preview" class="registry-lookup-preview muted">Saisissez un numéro : l’application cherchera automatiquement le bovin dans le fichier Reproduction de cette visite.</div>
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.6.8</span></div>
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.6.8</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
...
function renderFollowup(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Historique & évolution</h2><div class="muted">Comparer deux visites d’une même exploitation : reproduction, mortalité, mesures, alimentation, bâtiment et plans d’action.</div></div><span class="badge autosave">v14.6.8</span></div>
...
initWorkMode();
initGlobalSearch();
if ('serviceWorker' in navigator) navigator.serviceWorker.register('./sw.js?v=14.6.8',{updateViaCache:'none'}).then(r=>r.update()).catch(console.error);
render();
