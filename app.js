...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Repères visuels, fiches pratiques et carnet technique, disponibles hors ligne.</div></div><span class="badge autosave">v14.6.21.44</span></div>
...
window.addEventListener('audit-bovin-cloud-merged',event=>{
  ...
  // IMPORTANT tablette : une fusion cloud ne doit jamais reconstruire l'écran pendant
  // que le technicien parcourt une longue page. La reconstruction faisait sauter
  // Questions terrain et ramenait la liste des visites vers la visite active.
  // Les données fusionnées sont déjà en mémoire et seront affichées à la prochaine
  // ouverture/changement d'écran. Les écrans courts peuvent être rafraîchis.
  const noHotReload=new Set(['audit','visits','building','analysis','animals','feeding','questionnaires','herddata']);
  if(!noHotReload.has(currentView)) render();
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.6.21.44</span></div>
...
function renderVisits() {
  ...
  app.innerHTML = `
    <div class="section-title"><div><h2>Visites</h2><div class="muted">Toutes les visites enregistrées restent accessibles, y compris les plus anciennes.</div></div><span class="badge autosave">v14.6.21.44</span></div>
...
function renderFarmerQuestionnaires(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📨 Questionnaires éleveur</h2><div class="muted">Préparer la visite puis récupérer uniquement les réponses encore manquantes.</div></div><span class="badge autosave">v14.6.21.44</span></div>${activeVisitBanner(visit)}
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.6.21.44</span></div>
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.6.21.44</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
...
function renderFollowup(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Historique & évolution</h2><div class="muted">Comparer deux visites d’une même exploitation : reproduction, mortalité, mesures, alimentation, bâtiment et plans d’action.</div></div><span class="badge autosave">v14.6.21.44</span></div>
...
if ('serviceWorker' in navigator){
  ...
  const AUDIT_SW=location.origin+AUDIT_PATH+'sw.js?v=14.6.21.44';
...
function smartNumericTokens(text=''){return [...String(text||'').matchAll(/(?:<|>|≤|≥)?\s*-?\d+(?:[.,]\d+)?/g)].map(m=>({raw:m[0].replace(/\s+/g,'').replace(',','.'),index:m.index||0}))}
function smartNumberFromLine(line,aliases=[]){const plain=String(line||''),n=normalizeSearchText(plain);let best=-1,bestAlias='';for(const a of aliases){const aa=normalizeSearchText(a);const idx=n.indexOf(aa);if(idx>=0&&(best<0||idx<best)){best=idx;bestAlias=aa}}if(best<0)return null;const cleaned=plain.replace(/\b(?:19|20)\d{2}[-/.]\d{1,2}[-/.]\d{1,2}\b/g,' ');const normClean=normalizeSearchText(cleaned);const ali=bestAlias?normClean.indexOf(bestAlias):-1;const tokens=smartNumericTokens(cleaned);if(!tokens.length)return null;if(ali>=0){const roughRatio=Math.max(0,Math.min(1,(ali+bestAlias.length)/Math.max(1,normClean.length)));const roughPos=Math.floor(roughRatio*cleaned.length);const after=tokens.find(t=>t.index>=roughPos-2);if(after)return after.raw}return tokens[0].raw}
function smartUnitFromLine(line=''){const m=String(line).match(/(?:µg\/L|ug\/L|mg\/L|mg\/dL|µmol\/L|umol\/L|ng\/mL|µg\/dL|ug\/dL|µg\/mL|ug\/mL|mUT|OPG|oeufs?\/g|oocystes?\/g|UFC\/mL|UFC\/100\s*mL|NPP\/100\s*mL|NFU|µS\/cm|uS\/cm|°f|ppm)/i);return m?m[0]:''}
function smartFindValues(text,defs){const lines=smartTextLines(text),out=[];for(const d of defs){let found=null;for(let i=0;i<lines.length;i++){const here=lines[i],nh=normalizeSearchText(here);if(!(d.aliases||[d.label]).some(a=>nh.includes(normalizeSearchText(a))))continue;for(let span=0;span<=2&&!found;span++){const candidate=[here,...lines.slice(i+1,i+1+span)].join(' ');const value=smartNumberFromLine(candidate,d.aliases||[d.label]);if(value!==null)found={...d,value,unit:smartUnitFromLine(candidate)||d.unit||'',sourceLine:candidate}}if(found)break}if(found)out.push(found)}return out}
function integrateSmartLabText(view,visit,text,fileName=''){if(!text?.trim())return 0;let count=0;const lab=detectLabNameFromText(text);if(view==='metabolic'){const m=ensureMetabolic(visit);if(lab){const lp=metabolicLabProfiles().find(x=>normalizeSearchText(x.name).includes(normalizeSearchText(lab)));if(lp){m.labId=lp.id;m.lab=lp.name}else m.lab=lab;}const defs=Object.entries(METABOLIC_KNOWLEDGE).map(([key,x])=>({key,label:x.label,aliases:[x.label,...(x.aliases||[])]}));smartFindValues(text,defs).forEach(v=>{if((m.rows||[]).some(r=>metabolicKey(r.analyte)===v.key&&String(r.value)===String(v.value)))return;const row={id:uid('met'),animal:m.paperSubject||m.category||'',analyte:v.label,value:v.value,unit:v.unit||'',refMin:'',refMax:'',labStatus:'auto',importSource:fileName};applyMetabolicLabReference(row,metabolicLabById(m.labId),m.sampleType);m.rows.push(row);count++});if(count){m.sourceFile=fileName;m.importedAt=new Date().toISOString();}}
else if(view==='parasitism'){const p=ensureParasitism(visit);if(lab)p.lab=lab;const defs=[{label:'Strongles digestifs',aliases:['strongles digestifs','strongles gastro intestinaux','strongyles digestifs','strongyles gastro']},{label:'Nematodirus',aliases:['nematodirus']},{label:'Coccidies',aliases:['coccidies','coccidia','oocystes']},{label:'Paramphistomes',aliases:['paramphistome','paramphistomum']},{label:'Grande douve',aliases:['fasciola','grande douve']},{label:'Strongles respiratoires',aliases:['strongles respiratoires','dictyocaulus','strongyles respiratoires']}];smartFindValues(text,defs).forEach(v=>{if((p.rows||[]).some(r=>normalizeSearchText(r.parasite)===normalizeSearchText(v.label)&&String(r.value)===String(v.value)))return;p.rows.push({id:uid('par'),animal:p.paperSubject||p.category||'',parasite:v.label,value:v.value,unit:v.unit||'OPG',labStatus:'auto',importSource:fileName});count++});const pep=smartFindValues(text,[{label:'Pepsinogène',aliases:['pepsinogene','pepsinogène'],unit:'mUT'}])[0];if(pep&&!p.pepsinogen.some(r=>String(r.value)===String(pep.value))){p.pepsinogen.push({id:uid('pep'),animal:p.paperSubject||p.category||'',value:pep.value,importSource:fileName});count++}}
else if(view==='waterlab'){const w=ensureWaterLab(visit);if(lab)w.lab=lab;const defs=[{label:'Turbidité',aliases:['turbidite','turbidité'],unit:'NFU'},{label:'pH',aliases:['ph']},{label:'Conductivité',aliases:['conductivite','conductivité'],unit:'µS/cm'},{label:'Dureté',aliases:['durete','dureté','th'],unit:'°f'},{label:'Nitrites',aliases:['nitrites'],unit:'mg/L'},{label:'Nitrates',aliases:['nitrates'],unit:'mg/L'},{label:'Coliformes totaux',aliases:['coliformes totaux','bacteries coliformes','bactéries coliformes'],unit:'/100 mL'},{label:'Escherichia coli',aliases:['escherichia coli','e. coli','e coli'],unit:'/100 mL'},{label:'Entérocoques intestinaux',aliases:['enterocoques','entérocoques'],unit:'/100 mL'},{label:'Spores ASR',aliases:['asr','anaerobies sulfito reducteurs','anaérobies sulfito-réducteurs'],unit:'/100 mL'},{label:'Flore totale 22°C',aliases:['flore 22','22°c'],unit:'UFC/mL'},{label:'Flore totale 37°C',aliases:['flore 37','37°c'],unit:'UFC/mL'}];const vals=smartFindValues(text,defs);if(vals.length){let pt=w.points.find(x=>x.importSource===fileName);if(!pt){pt={id:uid('wpt'),name:`Import ${fileName}`.slice(0,80),location:'',linkedDrinkerId:'',importSource:fileName,rows:[]};w.points.unshift(pt)}vals.forEach(v=>{if(pt.rows.some(r=>normalizeSearchText(r.parameter)===normalizeSearchText(v.label)&&String(r.value)===String(v.value)))return;pt.rows.push({id:uid('wrow'),parameter:v.label,value:v.value,unit:v.unit||'',importSource:fileName});count++})}}return count}
...
function renderSmartLabImportCard(view,visit){if(!visit||!['metabolic','parasitism','waterlab'].includes(view))return;const labels={metabolic:'profil métabolique / oligo',parasitism:'parasitisme',waterlab:'analyse d’eau'};visit.labImports=Array.isArray(visit.labImports)?visit.labImports:[];const existing=visit.labImports.filter(x=>x.module===view),integrated=existing.reduce((n,x)=>n+(Number(x.integratedCount)||0),0);const card=document.createElement('section');card.className='card smart-lab-import';card.innerHTML=`<div class="section-title"><div><h3>📄 Résultats labo — ${escapeHtml(labels[view])}</h3><div class="muted">${existing.length?`✅ ${existing.length} document(s) chargé(s) · ${integrated} valeur(s) intégrée(s) dans ce module`:'Aucun résultat labo chargé dans ce module.'}</div></div>${existing.length?`<span class="badge complete">${existing.length} chargé(s)</span>`:''}</div><p class="muted">PDF : le texte est lu directement quand il existe ; si le PDF est scanné, ou pour une photo, l’application lance un OCR. Les valeurs reconnues sont intégrées sans écraser une valeur déjà existante. Le document original reste rattaché à la visite.</p><input type="file" accept="image/*,.pdf,application/pdf" data-smart-lab-file multiple><div class="small lab-import-progress" data-smart-lab-info></div><div class="lab-import-list">${existing.length?existing.map(x=>`<div class="lab-import-item"><span><b>${escapeHtml(x.name)}</b><small>${formatDateTime(x.createdAt)} · ${Number(x.integratedCount)||0} valeur(s) intégrée(s) · ${escapeHtml(labReadModeLabel(x.readMode|| (x.ocrText?'ocr-natif':'')))}${x.readError?` · ⚠️ ${escapeHtml(x.readError)}`:''}</small></span>${x.dataUrl?`<button class="btn small" data-retry-lab-import="${x.id}">↻ Relire</button><button class="btn small" data-open-lab-import="${x.id}">Ouvrir</button>`:''}<button class="btn small danger" data-delete-lab-import="${x.id}">Supprimer</button></div>`).join(''):'<div class="empty compact">Aucun résultat labo importé dans ce module.</div>'}</div>`;app.prepend(card);
 const processFile=async(file,existingImport=null)=>{const info=card.querySelector('[data-smart-lab-info]');const progress=msg=>{if(info)info.textContent=`${file.name} — ${msg}`};progress('préparation…');const{text,dataUrl,readMode,error}=await smartLabReadFile(file,progress);progress(text.trim()?'Recherche des paramètres et valeurs…':error||'Aucun texte détecté.');const integratedCount=integrateSmartLabText(view,visit,text,file.name);if(existingImport){existingImport.ocrText=text;existingImport.readMode=readMode;existingImport.readError=error;existingImport.integratedCount=(Number(existingImport.integratedCount)||0)+integratedCount;existingImport.reReadAt=new Date().toISOString();if(dataUrl&&!existingImport.dataUrl)existingImport.dataUrl=dataUrl}else visit.labImports.unshift({id:uid('labimp'),module:view,name:file.name,type:file.type,size:file.size,createdAt:new Date().toISOString(),ocrText:text,dataUrl,integratedCount,readMode,readError:error});return{integratedCount,text,error,readMode}};
 card.querySelector('[data-smart-lab-file]').onchange=async e=>{const files=[...(e.target.files||[])];if(!files.length)return;let ok=0,totalValues=0,readDocs=0,errors=[];for(const file of files){const r=await processFile(file);ok++;totalValues+=r.integratedCount;if(r.text.trim())readDocs++;if(r.error)errors.push(r.error)}visit.updatedAt=new Date().toISOString();saveDatabase(db);const info=card.querySelector('[data-smart-lab-info]');if(info)info.textContent='';if(totalValues)showToast(`${ok} document(s) chargé(s) · ${totalValues} valeur(s) intégrée(s).`);else if(readDocs)showToast(`${ok} document(s) lu(s), mais aucun paramètre reconnu dans le modèle actuel.`);else showToast(errors.length?'Le document est conservé, mais l’OCR n’a pas pu démarrer. Vérifiez la connexion lors de la première utilisation.':'Document conservé, mais aucun texte exploitable n’a été détecté.');render()};
 card.querySelectorAll('[data-retry-lab-import]').forEach(b=>b.onclick=async()=>{const x=visit.labImports.find(y=>y.id===b.dataset.retryLabImport);if(!x?.dataUrl)return showToast('Fichier non disponible sur cet appareil.');try{const blob=dataUrlToBlob(x.dataUrl),file=new File([blob],x.name||'resultat-labo',{type:x.type||blob.type||'application/octet-stream'}),r=await processFile(file,x);visit.updatedAt=new Date().toISOString();saveDatabase(db);showToast(r.integratedCount?`${r.integratedCount} nouvelle(s) valeur(s) intégrée(s).`:r.text.trim()?'Document relu, mais aucune nouvelle valeur reconnue.':'Impossible de lire le texte de ce document.');render()}catch(e){console.error(e);showToast('Impossible de relire ce document.')}});
 card.querySelectorAll('[data-open-lab-import]').forEach(b=>b.onclick=()=>{const x=visit.labImports.find(y=>y.id===b.dataset.openLabImport);if(!x?.dataUrl)return showToast('Fichier non disponible sur cet appareil.');try{const blob=dataUrlToBlob(x.dataUrl),url=URL.createObjectURL(blob);window.open(url,'_blank');setTimeout(()=>URL.revokeObjectURL(url),30000)}catch{showToast('Impossible d’ouvrir ce fichier.')}});card.querySelectorAll('[data-delete-lab-import]').forEach(b=>b.onclick=()=>{if(!confirm('Supprimer ce document labo de la visite ? Les valeurs déjà intégrées dans les tableaux ne seront pas supprimées automatiquement.'))return;visit.labImports=visit.labImports.filter(x=>x.id!==b.dataset.deleteLabImport);saveDatabase(db);render()})}
...
// v14.6.21.44 — check-up, suivi d'étude, économie et conservation globale du défilement horizontal
...
// v14.6.21.44 — suivi du temps par intervenant + temps GDS préparation/terrain/compte rendu/suivi

// v14.6.21.44 — questionnaires persistants, repères de saisie, bilan sanitaire/éco partenaire, supports papier, check-up responsive.

window.addEventListener('DOMContentLoaded',()=>{initGlobalSearch();initAccordionMemory();});
// v14.6.21.44 — cohérence, imprimables préparation, animaux présents, recherche globale.
