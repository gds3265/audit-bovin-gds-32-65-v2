...
// Cette fonction manquait dans la v14.6.21.92 et provoquait l'erreur splitUsefulLines is not defined.
...
// v14.6.21.92 — Audits ciblés + motif libre + orientation syndromique veaux
...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Repères visuels, fiches pratiques et carnet technique, disponibles hors ligne.</div></div><span class="badge autosave">v14.6.21.92</span></div>
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.6.21.92</span></div>
...
function renderVisits() {
  ...
  app.innerHTML = `
    <div class="section-title"><div><h2>Visites</h2><div class="muted">Toutes les visites enregistrées restent accessibles, y compris les plus anciennes.</div></div><span class="badge autosave">v14.6.21.92</span></div>
...
function renderFarmerQuestionnaires(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📨 Questionnaires éleveur</h2><div class="muted">Préparer la visite puis récupérer uniquement les réponses encore manquantes.</div></div><span class="badge autosave">v14.6.21.92</span></div>${activeVisitBanner(visit)}
...
function printReproMortalityPrep(){
  ...
  if(!x){const charts=item?`<h2>Graphiques Xper — synthèse technique</h2><div class="herd-tech-origin"><p><b>Source :</b> ${escapeHtml(item.sourceFile||'fichier Xper')}</p><p><b>Période :</b> ${escapeHtml(item.period?.start||'—')} au ${escapeHtml(item.period?.end||'—')}</p></div>${herdChartsHtml(item,{forReport:true})}`:'<p class="muted">Aucun fichier Xper / technico-économique lié à cette visite.</p>';paperWindow('Bilan technique préparatoire',`<h1>Bilan technique préparatoire</h1>${activeFarmNameForPrint(v)}<div class="notice warning">Aucun registre bovins avec mouvements n’est disponible : les listes individuelles et calculs détaillés de reproduction ne peuvent pas être générés.</div>${charts}`);return;}
  ...
  const ageStructure=`<h2>2. Répartition des femelles par âge</h2>${reproAgeDistributionHtml(x.females24,x.date,{interactive:false,wrap:false,detailTargetId:'print-prep-age-year-detail'})}`;
  ...
  const repro=`<h2>3. Reproduction — 12 derniers mois</h2><p class="muted"><i>Période : ${formatDate(rolling.start)} → ${formatDate(rolling.end)}. Les IVV sont rattachés à la date du vêlage qui termine l’intervalle.</i></p><div class="print-metrics">${metric('Vaches ayant vêlé',rolling.dams)}${metric('IVV calculables',rolling.intervals.length)}${metric('IVV moyen',rolling.ivvMean==null?'—':rolling.ivvMean+' j')}${metric('IVV ≤400 j',rolling.ivvLe400)}${metric('IVV 401–450 j',rolling.ivv401to450)}${metric('IVV 451–500 j',rolling.ivv451to500)}${metric('IVV >500 j',rolling.ivvOver500)}${metric('Âge moyen au 1er vêlage',rolling.firstCalvingMean==null?'—':rolling.firstCalvingMean+' mois')}${metric('Vaches à problème actuelles',x.cowProblems.length)}</div>`;
  const mortality=`<h2>4. Mortalité — 12 derniers mois</h2><p class="muted"><i>Période : veaux nés entre ${formatDate(rolling.start)} et ${formatDate(rolling.end)}. Le taux porte sur les décès avant 6 mois connus à la date de visite.</i></p><div class="print-metrics">${metric('Veaux nés / suivis',rolling.births.length)}${metric('Veaux morts <6 mois',rolling.dead6.length)}${metric('Mortalité veaux <6 mois',rolling.mortalityRate==null?'—':rolling.mortalityRate+' %')}</div>${annualHistoryTablesHtml(annualNative,{forPrint:true})}`;
  ...
  const technicalCharts=item?`<h2>5. Graphiques Xper — synthèse technique</h2><div class="herd-tech-origin"><p><b>Source :</b> ${escapeHtml(item.sourceFile||'fichier Xper')}</p><p><b>Période :</b> ${escapeHtml(item.period?.start||'—')} au ${escapeHtml(item.period?.end||'—')}</p></div>${herdChartsHtml(item,{forReport:true})}`:`<h2>4. Graphiques Xper — synthèse technique</h2><p class="muted">Aucun fichier Xper / technico-économique lié à cette visite : les graphiques IVV, mortalité et introductions ne peuvent pas être générés.</p>`;
  ...
  const problems=`<h2>7. Vaches à problèmes</h2><table><thead><tr><th>N° travail / animal</th><th>Score</th><th>Pourquoi ce score ?</th><th>Âge</th><th>Dernier vêlage</th><th>Jours depuis vêlage</th><th>IVV moyen</th><th>IVV complet</th><th>1er vêlage</th><th>Veaux morts &lt;6m</th></tr></thead><tbody>${x.cowProblems.map(r=>{const sd=reproductionScoreDetails(r),why=sd.lines.filter(z=>z.delta<0).map(z=>`${escapeHtml(z.label)} (${z.delta} pt${z.delta<-1?'s':''})`).join(' · ')||'Aucune pénalité calculée',flags=reproductionPenaltyFlags(r),scoreClass=sd.score>=75?'report-score-good':sd.score>=60?'report-score-warning':'report-score-danger',scoreStyle=sd.score>=75?'color:#176238;font-weight:800;background:#dff3e6':sd.score>=60?'color:#9a6700;font-weight:800;background:#fff0c7':'color:#a01818;font-weight:800;background:#ffd9d6',penaltyStyle=(warn,severe)=>warn?(severe?'color:#a01818;font-weight:800;background:#ffd9d6':'color:#9a6700;font-weight:800;background:#fff1c7'):'';return `<tr><td><b>${escapeHtml(r.cow.workNumber||r.cow.id)}</b>${r.cow.name?` · ${escapeHtml(r.cow.name)}`:''}<br><small>${escapeHtml(r.cow.id||'')}</small></td><td class="${scoreClass}" style="${scoreStyle}">${sd.score}/100</td><td>${why}</td><td>${escapeHtml(ageLabelAt(r.cow.birthDate,x.date)||'—')}</td><td>${r.lastCalvingDate?formatDate(r.lastCalvingDate):'—'}</td><td>${r.daysSinceLast??'—'}</td><td class="${flags.meanIVVWarn?'report-penalty-hit':''} ${flags.meanIVVSevere?'severe':''}" style="${penaltyStyle(flags.meanIVVWarn,flags.meanIVVSevere)}">${r.meanIVV??'—'}</td><td class="${flags.maxIVVWarn?'report-penalty-hit':''} ${flags.maxIVVSevere?'severe':''}" style="${penaltyStyle(flags.maxIVVWarn,flags.maxIVVSevere)}">${r.intervals?.length?r.intervals.join(' / ')+' j':'—'}</td><td class="${flags.firstCalvingWarn?'report-penalty-hit':''} ${flags.firstCalvingSevere?'severe':''}" style="${penaltyStyle(flags.firstCalvingWarn,flags.firstCalvingSevere)}">${r.firstCalvingAgeMonths??'—'}${r.firstCalvingAgeMonths!=null?' mois':''}</td><td class="${flags.deadWarn?'report-penalty-hit':''} ${flags.deadSevere?'severe':''}" style="${penaltyStyle(flags.deadWarn,flags.deadSevere)}">${(r.deadBefore6||[]).length}</td></tr>`}).join('')||'<tr><td colspan="10">Aucune alerte selon les critères actuels.</td></tr>'}</tbody></table><p class="muted"><i>Les indicateurs qui ont réellement fait perdre des points sont en gras et colorés. Score : vert ≥75, orange 60–74, rouge &lt;60.</i></p>`;
  const heifers=`<h2>8. Femelles &gt;36 mois sans vêlage</h2>${x.heifers36.length?`<table><thead><tr><th>N° travail / animal</th><th>Date naissance</th><th>Âge</th><th>Race</th><th>Date entrée</th></tr></thead><tbody>${x.heifers36.slice().sort(sortByWorkNumber).map(a=>`<tr><td><b>${escapeHtml(a.workNumber||a.id)}</b>${a.name?` · ${escapeHtml(a.name)}`:''}<br><small>${escapeHtml(a.id||'')}</small></td><td>${a.birthDate?formatDate(a.birthDate):'—'}</td><td>${escapeHtml(ageLabelAt(a.birthDate,x.date)||'—')}</td><td>${escapeHtml(a.breed||'—')}</td><td>${a.entryDate?formatDate(a.entryDate):'—'}</td></tr>`).join('')}</tbody></table>`:'<p>Aucune femelle &gt;36 mois sans vêlage détectée.</p>'}`;
  ...
  const body=`<h1>Bilan technique préparatoire</h1>${activeFarmNameForPrint(v)}<p><b>Objectif :</b> regrouper au même endroit les données techniques connues avant la visite : structure du troupeau, reproduction, mortalité, introductions/mouvements et historique Xper, afin de repérer les évolutions et les points à éclaircir avec l’éleveur.</p>${targetedAuditHtml(v,{forPrint:true})}${decisionSupport}${structure}${ageStructure}${repro}${mortality}${annual}${technicalCharts}${paternity}${problems}${heifers}${chrono}${economy}${toClarify}`;
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.6.21.92</span></div>
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.6.21.92</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
  visit.feeding=visit.feeding&&typeof visit.feeding==='object'?visit.feeding:{rations:[],settings:{},history:[]};
  visit.feeding.nutrition=visit.feeding.nutrition&&typeof visit.feeding.nutrition==='object'?visit.feeding.nutrition:{};
...
function renderFollowup(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Historique & évolution</h2><div class="muted">Comparer deux visites d’une même exploitation : reproduction, mortalité, mesures, alimentation, bâtiment et plans d’action.</div></div><span class="badge autosave">v14.6.21.92</span></div>
...
function reproRolling12Stats(visit,farm,x=null){
  ...
  const intervals=[];byDam.forEach(ds=>{const dates=[...new Set(ds)].sort();for(let i=1;i<dates.length;i++){if(dates[i]>=start&&dates[i]<=end){const d=daysBetweenDates(dates[i-1],dates[i]);if(d!=null)intervals.push(d);}}});
...
function reproMortalityAnnualHistory(visit,farm,yearsCount=10){
  ...
  const out=[];for(let y=firstYear;y<=endYear;y++){
    ...
    out.push({year:y,births:births.length,cowsCalved:dams.size,ivvCount:ivvs.length,ivvMean:mean(ivvs),ivvLe400:ivvs.filter(d=>d<=400).length,ivv401to450:ivvs.filter(d=>d>=401&&d<=450).length,ivv451to500:ivvs.filter(d=>d>=451&&d<=500).length,ivvOver500:ivvs.filter(d=>d>500).length,firstCalvingCount:ages.length,firstCalvingMean:mean1(ages),dead6:dead6.length,mortalityRate:births.length?Math.round(dead6.length/births.length*1000)/10:null});
  }return out;
}
...
// v14.6.21.92 — aide à la décision transversale : priorités, tendances et références internes.
...
// v14.6.21.92 — cohérence pré-visite, tableau global mouvements/renouvellement et âges lisibles.
...
// v14.6.21.92 — « fabrique du troupeau » : naissance femelles → conservation jusqu’à la reproduction → 1er vêlage → réforme.
...
// v14.6.21.92 — estimation économique pré-visite, calculée uniquement sur les données déjà disponibles.
...
function reproAgeDistributionHtml(animals=[],analysisDate='',opts={}){
  ...
  const interactive=options.interactive!==false,wrap=options.wrap!==false,detailTargetId=options.detailTargetId===undefined?'repro-age-year-detail':String(options.detailTargetId||'');
  ...
  const bucketLegend=buckets.map(b=>{const pct=Math.round(b.count/total*1000)/10;return `<span><i style="background:${b.color}"></i><strong>${escapeHtml(b.label)}</strong> — ${b.count} (${pct}%)</span>`;}).join('');
...
// v14.6.21.92 — check-up, suivi d'étude, économie et conservation globale du défilement horizontal
...
// v14.6.21.92 — suivi du temps par intervenant + temps GDS préparation/terrain/compte rendu/suivi

// v14.6.21.92 — questionnaires persistants, repères de saisie, bilan sanitaire/éco partenaire, supports papier, check-up responsive.

window.addEventListener('DOMContentLoaded',()=>{initGlobalSearch();initAccordionMemory();});
// v14.6.21.92 — cohérence, imprimables préparation, animaux présents, recherche globale.
