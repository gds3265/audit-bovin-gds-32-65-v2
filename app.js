...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Repères visuels, fiches pratiques et carnet technique, disponibles hors ligne.</div></div><span class="badge autosave">v14.6.21.76</span></div>
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.6.21.76</span></div>
...
function renderVisits() {
  ...
  app.innerHTML = `
    <div class="section-title"><div><h2>Visites</h2><div class="muted">Toutes les visites enregistrées restent accessibles, y compris les plus anciennes.</div></div><span class="badge autosave">v14.6.21.76</span></div>
...
function renderFarmerQuestionnaires(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📨 Questionnaires éleveur</h2><div class="muted">Préparer la visite puis récupérer uniquement les réponses encore manquantes.</div></div><span class="badge autosave">v14.6.21.76</span></div>${activeVisitBanner(visit)}
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.6.21.76</span></div>
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.6.21.76</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
...
function renderFollowup(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Historique & évolution</h2><div class="muted">Comparer deux visites d’une même exploitation : reproduction, mortalité, mesures, alimentation, bâtiment et plans d’action.</div></div><span class="badge autosave">v14.6.21.76</span></div>
...
function prepReproNativeCowTable(rows,date){return `<div class="table-wrap prep-native-detail-table"><table><thead><tr><th>Vache</th><th>Âge</th><th>Dernier vêlage</th><th>IVV moyen</th><th>IVV complet</th><th>Score</th><th>Pourquoi ?</th></tr></thead><tbody>${(rows||[]).map(z=>{const sd=reproductionScoreDetails(z),why=sd.lines.filter(q=>q.delta<0).map(q=>q.label).join(' · ')||'Pas de pénalité',flags=reproductionPenaltyFlags(z);return `<tr><td><strong>${escapeHtml(z.cow.workNumber||z.cow.id)}</strong>${z.cow.name?`<br>${escapeHtml(z.cow.name)}`:''}<br><small>${escapeHtml(z.cow.id)}</small></td><td>${escapeHtml(ageLabelAt(z.cow.birthDate,date)||'—')}</td><td>${reproPenaltyCell(`${z.lastCalvingDate?formatDate(z.lastCalvingDate):'—'}${z.firstCalvingAgeMonths!=null?`<br><small>1er vêlage ${z.firstCalvingAgeMonths} mois</small>`:''}`,flags.firstCalvingWarn,flags.firstCalvingSevere)}</td><td>${reproPenaltyCell(String(z.meanIVV??'—'),flags.meanIVVWarn,flags.meanIVVSevere)}</td><td>${reproPenaltyCell(z.intervals.length?z.intervals.join(' / ')+' j':'—',flags.maxIVVWarn,flags.maxIVVSevere)}</td><td>${sd.score}/100</td><td>${escapeHtml(why)}</td></tr>`}).join('')||'<tr><td colspan="7">Aucune vache.</td></tr>'}</tbody></table></div>`;}
...
function bindPrepReproClicks(visit,farm){
  ...
  const reproRows=r=>`<div class="table-wrap"><table><thead><tr><th>Vache</th><th>Âge</th><th>Dernier vêlage</th><th>IVV moyen</th><th>IVV complet</th><th>Score</th><th>Pourquoi ?</th></tr></thead><tbody>${r.map(z=>{const sd=reproductionScoreDetails(z),why=sd.lines.filter(q=>q.delta<0).map(q=>q.label).join(' · ')||'Pas de pénalité',flags=reproductionPenaltyFlags(z);return `<tr><td><strong>${escapeHtml(z.cow.workNumber||z.cow.id)}</strong>${z.cow.name?`<br>${escapeHtml(z.cow.name)}`:''}<br><small>${escapeHtml(z.cow.id)}</small></td><td>${escapeHtml(ageLabelAt(z.cow.birthDate,x.date)||'—')}</td><td>${reproPenaltyCell(`${z.lastCalvingDate?formatDate(z.lastCalvingDate):'—'}${z.firstCalvingAgeMonths!=null?`<br><small>1er vêlage ${z.firstCalvingAgeMonths} mois</small>`:''}`,flags.firstCalvingWarn,flags.firstCalvingSevere)}</td><td>${reproPenaltyCell(String(z.meanIVV??'—'),flags.meanIVVWarn,flags.meanIVVSevere)}</td><td>${reproPenaltyCell(z.intervals.length?z.intervals.join(' / ')+' j':'—',flags.maxIVVWarn,flags.maxIVVSevere)}</td><td>${sd.score}/100</td><td>${escapeHtml(why)}</td></tr>`}).join('')||'<tr><td colspan="7">Aucune vache.</td></tr>'}</tbody></table></div>`;
  const openTile=(b)=>{
...
// v14.6.21.76 — estimation économique pré-visite, calculée uniquement sur les données déjà disponibles.
...
// v14.6.21.76 — check-up, suivi d'étude, économie et conservation globale du défilement horizontal
...
// v14.6.21.76 — suivi du temps par intervenant + temps GDS préparation/terrain/compte rendu/suivi

// v14.6.21.76 — questionnaires persistants, repères de saisie, bilan sanitaire/éco partenaire, supports papier, check-up responsive.

window.addEventListener('DOMContentLoaded',()=>{initGlobalSearch();initAccordionMemory();});
// v14.6.21.76 — cohérence, imprimables préparation, animaux présents, recherche globale.
