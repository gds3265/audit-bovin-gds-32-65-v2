...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Quelques repères visuels utiles, intégrés à l’application et disponibles hors ligne.</div></div><span class="badge autosave">v14.5.5</span></div>
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.5.5</span></div>
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.5.5</span></div>
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.5.5</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
...
function reproductionPeriodIndicators(farm,start,end){
  ...
  const males=new Set(registry.filter(a=>String(a.sex||'').toUpperCase()==='M').map(a=>normalizeAnimalId(a.id)).filter(Boolean));
  let ia=0,natural=0,unknown=0;calves.forEach(c=>{const sire=normalizeAnimalId(c.sireId);if(!sire)unknown++;else if(males.has(sire))natural++;else ia++;});
  const known=ia+natural;return {eligible,calves,calved,withoutCalving,primiparous,ivv12,ia,natural,unknown,calvingRate:eligible.length?Math.round(calved.length/eligible.length*1000)/10:null,primiparousRate:calved.length?Math.round(primiparous.length/calved.length*1000)/10:null,ivv12Mean:ivv12.length?Math.round(ivv12.reduce((a,b)=>a+b,0)/ivv12.length):null,iaRate:known?Math.round(ia/known*1000)/10:null};
}
function renderReproduction(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🐄 Reproduction</h2><div class="muted">Registre historique pour les fiches individuelles ; indicateurs calculés uniquement sur les vaches présentes ayant déjà eu au moins un veau.</div></div><span class="badge autosave">v14.5.5</span></div>${visit?activeVisitBanner(visit):''}
  ${sourceHtml}
