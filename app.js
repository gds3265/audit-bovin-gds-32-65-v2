...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Quelques repères visuels utiles, intégrés à l’application et disponibles hors ligne.</div></div><span class="badge autosave">v14.6.7</span></div>
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.6.7</span></div>
...
function renderVisits() {
  ...
  app.innerHTML = `
    <div class="section-title"><div><h2>Visites</h2><div class="muted">Toutes les visites enregistrées restent accessibles, y compris les plus anciennes.</div></div><span class="badge autosave">v14.6.7</span></div>
...
function enrichVisitSubjectsFromReproduction(visit){
  ...
  const farm=db.farms.find(f=>f.id===visit.farmId);if(!farm)return false;
  const source=reproductionSourceForVisit(visit,farm),registry=source.registry||[];if(!registry.length)return false;
  const registryFarm={...farm,herdRegistry:registry};let changed=false;
  (visit.subjects||[]).forEach(subject=>{
    const match=resolveRegistryAnimal(registryFarm,subject.tag||subject.identifier||'');if(!match?.animal)return;
    const a=match.animal,repro=reproductionForCow(registryFarm,a.id),atDate=visit.date||new Date().toISOString().slice(0,10);
    const fill=(key,value)=>{if((subject[key]===undefined||subject[key]===null||subject[key]==='')&&value!==undefined&&value!==null&&value!==''){subject[key]=value;changed=true;}};
    ...
    fill('lastCalvingDate',repro?.lastCalvingDate||'');fill('lastCalfId',repro?.lastCalf?.id||'');fill('firstCalvingAgeMonths',repro?.firstCalvingAgeMonths??'');fill('lastIVV',repro?.lastIVV??'');fill('meanIVV',repro?.meanIVV??'');
    if(!subject.registryLinked){subject.registryLinked=true;changed=true;}
  });
  if(changed){visit.updatedAt=new Date().toISOString();saveDatabase(db);}return changed;
}
...
function subjectCardHtml(subject, index) {
  ...
  return `<article class="subject-card ${isOpen ? 'open' : ''}" data-subject-card="${subject.id}">
    <button type="button" class="subject-summary" data-toggle-subject="${subject.id}" aria-expanded="${isOpen}">
      ...
      <span class="subject-main"><strong>${escapeHtml(subject.tag || `Sujet ${index + 1}`)}${subject.name?` · ${escapeHtml(subject.name)}`:''}</strong><small>${escapeHtml(subject.location || 'Emplacement non renseigné')}${subject.age?` · ${escapeHtml(subject.age)}`:''}${subject.lastCalvingDate?` · Dernier vêlage ${formatDate(subject.lastCalvingDate)}`:''}</small></span>
...
function subjectDetailsHtml(subject) {
  return `<form class="subject-details" data-subject-form="${subject.id}">
    ...
    ${subject.registryLinked?`<section class="subject-registry-info"><h4>📋 Données du registre Reproduction</h4><div class="subject-registry-grid"><span><small>Nom</small><strong>${escapeHtml(subject.name||'—')}</strong></span><span><small>Date de naissance</small><strong>${subject.birthDate?formatDate(subject.birthDate):'—'}</strong></span><span><small>Âge à la visite</small><strong>${escapeHtml(subject.age||'—')}</strong></span><span><small>Race</small><strong>${escapeHtml(subject.breedCode||'—')}</strong></span><span><small>Dernier vêlage</small><strong>${subject.lastCalvingDate?formatDate(subject.lastCalvingDate):'—'}</strong></span><span><small>Dernier IVV</small><strong>${subject.lastIVV!==''&&subject.lastIVV!=null?`${subject.lastIVV} j`:'—'}</strong></span></div></section>`:''}
...
function renderAnimals() {
  ...
  quickForm?.addEventListener('submit', event => {
    ...
    const match = resolveRegistryAnimal(registryFarm, tag);
    if (match?.ambiguous) {
    ...
    const registryAnimal = resolveRegistryAnimal(registryFarm, tag)?.animal || null;
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.6.7</span></div>
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.6.7</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
...
function renderFollowup(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Historique & évolution</h2><div class="muted">Comparer deux visites d’une même exploitation : reproduction, mortalité, mesures, alimentation, bâtiment et plans d’action.</div></div><span class="badge autosave">v14.6.7</span></div>
...
function renderReproduction(){
  ...
  let periodStart=saved.start||periodMin,periodEnd=saved.end||periodMax;
  ...
  const updatePeriod=()=>{const start=ps?.value||periodMin,end=pe?.value||periodMax;if(start&&end&&start>end){showToast('La date de début doit être antérieure à la date de fin.');return;}localStorage.setItem(periodKey,JSON.stringify({start,end}));renderReproduction();};if(ps)ps.onchange=updatePeriod;if(pe)pe.onchange=updatePeriod;if(pr)pr.onclick=()=>{localStorage.removeItem(periodKey);renderReproduction();};
...
initWorkMode();
initGlobalSearch();
if ('serviceWorker' in navigator) navigator.serviceWorker.register('./sw.js?v=14.6.7',{updateViaCache:'none'}).then(r=>r.update()).catch(console.error);
render();
