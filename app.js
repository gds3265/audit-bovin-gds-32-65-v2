...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Repères visuels, fiches pratiques et carnet technique, disponibles hors ligne.</div></div><span class="badge autosave">v14.6.21.42</span></div>
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.6.21.42</span></div>
...
function renderVisits() {
  ...
  app.innerHTML = `
    <div class="section-title"><div><h2>Visites</h2><div class="muted">Toutes les visites enregistrées restent accessibles, y compris les plus anciennes.</div></div><span class="badge autosave">v14.6.21.42</span></div>
...
function renderFarmerQuestionnaires(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📨 Questionnaires éleveur</h2><div class="muted">Préparer la visite puis récupérer uniquement les réponses encore manquantes.</div></div><span class="badge autosave">v14.6.21.42</span></div>${activeVisitBanner(visit)}
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.6.21.42</span></div>
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.6.21.42</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
...
function renderFollowup(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Historique & évolution</h2><div class="muted">Comparer deux visites d’une même exploitation : reproduction, mortalité, mesures, alimentation, bâtiment et plans d’action.</div></div><span class="badge autosave">v14.6.21.42</span></div>
...
function renderSmartLabImportCard(view,visit){if(!visit||!['metabolic','parasitism','waterlab'].includes(view))return;const labels={metabolic:'profil métabolique / oligo',parasitism:'parasitisme',waterlab:'analyse d’eau'};visit.labImports=Array.isArray(visit.labImports)?visit.labImports:[];const existing=visit.labImports.filter(x=>x.module===view);const card=document.createElement('section');card.className='card smart-lab-import';card.innerHTML=`<h3>📄 Importer des résultats labo</h3><p class="muted">Vous pouvez sélectionner <b>plusieurs PDF ou photos en une fois</b> et en ajouter d’autres ensuite. Chaque document reste rattaché à la visite. Les images sont lues automatiquement si le navigateur dispose d’un moteur de reconnaissance ; toute recopie de valeur reste à valider.</p><input type="file" accept="image/*,.pdf,application/pdf" data-smart-lab-file multiple><div class="small" data-smart-lab-info></div><div class="lab-import-list">${existing.length?existing.map(x=>`<div class="lab-import-item"><span><b>${escapeHtml(x.name)}</b><small>${formatDateTime(x.createdAt)}${x.ocrText?' · texte détecté':''}</small></span>${x.dataUrl?`<button class="btn small" data-open-lab-import="${x.id}">Ouvrir</button>`:''}<button class="btn small danger" data-delete-lab-import="${x.id}">Supprimer</button></div>`).join(''):'<div class="empty compact">Aucun résultat labo importé dans ce module.</div>'}</div>`;app.prepend(card);
  card.querySelector('[data-smart-lab-file]').onchange=async e=>{const files=[...(e.target.files||[])];if(!files.length)return;const info=card.querySelector('[data-smart-lab-info]');info.textContent=`Import de ${files.length} fichier(s)…`;let ok=0;for(const file of files){let text='',dataUrl='';if(file.size<=4*1024*1024){try{dataUrl=await new Promise((res,rej)=>{const fr=new FileReader();fr.onload=()=>res(fr.result);fr.onerror=rej;fr.readAsDataURL(file)})}catch{}}if(file.type.startsWith('image/')&&'TextDetector'in window){try{const bmp=await createImageBitmap(file),det=new TextDetector(),res=await det.detect(bmp);text=res.map(x=>x.rawValue||'').join('\n')}catch{}}visit.labImports.unshift({id:uid('labimp'),module:view,name:file.name,type:file.type,size:file.size,createdAt:new Date().toISOString(),ocrText:text,dataUrl});ok++;}saveDatabase(db);showToast(`${ok} résultat(s) labo importé(s).`);render();};
  ...
  card.querySelectorAll('[data-delete-lab-import]').forEach(b=>b.onclick=()=>{if(!confirm('Supprimer ce résultat labo de la visite ?'))return;visit.labImports=visit.labImports.filter(x=>x.id!==b.dataset.deleteLabImport);saveDatabase(db);render();});
}
...
// v14.6.21.42 — check-up, suivi d'étude, économie et conservation globale du défilement horizontal
...
// v14.6.21.42 — suivi du temps par intervenant + temps GDS préparation/terrain/compte rendu/suivi

// v14.6.21.42 — questionnaires persistants, repères de saisie, bilan sanitaire/éco partenaire, supports papier, check-up responsive.

window.addEventListener('DOMContentLoaded',()=>{initGlobalSearch();initAccordionMemory();});
// v14.6.21.42 — cohérence, imprimables préparation, animaux présents, recherche globale.
