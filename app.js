...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Quelques repères visuels utiles, intégrés à l’application et disponibles hors ligne.</div></div><span class="badge autosave">v14.6.3</span></div>
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.6.3</span></div>
...
function renderVisits() {
  ...
  app.innerHTML = `
    <div class="section-title"><div><h2>Visites</h2><div class="muted">Toutes les visites enregistrées restent accessibles, y compris les plus anciennes.</div></div><span class="badge autosave">v14.6.3</span></div>
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.6.3</span></div>
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.6.3</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
...
function renderFollowup(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Historique & évolution</h2><div class="muted">Comparer deux visites d’une même exploitation : reproduction, mortalité, mesures, alimentation, bâtiment et plans d’action.</div></div><span class="badge autosave">v14.6.3</span></div>
...
function renderReproduction(){
  ...
  const reproFarm={...farm,herdRegistry:registry},today=new Date().toISOString().slice(0,10),presentFemales=registry.filter(a=>a.sex==='F'&&isRegistryAnimalPresent(a,today)),currentCows=currentReproductionCows(reproFarm),cowIds=new Set(currentCows.map(r=>normalizeAnimalId(r.cow.id))),breedingFemales=presentFemales.filter(a=>monthsBetweenDates(a.birthDate,today)>24),presentHeifers=breedingFemales.filter(a=>!cowIds.has(normalizeAnimalId(a.id)));
  const allIvvs=currentCows.flatMap(r=>r.intervals),firstIvvs=currentCows.map(r=>r.intervals[0]).filter(x=>x!=null),allCalves=currentCows.flatMap(r=>r.calves),deadCalves=allCalves.filter(c=>c.exitCause==='M'&&c.exitDate&&daysBetweenDates(c.birthDate,c.exitDate)<183),maleIds=new Set(registry.filter(a=>a.sex==='M').map(a=>normalizeAnimalId(a.id))),knownFather=allCalves.filter(c=>c.fatherId),probableIA=knownFather.filter(c=>!maleIds.has(normalizeAnimalId(c.fatherId)));
  const primipares=currentCows.filter(r=>r.calves.length===1),yearAgo=new Date(today+'T12:00:00');yearAgo.setFullYear(yearAgo.getFullYear()-1);const yearAgoIso=yearAgo.toISOString().slice(0,10),calvedLast12=new Set(allCalves.filter(c=>c.birthDate>=yearAgoIso&&c.birthDate<=today).map(c=>normalizeAnimalId(c.motherId))).size;
  const stats={ivvMean:allIvvs.length?Math.round(allIvvs.reduce((a,b)=>a+b,0)/allIvvs.length):null,ivv12:firstIvvs.length?Math.round(firstIvvs.reduce((a,b)=>a+b,0)/firstIvvs.length):null,firstMean:(()=>{const v=currentCows.map(r=>r.firstCalvingAgeMonths).filter(x=>x!=null);return v.length?Math.round(v.reduce((a,b)=>a+b,0)/v.length*10)/10:null})(),calvingRate:breedingFemales.length?Math.round(calvedLast12/breedingFemales.length*1000)/10:null,primipRate:currentCows.length?Math.round(primipares.length/currentCows.length*1000)/10:null};
