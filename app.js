...
// Cette fonction manquait dans la v14.6.21.98 et provoquait l'erreur splitUsefulLines is not defined.
...
// v14.6.21.98 — Audits ciblés + motif libre + orientation syndromique veaux
...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Repères visuels, fiches pratiques et carnet technique, disponibles hors ligne.</div></div><span class="badge autosave">v14.6.21.98</span></div>
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.6.21.98</span></div>
...
function renderVisits() {
  ...
  app.innerHTML = `
    <div class="section-title"><div><h2>Visites</h2><div class="muted">Toutes les visites enregistrées restent accessibles, y compris les plus anciennes.</div></div><span class="badge autosave">v14.6.21.98</span></div>
...
function renderFarmerQuestionnaires(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📨 Questionnaires éleveur</h2><div class="muted">Préparer la visite puis récupérer uniquement les réponses encore manquantes.</div></div><span class="badge autosave">v14.6.21.98</span></div>${activeVisitBanner(visit)}
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.6.21.98</span></div>
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.6.21.98</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
  visit.feeding=visit.feeding&&typeof visit.feeding==='object'?visit.feeding:{rations:[],settings:{},history:[]};
  visit.feeding.nutrition=visit.feeding.nutrition&&typeof visit.feeding.nutrition==='object'?visit.feeding.nutrition:{};
...
function renderFollowup(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Historique & évolution</h2><div class="muted">Comparer deux visites d’une même exploitation : reproduction, mortalité, mesures, alimentation, bâtiment et plans d’action.</div></div><span class="badge autosave">v14.6.21.98</span></div>
...
function herdGroupedBarLineChartSvg(labels,barSeries,lineSeries,{height=340}={}){
  ...
  let bars='';const pts=[];
  labels.forEach((lbl,i)=>{const center=pad.left+i*groupW+groupW/2,baseX=center-(barSeries.length*barW)/2;barSeries.forEach((bs,j)=>{const v=chartValue(bs.values[i]);if(v===null)return;const bh=(v/max)*plotH,bx=baseX+j*barW,by=pad.top+plotH-bh;bars+=`<rect x="${bx.toFixed(1)}" y="${by.toFixed(1)}" width="${Math.max(5,barW-4).toFixed(1)}" height="${bh.toFixed(1)}" rx="4" fill="${bs.color}"/>`;});const lv=chartValue(lineSeries?.values?.[i]);if(lv!==null)pts.push({x:center,y:y(lv),v:lv});bars+=`<text x="${center.toFixed(1)}" y="${height-14}" text-anchor="middle" font-size="15" fill="#4a5750">${escapeHtml(lbl)}</text>`;});
...
function prepSaleExitChartHtml(visit,farm,labels){
  const rows=prepAnnualSaleExitHistory(visit,farm,10),colors={lt8:'#2F5D8A',m8_16:'#5C8A4D',m16_36:'#C99324',gt36:'#8A5E8F'};
  const panel=(sex,title)=>{const series=[];[['lt8','<8 mois'],['m8_16','8–16 mois'],['m16_36','16–36 mois'],['gt36','>36 mois']].forEach(([k,l])=>{series.push({label:`${l} · E/H`,color:colors[k],values:rows.map(r=>r[`${k}_${sex}_EH`]||0)});series.push({label:`${l} · B`,color:colors[k]+'99',values:rows.map(r=>r[`${k}_${sex}_B`]||0)});});return `<div class="sale-exit-panel"><h5>${title}</h5>${herdStackedBarChartSvg(labels,series,{height:300})}</div>`;};
  ...
  return `<div class="sale-exit-chart"><p class="muted">Sorties de vente uniquement : causes <b>E et H regroupées</b>, et cause <b>B</b>. Les sorties cause M sont exclues car elles sont déjà analysées dans la mortalité. Répartition par âge à la sortie et par sexe.</p><div class="sale-exit-panels">${panel('F','Femelles')}${panel('M','Mâles')}</div></div>`;
}
...
// v14.6.21.98 — aide à la décision transversale : priorités, tendances et références internes.
...
// v14.6.21.98 — cohérence pré-visite, tableau global mouvements/renouvellement et âges lisibles.
...
// v14.6.21.98 — « fabrique du troupeau » : naissance femelles → conservation jusqu’à la reproduction → 1er vêlage → réforme.
...
// v14.6.21.98 — estimation économique pré-visite, calculée uniquement sur les données déjà disponibles.
...
function reproAgeDonutSvg(bucketRows,total){
  const cx=140,cy=120,r=60,stroke=34,circ=2*Math.PI*r;
  ...
  const labels=bucketRows.map(row=>{
    if(!row.count)return '';
    const frac=row.count/total,mid=startAngle+frac*Math.PI,labelAngle=mid,innerX=cx+Math.cos(labelAngle)*(r+stroke/2-2),innerY=cy+Math.sin(labelAngle)*(r+stroke/2-2),outerX=cx+Math.cos(labelAngle)*(r+stroke/2+18),outerY=cy+Math.sin(labelAngle)*(r+stroke/2+18),textX=cx+Math.cos(labelAngle)*(r+stroke/2+42),textY=cy+Math.sin(labelAngle)*(r+stroke/2+26),pct=Math.round(row.count/total*1000)/10;startAngle+=frac*2*Math.PI;return `<g class="repro-age-donut-label"><line x1="${innerX.toFixed(1)}" y1="${innerY.toFixed(1)}" x2="${outerX.toFixed(1)}" y2="${outerY.toFixed(1)}"></line><text x="${textX.toFixed(1)}" y="${(textY-2).toFixed(1)}" text-anchor="${reproDonutLabelAnchor(textX,cx)}"><tspan class="bucket">${escapeHtml(row.label)}</tspan><tspan x="${textX.toFixed(1)}" dy="1.15em" class="count">${row.count} · ${pct}%</tspan></text></g>`;
  }).join('');
  return `<svg class="repro-age-donut-svg" viewBox="-85 -25 490 300" role="img" aria-label="Répartition des femelles par tranche d'âge"><g transform="rotate(-90 ${cx} ${cy})">${base}${segments}</g><circle cx="${cx}" cy="${cy}" r="41" fill="var(--card,#fff)"></circle><text x="${cx}" y="${cy-4}" text-anchor="middle" class="repro-age-donut-total">${total}</text><text x="${cx}" y="${cy+18}" text-anchor="middle" class="repro-age-donut-sub">femelles</text>${labels}</svg>`;
}
...
// v14.6.21.98 — check-up, suivi d'étude, économie et conservation globale du défilement horizontal
...
// v14.6.21.98 — suivi du temps par intervenant + temps GDS préparation/terrain/compte rendu/suivi

// v14.6.21.98 — questionnaires persistants, repères de saisie, bilan sanitaire/éco partenaire, supports papier, check-up responsive.

window.addEventListener('DOMContentLoaded',()=>{initGlobalSearch();initAccordionMemory();});
// v14.6.21.98 — cohérence, imprimables préparation, animaux présents, recherche globale.
