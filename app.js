...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Repères visuels, fiches pratiques et carnet technique, disponibles hors ligne.</div></div><span class="badge autosave">v14.6.21.108</span></div>
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.6.21.108</span></div>
...
function renderVisits() {
  ...
  app.innerHTML = `
    <div class="section-title"><div><h2>Visites</h2><div class="muted">Toutes les visites enregistrées restent accessibles, y compris les plus anciennes.</div></div><span class="badge autosave">v14.6.21.108</span></div>
...
function renderFarmerQuestionnaires(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📨 Questionnaires éleveur</h2><div class="muted">Préparer la visite puis récupérer uniquement les réponses encore manquantes.</div></div><span class="badge autosave">v14.6.21.108</span></div>${activeVisitBanner(visit)}
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.6.21.108</span></div>
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.6.21.108</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
  visit.feeding=visit.feeding&&typeof visit.feeding==='object'?visit.feeding:{rations:[],settings:{},history:[]};
  visit.feeding.nutrition=visit.feeding.nutrition&&typeof visit.feeding.nutrition==='object'?visit.feeding.nutrition:{};
...
function groupQuickNotes(rawText=''){
  ...
  const flushParagraph=()=>{
    ...
    if(text)add(explicitCat||classifyQuickNote(text),text);
    paragraph=[];
  };
  for(const sourceLine of lines){
    const line=sourceLine.trim();
    if(!line){flushParagraph();explicitCat='';continue;}
    const heading=quickNoteHeadingCategory(line);
    const looksHeading=heading && (line.endsWith(':') || line.length<48 || /^#{1,3}\s/.test(line));
    if(looksHeading){flushParagraph();explicitCat=heading;continue;}
    const isBullet=/^[•\-–]\s+/.test(line);
    if(isBullet){flushParagraph();add(explicitCat||classifyQuickNote(line),line);continue;}
    if(explicitCat){paragraph.push(line);continue;}
    // Un compte rendu déjà rédigé : on garde le paragraphe entier au lieu de créer une fiche par phrase.
    paragraph.push(line);
  }
  flushParagraph();
  return order.map(category=>({
    category,
    text:groups.get(category).map(x=>`• ${x}`).join('\n'),
    use:true
  }));
}
...
function renderQuickFieldNotes(){
  ...
  if(SpeechRecognition&&mic){
    ...
    let committedText='';
    let liveInterim='';
    let cycleFinalByIndex=new Map();
    let lastStopWasError=false;
    const cleanSpeech=s=>String(s||'').replace(/\s+/g,' ').trim();
    const normSpeech=s=>cleanSpeech(s).toLocaleLowerCase('fr-FR').replace(/[’']/g,"'").replace(/[^a-zà-ÿ0-9' -]/gi,'').replace(/\s+/g,' ').trim();
    const mergeSpeech=(base,next)=>{
      base=cleanSpeech(base); next=cleanSpeech(next);
      if(!next)return base;if(!base)return next;
      const nb=normSpeech(base),nn=normSpeech(next);
      if(!nn||nn===nb||nb.endsWith(nn))return base;
      if(nn.startsWith(nb))return next;
      if(nn.length>14&&nb.includes(nn))return base;
      const bw=base.split(/\s+/),nw=next.split(/\s+/);let overlap=0;
      for(let k=Math.min(bw.length,nw.length,40);k>=1;k--){if(normSpeech(bw.slice(-k).join(' '))===normSpeech(nw.slice(0,k).join(' '))){overlap=k;break;}}
      return cleanSpeech(base+' '+nw.slice(overlap).join(' '));
    };
    const combinedValue=(interim='')=>{
      const dictated=cleanSpeech([committedText,interim].filter(Boolean).join(' '));
      const base=baseBeforeDictation.trimEnd();
      return dictated?(base?(base+'\n'+dictated):dictated):baseBeforeDictation;
    };
    const renderLive=(interim='')=>{
      liveInterim=cleanSpeech(interim);
      raw.value=combinedValue(liveInterim);
      raw.scrollTop=raw.scrollHeight;
      rememberQuickFieldNotesUi(visit.id,raw);
    };
    const commitCurrentCycle=({includeInterim=false}={})=>{
      const cycle=[...cycleFinalByIndex.entries()].sort((a,b)=>a[0]-b[0]).map(([,t])=>t).filter(Boolean).join(' ');
      let piece=cleanSpeech(cycle);
      if(includeInterim&&liveInterim)piece=mergeSpeech(piece,liveInterim);
      if(piece)committedText=mergeSpeech(committedText,piece);
      cycleFinalByIndex.clear();liveInterim='';
      raw.value=combinedValue('');
      // Seuls les segments finalisés (ou le dernier segment récupéré à la coupure) sont persistés.
      saveRaw();raw.scrollTop=raw.scrollHeight;
    };
    ...
    const stopRecognition=()=>{
      dictationWanted=false;if(restartTimer){clearTimeout(restartTimer);restartTimer=null;}
      commitCurrentCycle({includeInterim:true});
      try{recognition?.stop();}catch(_){ }
      setMicUi(false);status.textContent='Dictée arrêtée. Le texte affiché est enregistré dans les notes.';
    };
    const startRecognition=()=>{
      if(!dictationWanted)return;
      try{
        cycleFinalByIndex=new Map();liveInterim='';lastStopWasError=false;
        ...
        rec.onstart=()=>{setMicUi(true);status.textContent='🎙️ Dictée en cours — le texte apparaît ci-dessus au fur et à mesure.';};
        rec.onresult=e=>{
          let interimParts=[];
          // Les index appartiennent à cette instance de reconnaissance : on remplace un index déjà reçu au lieu de l'ajouter.
          for(let i=0;i<e.results.length;i++){
            const result=e.results[i],t=cleanSpeech(result?.[0]?.transcript||'');if(!t)continue;
            if(result.isFinal)cycleFinalByIndex.set(i,t);else interimParts.push(t);
          }
          const cycleFinal=[...cycleFinalByIndex.entries()].sort((a,b)=>a[0]-b[0]).map(([,t])=>t).join(' ');
          const preview=mergeSpeech(cycleFinal,interimParts.join(' '));
          renderLive(preview);
          status.textContent=interimParts.length?'🎙️ Écoute en cours…':'🎙️ Phrase prise en compte.';
          // Dès qu'un résultat final arrive, on sécurise les données sans attendre l'arrêt du micro.
          if([...e.results].some(r=>r.isFinal)){
            const global=mergeSpeech(committedText,cycleFinal);
            const val=(baseBeforeDictation.trimEnd()?baseBeforeDictation.trimEnd()+'\n':'')+global;
            n.raw=val;n.updatedAt=new Date().toISOString();visit.updatedAt=n.updatedAt;saveDatabase(db);
          }
        };
        rec.onerror=e=>{
          const err=e?.error||'erreur';lastStopWasError=true;
          if(err==='not-allowed'||err==='service-not-allowed'||err==='audio-capture'){
            dictationWanted=false;commitCurrentCycle({includeInterim:true});setMicUi(false);status.textContent=`Dictée impossible (${err}). Vérifiez l’autorisation du micro.`;
          }else if(dictationWanted){status.textContent=`Dictée momentanément interrompue (${err}). Le texte déjà affiché est conservé, reprise automatique…`;}
        };
        rec.onend=()=>{
          recognition=null;__fieldNotesRecognition=null;
          // Sur iOS/Android, un petit dernier morceau peut rester "interim" au moment où le moteur se coupe : on le conserve.
          commitCurrentCycle({includeInterim:true});
          if(dictationWanted){status.textContent='🎙️ Reprise automatique de la dictée…';restartTimer=setTimeout(()=>{restartTimer=null;startRecognition();},lastStopWasError?700:250);}
          else{setMicUi(false);status.textContent='Dictée arrêtée. Le texte reconnu est conservé dans les notes.';}
        };
        rec.start();
      }catch(err){console.error(err);if(dictationWanted){status.textContent='La dictée a été interrompue. Le texte déjà pris est conservé ; nouvelle tentative…';restartTimer=setTimeout(()=>{restartTimer=null;startRecognition();},900);}}
    };
    mic.onclick=()=>{
      if(dictationWanted){stopRecognition();return;}
      dictationWanted=true;baseBeforeDictation=raw.value||'';committedText='';liveInterim='';cycleFinalByIndex=new Map();startRecognition();
    };
  }
...
function renderFollowup(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Historique & évolution</h2><div class="muted">Comparer deux visites d’une même exploitation : reproduction, mortalité, mesures, alimentation, bâtiment et plans d’action.</div></div><span class="badge autosave">v14.6.21.108</span></div>
...
if ('serviceWorker' in navigator){
  ...
  const AUDIT_SW=location.origin+AUDIT_PATH+'sw.js?v=14.6.21.108';
