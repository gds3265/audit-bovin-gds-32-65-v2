...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Repères visuels, fiches pratiques et carnet technique, disponibles hors ligne.</div></div><span class="badge autosave">v14.6.21.33</span></div>
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.6.21.33</span></div>
...
function renderVisits() {
  ...
  app.innerHTML = `
    <div class="section-title"><div><h2>Visites</h2><div class="muted">Toutes les visites enregistrées restent accessibles, y compris les plus anciennes.</div></div><span class="badge autosave">v14.6.21.33</span></div>
...
function renderFarmerQuestionnaires(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📨 Questionnaires éleveur</h2><div class="muted">Préparer la visite puis récupérer uniquement les réponses encore manquantes.</div></div><span class="badge autosave">v14.6.21.33</span></div>${activeVisitBanner(visit)}
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.6.21.33</span></div>
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.6.21.33</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
...
function renderFollowup(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Historique & évolution</h2><div class="muted">Comparer deux visites d’une même exploitation : reproduction, mortalité, mesures, alimentation, bâtiment et plans d’action.</div></div><span class="badge autosave">v14.6.21.33</span></div>
...
// v14.6.21.33 — navigation par workflow
...
// v14.6.21.33 — check-up, suivi d'étude, économie et conservation globale du défilement horizontal
...
function renderEndVisitCheckup(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>✅ Check-up fin de visite</h2><div class="muted">Dernier balayage avant de quitter l’élevage. Rien n’est bloquant : Fait / À faire / Non nécessaire.</div></div><span class="badge ${left?'in-progress':'complete'}">${left?left+' à vérifier':'Complet'}</span></div>${activeVisitBanner(v)}<section class="card"><div class="table-wrap"><table><thead><tr><th>Élément</th><th>Statut</th><th>Commentaire / ce qui manque</th></tr></thead><tbody>${rows.map(([k,x])=>`<tr data-checkup="${k}"><td><strong>${escapeHtml(x.label)}</strong></td><td><select data-checkup-status><option ${x.status==='Fait'?'selected':''}>Fait</option><option ${x.status==='À faire'?'selected':''}>À faire</option><option ${x.status==='Non nécessaire'?'selected':''}>Non nécessaire</option></select></td><td><input data-checkup-note value="${escapeHtml(x.note||'')}" placeholder="ex. prendre 6 copros, récupérer étiquette minéral…"></td></tr>`).join('')}</tbody></table></div></section><section class="card notice ${left?'warning':''}"><strong>${left?'⚠️ Il reste '+left+' point(s) à vérifier.':'✓ Check-up terminé.'}</strong><br>Vous pouvez néanmoins passer à Analyse & restitution à tout moment.</section>`;
...
function ensureStudyTracking(v){
  ...
  s.attendees=Array.isArray(s.attendees)?s.attendees:[];s.tests=Array.isArray(s.tests)?s.tests:[];s.farmerChanges=Array.isArray(s.farmerChanges)?s.farmerChanges:[];s.notes=s.notes||'';s.gdsTime=s.gdsTime&&typeof s.gdsTime==='object'?s.gdsTime:{preparation:'',onSite:'',reporting:'',followup:''};return s;
}
...
// v14.6.21.33 — suivi du temps par intervenant + temps GDS préparation/terrain/compte rendu/suivi
