...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Repères visuels, fiches pratiques et carnet technique, disponibles hors ligne.</div></div><span class="badge autosave">v14.6.21.23</span></div>
...
function render() {
  const renderers = { dashboard: renderDashboard, farms: renderFarms, journal: renderJournalSuivi, documents: renderFarmDocuments, visits: renderVisits, questionnaires: renderFarmerQuestionnaires, animals: renderAnimals, analysis: renderAnalysis, assistant: renderAssistantGDS, review: renderPeerReview, feeding: renderFeeding, nutrition: renderNutritionAnalysis, reproduction: renderReproduction, building: renderBuilding, audit: renderAuditGlobal, planches: renderPlanches, photos: renderPhotos, herddata: renderHerdData, followup: renderFollowup, pilotage: renderPilotageActions, reports: renderReports, backup: renderBackup };
  app.innerHTML = '';
  try {
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.6.21.23</span></div>
...
function renderVisits() {
  ...
  app.innerHTML = `
    <div class="section-title"><div><h2>Visites</h2><div class="muted">Toutes les visites enregistrées restent accessibles, y compris les plus anciennes.</div></div><span class="badge autosave">v14.6.21.23</span></div>
...
function renderFarmerQuestionnaires(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📨 Questionnaires éleveur</h2><div class="muted">Préparer la visite puis récupérer uniquement les réponses encore manquantes.</div></div><span class="badge autosave">v14.6.21.23</span></div>${activeVisitBanner(visit)}
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.6.21.23</span></div>
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.6.21.23</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
...
function renderFollowup(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Historique & évolution</h2><div class="muted">Comparer deux visites d’une même exploitation : reproduction, mortalité, mesures, alimentation, bâtiment et plans d’action.</div></div><span class="badge autosave">v14.6.21.23</span></div>
...
function renderReports(){
  ...
  const technicalDefaults={raw:true,observations:true,analysis:false,reasoning:false,audit:true,reproduction:true,'reproduction-details':false,feeding:true,building:true,herddata:true,photos:false};
  const expertDefaults={raw:false,observations:true,analysis:true,reasoning:true,audit:false,reproduction:true,'reproduction-details':true,feeding:true,building:true,herddata:true,photos:false};
...
function initInstallButton(){
  ...
  btn.onclick=async()=>{
    if(deferredInstallPrompt){deferredInstallPrompt.prompt();try{await deferredInstallPrompt.userChoice;}catch(_){}deferredInstallPrompt=null;btn.hidden=true;return;}
    showToast('Sur Chrome ordinateur : ⋮ → Caster, enregistrer et partager → Installer la page en tant qu’application. Sur Edge : ⋯ → Applications → Installer ce site en tant qu’application.');
  };
...
if ('serviceWorker' in navigator){
  const AUDIT_PWA_SCOPE='/audit-bovin-gds-32-65-v2/';
  const AUDIT_SW=AUDIT_PWA_SCOPE+'sw.js?v=14.6.21.23';
  navigator.serviceWorker.register(AUDIT_SW,{scope:AUDIT_PWA_SCOPE,updateViaCache:'none'}).then(async r=>{
    try{await r.update();}catch(_){}
    try{
      const controller=navigator.serviceWorker.controller;
      const foreign=controller && !String(controller.scriptURL||'').includes('/audit-bovin-gds-32-65-v2/sw.js');
      if(foreign && !sessionStorage.getItem('audit-pwa-scope-reload')){
        sessionStorage.setItem('audit-pwa-scope-reload','1');
        navigator.serviceWorker.addEventListener('controllerchange',()=>location.reload(),{once:true});
        setTimeout(()=>location.reload(),1200);
      }else if(!foreign){sessionStorage.removeItem('audit-pwa-scope-reload');}
    }catch(_){}
  }).catch(console.error);
}
render();
