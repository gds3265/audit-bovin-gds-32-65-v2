...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Quelques repères visuels utiles, intégrés à l’application et disponibles hors ligne.</div></div><span class="badge autosave">v14.6.5</span></div>
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.6.5</span></div>
...
function renderVisits() {
  ...
  app.innerHTML = `
    <div class="section-title"><div><h2>Visites</h2><div class="muted">Toutes les visites enregistrées restent accessibles, y compris les plus anciennes.</div></div><span class="badge autosave">v14.6.5</span></div>
  ...
  document.getElementById('visit-search')?.addEventListener('input',e=>{localStorage.setItem('audit-bovin-visits-search',e.target.value);rerender()});
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.6.5</span></div>
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.6.5</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
...
function renderFollowup(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Historique & évolution</h2><div class="muted">Comparer deux visites d’une même exploitation : reproduction, mortalité, mesures, alimentation, bâtiment et plans d’action.</div></div><span class="badge autosave">v14.6.5</span></div>
...
function currentReproductionCows(farm,atDate=new Date().toISOString().slice(0,10)){
  return (farm?.herdRegistry||[])
    ...
    .map(c=>{const r=reproductionForCow(farm,c.id);const calves=r.calves.filter(v=>!atDate||!v.birthDate||v.birthDate<=atDate);const dates=[...new Set(calves.map(a=>a.birthDate).filter(Boolean))].sort();const intervals=[];for(let i=1;i<dates.length;i++){const d=daysBetweenDates(dates[i-1],dates[i]);if(d!==null)intervals.push(d);}const deadBefore6=calves.filter(v=>v.exitCause==='M'&&v.exitDate&&daysBetweenDates(v.birthDate,v.exitDate)!==null&&daysBetweenDates(v.birthDate,v.exitDate)<183);const first=calves[0]||null,last=calves.at(-1)||null;const firstAge=r.cow&&first?monthsBetweenDates(r.cow.birthDate,first.birthDate):null;const mean=intervals.length?Math.round(intervals.reduce((a,b)=>a+b,0)/intervals.length):null;return {...r,calves,calvingDates:dates,intervals,firstCalvingDate:first?.birthDate||'',lastCalvingDate:last?.birthDate||'',firstCalvingAgeMonths:firstAge,lastCalf:last,lastIVV:intervals.at(-1)??null,meanIVV:mean,minIVV:intervals.length?Math.min(...intervals):null,maxIVV:intervals.length?Math.max(...intervals):null,deadBefore6,daysSinceLast:last?daysBetweenDates(last.birthDate,atDate):null};})
...
function renderReproduction(){
  ...
  const fallbackPeriod=reproductionRegistryPeriod(registry),periodBounds={from:meta?.period?.from||fallbackPeriod.from||'',to:meta?.period?.to||fallbackPeriod.to||new Date().toISOString().slice(0,10)};
  const periodStorageKey=`audit-bovin-repro-period-${visit?.id||selectedFarmId||'default'}`;
  let savedPeriod={};
  try{savedPeriod=JSON.parse(localStorage.getItem(periodStorageKey)||'{}')||{}}catch(_){savedPeriod={};}
  let periodStart=savedPeriod.start||periodBounds.from||'';
  let periodEnd=savedPeriod.end||periodBounds.to||new Date().toISOString().slice(0,10);
  if(periodBounds.from&&periodStart<periodBounds.from)periodStart=periodBounds.from;
  if(periodBounds.to&&periodEnd>periodBounds.to)periodEnd=periodBounds.to;
  if(periodStart&&periodEnd&&periodStart>periodEnd){periodStart=periodBounds.from||periodEnd;}
  const analysisDate=periodEnd||new Date().toISOString().slice(0,10);
  const presentFemales=registry.filter(a=>a.sex==='F'&&isRegistryAnimalPresent(a,analysisDate));
  const currentCows=currentReproductionCows(reproFarm,analysisDate),cowIds=new Set(currentCows.map(r=>normalizeAnimalId(r.cow.id)));
  const breedingFemales24=presentFemales.filter(a=>monthsBetweenDates(a.birthDate,analysisDate)>24),breedingFemales36=presentFemales.filter(a=>monthsBetweenDates(a.birthDate,analysisDate)>36),presentHeifers=breedingFemales24.filter(a=>!cowIds.has(normalizeAnimalId(a.id)));
  const intervalEvents=currentCows.flatMap(r=>r.intervals.map((value,i)=>({cowId:r.cow.id,value,date:r.calvingDates[i+1]||''}))).filter(e=>e.date&&(!periodStart||e.date>=periodStart)&&(!periodEnd||e.date<=periodEnd));
  const allIvvs=intervalEvents.map(e=>e.value),firstIvvs=currentCows.map(r=>({value:r.intervals[0],date:r.calvingDates[1]||''})).filter(e=>e.value!=null&&e.date&&(!periodStart||e.date>=periodStart)&&(!periodEnd||e.date<=periodEnd)).map(e=>e.value);
  const allCalves=currentCows.flatMap(r=>r.calves).filter(c=>c.birthDate&&(!periodStart||c.birthDate>=periodStart)&&(!periodEnd||c.birthDate<=periodEnd));
  const deadCalves=allCalves.filter(c=>c.exitCause==='M'&&c.exitDate&&daysBetweenDates(c.birthDate,c.exitDate)<183),maleIds=new Set(registry.filter(a=>a.sex==='M').map(a=>normalizeAnimalId(a.id))),knownFather=allCalves.filter(c=>c.fatherId),probableIA=knownFather.filter(c=>!maleIds.has(normalizeAnimalId(c.fatherId)));
  const primipares=currentCows.filter(r=>r.calves.length===1||(!periodStart&&r.calves.length===1));
  const mothersInPeriod=new Set(allCalves.map(c=>normalizeAnimalId(c.motherId)));
  const breeding24Ids=new Set(breedingFemales24.map(a=>normalizeAnimalId(a.id))),breeding36Ids=new Set(breedingFemales36.map(a=>normalizeAnimalId(a.id)));
  const calved24=[...mothersInPeriod].filter(id=>breeding24Ids.has(id)).length,calved36=[...mothersInPeriod].filter(id=>breeding36Ids.has(id)).length;
  const stats={
    ivvMean:allIvvs.length?Math.round(allIvvs.reduce((a,b)=>a+b,0)/allIvvs.length):null,
    ivv12:firstIvvs.length?Math.round(firstIvvs.reduce((a,b)=>a+b,0)/firstIvvs.length):null,
    ivvMin:allIvvs.length?Math.min(...allIvvs):null,
    ivvMax:allIvvs.length?Math.max(...allIvvs):null,
    firstMean:(()=>{const v=currentCows.map(r=>r.firstCalvingAgeMonths).filter(x=>x!=null);return v.length?Math.round(v.reduce((a,b)=>a+b,0)/v.length*10)/10:null})(),
    calvingRate24:breedingFemales24.length?Math.round(calved24/breedingFemales24.length*1000)/10:null,
    calvingRate36:breedingFemales36.length?Math.round(calved36/breedingFemales36.length*1000)/10:null,
    primipRate:currentCows.length?Math.round(primipares.length/currentCows.length*1000)/10:null
  };
  ...
  const sourceInfo=meta?`<div class="repro-source-card"><div class="repro-source-head"><strong>📄 CSV utilisé pour cette visite</strong><span class="repro-file">${escapeHtml(meta.fileName||'Nom non disponible')}</span></div><div class="repro-source-meta"><span class="repro-meta-pill">${meta.rowCount||registry.length} ligne(s)</span>${meta.importedAt?`<span class="repro-meta-pill">Importé le ${formatDateTime(meta.importedAt)}</span>`:''}${meta.period?.from||meta.period?.to?`<span class="repro-meta-pill">Données CSV ${meta.period?.from?formatDate(meta.period.from):'—'} → ${meta.period?.to?formatDate(meta.period.to):'—'}</span>`:''}</div><div class="repro-period-bar"><div class="field"><label>Période d’analyse du registre</label><div class="repro-period-inputs"><input id="repro-period-start" type="date" min="${periodBounds.from||''}" max="${periodBounds.to||''}" value="${periodStart||''}"><span>→</span><input id="repro-period-end" type="date" min="${periodBounds.from||''}" max="${periodBounds.to||''}" value="${periodEnd||''}"></div></div><button class="btn secondary btn small" id="repro-period-reset" type="button">Toute la période importée</button></div></div>`:`<div class="notice warning"><strong>CSV source non identifié.</strong> Réimportez le registre une fois pour enregistrer son nom avec cette visite.</div>`;
  app.innerHTML=`<div class="section-title"><div><h2>Reproduction</h2><div class="muted">IVV, vêlages, primipares, femelles sans vêlage, mortalité des veaux et origine probable IA.</div></div><span class="badge autosave">v14.6.5</span></div>${activeVisitBanner(visit)}<section class="card repro-import-card"><div class="row"><div class="field"><label>Exploitation</label><select id="repro-farm" ${visit?'disabled':''}>${db.farms.map(f=>`<option value="${f.id}" ${f.id===selectedFarmId?'selected':''}>${escapeHtml(f.name)}</option>`).join('')}</select></div><div class="field"><label>${meta?'Remplacer le registre bovins CSV':'Importer le registre bovins CSV'}</label><input id="repro-file" type="file" accept=".csv,text/csv"></div></div>${sourceInfo}${registry.length?`<div class="repro-summary-chips"><span>${registry.length} bovin(s) historiques</span><span>${presentFemales.length} femelle(s) présente(s) à la fin de période</span><span>${currentCows.length} vache(s) avec vêlage</span></div>`:'<div class="muted">Aucun registre importé pour cette visite.</div>'}</section>${!registry.length?'<section class="empty" style="margin-top:16px">Importez le fichier CSV Reproduction.</section>':`<section class="repro-kpi-grid compact"><article class="card metric"><strong>${stats.calvingRate24??'—'}%</strong><span>Taux de vêlage &gt; 24 mois</span><small>Mères ayant vêlé sur la période / femelles présentes &gt; 24 mois</small></article><article class="card metric"><strong>${stats.calvingRate36??'—'}%</strong><span>Taux de vêlage &gt; 36 mois</span><small>Mères ayant vêlé sur la période / femelles présentes &gt; 36 mois</small></article><article class="card metric"><strong>${primipares.length}</strong><span>Primipares</span><small>${stats.primipRate??'—'}% des vaches avec vêlage</small></article><article class="card metric"><strong>${stats.ivvMean??'—'}</strong><span>IVV moyen</span><small>En jours sur la période choisie</small></article><article class="card metric"><strong>${stats.ivvMin??'—'}</strong><span>IVV mini</span><small>Intervalle minimal observé</small></article><article class="card metric"><strong>${stats.ivvMax??'—'}</strong><span>IVV maxi</span><small>Intervalle maximal observé</small></article><article class="card metric"><strong>${stats.ivv12??'—'}</strong><span>IVV1–IVV2 moyen</span><small>En jours</small></article><article class="card metric"><strong>${stats.firstMean??'—'}</strong><span>Âge moyen au 1er vêlage</span><small>En mois</small></article><article class="card metric"><strong>${presentHeifers.length}</strong><span>Femelles &gt; 24 mois sans vêlage</span><small>Femelles présentes sans vêlage enregistré</small></article><article class="card metric"><strong>${allCalves.length?Math.round(deadCalves.length/allCalves.length*1000)/10:'—'}%</strong><span>Mortalité veaux &lt; 6 mois</span><small>${deadCalves.length}/${allCalves.length} veaux sur la période</small></article><article class="card metric"><strong>${knownFather.length?probableIA.length:'—'}</strong><span>Veaux probablement issus d’IA</span><small>${knownFather.length?`${Math.round(probableIA.length/knownFather.length*1000)/10}% des pères renseignés`:'Colonne père absente ou vide'}</small></article></section><section class="card repro-ivv-card"><h3>Répartition des IVV sur la période choisie</h3><div class="ivv-band-grid">${ivvBands.map(([l,n])=>`<div><strong>${n}</strong><span>${l}</span></div>`).join('')}</div></section><section class="card"><div class="row"><div class="field"><label>Filtre <span class="badge">${rows.length}</span></label><select id="repro-filter"><option value="all">Toutes les vaches présentes</option><option value="400">Sans vêlage depuis plus de 400 j</option><option value="ivvLe400">IVV moyen ≤ 400 j</option><option value="ivv401to450">IVV moyen 401–450 j</option><option value="ivv451to500">IVV moyen 451–500 j</option><option value="ivvOver500">IVV moyen &gt; 500 j</option><option value="dead">Au moins 2 veaux morts avant 6 mois</option><option value="under28">1er vêlage avant 28 mois</option><option value="28to36">1er vêlage 28–36 mois</option><option value="over36">1er vêlage après 36 mois</option></select></div><div class="field"><label>Classement</label><select id="repro-sort"><option value="lastCalvingOld">Dernier vêlage le plus ancien</option><option value="ivvHigh">IVV moyen le plus élevé</option><option value="firstAgeHigh">1er vêlage le plus tardif</option><option value="deadHigh">Mortalité veaux la plus élevée</option><option value="scoreLow">Score le plus faible</option></select></div></div><div class="muted repro-filter-count"><strong>${rows.length}</strong> vache(s) correspondent au filtre sur ${currentCows.length} à la date du ${periodEnd?formatDate(periodEnd):'dernier enregistrement'}.</div><div class="table-wrap"><table><thead><tr><th>Vache</th><th>Âge / race</th><th>1er vêlage</th><th>Dernier vêlage</th><th>IVV complet</th><th>Veaux</th><th>Score</th><th></th></tr></thead><tbody>${rows.map(r=>`<tr><td><strong>${escapeHtml(r.cow.workNumber||r.cow.id)}</strong><br><small>${escapeHtml(r.cow.id)}</small></td><td>${ageLabelAt(r.cow.birthDate,analysisDate)||'—'}<br><small>Race ${escapeHtml(r.cow.breed||'—')}</small></td><td>${r.firstCalvingDate?formatDate(r.firstCalvingDate):'—'}<br><small>${r.firstCalvingAgeMonths!=null?`${r.firstCalvingAgeMonths} mois`:'—'}</small></td><td>${r.lastCalvingDate?formatDate(r.lastCalvingDate):'—'}<br><small>${r.daysSinceLast??'—'} j</small></td><td>Moy. ${r.meanIVV??'—'} · mini ${r.minIVV??'—'} · maxi ${r.maxIVV??'—'}<br><small>${r.intervals.length?r.intervals.join(' / ')+' j':'Un seul vêlage'}</small></td><td>${r.calves.length}<br><small>${r.deadBefore6.length} mort(s) &lt;6 mois</small></td><td><button class="badge ${reproductionScore(r)>=75?'complete':reproductionScore(r)>=60?'in-progress':'danger'}" data-repro-score="${escapeHtml(r.cow.id)}">${reproductionScore(r)}/100</button></td><td><button class="btn small" data-repro-detail="${escapeHtml(r.cow.id)}">Voir</button></td></tr>`).join('')||'<tr><td colspan="8">Aucun animal correspondant.</td></tr>'}</tbody></table></div></section><section class="card"><div class="section-title"><div><h3>Femelles de plus de 24 mois présentes sans vêlage</h3><div class="muted">Les génisses plus jeunes ne sont pas comptées dans cette alerte.</div></div><span class="badge">${presentHeifers.length}</span></div>${presentHeifers.length?`<div class="table-wrap"><table><thead><tr><th>Femelle</th><th>Date de naissance</th><th>Âge</th><th>Race</th><th>Alerte</th></tr></thead><tbody>${presentHeifers.map(g=>{const age=monthsBetweenDates(g.birthDate,analysisDate);return `<tr><td><strong>${escapeHtml(g.workNumber||g.id)}</strong><br><small>${escapeHtml(g.id)}</small></td><td>${g.birthDate?formatDate(g.birthDate):'—'}</td><td>${ageLabelAt(g.birthDate,analysisDate)||'—'}</td><td>${escapeHtml(g.breed||'—')}</td><td>${age!=null&&age>36?'<span class="badge danger">Plus de 36 mois</span>':age!=null&&age>28?'<span class="badge in-progress">À surveiller</span>':'<span class="badge">24–28 mois</span>'}</td></tr>`}).join('')}</tbody></table></div>`:'<div class="empty">Aucune femelle de plus de 24 mois présente sans vêlage.</div>'}</section><section id="repro-detail"></section>`}`;
  ...
  const savePeriod=()=>{const start=ps?.value||periodBounds.from||'',end=pe?.value||periodBounds.to||'';if(ps&&pe&&start&&end&&start>end){showToast('La date de début doit être antérieure à la date de fin.');return;}localStorage.setItem(periodStorageKey,JSON.stringify({start,end}));renderReproduction();};
  if(ps)ps.onchange=savePeriod;if(pe)pe.onchange=savePeriod;if(pr)pr.onclick=()=>{localStorage.setItem(periodStorageKey,JSON.stringify({start:periodBounds.from||'',end:periodBounds.to||''}));renderReproduction();};
  ...
  app.querySelectorAll('[data-repro-score]').forEach(b=>b.onclick=()=>{const r=currentCows.find(x=>normalizeAnimalId(x.cow.id)===normalizeAnimalId(b.dataset.reproScore))||reproductionForCow(reproFarm,b.dataset.reproScore),d=reproductionScoreDetails(r),box=document.getElementById('repro-detail');box.innerHTML=`<section class="card"><h3>Détail du score · ${escapeHtml(r.cow.workNumber||r.cow.id)}</h3><div class="table-wrap"><table><tbody>${d.lines.map(x=>`<tr><td>${escapeHtml(x.label)}</td><td>${escapeHtml(x.value)}</td><td>${x.delta}</td></tr>`).join('')}</tbody></table></div></section>`;box.scrollIntoView({behavior:'smooth'})});
  app.querySelectorAll('[data-repro-detail]').forEach(b=>b.onclick=()=>{const r=currentCows.find(x=>normalizeAnimalId(x.cow.id)===normalizeAnimalId(b.dataset.reproDetail))||reproductionForCow(reproFarm,b.dataset.reproDetail),box=document.getElementById('repro-detail');box.innerHTML=`<section class="card"><h3>${escapeHtml(r.cow.workNumber||r.cow.id)} — historique complet</h3><div class="table-wrap"><table><thead><tr><th>Date</th><th>Veau</th><th>Père</th><th>Sortie</th><th>IVV précédent</th></tr></thead><tbody>${r.calves.map((c,i)=>`<tr><td>${formatDate(c.birthDate)}</td><td>${escapeHtml(c.workNumber||c.id)}</td><td>${escapeHtml(c.fatherId||'Non renseigné')}</td><td>${escapeHtml(c.exitCause||'Présent')}</td><td>${i?r.intervals[i-1]+' j':'—'}</td></tr>`).join('')}</tbody></table></div></section>`;box.scrollIntoView({behavior:'smooth'})});
}
function applyWorkModefunction applyWorkMode(mode){
...
initWorkMode();
initGlobalSearch();
if ('serviceWorker' in navigator) navigator.serviceWorker.register('./sw.js?v=14.6.2',{updateViaCache:'none'}).then(r=>r.update()).catch(console.error);
render();
