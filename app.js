...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Quelques repères visuels utiles, intégrés à l’application et disponibles hors ligne.</div></div><span class="badge autosave">v14.6.12</span></div>
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.6.12</span></div>
...
function renderVisits() {
  ...
  app.innerHTML = `
    <div class="section-title"><div><h2>Visites</h2><div class="muted">Toutes les visites enregistrées restent accessibles, y compris les plus anciennes.</div></div><span class="badge autosave">v14.6.12</span></div>
...
function scheduleAnalysisSave(delay=450){
  clearTimeout(analysisSaveTimer);
  analysisSaveTimer=setTimeout(()=>{analysisSaveTimer=null;saveDatabase(db);},delay);
}
function flushAnalysisSave(){
  clearTimeout(analysisSaveTimer);analysisSaveTimer=null;saveDatabase(db);
}
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.6.12</span></div>
...
function renderAnalysis() {
  ...
  if(!activeVisitId&&visits.length)setActiveVisit(visits[0].id);
  const visit=activeVisit();
  if(visit){enrichVisitSubjectsFromReproduction(visit);ensureAnalysisVisit(visit);}
...
function bindAnalysisEvents(visit) {
  if(!visit)return;
  enhanceNumericEntry(app);
  app.querySelectorAll('.analysis-input').forEach(input=>{const persist=(normalize=false,immediate=false)=>{const s=visit.subjects.find(x=>x.id===input.dataset.subjectId);if(!s)return;let value=input.value;if(normalize){value=normalizedMeasurementValue(value);input.value=value;}s.measurements.analysis[input.dataset.param]=value;s.updatedAt=new Date().toISOString();visit.updatedAt=new Date().toISOString();if(immediate)flushAnalysisSave();else scheduleAnalysisSave();const result=s.category&&s.category!=='Non classé'?classifyValue(value,thresholdFor(s,input.dataset.param)):(value===''?{status:'empty',label:'Non mesuré'}:{status:'unclassified',label:'Classer le sujet'});const cell=input.closest('.analysis-value-cell');if(cell){cell.className=`analysis-value-cell ${result.status}`;const small=cell.querySelector('small');if(small)small.textContent=result.label;}};input.oninput=()=>persist(false,false);input.onchange=()=>persist(true,true);input.onblur=()=>persist(true,true);});
  app.querySelectorAll('[data-family-comment]').forEach(el=>{const save=(immediate=false)=>{const s=visit.subjects.find(x=>x.id===el.dataset.subjectId);if(!s)return;s.measurements.comments=s.measurements.comments||{};s.measurements.comments[el.dataset.family]=el.value;s.updatedAt=new Date().toISOString();visit.updatedAt=new Date().toISOString();if(immediate)flushAnalysisSave();else scheduleAnalysisSave();};el.oninput=()=>save(false);el.onchange=()=>save(true);el.onblur=()=>save(true);});
  app.querySelectorAll('[data-observation]').forEach(el=>{const save=(normalize=false,immediate=false)=>{const s=visit.subjects.find(x=>x.id===el.dataset.subjectId);if(!s)return;let value=el.value;if(normalize&&el.hasAttribute('data-numeric-observation')){value=normalizedMeasurementValue(value);el.value=value;}s.measurements.observations[el.dataset.key]=value;visit.updatedAt=new Date().toISOString();if(immediate)flushAnalysisSave();else scheduleAnalysisSave();};el.oninput=()=>save(false,false);el.onchange=()=>save(true,true);el.onblur=()=>save(true,true);});
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.6.12</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
...
function renderFollowup(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Historique & évolution</h2><div class="muted">Comparer deux visites d’une même exploitation : reproduction, mortalité, mesures, alimentation, bâtiment et plans d’action.</div></div><span class="badge autosave">v14.6.12</span></div>
...
initWorkMode();
initGlobalSearch();
if ('serviceWorker' in navigator) navigator.serviceWorker.register('./sw.js?v=14.6.12',{updateViaCache:'none'}).then(r=>r.update()).catch(console.error);
render();
