...
// Cette fonction manquait dans la v14.6.21.94 et provoquait l'erreur splitUsefulLines is not defined.
...
// v14.6.21.94 — Audits ciblés + motif libre + orientation syndromique veaux
...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Repères visuels, fiches pratiques et carnet technique, disponibles hors ligne.</div></div><span class="badge autosave">v14.6.21.94</span></div>
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.6.21.94</span></div>
...
function renderVisits() {
  ...
  app.innerHTML = `
    <div class="section-title"><div><h2>Visites</h2><div class="muted">Toutes les visites enregistrées restent accessibles, y compris les plus anciennes.</div></div><span class="badge autosave">v14.6.21.94</span></div>
...
function renderFarmerQuestionnaires(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📨 Questionnaires éleveur</h2><div class="muted">Préparer la visite puis récupérer uniquement les réponses encore manquantes.</div></div><span class="badge autosave">v14.6.21.94</span></div>${activeVisitBanner(visit)}
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.6.21.94</span></div>
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.6.21.94</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
  visit.feeding=visit.feeding&&typeof visit.feeding==='object'?visit.feeding:{rations:[],settings:{},history:[]};
  visit.feeding.nutrition=visit.feeding.nutrition&&typeof visit.feeding.nutrition==='object'?visit.feeding.nutrition:{};
...
function renderFollowup(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Historique & évolution</h2><div class="muted">Comparer deux visites d’une même exploitation : reproduction, mortalité, mesures, alimentation, bâtiment et plans d’action.</div></div><span class="badge autosave">v14.6.21.94</span></div>
...
function reproMortalityAnnualHistory(visit,farm,yearsCount=10){
  ...
  const out=[];for(let y=firstYear;y<=endYear;y++){
    ...
    const deathAgeDays=a=>daysBetweenDates(a.birthDate,a.exitDate),deathAgeMonths=a=>monthsBetweenDates(a.birthDate,a.exitDate);
    ...
    const d1_6=deaths.filter(a=>{const m=deathAgeMonths(a);return m!=null&&m>=1&&m<6}).length;
    const d6_12=deaths.filter(a=>{const m=deathAgeMonths(a);return m!=null&&m>=6&&m<12}).length;
    const d12_24=deaths.filter(a=>{const m=deathAgeMonths(a);return m!=null&&m>=12&&m<24}).length;
    const dOver24=deaths.filter(a=>{const m=deathAgeMonths(a);return m!=null&&m>=24}).length;
...
function prepRegistryTenYearChartsHtml(visit,farm,annualRows=null,{forPrint=false}={}){
  ...
  const mortalitySeries=[
    {label:'0–2 j',color:'#2F5D8A',values:hist.map(r=>r.deaths0_2Pct)},
    {label:'2 j–1 mois',color:'#B85C38',values:hist.map(r=>r.deaths2_30Pct)},
    {label:'1–6 mois',color:'#6B7C72',values:hist.map(r=>r.deaths1_6Pct)},
    {label:'6–12 mois',color:'#C99324',values:hist.map(r=>r.deaths6_12Pct)},
    {label:'12–24 mois',color:'#587FA8',values:hist.map(r=>r.deaths12_24Pct)},
    {label:'≥24 mois',color:'#3E7655',values:hist.map(r=>r.deathsOver24Pct)}
  ];
...
// v14.6.21.94 — aide à la décision transversale : priorités, tendances et références internes.
...
// v14.6.21.94 — cohérence pré-visite, tableau global mouvements/renouvellement et âges lisibles.
...
// v14.6.21.94 — « fabrique du troupeau » : naissance femelles → conservation jusqu’à la reproduction → 1er vêlage → réforme.
...
function prepExtendedDecisionHtml(visit,farm,x,hist,rolling,d,{forPrint=false}={}){
  const movements=prepAnnualMovementHistory(visit,farm,10),renewalFactory=prepRenewalFactoryData(visit,farm,10),feed=prepFeedingRiskData(visit),hyp=prepCrossHypotheses(visit,farm,x,hist,rolling,d),follow=prepPreviousRecommendations(visit),san=prepSanitarySummary(visit);
  const auditNow=ensureAuditGlobal(visit),sanSection=auditGlobalSections.find(q=>q.id==='sanitaire'),hasSanitaryData=sanSection?auditSectionProgress(sanSection,auditNow).done>0:false;
  const strengths=d.themes.filter(t=>Number.isFinite(t.score)&&t.score>=75).sort((a,b)=>b.score-a.score).slice(0,5);
  const feedText=!feed.hasData?'Données de ration à compléter':feed.low.length?`${feed.low.length} couverture(s) <80% : ${feed.low.map(r=>r.label).join(', ')}`:feed.high.length?`${feed.high.length} surapport(s) potentiel(s) >180% : ${feed.high.map(r=>r.label).join(', ')}`:'Pas de déficit majeur détecté sur les éléments calculables';
  ...
  return `<div class="decision-extended ${forPrint?'for-print':''}"><div class="decision-onepage"><h3>Repères complémentaires</h3><div><h4>Points forts</h4>${strengths.length?`<ul>${strengths.map(t=>`<li><strong>${escapeHtml(t.label)}</strong> — ${escapeHtml(t.why)}</li>`).join('')}</ul>`:'<p class="muted">Pas assez de données pour dégager des points forts fiables.</p>'}</div>
  <div class="decision-topic-grid"><article><h4>🥣 Alimentation — ration à risque</h4><p>${escapeHtml(feedText)}</p><small>Les champs énergie/protéines ne sont interprétés que s’ils sont renseignés.</small></article><article><h4>🩺 Sanitaire</h4><p>${hasSanitaryData?escapeHtml(san.status):'À évaluer / confirmer pendant la visite'}</p><small>${hasSanitaryData?`${san.cur} point(s) d’attention dans l’audit sanitaire${san.prevCount!=null?` · précédente visite : ${san.prevCount}`:''}.`:'Aucun score actuel tant que les informations terrain ne sont pas renseignées.'}</small></article></div>
...
function prepDecisionSupportHtml(visit,farm,x,hist,rolling,{forPrint=false}={}){
  const d=prepDecisionSupportData(visit,farm,x,hist,rolling),status=s=>s==null?'pending':s>=75?'good':s>=55?'warn':'bad',label=s=>s==null?'À évaluer':s>=75?'Satisfaisant':s>=55?'À surveiller':'À creuser';
  const priorityAlerts=d.alerts.slice(0,5);
  ...
  return `<section class="decision-support ${forPrint?'for-print':''}"><h2>${forPrint?'0. Synthèse prioritaire avant visite':'🎯 Synthèse avant visite'}</h2><p class="muted">Une seule liste de priorités pour éviter les répétitions. Les autres blocs apportent uniquement du contexte ou des éléments à vérifier.</p><div class="decision-theme-grid">${d.themes.map(t=>`<article class="decision-theme ${status(t.score)}"><strong>${escapeHtml(t.label)}</strong><span>${t.score==null?'À évaluer pendant la visite':`${t.score}/100 · ${label(t.score)}`}</span><small>${escapeHtml(t.why)}</small></article>`).join('')}</div><h3>À creuser en priorité</h3><div class="decision-alert-list">${alertHtml}</div>${prepAnalysisComparisonHtml(hist,{forPrint})}${prepExtendedDecisionHtml(visit,farm,x,hist,rolling,d,{forPrint})}</section>`;
}
...
// v14.6.21.94 — estimation économique pré-visite, calculée uniquement sur les données déjà disponibles.
...
// v14.6.21.94 — check-up, suivi d'étude, économie et conservation globale du défilement horizontal
...
// v14.6.21.94 — suivi du temps par intervenant + temps GDS préparation/terrain/compte rendu/suivi

// v14.6.21.94 — questionnaires persistants, repères de saisie, bilan sanitaire/éco partenaire, supports papier, check-up responsive.

window.addEventListener('DOMContentLoaded',()=>{initGlobalSearch();initAccordionMemory();});
// v14.6.21.94 — cohérence, imprimables préparation, animaux présents, recherche globale.
