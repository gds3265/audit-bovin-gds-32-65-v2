...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Quelques repères visuels utiles, intégrés à l’application et disponibles hors ligne.</div></div><span class="badge autosave">v14.4.6</span></div>
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.4.6</span></div>
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.4.6</span></div>
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.4.6</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
...
function repairHerdImportFarmLinks(){
  // Ne jamais rattacher automatiquement un CSV à une autre exploitation.
  // Les anciens imports conservent uniquement leur farmId explicite.
  repairLegacyFarmNumbers();
  let cleaned=0;
  (db.herdImports||[]).forEach(item=>{
    if(item && item.farmId && !db.farms.some(f=>f.id===item.farmId)){item.farmId='';cleaned++;}
  });
  if(cleaned)saveDatabase(db);
  return{linked:0,cleaned,applied:0};
}
...
function herdImportSnapshot(item){
  if(!item)return null;
  const clean={...item};
  delete clean.raw;
  return JSON.parse(JSON.stringify(clean));
}
function linkedHerdImportForVisit(visit){
  // Source strictement figée dans la visite : aucun repli vers le dernier CSV global.
  const direct=visit?.technicalEconomicImport?.snapshot;
  if(direct)return repairHerdImport(JSON.parse(JSON.stringify(direct)));
  const link=ensureAuditGlobal(visit).importedHerdData;
  if(link?.snapshot)return repairHerdImport(JSON.parse(JSON.stringify(link.snapshot)));
  return null;
}
...
function removeHerdImportFromVisit(visit,sourceId=''){
  const a=ensureAuditGlobal(visit),oldId=sourceId||a.importedHerdData?.sourceId||'',item=a.importedHerdData?.snapshot||((db.herdImports||[]).find(x=>x.id===oldId));
  if(!oldId)return 0;let removed=0;
  if(item){const fixed=repairHerdImport(item),st=fixed.current?.structure||{},mv=fixed.current?.movements||{},yr=fixed.years?.N||{},mort=yr.mortality||{},rep=yr.reproduction||{};
    const clearIfSame=(obj,key,val)=>{if(val!==null&&val!==undefined&&val!==''&&String(obj?.[key]??'')===String(val)){obj[key]='';removed++;}};
    clearIfSame(a.renewal,'cowsTotal',st.femalesOver36);clearIfSame(a.renewal,'calvesUnder6',st.total0_6);clearIfSame(a.renewal,'heifers6_12',st.females6_12);clearIfSame(a.renewal,'heifers12_24',st.females12_24);clearIfSame(a.renewal,'replacementHeifers',((st.females6_12??0)+(st.females12_24??0))||null);clearIfSame(a.renewal,'annualReforms',mv.salesSlaughter);
    const mm={'0–2 jours':mort.h0_48,'2 jours–1 mois':((mort.d2_7??0)+(mort.d8_30??0))||null,'1–6 mois':mort.m1_6,'6–12 mois':mort.m6_12,'12–24 mois':mort.m12_24,'> 24 mois':mort.over24};
    Object.entries(mm).forEach(([cl,val])=>{const r=a.mortality?.[cl];if(r&&String(r.count??'')===String(val??'')&&/import/i.test(r.comment||'')){r.count='';r.comment='';removed++;}});
    const f=a.answers?.['Âge moyen au premier vêlage'];if(f&&String(f.answer??'')===String(rep.firstCalvingAge??'')&&/import/i.test(f.comment||'')){f.answer='';f.comment='';removed++;}
    const ivv=a.answers?.['Intervalle vêlage-vêlage'];if(ivv&&String(ivv.answer??'')===String(rep.ivv??'')&&/import/i.test(ivv.comment||'')){ivv.answer='';ivv.comment='';removed++;}
  }
  ['purchases','sales'].forEach(k=>{const before=(a[k]||[]).length;a[k]=(a[k]||[]).filter(x=>x.importSourceId!==oldId);removed+=before-a[k].length;});
  a.importedHerdData=null;visit.technicalEconomicImport=null;visit.updatedAt=new Date().toISOString();return removed;

function applyHerdImportToVisit(item,visit,overwrite=false){
  item=repairHerdImport(JSON.parse(JSON.stringify(item)));
  if(!item?.id||!visit?.id)throw new Error('Import ou visite invalide.');
  if(!item.farmId||item.farmId!==visit.farmId)throw new Error('Le CSV et la visite ne sont pas liés à la même exploitation.');
  // Casse toute éventuelle référence partagée héritée d'anciennes versions.
  visit.auditGlobal=JSON.parse(JSON.stringify(ensureAuditGlobal(visit)));
  ...
  const frozenSnapshot=herdImportSnapshot(item);
  const appliedAt=new Date().toISOString();
  a.importedHerdData={sourceId:item.id,importInstanceId:item.importInstanceId||item.id,farmId:visit.farmId,visitId:visit.id,sourceFile:item.sourceFile||'',period:JSON.parse(JSON.stringify(item.period||{})),appliedAt,changes:[...changes],snapshot:frozenSnapshot,summary:{totalHerd:st.total,births:mv.births,purchases:mv.purchases,totalOutputs:mv.totalOutputs,mortalityTotal:mort.total,mortalityYoungRate:mort.youngRate,abortions:rep.abortions,productivity:rep.productivity,unproductiveFemales:item.current?.unproductiveFemales}};
  visit.technicalEconomicImport={sourceId:item.id,importInstanceId:item.importInstanceId||item.id,farmId:visit.farmId,visitId:visit.id,sourceFile:item.sourceFile||'',appliedAt,snapshot:JSON.parse(JSON.stringify(frozenSnapshot))};
  visit.updatedAt=appliedAt;saveDatabase(db);return changes;
}
...
function renderHerdData(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Données technico-économiques</h2><span class="muted">Effectifs, mouvements, mortalité et reproduction depuis un export CSV.</span></div><span class="badge autosave">v14.4.6</span></div>${repairedLinks.cleaned?`<section class="notice warning"><strong>Import orphelin détecté :</strong> ${repairedLinks.cleaned} import(s) ne sont plus liés à une exploitation et devront être rattachés manuellement.</section>`:''}
  <section class="card"><h3>Importer un fichier d’un autre logiciel</h3><p class="muted">Le fichier est d’abord contrôlé puis enregistré pour une exploitation. <strong>Il n’est jamais appliqué automatiquement à une visite.</strong></p><div class="row"><div class="field"><label>Exploitation de destination</label><select id="herd-farm-select"><option value="">Choisir obligatoirement une exploitation</option>${db.farms.map(f=>`<option value="${f.id}" ${f.id===(herdImportPreview?.targetFarmId||'')?'selected':''}>${escapeHtml(f.name)}${f.farmNumber?` — EDE ${escapeHtml(f.farmNumber)}`:''}</option>`).join('')}</select></div><div class="field"><label>Fichier CSV</label><input id="herd-csv-input" type="file" accept=".csv,text/csv" /></div></div><div id="herd-preview">${herdImportPreview?`<div class="notice"><strong>Fichier sélectionné : ${escapeHtml(herdImportPreview.fileName)}</strong><br>${herdImportPreview.items.length} ligne(s) prête(s) à importer.<br>${herdImportPreview.items.map(herdImportLabel).map(escapeHtml).join('<br>')}<div class="actions" style="margin-top:10px"><button class="btn primary" id="confirm-herd-import">Enregistrer précisément ce fichier</button><button class="btn secondary" id="cancel-herd-import">Annuler</button></div></div>`:'<div class="empty">Sélectionnez un CSV pour afficher son nom et son aperçu avant validation.</div>'}</div></section>
  ${byDate.length?byDate.map(renderHerdImportDetail).join(''):'<section class="empty">Aucune donnée d’élevage importée.</section>'}`;
  document.getElementById('herd-farm-select')?.addEventListener('change',e=>{if(herdImportPreview)herdImportPreview.targetFarmId=e.target.value||'';});
  document.getElementById('herd-csv-input')?.addEventListener('change',async e=>{const file=e.target.files?.[0];if(!file)return;const token=++herdImportReadToken;const targetFarmId=document.getElementById('herd-farm-select')?.value||'';try{const text=await file.text();if(token!==herdImportReadToken)return;const rows=parseCsvText(text);if(!rows.length)throw new Error('Aucune ligne');herdImportPreview={fileName:file.name,fileSize:file.size,lastModified:file.lastModified,targetFarmId,items:rows.map(r=>extractHerdRow(r,file.name))};renderHerdData();}catch(err){if(token!==herdImportReadToken)return;console.error(err);herdImportPreview=null;alert('Impossible de lire ce CSV. Vérifiez qu’il contient une ligne d’en-têtes et des données séparées par des points-virgules ou des virgules.');renderHerdData();}});
  ...
  document.getElementById('confirm-herd-import')?.addEventListener('click',()=>{if(!herdImportPreview?.items?.length)return showToast('Aucun fichier prêt à importer.');const chosen=herdImportPreview.targetFarmId||document.getElementById('herd-farm-select')?.value||'';if(!chosen)return showToast('Choisissez obligatoirement l’exploitation de destination avant l’import.');const farm=db.farms.find(f=>f.id===chosen);if(!farm)return showToast('Exploitation introuvable.');const importedFile=herdImportPreview.fileName;const fileBatchId=uid('csv-batch');herdImportPreview.items.forEach(rawItem=>{let item=repairHerdImport(JSON.parse(JSON.stringify(rawItem)));item.farmId=farm.id;item.id=uid('herd-import');item.importedAt=new Date().toISOString();item.importInstanceId=uid('csv');item.fileBatchId=fileBatchId;db.herdImports.push(item);});saveDatabase(db);herdImportPreview=null;showToast(`Fichier « ${importedFile} » enregistré uniquement pour ${farm.name}. Choisissez ensuite explicitement la visite à compléter.`);renderHerdData();});
  ...
  app.querySelectorAll('[data-confirm-relink-herd]').forEach(b=>b.onclick=()=>{const item=db.herdImports.find(x=>x.id===b.dataset.confirmRelinkHerd);const farmId=app.querySelector(`[data-relink-herd-import="${CSS.escape(b.dataset.confirmRelinkHerd)}"]`)?.value;if(!item||!farmId)return showToast('Sélectionnez une exploitation.');item.farmId=farmId;const farm=db.farms.find(f=>f.id===farmId);if(farm&&!farm.farmNumber)farm.farmNumber=normalizeHerdNumber(item.identity?.farmNumber)||'';saveDatabase(db);showToast(`Import relié à ${farm?.name||'l’exploitation'}. Aucune visite n’a été modifiée.`);renderHerdData();});
  app.querySelectorAll('[data-apply-herd-audit]').forEach(b=>b.onclick=()=>{let item=db.herdImports.find(x=>x.id===b.dataset.applyHerdAudit);item=repairHerdImport(item);const visitId=app.querySelector(`[data-herd-target-visit="${CSS.escape(b.dataset.applyHerdAudit)}"]`)?.value;const visit=db.visits.find(v=>v.id===visitId);if(!item||!visit)return showToast('Visite introuvable.');if(visit.farmId!==item.farmId)return showToast('Cette visite n’appartient pas à l’exploitation liée à ce fichier.');const currentSource=ensureAuditGlobal(visit).importedHerdData?.sourceId;if(currentSource&&currentSource!==item.id&&!confirm('Cette visite contient déjà des données issues d’un autre CSV. Les anciennes données importées seront retirées avant d’appliquer ce fichier. Continuer ?'))return;const overwrite=!!app.querySelector(`[data-herd-overwrite="${CSS.escape(item.id)}"]`)?.checked;if(overwrite&&!confirm('Remplacer aussi les valeurs déjà saisies avec les données de ce fichier ?'))return;const changes=applyHerdImportToVisit(item,visit,overwrite);setActiveVisit(visit.id);showToast(changes.length?`${changes.length} rubrique(s) complétée(s) uniquement dans la visite du ${formatDate(visit.date)}.`:'Aucune nouvelle rubrique complétée dans cette visite.');renderAuditGlobal();});
}
...
function reproductionScore(r){let score=100;if(r.firstCalvingAgeMonths!=null){if(r.firstCalvingAgeMonths>36)score-=20;else if(r.firstCalvingAgeMonths>28)score-=8;}if(r.meanIVV!=null){if(r.meanIVV>500)score-=30;else if(r.meanIVV>450)score-=20;else if(r.meanIVV>390)score-=10;}if(r.maxIVV>730)score-=15;else if(r.maxIVV>500)score-=8;score-=Math.min(30,r.deadBefore6.length*12);return Math.max(0,Math.round(score));}
...
function renderReproduction(){
  const visit=activeVisit(); const selectedFarmId=localStorage.getItem('audit-bovin-repro-farm')||visit?.farmId||db.farms[0]?.id||''; const farm=db.farms.find(f=>f.id===selectedFarmId);
  ...
  const currentCows=currentReproductionCows(farm);
  ...
  if(reproductionFilter==='390')rows=rows.filter(r=>r.daysSinceLast>390);if(reproductionFilter==='dead')rows=rows.filter(r=>r.deadBefore6.length>=2);if(reproductionFilter==='under28')rows=rows.filter(r=>r.firstCalvingAgeMonths<28);if(reproductionFilter==='28to36')rows=rows.filter(r=>r.firstCalvingAgeMonths>=28&&r.firstCalvingAgeMonths<=36);if(reproductionFilter==='over36')rows=rows.filter(r=>r.firstCalvingAgeMonths>36);
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🐄 Reproduction</h2><div class="muted">Registre historique pour les fiches individuelles ; indicateurs calculés uniquement sur les vaches présentes ayant déjà eu au moins un veau.</div></div><span class="badge autosave">v14.4.6</span></div>
  <section class="card"><div class="row"><div class="field"><label>Exploitation</label><select id="repro-farm">${db.farms.map(f=>`<option value="${f.id}" ${f.id===selectedFarmId?'selected':''}>${escapeHtml(f.name)}${f.farmNumber?` · ${escapeHtml(f.farmNumber)}`:''}</option>`).join('')}</select></div><div class="field"><label>Importer le registre bovins CSV</label><input id="repro-file" type="file" accept=".csv,text/csv"></div></div><div class="muted">${registry.length?`${registry.length} bovin(s) historiques · ${presentFemales.length} femelle(s) présente(s) · ${currentCows.length} vache(s) avec au moins un veau · mise à jour ${formatDateTime(farm.herdRegistryImportedAt)}`:'Aucun registre importé pour cette exploitation.'}</div></section>
  ${!registry.length?'<section class="empty" style="margin-top:16px">Sélectionnez l’exploitation puis importez le fichier CSV contenant tous les bovins.</section>':`
  <section class="grid cols-4" style="margin-top:16px"><article class="card metric"><strong>${stats.firstMean??'—'}</strong><span>Âge moyen 1er vêlage (mois)</span><small>${stats.firstMin??'—'} à ${stats.firstMax??'—'}</small></article><article class="card metric"><strong>${stats.ivvMean??'—'}</strong><span>IVV moyen (jours)</span><small>${stats.ivvMin??'—'} à ${stats.ivvMax??'—'}</small></article><article class="card metric"><strong>${currentCows.filter(r=>r.daysSinceLast>390).length}</strong><span>Vaches &gt; 390 jours</span></article><article class="card metric"><strong>${currentCows.filter(r=>r.deadBefore6.length>=2).length}</strong><span>Mères avec ≥2 morts &lt;6 mois</span></article></section>
  <section class="card" style="margin-top:16px"><div class="row"><div class="field"><label>Filtre</label><select id="repro-filter"><option value="all">Toutes les vaches présentes</option><option value="390">Sans vêlage depuis plus de 390 j</option><option value="dead">Au moins 2 veaux morts avant 6 mois</option><option value="under28">1er vêlage avant 28 mois</option><option value="28to36">1er vêlage entre 28 et 36 mois</option><option value="over36">1er vêlage après 36 mois</option></select></div><div class="field"><label>Classement</label><select id="repro-sort"><option value="lastCalvingOld">Dernier vêlage le plus ancien</option><option value="ivvHigh">IVV moyen le plus élevé</option><option value="firstAgeHigh">1er vêlage le plus tardif</option><option value="deadHigh">Mortalité veaux la plus élevée</option><option value="scoreLow">Score le plus faible</option></select></div></div>
  <div class="table-wrap"><table><thead><tr><th>Vache</th><th>Race / âge</th><th>1er vêlage</th><th>Dernier vêlage</th><th>IVV</th><th>Veaux</th><th>Score</th><th>Détail</th></tr></thead><tbody>${rows.map(r=>`<tr><td><strong>${escapeHtml(r.cow.workNumber||r.cow.id)}</strong><br><small>${escapeHtml(r.cow.id)}</small></td><td>Race ${escapeHtml(r.cow.breed||'—')}<br><small>${ageLabelAt(r.cow.birthDate,today)||'—'}</small></td><td>${r.firstCalvingDate?formatDate(r.firstCalvingDate):'—'}<br><small>${r.firstCalvingAgeMonths!=null?`${Math.round(r.firstCalvingAgeMonths*10)/10} mois`:'—'}</small></td><td>${r.lastCalvingDate?formatDate(r.lastCalvingDate):'—'}<br><small>${r.daysSinceLast!=null?`${r.daysSinceLast} j`:'—'}</small></td><td>Dernier ${r.lastIVV??'—'} j<br><small>Moy. ${r.meanIVV??'—'} · mini ${r.minIVV??'—'} · maxi ${r.maxIVV??'—'}</small></td><td>${r.calves.length}<br><small class="${r.deadBefore6.length>=2?'danger-text':''}">${r.deadBefore6.length} mort(s) &lt;6 mois</small></td><td><span class="badge ${reproductionScore(r)>=75?'complete':reproductionScore(r)>=60?'in-progress':'danger'}">${reproductionScore(r)}/100</span></td><td><button class="btn small" data-repro-detail="${escapeHtml(r.cow.id)}">Voir</button></td></tr>`).join('')||'<tr><td colspan="8">Aucun animal correspondant au filtre.</td></tr>'}</tbody></table></div></section><section id="repro-detail"></section>`}`;
  const fs=document.getElementById('repro-farm');if(fs)fs.onchange=()=>{localStorage.setItem('audit-bovin-repro-farm',fs.value);renderReproduction();};
  const fi=document.getElementById('repro-file');if(fi)fi.onchange=async()=>{const file=fi.files?.[0];if(!file||!farm)return;try{const text=await file.text();const animals=importHerdRegistryRows(parseSemicolonCsv(text));farm.herdRegistry=animals;farm.herdRegistryImportedAt=new Date().toISOString();const ede=[...new Set(animals.map(a=>a.farmNumber).filter(Boolean))];if(!farm.farmNumber&&ede.length===1)farm.farmNumber=ede[0];saveDatabase(db);showToast(`${animals.length} bovin(s) importé(s).`);renderReproduction();}catch(e){console.error(e);showToast(`Import impossible : ${e.message}`);}};
