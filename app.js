...
async function exportPartnerWorkbook(visit,phase='auto'){
  ...
  const response=await fetch('./modele-partenaires-passage-bv.xlsx?v=11.11.0',{cache:'no-store'});if(!response.ok)throw new Error('Modèle partenaire introuvable');
  const zip=await JSZip.loadAsync(await response.arrayBuffer()),cache={};const a=ensureAuditGlobal(visit),item=linkedHerdImportForVisit(visit),col=partnerPhaseColumn(visit,phase),summary=partnerVisitSummary(visit),st=item?.current?.structure||{},mv=item?.current?.movements||{},mort=item?.years?.N?.mortality||{},rep=item?.years?.N?.reproduction||{};
  ...
  const blob=await zip.generateAsync({type:'blob',compression:'DEFLATE',compressionOptions:{level:6}});const url=URL.createObjectURL(blob),link=document.createElement('a');link.href=url;link.download=`PASSAGE-${slugify(farm.name)}-${visit.date||'visite'}.xlsx`;document.body.appendChild(link);link.click();link.remove();setTimeout(()=>URL.revokeObjectURL(url),1500);showToast('Fichier partenaire Excel généré. Vérifiez les champs laissés vides avant transmission.');
}
