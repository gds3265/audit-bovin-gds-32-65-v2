...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Repères visuels, fiches pratiques et carnet technique, disponibles hors ligne.</div></div><span class="badge autosave">v14.6.21.38</span></div>
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.6.21.38</span></div>
...
function renderVisits() {
  ...
  app.innerHTML = `
    <div class="section-title"><div><h2>Visites</h2><div class="muted">Toutes les visites enregistrées restent accessibles, y compris les plus anciennes.</div></div><span class="badge autosave">v14.6.21.38</span></div>
...
function renderFarmerQuestionnaires(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📨 Questionnaires éleveur</h2><div class="muted">Préparer la visite puis récupérer uniquement les réponses encore manquantes.</div></div><span class="badge autosave">v14.6.21.38</span></div>${activeVisitBanner(visit)}
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.6.21.38</span></div>
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.6.21.38</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
...
function renderFollowup(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Historique & évolution</h2><div class="muted">Comparer deux visites d’une même exploitation : reproduction, mortalité, mesures, alimentation, bâtiment et plans d’action.</div></div><span class="badge autosave">v14.6.21.38</span></div>
...
function renderSmartLabImportCard(view,visit){if(!visit||!['metabolic','parasitism','waterlab'].includes(view))return;const labels={metabolic:'profil métabolique / oligo',parasitism:'parasitisme',waterlab:'analyse d’eau'};const card=document.createElement('section');card.className='card smart-lab-import';card.innerHTML=`<h3>📄 Recopie intelligente d’un résultat labo</h3><p class="muted">Importez une photo ou un PDF du compte rendu. Les images peuvent être lues automatiquement si le navigateur le permet. Les valeurs détectées sont toujours proposées pour vérification avant intégration dans ${labels[view]}.</p><input type="file" accept="image/*,.pdf,application/pdf" data-smart-lab-file><div class="small" data-smart-lab-info></div>`;app.prepend(card);card.querySelector('[data-smart-lab-file]').onchange=async e=>{const file=e.target.files?.[0];if(!file)return;const info=card.querySelector('[data-smart-lab-info]');info.textContent='Lecture en cours…';let text='';if(file.type.startsWith('image/')&&'TextDetector'in window){try{const bmp=await createImageBitmap(file),det=new TextDetector(),res=await det.detect(bmp);text=res.map(x=>x.rawValue||'').join('\n')}catch{}}visit.labImports=Array.isArray(visit.labImports)?visit.labImports:[];visit.labImports.unshift({id:uid('labimp'),module:view,name:file.name,type:file.type,createdAt:new Date().toISOString(),ocrText:text});saveDatabase(db);info.innerHTML=text?`✓ Texte détecté. <b>Vérification obligatoire</b> avant recopie des valeurs.`:`✓ Document rattaché à la visite. Lecture automatique non disponible pour ce fichier sur cet appareil ; il reste accessible pour la saisie assistée.`;};}

render();

// v14.6.21.38 — navigation par workflow
...
// v14.6.21.38 — check-up, suivi d'étude, économie et conservation globale du défilement horizontal
...
// v14.6.21.38 — suivi du temps par intervenant + temps GDS préparation/terrain/compte rendu/suivi

// v14.6.21.38 — questionnaires persistants, repères de saisie, bilan sanitaire/éco partenaire, supports papier, check-up responsive.

window.addEventListener('DOMContentLoaded',()=>{initGlobalSearch();initAccordionMemory();});
// v14.6.21.38 — cohérence, imprimables préparation, animaux présents, recherche globale.
