...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Repères visuels, fiches pratiques et carnet technique, disponibles hors ligne.</div></div><span class="badge autosave">v14.6.21.45</span></div>
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.6.21.45</span></div>
...
function renderVisits() {
  ...
  app.innerHTML = `
    <div class="section-title"><div><h2>Visites</h2><div class="muted">Toutes les visites enregistrées restent accessibles, y compris les plus anciennes.</div></div><span class="badge autosave">v14.6.21.45</span></div>
...
function renderFarmerQuestionnaires(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📨 Questionnaires éleveur</h2><div class="muted">Préparer la visite puis récupérer uniquement les réponses encore manquantes.</div></div><span class="badge autosave">v14.6.21.45</span></div>${activeVisitBanner(visit)}
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.6.21.45</span></div>
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.6.21.45</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
...
function renderFollowup(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Historique & évolution</h2><div class="muted">Comparer deux visites d’une même exploitation : reproduction, mortalité, mesures, alimentation, bâtiment et plans d’action.</div></div><span class="badge autosave">v14.6.21.45</span></div>
...
if ('serviceWorker' in navigator){
  ...
  const AUDIT_SW=location.origin+AUDIT_PATH+'sw.js?v=14.6.21.45';
...
function smartAliasMatch(line,aliases=[]){const n=normalizeSearchText(line);let best=null;for(const a of aliases){const aa=normalizeSearchText(a);if(!aa)continue;let idx=-1;if(aa.length<=3){const re=new RegExp(`(^|[^a-z0-9])${aa.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')}([^a-z0-9]|$)`,'i');const m=n.match(re);if(m)idx=m.index+(m[1]?.length||0)}else idx=n.indexOf(aa);if(idx>=0&&(!best||idx<best.idx))best={alias:aa,idx}}return best}
function smartResultFromCandidate(line,aliases=[]){const match=smartAliasMatch(line,aliases);if(!match)return null;const plain=String(line||'');const n=normalizeSearchText(plain);const ratio=Math.max(0,Math.min(1,(match.idx+match.alias.length)/Math.max(1,n.length)));const rough=Math.floor(ratio*plain.length);const toks=smartNumericTokens(plain).filter(t=>t.index>=Math.max(0,rough-2));if(!toks.length)return null;const raw=toks[0].raw;const numeric=parseFloat(raw.replace(/[<>≤≥]/g,''));if(!Number.isFinite(numeric))return null;if(numeric>=1900&&numeric<=2100)return null;return raw}
function smartQualitativeResult(line=''){const n=normalizeSearchText(line);if(/\b(absence|absent|negatif|negative|non detecte|non detectee|nd)\b/.test(n))return 'Négatif';if(/\b(positif|positive|presence|present)\b/.test(n))return 'Positif';if(/\b(douteux|douteuse|equivoque|ininterpretable)\b/.test(n))return 'Douteux';return ''}
function smartFindProposals(text,defs){const lines=smartTextLines(text),out=[];for(const d of defs){for(let i=0;i<lines.length;i++){const here=lines[i],m=smartAliasMatch(here,d.aliases||[d.label]);if(!m)continue;let source=here,value=smartResultFromCandidate(here,d.aliases||[d.label]);if(value===null&&d.qualitative)value=smartQualitativeResult(here)||null;if(value===null&&i+1<lines.length){const next=lines[i+1];if(/^\s*(?:<|>|≤|≥)?\s*-?\d/.test(next)){source=`${here} ${next}`;value=smartNumericTokens(next)[0]?.raw||null}else if(d.qualitative){const q=smartQualitativeResult(next);if(q){source=`${here} ${next}`;value=q}}}
...
function smartLabDefinitions(view){if(view==='metabolic')return Object.entries(METABOLIC_KNOWLEDGE).map(([key,x])=>({key,label:x.label,aliases:[x.label,...(x.aliases||[])]}));if(view==='parasitism')return [{key:'strongles_digestifs',label:'Strongles digestifs',aliases:['strongles digestifs','strongles gastro intestinaux','strongyles digestifs','strongyles gastro'],qualitative:true},{key:'nematodirus',label:'Nematodirus',aliases:['nematodirus'],qualitative:true},{key:'coccidies',label:'Coccidies',aliases:['coccidies','coccidia','oocystes'],qualitative:true},{key:'paramphistomes',label:'Paramphistomes',aliases:['paramphistome','paramphistomum'],qualitative:true},{key:'grande_douve',label:'Grande douve',aliases:['fasciola','grande douve'],qualitative:true},{key:'strongles_respiratoires',label:'Strongles respiratoires',aliases:['strongles respiratoires','dictyocaulus','strongyles respiratoires'],qualitative:true},{key:'pepsinogene',label:'Pepsinogène',aliases:['pepsinogene','pepsinogène'],unit:'mUT'}];return [{key:'turbidite',label:'Turbidité',aliases:['turbidite','turbidité'],unit:'NFU'},{key:'ph',label:'pH',aliases:['ph']},{key:'conductivite',label:'Conductivité',aliases:['conductivite','conductivité'],unit:'µS/cm'},{key:'durete',label:'Dureté',aliases:['durete','dureté','th'],unit:'°f'},{key:'nitrites',label:'Nitrites',aliases:['nitrites'],unit:'mg/L'},{key:'nitrates',label:'Nitrates',aliases:['nitrates'],unit:'mg/L'},{key:'coliformes',label:'Coliformes totaux',aliases:['coliformes totaux','bacteries coliformes','bactéries coliformes'],unit:'/100 mL'},{key:'ecoli',label:'Escherichia coli',aliases:['escherichia coli','e. coli','e coli'],unit:'/100 mL'},{key:'enterocoques',label:'Entérocoques intestinaux',aliases:['enterocoques','entérocoques'],unit:'/100 mL'},{key:'asr',label:'Spores ASR',aliases:['asr','anaerobies sulfito reducteurs','anaérobies sulfito-réducteurs'],unit:'/100 mL'},{key:'flore22',label:'Flore totale 22°C',aliases:['flore 22','22°c'],unit:'UFC/mL'},{key:'flore37',label:'Flore totale 37°C',aliases:['flore 37','37°c'],unit:'UFC/mL'}]}
...
function renderSmartLabImportCard(view,visit){
 ...
 <div class="lab-import-list">${existing.length?existing.map(x=>{const props=Array.isArray(x.proposals)?x.proposals:[];return `<article class="lab-import-review" data-lab-doc="${x.id}"><div class="lab-import-item"><span><b>${escapeHtml(x.name)}</b><small>${formatDateTime(x.createdAt)} · ${props.length} proposition(s) détectée(s) · ${Number(x.integratedCount)||0} intégrée(s) · ${escapeHtml(labReadModeLabel(x.readMode|| (x.ocrText?'ocr-natif':'')))}${x.readError?` · ⚠️ ${escapeHtml(x.readError)}`:''}</small></span><div class="actions">${x.dataUrl?`<button class="btn small" data-retry-lab-import="${x.id}">↻ Relire</button><button class="btn small" data-open-lab-import="${x.id}">Ouvrir</button>`:''}${Number(x.integratedCount)?`<button class="btn small secondary" data-remove-lab-values="${x.id}">Retirer les valeurs importées</button>`:''}<button class="btn small danger" data-delete-lab-import="${x.id}">Supprimer</button></div></div>
 ${props.length?`<div class="table-wrap"><table class="compact-table lab-review-table"><thead><tr><th>✓</th><th>Paramètre</th><th>Valeur détectée</th><th>Unité</th><th>Ligne réellement lue dans le document</th></tr></thead><tbody>${props.map(p=>smartLabProposalHtml(p,x.id)).join('')}</tbody></table></div><div class="actions"><button class="btn primary" data-integrate-lab-proposals="${x.id}">✓ Intégrer les valeurs cochées</button><span class="muted small-text">Vous pouvez corriger la valeur ou l’unité avant validation.</span></div>`:`<div class="notice"><strong>Aucune proposition exploitable.</strong> Le document est bien conservé, mais aucune valeur ne sera inventée. Ouvrez le document et saisissez manuellement si nécessaire.</div>`}</article>`}).join(''):'<div class="empty compact">Aucun résultat labo importé dans ce module.</div>'}</div>`;
 app.prepend(card);
 const processFile=async(file,existingImport=null)=>{const info=card.querySelector('[data-smart-lab-info]');const progress=msg=>{if(info)info.textContent=`${file.name} — ${msg}`};progress('préparation…');const{text,dataUrl,readMode,error}=await smartLabReadFile(file,progress);progress(text.trim()?'Analyse prudente des lignes du document…':error||'Aucun texte détecté.');const proposals=extractSmartLabProposals(view,text);if(existingImport){existingImport.ocrText=text;existingImport.readMode=readMode;existingImport.readError=error;existingImport.proposals=proposals;existingImport.reReadAt=new Date().toISOString();if(dataUrl&&!existingImport.dataUrl)existingImport.dataUrl=dataUrl}else visit.labImports.unshift({id:uid('labimp'),module:view,name:file.name,type:file.type,size:file.size,createdAt:new Date().toISOString(),ocrText:text,dataUrl,integratedCount:0,proposals,readMode,readError:error});return{proposalCount:proposals.length,text,error,readMode}};
 card.querySelector('[data-smart-lab-file]').onchange=async e=>{const files=[...(e.target.files||[])];if(!files.length)return;let ok=0,total=0,readDocs=0,errors=[];for(const file of files){const r=await processFile(file);ok++;total+=r.proposalCount;if(r.text.trim())readDocs++;if(r.error)errors.push(r.error)}visit.updatedAt=new Date().toISOString();saveDatabase(db);if(total)showToast(`${ok} document(s) lu(s) · ${total} valeur(s) proposée(s) à vérifier.`);else if(readDocs)showToast(`${ok} document(s) lu(s), mais aucune valeur suffisamment fiable n’a été proposée.`);else showToast(errors.length?'Le document est conservé, mais l’OCR n’a pas pu démarrer.':'Document conservé, mais aucun texte exploitable n’a été détecté.');render()};
...
// v14.6.21.45 — check-up, suivi d'étude, économie et conservation globale du défilement horizontal
...
// v14.6.21.45 — suivi du temps par intervenant + temps GDS préparation/terrain/compte rendu/suivi

// v14.6.21.45 — questionnaires persistants, repères de saisie, bilan sanitaire/éco partenaire, supports papier, check-up responsive.

window.addEventListener('DOMContentLoaded',()=>{initGlobalSearch();initAccordionMemory();});
// v14.6.21.45 — cohérence, imprimables préparation, animaux présents, recherche globale.
