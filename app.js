...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Repères visuels, fiches pratiques et carnet technique, disponibles hors ligne.</div></div><span class="badge autosave">v14.6.21.56</span></div>
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.6.21.56</span></div>
...
function renderVisits() {
  ...
  app.innerHTML = `
    <div class="section-title"><div><h2>Visites</h2><div class="muted">Toutes les visites enregistrées restent accessibles, y compris les plus anciennes.</div></div><span class="badge autosave">v14.6.21.56</span></div>
...
function renderFarmerQuestionnaires(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📨 Questionnaires éleveur</h2><div class="muted">Préparer la visite puis récupérer uniquement les réponses encore manquantes.</div></div><span class="badge autosave">v14.6.21.56</span></div>${activeVisitBanner(visit)}
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.6.21.56</span></div>
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.6.21.56</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
...
function renderFollowup(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Historique & évolution</h2><div class="muted">Comparer deux visites d’une même exploitation : reproduction, mortalité, mesures, alimentation, bâtiment et plans d’action.</div></div><span class="badge autosave">v14.6.21.56</span></div>
...
function reproWeatherBundledFiles(dept){const d=String(dept||'').padStart(2,'0');if(!['32','65'].includes(d))return [];return [`./meteo/Q_${d}_previous-1950-2024_RR-T-Vent.csv.gz`,`./meteo/Q_${d}_latest-2025-2026_RR-T-Vent.csv.gz`];}
async function reproWeatherLoadBundled(dept,start,end,onProgress){const files=reproWeatherBundledFiles(dept);if(!files.length)throw new Error('Les fichiers inclus concernent uniquement les départements 32 et 65.');const all=[];let used=0;for(let i=0;i<files.length;i++){onProgress?.(`Lecture du fichier météo inclus ${i+1}/${files.length}…`);const r=await fetch(files[i]);if(!r.ok)throw new Error(`Fichier météo inclus absent (${r.status}). Réimporte le ZIP complet sur GitHub.`);const text=await reproWeatherUngzipResponse(r,files[i]);const rows=reproWeatherParseDetailedCsv(text).filter(z=>(!start||z.date>=start)&&(!end||z.date<=end));if(rows.length){all.push(...rows);used++;}}const uniq=new Map();for(const x of all){const k=`${x.stationId}|${x.date}`;uniq.set(k,x);}if(!uniq.size)throw new Error('Aucune donnée exploitable dans les fichiers météo inclus pour cette période.');return {rows:[...uniq.values()],resourcesUsed:used};}
...
function reproQuarterOrdinal(k){const m=String(k||'').match(/(\d{4})-T([1-4])/);return m?Number(m[1])*4+Number(m[2])-1:0;}
...
function reproChronologyInsights(chrono){
  const buckets=chrono?.buckets||[];if(!buckets.length)return [];
  const ivvBase=reproMedian(buckets.filter(b=>b.ivvs?.length>=3&&b.avg!=null).map(b=>b.avg));
  const mortBase=reproMedian(buckets.filter(b=>b.calves>=5&&b.mortRate!=null).map(b=>b.mortRate));
  const flagged=buckets.filter(b=>{
    const ivvBreak=b.ivvs?.length>=3&&b.avg!=null&&((ivvBase!=null&&b.avg>=ivvBase+35)||b.avg>=470||b.badRate>=50);
    const mortBreak=b.calves>=5&&b.mortRate!=null&&((mortBase!=null&&b.mortRate>=mortBase+5&&b.mortRate>=8)||b.mortRate>=12);
    return b.severity>=4||ivvBreak||mortBreak;
  }).sort((a,b)=>reproQuarterOrdinal(a.key)-reproQuarterOrdinal(b.key));
  if(!flagged.length)return [];
  const groups=[];for(const b of flagged){const last=groups.at(-1);if(last&&reproQuarterOrdinal(b.key)<=reproQuarterOrdinal(last.at(-1).key)+1)last.push(b);else groups.push([b]);}
  const byOrd=new Map(buckets.map(b=>[reproQuarterOrdinal(b.key),b]));
  return groups.map(g=>{
    const startOrd=reproQuarterOrdinal(g[0].key),endOrd=reproQuarterOrdinal(g.at(-1).key),context=[];
    for(let o=startOrd-1;o<=endOrd;o++)if(byOrd.has(o))context.push(byOrd.get(o));
    const ivvs=g.flatMap(b=>b.ivvs||[]).map(x=>x.ivv),avg=ivvs.length?Math.round(ivvs.reduce((a,c)=>a+c,0)/ivvs.length):null,badRate=ivvs.length?Math.round(ivvs.filter(v=>v>450).length/ivvs.length*100):null;
    const mort=g.filter(b=>b.calves>=5&&b.mortRate!=null).map(b=>b.mortRate),mortMax=mort.length?Math.max(...mort):null;
    const intros=context.flatMap(b=>b.introductions||[]),events=context.flatMap(b=>b.events||[]),weather=[...new Set(context.flatMap(b=>b.weather?.tags||[]))];
    const signals=[];if(avg!=null)signals.push(`IVV moyen ${avg} j${ivvBase!=null?` (${avg>=ivvBase?'+':''}${Math.round(avg-ivvBase)} j vs niveau habituel ≈ ${Math.round(ivvBase)} j)`:''}`);if(badRate!=null&&badRate>=30)signals.push(`${badRate}% des IVV > 450 j`);if(mortMax!=null&&mortMax>=6)signals.push(`mortalité veaux jusqu’à ${mortMax}%`);
    const links=[];if(intros.length)links.push(`${intros.length} introduction(s) dans les 3 mois précédant ou pendant la dégradation`);if(weather.length)links.push(`météo : ${weather.join(', ')}`);if(events.length)links.push(`événement(s) saisi(s) : ${[...new Set(events.map(e=>e.label||e.type).filter(Boolean))].join(', ')}`);
    const questions=[];if(intros.length)questions.push('Vérifier le statut sanitaire des animaux introduits et la survenue de signes cliniques après leur arrivée.');if(weather.some(x=>/sec|chaud/i.test(x)))questions.push('Vérifier disponibilité/qualité des fourrages, état corporel, abreuvement et complémentation pendant cette période.');if(weather.some(x=>/humide|pluie/i.test(x)))questions.push('Vérifier conditions de pâturage, conservation des fourrages et pression sanitaire pendant la période humide.');if(events.some(e=>/aliment|fourrage/i.test(`${e.type} ${e.label}`)))questions.push('Comparer précisément la ration avant/après le changement alimentaire.');if(mortMax!=null&&mortMax>=8&&avg!=null&&avg>430)questions.push('Reproduction et mortalité se dégradent ensemble : rechercher une cause commune sanitaire, alimentaire ou environnementale.');if(!questions.length)questions.push('Pendant l’audit, cibler détection des chaleurs, organisation des lots/taureaux, alimentation et événements sanitaires sur cette période.');
    const score=Math.max(...g.map(b=>b.severity||0))+(intros.length?1:0)+(weather.length?1:0)+(events.length?1:0);
    return {from:g[0].key,to:g.at(-1).key,label:g.length===1?g[0].key:`${g[0].key} → ${g.at(-1).key}`,signals,links,questions,score};
  }).sort((a,b)=>b.score-a.score).slice(0,5);
}
function reproChronologyInsightHtml(chrono,{print=false}={}){
  const insights=reproChronologyInsights(chrono);if(!insights.length)return '<div class="notice"><strong>✓ Pas de dégradation majeure isolée par l’analyse croisée.</strong><br><span class="muted">Les fluctuations trimestrielles mineures ne sont volontairement pas listées.</span></div>';
  return `<div class="repro-cross-analysis">${insights.map((x,i)=>`<article class="notice ${i===0?'warning':''}"><strong>${i===0?'🔴':'🟠'} ${escapeHtml(x.label)} — dégradation à investiguer</strong><div><b>Ce qui change :</b> ${escapeHtml(x.signals.join(' · ')||'rupture de performance')}</div>${x.links.length?`<div><b>Coïncidences à vérifier :</b> ${escapeHtml(x.links.join(' · '))}</div>`:'<div class="muted">Aucun événement daté connu ne coïncide actuellement avec cette rupture.</div>'}<div><b>Questions ciblées :</b> ${escapeHtml(x.questions.join(' '))}</div></article>`).join('')}</div>`;
}
function reproChronologyHtml(chrono){
  ...
  const timeline=`<details class="chrono-raw-detail"><summary><strong>Voir le détail trimestre par trimestre (contrôle technique)</strong></summary><div class="muted">Ce tableau est volontairement replié : la synthèse ci-dessus sert à préparer l’enquête, le détail reste disponible pour vérification.</div><div class="table-wrap"><table class="compact-table"><thead><tr><th>Période</th><th>IVV conception estimée</th><th>IVV &gt;450 j</th><th>Naissances</th><th>Morts &lt;6m</th><th>Introductions</th><th>Météo-France</th><th>Événements</th></tr></thead><tbody>${chrono.buckets.map(b=>`<tr class="${b.severity>=4?'repro-period-danger':b.severity>=2?'repro-period-warning':''}"><td><strong>${b.key}</strong></td><td>${b.avg??'—'}${b.avg!=null?' j':''} <small>(${b.ivvs.length})</small></td><td>${b.ivvs.length?b.badRate+'%':'—'}</td><td>${b.calves}</td><td>${b.mortRate==null?'—':b.mortRate+'%'}</td><td>${b.introductions.length}</td><td>${b.weather?`${b.weather.rain==null?'—':b.weather.rain+' mm'}<br><small>${b.weather.temp==null?'—':b.weather.temp+' °C'}${b.weather.tags?.length?' · '+escapeHtml(b.weather.tags.join(', ')):''}</small>`:'—'}</td><td>${b.events.length?b.events.map(e=>escapeHtml(e.label||e.type||'Événement')).join('<br>'):'—'}</td></tr>`).join('')}</tbody></table></div></details>`;
...
function renderReproduction(){
  ...
  if(visit){const saveInv=()=>{visit.updatedAt=new Date().toISOString();saveDatabase(db)};app.querySelectorAll('[data-repro-heat]').forEach(el=>{const f=el.dataset.reproHeat;const fn=()=>{inv.heat[f]=el.value;saveInv()};el.onchange=()=>{fn();renderReproduction()};el.oninput=fn});app.querySelectorAll('[data-repro-setting]').forEach(el=>{const f=el.dataset.reproSetting;const fn=()=>{inv.settings[f]=el.type==='number'?Number(el.value||0):el.value;saveInv()};el.onchange=()=>{fn();renderReproduction()};el.oninput=fn});app.querySelectorAll('[data-bull-field]').forEach(el=>{const row=el.closest('[data-repro-bull-row]'),i=Number(row?.dataset.reproBullRow),f=el.dataset.bullField;const fn=()=>{if(!inv.bulls[i])return;inv.bulls[i][f]=el.type==='number'?Number(el.value||0):el.value;saveInv()};el.oninput=fn;el.onchange=()=>{fn();renderReproduction()}});document.getElementById('add-repro-bull')?.addEventListener('click',()=>{inv.bulls.push({id:uid('bull'),name:'',ageClass:'adult',females:'',contactDays:'',fertility:''});saveInv();renderReproduction()});app.querySelectorAll('[data-delete-bull]').forEach(b=>b.onclick=()=>{inv.bulls.splice(Number(b.dataset.deleteBull),1);saveInv();renderReproduction()});document.getElementById('add-repro-event')?.addEventListener('click',()=>{const date=document.getElementById('repro-event-date')?.value,label=document.getElementById('repro-event-label')?.value.trim(),type=document.getElementById('repro-event-type')?.value;if(!date||!label){showToast('Renseignez la date et l’événement.');return;}inv.events.push({id:uid('reproevt'),date,type,label});saveInv();renderReproduction()});app.querySelectorAll('[data-delete-repro-event]').forEach(b=>b.onclick=()=>{inv.events=inv.events.filter(e=>e.id!==b.dataset.deleteReproEvent);saveInv();renderReproduction()});
    ...
    let weatherDeptRows=[];
    const weatherFrom=fullPeriod.from||periodStart,weatherTo=periodMax||periodEnd;
...
// v14.6.21.56 — check-up, suivi d'étude, économie et conservation globale du défilement horizontal
...
// v14.6.21.56 — suivi du temps par intervenant + temps GDS préparation/terrain/compte rendu/suivi

// v14.6.21.56 — questionnaires persistants, repères de saisie, bilan sanitaire/éco partenaire, supports papier, check-up responsive.

window.addEventListener('DOMContentLoaded',()=>{initGlobalSearch();initAccordionMemory();});
// v14.6.21.56 — cohérence, imprimables préparation, animaux présents, recherche globale.
