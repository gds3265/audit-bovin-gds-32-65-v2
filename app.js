...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Repères visuels, fiches pratiques et carnet technique, disponibles hors ligne.</div></div><span class="badge autosave">v14.6.21.106</span></div>
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.6.21.106</span></div>
...
function renderVisits() {
  ...
  app.innerHTML = `
    <div class="section-title"><div><h2>Visites</h2><div class="muted">Toutes les visites enregistrées restent accessibles, y compris les plus anciennes.</div></div><span class="badge autosave">v14.6.21.106</span></div>
...
function renderFarmerQuestionnaires(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📨 Questionnaires éleveur</h2><div class="muted">Préparer la visite puis récupérer uniquement les réponses encore manquantes.</div></div><span class="badge autosave">v14.6.21.106</span></div>${activeVisitBanner(visit)}
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.6.21.106</span></div>
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.6.21.106</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
  visit.feeding=visit.feeding&&typeof visit.feeding==='object'?visit.feeding:{rations:[],settings:{},history:[]};
  visit.feeding.nutrition=visit.feeding.nutrition&&typeof visit.feeding.nutrition==='object'?visit.feeding.nutrition:{};
...
function renderQuickFieldNotes(){
  ...
  if(SpeechRecognition&&mic){
    let listening=false,sessionPrefix='';
    const stop=()=>{try{__fieldNotesRecognition?.stop();}catch(_){ } };
    mic.onclick=()=>{
      if(listening){stop();return;}
      try{
        const rec=new SpeechRecognition();__fieldNotesRecognition=rec;rec.lang='fr-FR';rec.continuous=true;rec.interimResults=true;
        sessionPrefix=raw.value+(raw.value&&!raw.value.endsWith('\n')?'\n':'');
        const finalByIndex=new Map();
        rec.onstart=()=>{listening=true;mic.textContent='⏹️ Arrêter la dictée';mic.classList.add('danger');status.textContent='🎙️ Dictée en cours… parlez normalement.';};
        rec.onresult=e=>{
          let interim='';
          for(let i=0;i<e.results.length;i++){
            const result=e.results[i],t=(result?.[0]?.transcript||'').trim();
            if(!t)continue;
            if(result.isFinal)finalByIndex.set(i,t);
            else if(i>=e.resultIndex)interim+=(interim?' ':'')+t;
          }
          const finalText=[...finalByIndex.entries()].sort((a,b)=>a[0]-b[0]).map(x=>x[1]).join(' ').trim();
          raw.value=sessionPrefix+finalText;
          if(finalText)saveRaw();
          status.textContent=interim?`🎙️ ${interim}`:'🎙️ Dictée en cours…';
          raw.scrollTop=raw.scrollHeight;
        };
        rec.onerror=e=>{status.textContent=`Dictée interrompue (${e.error||'erreur'}). Le texte déjà reconnu est conservé.`;};
        rec.onend=()=>{listening=false;mic.textContent='🎙️ Démarrer la dictée';mic.classList.remove('danger');status.textContent='Dictée arrêtée. Le texte reconnu est conservé dans les notes.';saveRaw();__fieldNotesRecognition=null;};
        rec.start();
      }catch(err){console.error(err);status.textContent='Impossible de démarrer la dictée sur ce navigateur.';}
    };
  }
...
function renderFollowup(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Historique & évolution</h2><div class="muted">Comparer deux visites d’une même exploitation : reproduction, mortalité, mesures, alimentation, bâtiment et plans d’action.</div></div><span class="badge autosave">v14.6.21.106</span></div>
