...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Repères visuels, fiches pratiques et carnet technique, disponibles hors ligne.</div></div><span class="badge autosave">v14.6.21.109</span></div>
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.6.21.109</span></div>
...
function renderVisits() {
  ...
  app.innerHTML = `
    <div class="section-title"><div><h2>Visites</h2><div class="muted">Toutes les visites enregistrées restent accessibles, y compris les plus anciennes.</div></div><span class="badge autosave">v14.6.21.109</span></div>
...
function renderFarmerQuestionnaires(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📨 Questionnaires éleveur</h2><div class="muted">Préparer la visite puis récupérer uniquement les réponses encore manquantes.</div></div><span class="badge autosave">v14.6.21.109</span></div>${activeVisitBanner(visit)}
...
const supplementalBloodCatalog=[
  {key:'labUreaGL',label:'Urée',unit:'g/L'},
  {key:'labGlucoseGL',label:'Glucose',unit:'g/L'},
  {key:'labHctPct',label:'Hématocrite (Hct)',unit:'%'},
  {key:'labPH',label:'pH',unit:''},
  {key:'labNa',label:'Sodium (Na)',unit:'mmol/L'},
  {key:'labK',label:'Potassium (K)',unit:'mmol/L'},
  {key:'labCl',label:'Chlorures (Cl)',unit:'mmol/L'},
  {key:'labICa',label:'Calcium ionisé (iCa)',unit:'mmol/L'},
  {key:'labLactate',label:'Lactate',unit:'mmol/L'}
];
function ensureSupplementalBlood(visit){
  ...
  const x=visit.supplementalBlood;
  x.enabled=Array.isArray(x.enabled)?x.enabled:[];
  x.source=x.source||''; x.date=x.date||visit.date||''; x.comment=x.comment||'';
  (visit.subjects||[]).forEach(subject=>{
...
function fmtGlucose(v,d=2){return Number.isFinite(v)?v.toLocaleString('fr-FR',{maximumFractionDigits:d}):'—';}
function renderSupplementalBloodSection(visit){
  const cfg=ensureSupplementalBlood(visit);
  const enabled=supplementalBloodCatalog.filter(p=>cfg.enabled.includes(p.key));
  const minWidth=170+enabled.length*125+180;
  return `<section class="card supplemental-blood-card">
    <div class="section-title"><div><h3>🧪 Analyses sanguines complémentaires</h3><div class="muted">Résultats d'un analyseur vétérinaire ou de laboratoire, séparés des mesures terrain.</div></div></div>
    <details ${cfg.enabled.length?'open':''}>
      <summary><strong>+ Choisir les analyses à ajouter</strong></summary>
      <div class="chip-options supplemental-blood-choices">${supplementalBloodCatalog.map(p=>`<label class="choice-chip ${cfg.enabled.includes(p.key)?'selected':''}"><input type="checkbox" data-supp-blood-enable="${p.key}" ${cfg.enabled.includes(p.key)?'checked':''}>${escapeHtml(p.label)}${p.unit?` <small>(${escapeHtml(p.unit)})</small>`:''}</label>`).join('')}</div>
    </details>
    <div class="grid cols-3" style="margin-top:12px">
      <div class="field"><label>Source / appareil / vétérinaire</label><input id="supp-blood-source" value="${escapeHtml(cfg.source)}" placeholder="Ex. analyseur du cabinet vétérinaire"></div>
      <div class="field"><label>Date d'analyse</label><input id="supp-blood-date" type="date" value="${escapeHtml(cfg.date)}"></div>
      <div class="field"><label>Commentaire</label><input id="supp-blood-comment" value="${escapeHtml(cfg.comment)}" placeholder="Lot, conditions, remarque…"></div>
    </div>
    ${enabled.length?`<div class="table-wrap" style="margin-top:12px"><table class="analysis-table matrix-table supplemental-blood-table" style="min-width:${minWidth}px"><thead><tr><th style="min-width:150px">Sujet</th>${enabled.map(p=>`<th style="min-width:120px">${escapeHtml(p.label)}${p.unit?`<br><small>${escapeHtml(p.unit)}</small>`:''}</th>`).join('')}<th style="min-width:180px">Commentaire</th></tr></thead><tbody>${(visit.subjects||[]).map(subject=>`<tr><td><strong>${escapeHtml(subject.tag||subject.name||'Sujet')}</strong></td>${enabled.map(p=>`<td><input class="decimal-input" inputmode="decimal" data-supp-blood-value data-subject-id="${subject.id}" data-param="${p.key}" value="${escapeHtml(subject.measurements?.supplementalBlood?.[p.key]||'')}" placeholder="—"></td>`).join('')}<td><input data-supp-blood-subject-comment data-subject-id="${subject.id}" value="${escapeHtml(subject.measurements?.supplementalBlood?.comment||'')}"></td></tr>`).join('')}</tbody></table></div>`:'<div class="empty" style="margin-top:12px">Cochez uniquement les paramètres disponibles pour cette visite.</div>'}
    <div class="glucose-converter"><h4>Convertisseur glycémie</h4>
      <div class="row">
        <div class="field"><label>Valeur</label><input id="glucose-convert-value" class="decimal-input" inputmode="decimal" placeholder="Ex. 0,60"></div>
        <div class="field"><label>Unité saisie</label><select id="glucose-convert-unit"><option>g/L</option><option>mmol/L</option><option>mg/dL</option></select></div>
      </div>
      <div id="glucose-convert-result" class="notice"><strong>Entrez une valeur</strong> pour afficher les trois unités.</div>
    </div>
  </section>`;
}
function bindSupplementalBloodEvents(visit){
  const cfg=ensureSupplementalBlood(visit),save=()=>{visit.updatedAt=new Date().toISOString();saveDatabase(db);};
  app.querySelectorAll('[data-supp-blood-enable]').forEach(el=>el.onchange=()=>{
    const k=el.dataset.suppBloodEnable;
    cfg.enabled=el.checked?[...new Set([...cfg.enabled,k])]:cfg.enabled.filter(x=>x!==k);
    save(); renderAnalysis();
  });
  ...
  if(source)source.oninput=()=>{cfg.source=source.value;save();};
  if(date)date.onchange=()=>{cfg.date=date.value;save();};
  if(comment)comment.oninput=()=>{cfg.comment=comment.value;save();};
  app.querySelectorAll('[data-supp-blood-value]').forEach(el=>{
    const fn=()=>{
      const subject=(visit.subjects||[]).find(x=>x.id===el.dataset.subjectId); if(!subject)return;
      subject.measurements=subject.measurements||{};
      subject.measurements.supplementalBlood=subject.measurements.supplementalBlood||{};
      subject.measurements.supplementalBlood[el.dataset.param]=el.value; save();
    };
    el.oninput=fn; el.onchange=fn;
  });
  app.querySelectorAll('[data-supp-blood-subject-comment]').forEach(el=>el.oninput=()=>{
    const subject=(visit.subjects||[]).find(x=>x.id===el.dataset.subjectId); if(!subject)return;
    subject.measurements=subject.measurements||{};
    subject.measurements.supplementalBlood=subject.measurements.supplementalBlood||{};
    subject.measurements.supplementalBlood.comment=el.value; save();
  });
  ...
  const update=()=>{
    if(!gv||!gu||!gr)return;
    const c=glucoseConversions(gv.value,gu.value);
    gr.innerHTML=c?`<strong>${fmtGlucose(c.gl,3)} g/L</strong> · <strong>${fmtGlucose(c.mmol,2)} mmol/L</strong> · <strong>${fmtGlucose(c.mgdl,1)} mg/dL</strong>`:'<strong>Entrez une valeur</strong> pour afficher les trois unités.';
  };
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.6.21.109</span></div>
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.6.21.109</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
  visit.feeding=visit.feeding&&typeof visit.feeding==='object'?visit.feeding:{rations:[],settings:{},history:[]};
  visit.feeding.nutrition=visit.feeding.nutrition&&typeof visit.feeding.nutrition==='object'?visit.feeding.nutrition:{};
...
function groupQuickNotes(rawText=''){
  ...
  if(!raw)return [];
  const groups=new Map();
  const order=['Alimentation / ration','Eau / bâtiment','Reproduction','Mortalité / sanitaire','Animaux / observations','Organisation / économie','À classer'];
  const add=(cat,text)=>{
    const clean=String(text||'').replace(/^[•\-–]\s*/,'').replace(/\s+/g,' ').trim();
    if(!clean)return;
    const key=cat||'À classer';
    if(!groups.has(key))groups.set(key,[]);
    const arr=groups.get(key);
    if(!arr.some(x=>normalizeSearchText(x)===normalizeSearchText(clean)))arr.push(clean);
  };
  let explicitCat='';
  const splitFragments=text=>{
    let parts=String(text||'').split(/(?<=[.!?;])\s+|\s+[–—]\s+/).map(x=>x.trim()).filter(Boolean);
    if(parts.length===1 && parts[0].length>260){
      parts=parts[0].split(/\s+(?=(?:au niveau de|concernant|pour ce qui est de|côté|en reproduction|en alimentation|pour l'eau|pour les vaches|pour les génisses|sanitaire|au pâturage|dans le bâtiment)\b)/i).map(x=>x.trim()).filter(Boolean);
    }
    if(parts.length===1 && parts[0].length>420){
      const words=parts[0].split(/\s+/); parts=[]; let buf=[];
      words.forEach(w=>{buf.push(w);if(buf.join(' ').length>210){parts.push(buf.join(' '));buf=[];}});
      if(buf.length)parts.push(buf.join(' '));
    }
    return parts;
  };
  for(const source of raw.split(/\n+/).map(x=>x.trim()).filter(Boolean)){
    const heading=quickNoteHeadingCategory(source);
    const looksHeading=heading && (source.endsWith(':') || source.length<48 || /^#{1,3}\s/.test(source));
    if(looksHeading){explicitCat=heading;continue;}
    const line=source.replace(/^[•\-–]\s*/,'').trim();
    for(const frag of splitFragments(line))add(explicitCat||classifyQuickNote(frag),frag);
  }
...
function renderQuickFieldNotes(){
  ...
  if(status&&!document.getElementById('quick-note-live-preview'))status.insertAdjacentHTML('afterend','<div id="quick-note-live-preview" class="notice" style="display:none;margin-top:8px"><strong>Texte entendu :</strong> <span></span><div class="muted small-text">Le texte provisoire s’affiche ici ; seules les phrases validées sont ajoutées aux notes.</div></div>');
  ...
  if(SpeechRecognition&&mic){
    let dictationWanted=false,recognition=null,restartTimer=null,lastFinalNorm='',lastFinalAt=0;
    const cleanSpeech=x=>String(x||'').replace(/\s+/g,' ').trim();
    const norm=x=>normalizeSearchText(cleanSpeech(x)).replace(/[^a-z0-9à-ÿ ]/gi,'').replace(/\s+/g,' ').trim();
    const appendFinal=text=>{
      const piece=cleanSpeech(text),np=norm(piece); if(!np)return;
      const now=Date.now();
      if(np===lastFinalNorm && now-lastFinalAt<15000)return;
      const tail=norm((raw.value||'').slice(-Math.max(700,piece.length*4)));
      if(np.length>8 && (tail.endsWith(np)||tail.includes(np))){lastFinalNorm=np;lastFinalAt=now;return;}
      raw.value=(raw.value||'')+(raw.value&&!/\s$/.test(raw.value)?' ':'')+piece;
      lastFinalNorm=np;lastFinalAt=now; saveRaw(); raw.scrollTop=raw.scrollHeight;
    };
    const showLive=text=>{if(liveSpan)liveSpan.textContent=cleanSpeech(text)||'…';if(liveBox)liveBox.style.display=dictationWanted?'block':'none';};
    const setMicUi=active=>{mic.textContent=active?'⏹️ Arrêter la dictée':'🎙️ Démarrer la dictée';mic.classList.toggle('danger',active);};
    const stop=()=>{dictationWanted=false;if(restartTimer){clearTimeout(restartTimer);restartTimer=null;}try{recognition?.stop();}catch(_){}setMicUi(false);if(liveBox)liveBox.style.display='none';status.textContent='Dictée arrêtée. Les phrases validées sont enregistrées.';};
    const start=()=>{
      if(!dictationWanted)return;
      try{
        const rec=new SpeechRecognition(); recognition=rec; __fieldNotesRecognition=rec;
        rec.lang='fr-FR'; rec.continuous=true; rec.interimResults=true; rec.maxAlternatives=1;
        rec.onstart=()=>{setMicUi(true);status.textContent='🎙️ Dictée en cours. Le texte entendu s’affiche en direct ci-dessous.';showLive('…');};
        rec.onresult=e=>{
          let interim='';
          for(let i=e.resultIndex;i<e.results.length;i++){
            const r=e.results[i],t=cleanSpeech(r?.[0]?.transcript||''); if(!t)continue;
            if(r.isFinal)appendFinal(t); else interim=t;
          }
          showLive(interim||'✓ phrase enregistrée');
        };
        rec.onerror=e=>{
          const err=e?.error||'erreur';
          if(['not-allowed','service-not-allowed','audio-capture'].includes(err)){
            dictationWanted=false;setMicUi(false);if(liveBox)liveBox.style.display='none';
            status.textContent=`Dictée impossible (${err}). Vérifiez l’autorisation du micro.`;
          }else if(dictationWanted)status.textContent=`Dictée interrompue (${err}), reprise automatique…`;
        };
        rec.onend=()=>{recognition=null;__fieldNotesRecognition=null;if(dictationWanted){status.textContent='🎙️ Reprise automatique…';restartTimer=setTimeout(()=>{restartTimer=null;start();},600);}else{setMicUi(false);if(liveBox)liveBox.style.display='none';}};
        rec.start();
      }catch(err){console.error(err);if(dictationWanted){status.textContent='Dictée interrompue, nouvelle tentative…';restartTimer=setTimeout(()=>{restartTimer=null;start();},1000);}}
    };
    mic.onclick=()=>{if(dictationWanted){stop();return;}dictationWanted=true;lastFinalNorm='';lastFinalAt=0;start();};
  }
...
function renderFollowup(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Historique & évolution</h2><div class="muted">Comparer deux visites d’une même exploitation : reproduction, mortalité, mesures, alimentation, bâtiment et plans d’action.</div></div><span class="badge autosave">v14.6.21.109</span></div>
...
function measurementExportSheets(visit){
  ...
  for(const [kind,cfg] of Object.entries(generalConfigs)){
    ...
    const sb=ensureSupplementalBlood(visit),sbParams=supplementalBloodCatalog.filter(p=>sb.enabled.includes(p.key));
  if(sbParams.length){
    ...
    const rows=(visit.subjects||[]).map(subject=>[...subjectBase(subject),...sbParams.map(p=>{const v=subject.measurements?.supplementalBlood?.[p.key]??'';const n=numericValue(v);return n===null?v:n;}),subject.measurements?.supplementalBlood?.comment||'',sb.source||'',sb.date||'',sb.comment||'']);
...
if ('serviceWorker' in navigator){
  ...
  const AUDIT_SW=location.origin+AUDIT_PATH+'sw.js?v=14.6.21.109';
