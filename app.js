...
// Cette fonction manquait dans la v14.6.21.96 et provoquait l'erreur splitUsefulLines is not defined.
...
// v14.6.21.96 — Audits ciblés + motif libre + orientation syndromique veaux
...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Repères visuels, fiches pratiques et carnet technique, disponibles hors ligne.</div></div><span class="badge autosave">v14.6.21.96</span></div>
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.6.21.96</span></div>
...
function renderVisits() {
  ...
  app.innerHTML = `
    <div class="section-title"><div><h2>Visites</h2><div class="muted">Toutes les visites enregistrées restent accessibles, y compris les plus anciennes.</div></div><span class="badge autosave">v14.6.21.96</span></div>
...
function renderFarmerQuestionnaires(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📨 Questionnaires éleveur</h2><div class="muted">Préparer la visite puis récupérer uniquement les réponses encore manquantes.</div></div><span class="badge autosave">v14.6.21.96</span></div>${activeVisitBanner(visit)}
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.6.21.96</span></div>
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.6.21.96</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
  visit.feeding=visit.feeding&&typeof visit.feeding==='object'?visit.feeding:{rations:[],settings:{},history:[]};
  visit.feeding.nutrition=visit.feeding.nutrition&&typeof visit.feeding.nutrition==='object'?visit.feeding.nutrition:{};
...
function renderFollowup(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Historique & évolution</h2><div class="muted">Comparer deux visites d’une même exploitation : reproduction, mortalité, mesures, alimentation, bâtiment et plans d’action.</div></div><span class="badge autosave">v14.6.21.96</span></div>
...
function herdStackedBarChartSvg(labels,series,{height=330}={}){
  ...
  const grid=Array.from({length:4},(_,i)=>{const gv=max*(i/3),gy=y(gv);return `<line x1="${pad.left}" y1="${gy.toFixed(1)}" x2="${width-pad.right}" y2="${gy.toFixed(1)}" stroke="#d9e4dd" stroke-width="1"/><text x="${pad.left-6}" y="${(gy+4).toFixed(1)}" text-anchor="end" font-size="11" fill="#6b746e">${Math.round(gv)}</text>`;}).join('');
  let bars='';
  labels.forEach((lbl,i)=>{const x=pad.left+i*(plotW/Math.max(1,labels.length))+(plotW/Math.max(1,labels.length)-barW)/2;let cumulative=0;series.forEach(s=>{const v=chartValue(s.values[i])||0;if(v<=0)return;const y0=y(cumulative+v),y1=y(cumulative),h=y1-y0,label=pctLabel(s.labelValues?.[i]);bars+=`<rect x="${x.toFixed(1)}" y="${y0.toFixed(1)}" width="${barW.toFixed(1)}" height="${h.toFixed(1)}" fill="${s.color}"/>${label&&h>=11?`<text x="${(x+barW/2).toFixed(1)}" y="${(y0+h/2+3.5).toFixed(1)}" text-anchor="middle" font-size="9.5" font-weight="700" fill="#fff" stroke="#26332d" stroke-width="0.35" paint-order="stroke">${label}</text>`:''}`;cumulative+=v;});bars+=`<text x="${(x+barW/2).toFixed(1)}" y="${(y(cumulative)-8).toFixed(1)}" text-anchor="middle" font-size="11" font-weight="700" fill="#22312a">${cumulative||''}</text><text x="${(x+barW/2).toFixed(1)}" y="${height-14}" text-anchor="middle" font-size="12" fill="#4a5750">${escapeHtml(lbl)}</text>`;});
  return `${herdChartLegend(series)}<svg viewBox="0 0 ${width} ${height}" class="herd-svg-chart" aria-hidden="true"><rect x="0" y="0" width="${width}" height="${height}" rx="12" fill="#fffafd"/><g>${grid}<line x1="${pad.left}" y1="${height-pad.bottom}" x2="${width-pad.right}" y2="${height-pad.bottom}" stroke="#b9c9bf" stroke-width="1.2"/>${bars}</g></svg>`;
}
...
// v14.6.21.96 — aide à la décision transversale : priorités, tendances et références internes.
...
// v14.6.21.96 — cohérence pré-visite, tableau global mouvements/renouvellement et âges lisibles.
...
// v14.6.21.96 — « fabrique du troupeau » : naissance femelles → conservation jusqu’à la reproduction → 1er vêlage → réforme.
...
// v14.6.21.96 — estimation économique pré-visite, calculée uniquement sur les données déjà disponibles.
...
// v14.6.21.96 — check-up, suivi d'étude, économie et conservation globale du défilement horizontal
...
// v14.6.21.96 — suivi du temps par intervenant + temps GDS préparation/terrain/compte rendu/suivi

// v14.6.21.96 — questionnaires persistants, repères de saisie, bilan sanitaire/éco partenaire, supports papier, check-up responsive.

window.addEventListener('DOMContentLoaded',()=>{initGlobalSearch();initAccordionMemory();});
// v14.6.21.96 — cohérence, imprimables préparation, animaux présents, recherche globale.
