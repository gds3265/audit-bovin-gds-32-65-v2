...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Repères visuels, fiches pratiques et carnet technique, disponibles hors ligne.</div></div><span class="badge autosave">v14.6.21.120</span></div>
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.6.21.120</span></div>
...
function renderVisits() {
  ...
  app.innerHTML = `
    <div class="section-title"><div><h2>Visites</h2><div class="muted">Toutes les visites enregistrées restent accessibles, y compris les plus anciennes.</div></div><span class="badge autosave">v14.6.21.120</span></div>
...
function renderFarmerQuestionnaires(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📨 Questionnaires éleveur</h2><div class="muted">Préparer la visite puis récupérer uniquement les réponses encore manquantes.</div></div><span class="badge autosave">v14.6.21.120</span></div>${activeVisitBanner(visit)}
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.6.21.120</span></div>
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.6.21.120</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
  visit.feeding=visit.feeding&&typeof visit.feeding==='object'?visit.feeding:{rations:[],settings:{},history:[]};
  visit.feeding.nutrition=visit.feeding.nutrition&&typeof visit.feeding.nutrition==='object'?visit.feeding.nutrition:{};
...
function renderFollowup(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Historique & évolution</h2><div class="muted">Comparer deux visites d’une même exploitation : reproduction, mortalité, mesures, alimentation, bâtiment et plans d’action.</div></div><span class="badge autosave">v14.6.21.120</span></div>
...
function cycleStageData(visit){
  ...
  const compact=arr=>arr.map(x=>String(x||'').trim()).filter(Boolean).filter((x,i,a)=>a.indexOf(x)===i).slice(0,2).join(' · ');
  const solObs=compact(sol.map(a=>a.comment));
  const forageObs=compact([...forages.map(a=>a.comment),...veg.map(a=>a.comment)]);
  const waterObs=String(w.notes||'').trim();
  const rationObs=compact([ensureSupplementation(visit).notes,...(visit.feeding?.rations||[]).map(r=>r.comment)]);
  const animalObs=String(m.notes||'').trim();
...
function importChatgptRows(visit,rows,fileName){
  ...
  for(const row of rows){
    ...
    if(type.includes('fourrage')){
      ...
      if(date)a.date=date;if(source)a.source=source;if(comment)a.comment=[a.comment,comment].filter(Boolean).join(' · ');a.rows.push({id:uid('ctxrow'),parameter:param,value,unit,comment,importSource:tag});count++;
...
if ('serviceWorker' in navigator){
  ...
  const AUDIT_SW=location.origin+AUDIT_PATH+'sw.js?v=14.6.21.120';
