...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Repères visuels, fiches pratiques et carnet technique, disponibles hors ligne.</div></div><span class="badge autosave">v14.6.21.103</span></div>
...
window.addEventListener('audit-bovin-cloud-merged',event=>{
  ...
  const noHotReload=new Set(['audit','visits','documents','building','analysis','animals','feeding','questionnaires','herddata','metabolic','parasitism','waterlab']);
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.6.21.103</span></div>
...
function renderVisits() {
  ...
  app.innerHTML = `
    <div class="section-title"><div><h2>Visites</h2><div class="muted">Toutes les visites enregistrées restent accessibles, y compris les plus anciennes.</div></div><span class="badge autosave">v14.6.21.103</span></div>
...
function renderFarmerQuestionnaires(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📨 Questionnaires éleveur</h2><div class="muted">Préparer la visite puis récupérer uniquement les réponses encore manquantes.</div></div><span class="badge autosave">v14.6.21.103</span></div>${activeVisitBanner(visit)}
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.6.21.103</span></div>
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.6.21.103</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
  visit.feeding=visit.feeding&&typeof visit.feeding==='object'?visit.feeding:{rations:[],settings:{},history:[]};
  visit.feeding.nutrition=visit.feeding.nutrition&&typeof visit.feeding.nutrition==='object'?visit.feeding.nutrition:{};
...
function classifyQuickNote(text=''){
  ...
  const rules=[
    ['Alimentation / ration',/(ration|aliment|ensilage|foin|paille|concentre|mineral|sel|melangeuse|distribution|fourrage)/],
    ['Eau / bâtiment',/(eau|abreuvoir|batiment|ventilation|litiere|aire|couchage|logette|temperature|humidite|courant d air)/],
    ['Reproduction',/(repro|velage|ivv|chaleur|saillie|insemin|avort|gestation|taureau)/],
    ['Mortalité / sanitaire',/(mort|mortal|maladie|diarrhee|pneum|boiter|mammit|sanitaire|traitement|veto|vaccin)/],
    ['Animaux / observations',/(vache|veau|genisse|animal|troupeau|etat corporel|nec|bouse|urine|poil)/],
    ['Organisation / économie',/(travail|main d oeuvre|organisation|achat|vente|cout|prix|temps|objectif|projet)/]
  ];
  return rules.find(([,rx])=>rx.test(t))?.[0] || 'À classer';
}
function appendSectionNote(target,text){
  ...
  if(target==='Alimentation / ration'){
    visit.feeding=visit.feeding&&typeof visit.feeding==='object'?visit.feeding:{rations:[],settings:{},history:[]};
    visit.feeding.settings=visit.feeding.settings&&typeof visit.feeding.settings==='object'?visit.feeding.settings:{};
    visit.feeding.settings.generalComment=add(visit.feeding.settings.generalComment,'Note terrain');
  }else{
    const ag=ensureAuditGlobal(visit); ag.notes=add(ag.notes,target);
  }
}
function renderQuickFieldNotes(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📝 Notes terrain</h2><div class="muted">Bloc-notes libre pendant l’échange avec l’éleveur. Écrivez vite, classez après.</div></div><span class="badge autosave">Sauvegarde automatique</span></div>
  ${activeVisitBanner(visit)}
  <section class="card quick-field-notes">
    <div class="notice"><strong>Mode terrain :</strong> une idée par ligne si possible. Vous pouvez tout noter en vrac puis utiliser « Retraiter les notes ».</div>
    <div class="field"><label>Notes prises pendant la visite</label><textarea id="quick-field-notes-raw" rows="16" placeholder="Ex. Les génisses reçoivent le même foin que les vaches&#10;2e abreuvoir souvent vide l'été&#10;Taureau changé en 2024…">${escapeHtml(n.raw||'')}</textarea></div>
    <div class="actions"><button class="btn" id="quick-note-time">+ Heure</button><button class="btn primary" id="quick-notes-process">✨ Retraiter les notes</button></div>
  </section>
  <section class="card"><div class="section-title"><div><h3>Propositions de classement</h3><div class="muted">Rien n’est injecté dans l’audit sans validation.</div></div>${n.items.length?'<button class="btn primary" id="quick-notes-integrate">Intégrer les notes classées</button>':''}</div>
  <div id="quick-notes-items">${n.items.length?n.items.map((x,i)=>`<article class="quick-note-item"><div class="field"><label>Destination</label><select data-qn-cat="${i}">${cats.map(c=>`<option ${x.category===c?'selected':''}>${c}</option>`).join('')}</select></div><div class="field field-wide"><label>Note</label><textarea data-qn-text="${i}">${escapeHtml(x.text||'')}</textarea></div><label class="check"><input type="checkbox" data-qn-use="${i}" ${x.use!==false?'checked':''}> Intégrer</label></article>`).join(''):'<div class="empty">Aucune proposition. Cliquez sur « Retraiter les notes ».</div>'}</div></section>`;
  const raw=document.getElementById('quick-field-notes-raw');
  const saveRaw=()=>{n.raw=raw.value;n.updatedAt=new Date().toISOString();visit.updatedAt=n.updatedAt;saveDatabase(db);};
  raw.oninput=saveRaw; raw.onblur=saveRaw;
  ...
  document.getElementById('quick-notes-process').onclick=()=>{
    ...
    n.items=parts.map(text=>({text,category:classifyQuickNote(text),use:true}));
  ...
  document.getElementById('quick-notes-integrate')?.addEventListener('click',()=>{
    ...
    n.integratedAt=new Date().toISOString();visit.updatedAt=n.integratedAt;saveDatabase(db);
    showToast(`${chosen.length} note(s) intégrée(s). Les notes « À classer » restent dans le bloc-notes.`);
  });
}
...
function renderFollowup(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Historique & évolution</h2><div class="muted">Comparer deux visites d’une même exploitation : reproduction, mortalité, mesures, alimentation, bâtiment et plans d’action.</div></div><span class="badge autosave">v14.6.21.103</span></div>
