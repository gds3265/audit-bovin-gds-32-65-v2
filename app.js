...
function applyHerdImportToVisit(item,visit,overwrite=false){
  ...
  if(rep.firstCalvingAge!==null&&rep.firstCalvingAge!==undefined&&(overwrite||!a.answers['Âge moyen au premier vêlage'].answer)){a.answers['Âge moyen au premier vêlage'].answer=`${String(rep.firstCalvingAge).replace('.',',')} mois`;a.answers['Âge moyen au premier vêlage'].comment=`Donnée importée depuis ${item.sourceFile||'CSV'} (période N).`;changes.push('Âge au premier vêlage');}
  a.answers['Intervalle vêlage-vêlage']=a.answers['Intervalle vêlage-vêlage']||{};
  if(rep.ivv!==null&&rep.ivv!==undefined&&(overwrite||!a.answers['Intervalle vêlage-vêlage'].answer)){a.answers['Intervalle vêlage-vêlage'].answer=`${String(rep.ivv).replace('.',',')} jours`;a.answers['Intervalle vêlage-vêlage'].comment=`IVV > 390 j : ${rep.ivv390??'—'} ; IVV > 420 j : ${rep.ivv420??'—'}. Donnée importée.`;changes.push('IVV');}
...
function herdAuditLinkHtml(item){const visits=db.visits.filter(v=>v.farmId===item.farmId).slice().sort((a,b)=>(b.date||'').localeCompare(a.date||''));if(!visits.length)return `<div class="notice warning"><strong>Aucune visite liée.</strong> Créez d’abord une visite pour pouvoir compléter son audit.</div>`;const suggested=(activeVisit()?.farmId===item.farmId?activeVisit():latestVisitForFarm(item.farmId));return `<div class="herd-audit-link"><div class="field"><label>Visite à compléter</label><select data-herd-target-visit="${item.id}">${visits.map(v=>`<option value="${v.id}" ${v.id===suggested?.id?'selected':''}>${formatDate(v.date)} — ${escapeHtml(v.type||'Visite')}</option>`).join('')}</select></div><label class="checkbox-line"><input type="checkbox" data-herd-overwrite="${item.id}"> Remplacer aussi les valeurs déjà saisies</label><button class="btn primary" data-apply-herd-audit="${item.id}">Compléter l’audit avec ces données</button><div class="muted">Par défaut, seules les rubriques vides sont complétées. Les données importées restent identifiées.</div></div>`;}
...
function renderHerdData(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Données élevage importées</h2><span class="muted">Effectifs, mouvements, mortalité et reproduction depuis un export CSV.</span></div><span class="badge autosave">v11.10.1</span></div>
