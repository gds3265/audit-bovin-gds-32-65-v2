...
function planCanvasHtml(){return `<section class="card"><div class="section-title"><div><h3>Plan interactif</h3><div class="muted">Dessinez la structure puis placez les équipements. Les abreuvoirs, points électriques et zones de litière sont reliés aux tableaux de mesures.</div></div><span class="badge autosave">Auto</span></div>
  <div class="plan-toolbar plan-toolbar-groups">
    ...
    <div class="plan-tool-group"><strong>Équipements</strong><button class="plan-tool" data-tool="cornadis">▥ Cornadis</button><button class="plan-tool" data-tool="mangeoire">🥣 Mangeoire</button><button class="plan-tool" data-tool="logette">▱ Logette</button><button class="plan-tool" data-tool="ventilateur">🌀 Ventilateur</button><button class="plan-tool" data-tool="porte">🚪 Porte</button><button class="plan-tool" data-tool="fenetre">▣ Fenêtre</button></div>
    <div class="plan-tool-group"><strong>Mesures liées</strong><button class="plan-tool" data-tool="water">💧 Abreuvoir</button><button class="plan-tool" data-tool="electric">⚡ Point électrique</button><button class="plan-tool" data-tool="litter">🛏️ Litière</button></div>
  ...
  </div><div class="plan-layout"><div class="plan-canvas-wrap"><canvas id="building-canvas" width="1000" height="580"></canvas></div><aside id="plan-inspector" class="plan-inspector"><h4>Objet sélectionné</h4><p class="muted">Choisissez l’outil Sélection puis cliquez sur un objet.</p></aside></div><div class="muted small-text">Les équipements peuvent être déplacés avec l’outil Sélection. Cliquez sur un abreuvoir, un point électrique ou une litière pour ouvrir sa fiche de mesure.</div></section>`;}
...
function initPlanCanvas(building,audit,visit){
  ...
  const objectMeta={cornadis:{label:'Cornadis',icon:'C',color:'#64748b'},mangeoire:{label:'Mangeoire',icon:'M',color:'#ca8a04'},logette:{label:'Logette',icon:'L',color:'#8b5cf6'},ventilateur:{label:'Ventilateur',icon:'V',color:'#0f766e'},porte:{label:'Porte',icon:'P',color:'#92400e'},fenetre:{label:'Fenêtre',icon:'F',color:'#38bdf8'},water:{label:'Abreuvoir',icon:'A',color:'#0ea5e9'},electric:{label:'Électricité',icon:'E',color:'#eab308'},litter:{label:'Litière',icon:'Li',color:'#a16207'}};
  ...
  const isObject=s=>!!objectMeta[s.type];
  const drawShape=s=>{ctx.save();ctx.lineCap='round';ctx.lineJoin='round';ctx.strokeStyle=s.color||color(s.type);ctx.fillStyle=s.color||color(s.type);ctx.lineWidth=s.width||4;
    ...
    if(isObject(s)){const w=s.w||54,h=s.h||38;ctx.globalAlpha=.92;ctx.fillRect(s.x-w/2,s.y-h/2,w,h);ctx.globalAlpha=1;ctx.fillStyle='#fff';ctx.font='bold 15px sans-serif';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(objectMeta[s.type].icon,s.x,s.y-3);ctx.font='10px sans-serif';ctx.fillText((s.label||objectMeta[s.type].label).slice(0,12),s.x,s.y+12);if(s.id===selectedId){ctx.strokeStyle='#dc2626';ctx.lineWidth=3;ctx.strokeRect(s.x-w/2-4,s.y-h/2-4,w+8,h+8);}}
    if(s.type==='text'){ctx.font='18px sans-serif';ctx.fillText(s.text||'',s.x,s.y);}ctx.restore();};
  ...
  const hit=p=>[...history].reverse().find(s=>{if(isObject(s))return Math.abs(p.x-s.x)<(s.w||54)/2+8&&Math.abs(p.y-s.y)<(s.h||38)/2+8;if(s.type==='text')return Math.abs(p.x-s.x)<80&&Math.abs(p.y-s.y)<24;return false});
  ...
  const renderInspector=()=>{const box=document.getElementById('plan-inspector');if(!box)return;const s=history.find(x=>x.id===selectedId);if(!s){box.innerHTML='<h4>Objet sélectionné</h4><p class="muted">Choisissez l’outil Sélection puis cliquez sur un objet.</p>';return}const row=linkedRow(s);box.innerHTML=`<h4>${escapeHtml(s.label||objectMeta[s.type]?.label||s.type)}</h4><div class="field"><label>Libellé</label><input id="shape-label" value="${escapeHtml(s.label||'')}"></div><div class="muted small-text">Type : ${escapeHtml(objectMeta[s.type]?.label||s.type)}</div>${row?`<div class="plan-linked-summary">${s.linkKind==='drinker'?`Type : ${escapeHtml(row.type||'')}<br>Matériau : ${escapeHtml(row.material||'')}<br>Débit : ${escapeHtml(row.flow||'—')} L/min`:s.linkKind==='electric'?`Valeur : ${escapeHtml(row.value||'—')} ${escapeHtml(row.unit||'')}`:`Zone : ${escapeHtml(row.zone||'')}<br>pH : ${escapeHtml(row.ph||'—')}`}</div><button class="btn primary" id="open-linked-row">Ouvrir la fiche liée</button>`:'<p class="muted">Équipement permanent du plan.</p>'}<button class="btn danger" id="delete-shape-inspector">Supprimer cet objet</button>`;document.getElementById('shape-label')?.addEventListener('input',e=>{s.label=e.target.value;if(row)row.name=row.name||e.target.value;persist()});document.getElementById('delete-shape-inspector').onclick=()=>{const i=history.findIndex(x=>x.id===s.id);if(i>=0)history.splice(i,1);selectedId='';persist()};document.getElementById('open-linked-row')?.addEventListener('click',()=>{activeBuildingTab=s.linkKind==='drinker'?'water':s.linkKind==='electric'?'electric':'litter';localStorage.setItem('audit-bovin-building-tab',activeBuildingTab);renderBuilding();setTimeout(()=>document.querySelector(`[data-id="${s.linkId}"]`)?.scrollIntoView({behavior:'smooth',block:'center'}),100)});};
  const createLinkedObject=(type,p)=>{let row,label;if(type==='water'){const n=audit.drinkers.length+1;row={id:uid('drinker'),name:`Abreuvoir ${n}`,type:'Bac collectif',material:'Inox',origin:'Réseau'};audit.drinkers.push(row);label=row.name;saveBuildingAudit(visit);commit({type,x:p.x,y:p.y,label,linkKind:'drinker',linkId:row.id});}else if(type==='electric'){const n=audit.electric.length+1;row={id:uid('electric'),equipment:`Point électrique ${n}`,unit:'mV',current:'AC'};audit.electric.push(row);label=row.equipment;saveBuildingAudit(visit);commit({type,x:p.x,y:p.y,label,linkKind:'electric',linkId:row.id});}else if(type==='litter'){const n=audit.litters.length+1;row={id:uid('litter'),zone:`Zone ${n}`,type:'Paille',quantityUnit:'kg/j'};audit.litters.push(row);label=row.zone;saveBuildingAudit(visit);commit({type,x:p.x,y:p.y,label,linkKind:'litter',linkId:row.id});}};
  ...
  canvas.addEventListener('pointerdown',e=>{canvas.setPointerCapture(e.pointerId);const p=point(e);const width=Number(document.getElementById('plan-width')?.value||4);if(tool==='select'){const s=hit(p);selectedId=s?.id||'';if(s){dragOffset={x:p.x-s.x,y:p.y-s.y};drawing=true}else dragOffset=null;renderInspector();renderCanvas();return}if(['water','electric','litter'].includes(tool)){createLinkedObject(tool,p);return}if(objectMeta[tool]){commit({type:tool,x:p.x,y:p.y,label:objectMeta[tool].label});return}if(tool==='text'){const text=prompt('Texte à ajouter :');if(text)commit({type:'text',x:p.x,y:p.y,text});return}drawing=true;start=p;if(tool==='free')temp={type:'free',points:[p],width,color:color(tool)};});
  canvas.addEventListener('pointermove',e=>{if(!drawing)return;const p=point(e);if(tool==='select'){const s=history.find(x=>x.id===selectedId);if(s&&isObject(s)){s.x=p.x-dragOffset.x;s.y=p.y-dragOffset.y;renderCanvas()}return}const width=Number(document.getElementById('plan-width')?.value||4);if(tool==='free')temp.points.push(p);if(tool==='line')temp={type:'line',x1:start.x,y1:start.y,x2:p.x,y2:p.y,width,color:color(tool)};if(tool==='rect')temp={type:'rect',x:Math.min(start.x,p.x),y:Math.min(start.y,p.y),w:Math.abs(p.x-start.x),h:Math.abs(p.y-start.y),width,color:color(tool)};renderCanvas();});
  const finish=()=>{if(!drawing)return;drawing=false;if(tool==='select'){persist();return}if(temp&&(tool==='free'?temp.points.length>1:true))commit(temp);else{temp=null;renderCanvas()}};canvas.addEventListener('pointerup',finish);canvas.addEventListener('pointercancel',finish);
