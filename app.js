...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Repères visuels, fiches pratiques et carnet technique, disponibles hors ligne.</div></div><span class="badge autosave">v14.6.21.43</span></div>
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.6.21.43</span></div>
...
function renderVisits() {
  ...
  app.innerHTML = `
    <div class="section-title"><div><h2>Visites</h2><div class="muted">Toutes les visites enregistrées restent accessibles, y compris les plus anciennes.</div></div><span class="badge autosave">v14.6.21.43</span></div>
...
function renderFarmerQuestionnaires(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📨 Questionnaires éleveur</h2><div class="muted">Préparer la visite puis récupérer uniquement les réponses encore manquantes.</div></div><span class="badge autosave">v14.6.21.43</span></div>${activeVisitBanner(visit)}
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.6.21.43</span></div>
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.6.21.43</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
...
function renderFollowup(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Historique & évolution</h2><div class="muted">Comparer deux visites d’une même exploitation : reproduction, mortalité, mesures, alimentation, bâtiment et plans d’action.</div></div><span class="badge autosave">v14.6.21.43</span></div>
...
function smartNumberFromLine(line,aliases=[]){const n=normalizeSearchText(line);if(!aliases.some(a=>n.includes(normalizeSearchText(a))))return null;const cleaned=String(line).replace(/\b(?:19|20)\d{2}[-/.]\d{1,2}[-/.]\d{1,2}\b/g,' ');const m=cleaned.match(/(?:<|>|≤|≥)?\s*-?\d+(?:[.,]\d+)?/);return m?m[0].replace(/\s+/g,'').replace(',','.') : null}
...
function smartFindValues(text,defs){const lines=smartTextLines(text),out=[];for(const d of defs){for(const line of lines){const value=smartNumberFromLine(line,d.aliases||[d.label]);if(value!==null){out.push({...d,value,unit:smartUnitFromLine(line)||d.unit||'',sourceLine:line});break}}}return out}
...
async function smartLabReadFile(file){let text='',dataUrl='';if(file.size<=4*1024*1024){try{dataUrl=await new Promise((res,rej)=>{const fr=new FileReader();fr.onload=()=>res(fr.result);fr.onerror=rej;fr.readAsDataURL(file)})}catch{}}if(file.type.startsWith('image/')&&'TextDetector'in window){try{const bmp=await createImageBitmap(file),det=new TextDetector(),res=await det.detect(bmp);text=res.map(x=>x.rawValue||'').join('\n')}catch{}}if(!text&&/^text\//.test(file.type)){try{text=await file.text()}catch{}}return{text,dataUrl}}
function renderSmartLabImportCard(view,visit){if(!visit||!['metabolic','parasitism','waterlab'].includes(view))return;const labels={metabolic:'profil métabolique / oligo',parasitism:'parasitisme',waterlab:'analyse d’eau'};visit.labImports=Array.isArray(visit.labImports)?visit.labImports:[];const existing=visit.labImports.filter(x=>x.module===view),integrated=existing.reduce((n,x)=>n+(Number(x.integratedCount)||0),0);const card=document.createElement('section');card.className='card smart-lab-import';card.innerHTML=`<div class="section-title"><div><h3>📄 Résultats labo — ${escapeHtml(labels[view])}</h3><div class="muted">${existing.length?`✅ ${existing.length} document(s) chargé(s) · ${integrated} valeur(s) intégrée(s) dans ce module`:'Aucun résultat labo chargé dans ce module.'}</div></div>${existing.length?`<span class="badge complete">${existing.length} chargé(s)</span>`:''}</div><p class="muted">Sélectionnez PDF ou photos. Quand le navigateur peut lire le texte d’une image, les valeurs reconnues sont ajoutées directement au tableau sans écraser une valeur existante. Le document original reste rattaché à la visite.</p><input type="file" accept="image/*,.pdf,application/pdf" data-smart-lab-file multiple><div class="small" data-smart-lab-info></div><div class="lab-import-list">${existing.length?existing.map(x=>`<div class="lab-import-item"><span><b>${escapeHtml(x.name)}</b><small>${formatDateTime(x.createdAt)} · ${Number(x.integratedCount)||0} valeur(s) intégrée(s)${x.ocrText?' · texte détecté':' · document conservé'}</small></span>${x.dataUrl?`<button class="btn small" data-open-lab-import="${x.id}">Ouvrir</button>`:''}<button class="btn small danger" data-delete-lab-import="${x.id}">Supprimer</button></div>`).join(''):'<div class="empty compact">Aucun résultat labo importé dans ce module.</div>'}</div>`;app.prepend(card);card.querySelector('[data-smart-lab-file]').onchange=async e=>{const files=[...(e.target.files||[])];if(!files.length)return;const info=card.querySelector('[data-smart-lab-info]');info.textContent=`Import de ${files.length} fichier(s)…`;let ok=0,totalValues=0;for(const file of files){const{text,dataUrl}=await smartLabReadFile(file);const integratedCount=integrateSmartLabText(view,visit,text,file.name);visit.labImports.unshift({id:uid('labimp'),module:view,name:file.name,type:file.type,size:file.size,createdAt:new Date().toISOString(),ocrText:text,dataUrl,integratedCount});ok++;totalValues+=integratedCount}visit.updatedAt=new Date().toISOString();saveDatabase(db);showToast(totalValues?`${ok} document(s) chargé(s) · ${totalValues} valeur(s) intégrée(s).`:`${ok} document(s) chargé(s), mais aucune valeur n’a pu être lue automatiquement.`);render()};card.querySelectorAll('[data-open-lab-import]').forEach(b=>b.onclick=()=>{const x=visit.labImports.find(y=>y.id===b.dataset.openLabImport);if(!x?.dataUrl)return showToast('Fichier non disponible sur cet appareil.');try{const blob=dataUrlToBlob(x.dataUrl),url=URL.createObjectURL(blob);window.open(url,'_blank');setTimeout(()=>URL.revokeObjectURL(url),30000)}catch{showToast('Impossible d’ouvrir ce fichier.')}});card.querySelectorAll('[data-delete-lab-import]').forEach(b=>b.onclick=()=>{if(!confirm('Supprimer ce document labo de la visite ? Les valeurs déjà intégrées dans les tableaux ne seront pas supprimées automatiquement.'))return;visit.labImports=visit.labImports.filter(x=>x.id!==b.dataset.deleteLabImport);saveDatabase(db);render()})}
...
// v14.6.21.43 — check-up, suivi d'étude, économie et conservation globale du défilement horizontal
...
// v14.6.21.43 — suivi du temps par intervenant + temps GDS préparation/terrain/compte rendu/suivi

// v14.6.21.43 — questionnaires persistants, repères de saisie, bilan sanitaire/éco partenaire, supports papier, check-up responsive.

window.addEventListener('DOMContentLoaded',()=>{initGlobalSearch();initAccordionMemory();});
// v14.6.21.43 — cohérence, imprimables préparation, animaux présents, recherche globale.
