...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Quelques repères visuels utiles, intégrés à l’application et disponibles hors ligne.</div></div><span class="badge autosave">v14.4.8</span></div>
...
function migrateDatabase() {
  ...
  db.herdImports = Array.isArray(db.herdImports) ? db.herdImports.map(item => { if (!item || typeof item !== 'object') return item; const { raw, ...clean } = item; return clean; }) : [];
...
migrateDatabase();
window.addEventListener('audit-bovin-save-error', event => alert(event.detail?.message || 'La sauvegarde locale a échoué.'));
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.4.8</span></div>
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.4.8</span></div>
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.4.8</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
...
function repairHerdImportFarmLinks(){repairLegacyFarmNumbers();let linked=0;(db.herdImports||[]).forEach(item=>{const matched=findFarmForImport(item);if(matched&&item.farmId!==matched.id){item.farmId=matched.id;linked++;}});if(linked)saveDatabase(db);return{linked,applied:0};}
...
function linkedHerdImportForVisit(visit){const sourceId=ensureAuditGlobal(visit).importedHerdData?.sourceId;return sourceId?(db.herdImports?.find(x=>x.id===sourceId)||null):null;}
...
function renderHerdData(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Données technico-économiques</h2><span class="muted">Effectifs, mouvements, mortalité et reproduction depuis un export CSV.</span></div><span class="badge autosave">v14.4.8</span></div>${repairedLinks.linked?`<section class="notice"><strong>Liaison corrigée :</strong> ${repairedLinks.linked} import(s) rattaché(s) à l’exploitation correspondante. Aucun import n’a été appliqué automatiquement à une visite.</section>`:''}
...
function renderReproduction(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🐄 Reproduction</h2><div class="muted">Registre historique pour les fiches individuelles ; indicateurs calculés uniquement sur les vaches présentes ayant déjà eu au moins un veau.</div></div><span class="badge autosave">v14.4.8</span></div>
