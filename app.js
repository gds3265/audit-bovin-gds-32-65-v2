...
function ensureCoreDatabaseShape(){
...
// Découpe robuste des listes libres (conclusion, suites, recommandations).
// Cette fonction manquait dans la v14.6.21.99 et provoquait l'erreur splitUsefulLines is not defined.
...
// v14.6.21.99 — Audits ciblés + motif libre + orientation syndromique veaux
const targetedAuditModes = [
  ['complete','Audit complet'],
  ['young-mortality','Audit orienté mortalité des jeunes'],
  ['reproduction','Audit orienté reproduction'],
  ['automatic','Audit ciblé automatique']
];
function ensureAuditTargeting(visit){
  visit.auditTargeting=visit.auditTargeting||{};
  const t=visit.auditTargeting;
  if(!t.mode)t.mode='automatic';
  if(t.problemNarrative==null)t.problemNarrative='';
  t.calfSyndrome=t.calfSyndrome||{type:'',age:'',ageUnit:'jours',affected:'',total:'',fever:'Inconnu',blood:'Inconnu',spread:'',notes:''};
  return t;
}
function symptomAgeDays(age,unit){
  const n=parseFrenchNumber(age); if(n==null||n<0)return null;
  const u=normalizeSearchText(unit||'jours');
  return Math.round(n*(u.startsWith('semaine')?7:u.startsWith('mois')?30.4:1));
}
function ageFromNarrative(text){
  const raw=String(text||'');
  const rx=/(?:age|âgé(?:e)?s? de|ages? de|veaux? de|a)\s*(\d+(?:[,.]\d+)?)\s*(jours?|j\b|semaines?|sem\b|mois)/i;
  let m=raw.match(rx)||raw.match(/(\d+(?:[,.]\d+)?)\s*(jours?|j\b|semaines?|sem\b|mois)/i);
  if(!m)return null;
  const n=parseFrenchNumber(m[1]),u=normalizeSearchText(m[2]); if(n==null)return null;
  return {value:n,unit:u.startsWith('sem')?'semaines':u.startsWith('mois')?'mois':'jours',days:Math.round(n*(u.startsWith('sem')?7:u.startsWith('mois')?30.4:1))};
}
function calfSyndromeGuidance(type,ageDays,contextText=''){
  if(!type||ageDays==null)return null;
  const ctx=normalizeSearchText(contextText),isDia=type==='diarrhea',isResp=type==='respiratory';
  const causes=[],risk=[],questions=[],vet=[];
  if(isDia){
    if(ageDays<=4){causes.push('E. coli entérotoxigène (ETEC) compatible avec les tout premiers jours','Défaut de transfert colostral / septicémie à rechercher selon l’état général','Erreur de préparation, volume ou température du lait');}
    else if(ageDays<=21){causes.push('Cryptosporidium spp. particulièrement compatible avec cette tranche d’âge','Rotavirus et coronavirus entériques à envisager selon le contexte de lot','Facteurs alimentaires et pression d’infection à évaluer');}
    else if(ageDays<=60){causes.push('Coccidiose à envisager davantage à partir de 3–4 semaines','Cryptosporidiose encore possible chez certains lots','Transition alimentaire / lait / concentré et hygiène du logement');}
    else {causes.push('Coccidiose et causes parasitaires selon conduite et pâturage','Causes alimentaires / transition / qualité de l’eau','Salmonellose ou autre cause infectieuse si fièvre, abattement ou mortalité');}
    risk.push('Délai, quantité et qualité du colostrum','Hygiène des biberons/seaux et de la préparation lactée','Mélange des classes d’âge et nettoyage entre lots','Qualité de l’eau et régularité de la distribution','Densité, humidité et propreté des cases');
    questions.push('À quel âge exact débute la diarrhée et combien de jours dure-t-elle ?','Tous les veaux sont-ils touchés ou seulement certains lots ?','Fièvre, sang, abattement ou déshydratation ?','Changement récent de lait, poudre, concentration ou fréquence ?','Les cases sont-elles vidées/nettoyées/désinfectées entre lots ?');
    vet.push('Selon le tableau clinique : coprologie / recherche Cryptosporidium-coccidies','PCR/antigènes sur fèces selon le contexte (rota/corona/agents bactériens)','Prélèvement précoce sur veaux non encore traités, à discuter avec le vétérinaire');
  }
  if(isResp){
    if(ageDays<=30){causes.push('Défaut de transfert colostral et infections néonatales favorisant les pneumonies précoces','Fausse déglutition / aspiration à envisager si lien avec buvée ou sondage','Agents bactériens opportunistes selon tableau clinique');}
    else if(ageDays<=180){causes.push('Complexe respiratoire bovin : BRSV, PI3, coronavirus bovin selon contexte épidémiologique','Mannheimia haemolytica / Pasteurella multocida / Histophilus somni comme complications possibles','Mycoplasma bovis à envisager notamment si évolution traînante, otites ou échecs thérapeutiques');}
    else {causes.push('Complexe respiratoire bovin et agents bactériens secondaires','Strongylose respiratoire si animaux au pâturage et contexte compatible','Mycoplasma bovis ou causes chroniques selon l’évolution');}
    risk.push('Ventilation réelle au niveau des veaux, condensation et ammoniac','Densité et mélange d’âges / provenances','Stress de sevrage, allotement, transport ou changement de bâtiment','Transfert colostral et état nutritionnel','Vaccination et chronologie des épisodes');
    questions.push('Toux seule ou fièvre / jetage / dyspnée ?','Âge exact au début et vitesse de propagation dans le lot ?','Épisode après allotement, sevrage, transport ou changement météo ?','Ventilation et courants d’air au niveau des animaux ?','Traitements déjà réalisés et réponse observée ?');
    vet.push('Examen clinique et température sur plusieurs animaux du lot','Écouvillonnage profond / lavage broncho-alvéolaire selon décision vétérinaire','Autopsie/prélèvements sur cas récent si mortalité, avec choix des tests par le vétérinaire');
  }
  if(/sang|hemorrag|hémorrag/.test(ctx)&&isDia)causes.unshift('Présence de sang : renforcer la recherche de coccidiose, salmonellose ou atteinte intestinale sévère selon l’âge et l’examen clinique');
  return {type,ageDays,causes,risk,questions,vet};
}
function narrativeSyndrome(text){
  const n=normalizeSearchText(text),age=ageFromNarrative(text);
  let type=''; if(/diarrh|feces liquides|fèces liquides/.test(n))type='diarrhea'; else if(/respir|pneum|toux|jetage|dyspn/.test(n))type='respiratory';
  return type&&age?calfSyndromeGuidance(type,age.days,text):null;
}
function targetedAuditPlan(visit){
  const t=ensureAuditTargeting(visit),text=[visit.objective||'',t.problemNarrative||''].join(' '),n=normalizeSearchText(text),priorities=[],tags=[];
  const add=(rank,title,reason,section,questions)=>{const key=title+'|'+section;if(priorities.some(p=>p.key===key))return;priorities.push({key,rank,title,reason,section,questions});};
  const tag=(x)=>{if(!tags.includes(x))tags.push(x)};
  if(t.mode==='young-mortality'){tag('Mortalité jeunes');add(100,'Veaux / mortalité jeunes','Mode d’audit choisi','jeunes',['Colostrum : délai, quantité, qualité et traçabilité','Naissance, nombril, logement et hygiène','Diarrhées / respiratoire : âge de début, lot, chronologie']);add(90,'Sanitaire des veaux','Identifier les syndromes et leur chronologie','sanitaire',['Mortalité par âge et causes documentées','Vaccination / traitements / prélèvements','Isolement des malades et gestion des lots']);}
  if(t.mode==='reproduction'){tag('Reproduction');add(100,'Reproduction','Mode d’audit choisi','reproduction',['IVV, retours, diagnostics et périodes de reproduction','Âge au premier vêlage et croissance des génisses','Taureaux / IA, vaches vides et décisions de réforme']);add(85,'Alimentation liée à la reproduction','Rechercher les facteurs de conduite associés','fourrages',['État corporel et ingestion','Énergie/protéines/minéraux selon données disponibles','Transitions et accès à l’eau']);}
  if(/diarrh/.test(n)){tag('Diarrhée veaux');add(105,'Diarrhées des veaux','Problème explicitement signalé','jeunes',['Âge exact au début des signes','Colostrum, lait, eau et hygiène du matériel','Logement, mélange d’âges et nettoyage entre lots']);add(92,'Sanitaire ciblé diarrhées','Départager causes infectieuses, parasitaires et alimentaires','sanitaire',['Fièvre, sang, abattement, nombre atteints','Traitements déjà faits et réponse','Prélèvements à discuter avec le vétérinaire']);}
  if(/respir|pneum|toux|jetage|dyspn/.test(n)){tag('Respiratoire veaux');add(105,'Troubles respiratoires','Problème explicitement signalé','jeunes',['Âge exact, fièvre, toux, jetage, dyspnée','Ventilation au niveau des veaux et densité','Chronologie avec sevrage, allotement ou météo']);add(93,'Bâtiment / ambiance','Facteurs favorisants fréquents à vérifier','pratiques',['Ventilation, condensation, ammoniac','Courants d’air directs et humidité','Mélange d’âges et densité']);}
  if(/mortalite adulte|mortalité adulte|mort.*adulte/.test(n)){tag('Mortalité adulte');add(105,'Mortalité adulte','Problème explicitement signalé','sanitaire',['Âge/stade physiologique des morts','Chronologie, signes précurseurs, autopsies','Causes de réforme vs décès non programmés']);add(82,'Alimentation / métabolique','À croiser avec état corporel et stade physiologique','fourrages',['État corporel, ingestion et transition','Minéraux et ration si disponibles','Accès à l’eau']);}
  if(/etat corporel|état corporel|nec|maigr|amaig/.test(n)){tag('État corporel');add(100,'État corporel insuffisant','Problème explicitement signalé','fourrages',['NEC par lot et stade physiologique','Ingestion réelle, tri, accès auge/eau','Ration, fourrages, énergie/protéines/minéraux']);add(85,'Reproduction à croiser','L’état corporel peut accompagner des performances de reproduction dégradées','reproduction',['IVV et retours','Vaches vides / tardives','État des primipares et génisses']);}
  if(/ecartel|écartel|gliss|aplomb|boiter/.test(n)){tag('Locomotion / sols');add(100,'Locomotion / sécurité des sols','Problème locomoteur ou traumatique signalé','pratiques',['Sols glissants, pentes et zones de retournement','Aplombs / parage / lésions des membres','Couchage, circulation et compétition']);}
  if(/ivv|repro|vache vide|chaleur|gestation|fertil/.test(n)&&t.mode!=='reproduction'){tag('Reproduction');add(95,'Reproduction','Motif de visite compatible','reproduction',['IVV, retours et chaleurs','Diagnostics et conduite des vaches vides','Nutrition, état corporel et taureaux/IA']);}
  // Ajouter les alertes de pré-visite réellement calculables
  try{
    const farm=db.farms.find(f=>f.id===visit.farmId),x=farm?reproPreparationSnapshot(visit,farm):null;
    if(x){const rolling=reproRolling12Stats(visit,farm,x),hist=reproMortalityAnnualHistory(visit,farm,10),d=prepDecisionSupportData(visit,farm,x,hist,rolling);(d.alerts||[]).slice(0,4).forEach(a=>{const sec=/repro|renouvel/i.test(a.theme)?'reproduction':/mortal|veau|sanit/i.test(a.theme)?'sanitaire':/aliment/i.test(a.theme)?'fourrages':'pratiques';add(70+a.severity,a.theme,`Pré-visite : ${a.title}`,sec,[a.detail]);});}
  }catch(e){console.warn('Priorisation pré-visite indisponible',e)}
  if(t.mode==='complete'){add(20,'Audit complet','Toutes les rubriques restent à parcourir','sanitaire',['Le classement ci-dessous sert seulement à commencer par les points les plus utiles.']);}
  if(t.mode==='automatic'&&!priorities.length)add(30,'Audit ciblé automatique','Aucun motif précis ni alerte documentaire forte : commencer par les données terrain indispensables','sanitaire',['Préciser le motif de visite / problèmes rencontrés pour affiner automatiquement l’audit.']);
  const structured=t.calfSyndrome||{},structuredDays=symptomAgeDays(structured.age,structured.ageUnit),structuredGuide=structured.type&&structuredDays!=null?calfSyndromeGuidance(structured.type,structuredDays,[structured.notes,t.problemNarrative].join(' ')):null;
  const textGuide=narrativeSyndrome(t.problemNarrative||visit.objective||'');
  priorities.sort((a,b)=>b.rank-a.rank);
  return {mode:t.mode,tags,priorities:priorities.slice(0,8),syndrome:structuredGuide||textGuide};
}
function targetedAuditHtml(visit,{forPrint=false}={}){
  const t=ensureAuditTargeting(visit),p=targetedAuditPlan(visit),modeLabel=targetedAuditModes.find(x=>x[0]===t.mode)?.[1]||'Audit ciblé automatique';
  const syn=p.syndrome;
  if(forPrint){return `<section class="targeted-audit-print"><h2>Priorités de visite — audit adapté</h2><p><b>Mode :</b> ${escapeHtml(modeLabel)}${t.problemNarrative?`<br><b>Problèmes signalés :</b> ${escapeHtml(t.problemNarrative)}`:''}</p>${p.priorities.length?`<ol>${p.priorities.map(x=>`<li><b>${escapeHtml(x.title)}</b><br><span><b>Pourquoi cette priorité ?</b> ${escapeHtml(x.reason)}</span><br><small>${x.questions.map(q=>escapeHtml(q)).join(' · ')}</small></li>`).join('')}</ol>`:''}${syn?`<h3>Orientation syndromique veau — à confirmer sur le terrain</h3><p><b>Âge au début :</b> ≈ ${syn.ageDays} jours. <b>Ce bloc ne constitue pas un diagnostic.</b></p><p><b>Causes compatibles à explorer :</b> ${syn.causes.map(escapeHtml).join(' · ')}</p><p><b>Facteurs à vérifier :</b> ${syn.risk.map(escapeHtml).join(' · ')}</p>`:''}</section>`;}
  return `<section class="card targeted-audit-card"><div class="section-title"><div><h3>🎯 Orientation de l’audit après pré-visite</h3><div class="muted">À renseigner après les imports préparatoires. Le logiciel croise les données disponibles avec le motif signalé pour prioriser les questions, sans masquer le reste de l’audit. Les hypothèses pathologiques sont des pistes à explorer, jamais un diagnostic.</div></div><span class="badge">${escapeHtml(modeLabel)}</span></div>
  <div class="grid cols-2"><div class="field"><label>Mode d’audit</label><select data-target-audit="mode">${targetedAuditModes.map(([v,l])=>`<option value="${v}" ${t.mode===v?'selected':''}>${escapeHtml(l)}</option>`).join('')}</select></div><div class="field"><label>Motif / problèmes rencontrés</label><textarea data-target-audit="problemNarrative" placeholder="Ex. diarrhée sur veaux de 3 semaines ; mortalité adulte ; état corporel insuffisant…">${escapeHtml(t.problemNarrative||'')}</textarea></div></div>
  <details class="targeted-syndrome-editor" ${t.calfSyndrome?.type?'open':''}><summary><strong>🐮 Symptôme veau — préciser l’âge si utile</strong></summary><div class="grid cols-4"><div class="field"><label>Syndrome</label><select data-calf-target="type"><option value="">Aucun / texte libre</option><option value="diarrhea" ${t.calfSyndrome.type==='diarrhea'?'selected':''}>Diarrhée</option><option value="respiratory" ${t.calfSyndrome.type==='respiratory'?'selected':''}>Respiratoire</option></select></div><div class="field"><label>Âge au début</label><input inputmode="decimal" data-calf-target="age" value="${escapeHtml(t.calfSyndrome.age||'')}"></div><div class="field"><label>Unité</label><select data-calf-target="ageUnit">${['jours','semaines','mois'].map(v=>`<option ${t.calfSyndrome.ageUnit===v?'selected':''}>${v}</option>`).join('')}</select></div><div class="field"><label>Fièvre</label><select data-calf-target="fever">${['Inconnu','Non','Oui'].map(v=>`<option ${t.calfSyndrome.fever===v?'selected':''}>${v}</option>`).join('')}</select></div></div><div class="field"><label>Précisions</label><textarea data-calf-target="notes" placeholder="Ex. sang, abattement, lot entier, après changement de lait, toux après allotement…">${escapeHtml(t.calfSyndrome.notes||'')}</textarea></div></details>
  ${p.tags.length?`<div class="target-tags">${p.tags.map(x=>`<span class="badge">${escapeHtml(x)}</span>`).join('')}</div>`:''}
  <div class="target-priorities">${p.priorities.map((x,i)=>`<article><span>${i+1}</span><div><strong>${escapeHtml(x.title)}</strong><details class="priority-why"><summary>Pourquoi cette priorité ?</summary><p>${escapeHtml(x.reason)}</p></details><ul>${x.questions.map(q=>`<li>${escapeHtml(q)}</li>`).join('')}</ul>${x.section?`<button type="button" class="btn small secondary" data-target-open-section="${escapeHtml(x.section)}">Ouvrir cette rubrique</button>`:''}</div></article>`).join('')}</div>
  ${syn?`<div class="syndrome-guidance"><h4>🧭 Causes compatibles avec l’âge — à explorer</h4><p class="muted">Âge au début ≈ <b>${syn.ageDays} jours</b>. L’âge oriente les hypothèses mais ne permet pas de poser un diagnostic.</p><div class="grid cols-3"><article><strong>Causes possibles</strong><ul>${syn.causes.map(x=>`<li>${escapeHtml(x)}</li>`).join('')}</ul></article><article><strong>Facteurs à vérifier</strong><ul>${syn.risk.map(x=>`<li>${escapeHtml(x)}</li>`).join('')}</ul></article><article><strong>Questions / prélèvements</strong><ul>${syn.questions.map(x=>`<li>${escapeHtml(x)}</li>`).join('')}${syn.vet.map(x=>`<li>🧪 ${escapeHtml(x)}</li>`).join('')}</ul></article></div></div>`:''}</section>`;
}
function orderedAuditSectionsForVisit(visit){
  const plan=targetedAuditPlan(visit),ids=[];
  plan.priorities.forEach(p=>{if(p.section&&!ids.includes(p.section))ids.push(p.section)});
  const mode=ensureAuditTargeting(visit).mode;
  if(mode==='young-mortality')['jeunes','sanitaire','fourrages','pratiques','reproduction'].forEach(id=>{if(!ids.includes(id))ids.push(id)});
  if(mode==='reproduction')['reproduction','fourrages','sanitaire','pratiques','jeunes'].forEach(id=>{if(!ids.includes(id))ids.push(id)});
  return [...ids.map(id=>auditGlobalSections.find(s=>s.id===id)).filter(Boolean),...auditGlobalSections.filter(s=>!ids.includes(s.id))];
}
function bindTargetedAudit(visit){
  ensureAuditTargeting(visit);
  app.querySelectorAll('[data-target-audit]').forEach(el=>{el.addEventListener('change',()=>{visit.auditTargeting[el.dataset.targetAudit]=el.value;visit.updatedAt=new Date().toISOString();saveDatabase(db);renderAuditGlobal();});});
  app.querySelectorAll('[data-calf-target]').forEach(el=>{el.addEventListener('change',()=>{visit.auditTargeting.calfSyndrome[el.dataset.calfTarget]=el.value;visit.updatedAt=new Date().toISOString();saveDatabase(db);renderAuditGlobal();});});
  app.querySelectorAll('[data-target-open-section]').forEach(b=>b.onclick=()=>{const id=b.dataset.targetOpenSection,det=app.querySelector(`[data-audit-section-id="${CSS.escape(id)}"]`);if(det){det.open=true;det.scrollIntoView({behavior:'smooth',block:'start'});}});
}
...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Repères visuels, fiches pratiques et carnet technique, disponibles hors ligne.</div></div><span class="badge autosave">v14.6.21.123</span></div>
...
function migrateDatabase() {
  // Ancienne sauvegarde locale complète supprimée : elle doublait la taille de la base
  // et pouvait saturer le quota navigateur sur tablette. La sauvegarde de référence est le cloud.
  try { localStorage.removeItem('audit-bovin-v10-backup-before-10-7'); } catch (_) {}
...
window.addEventListener('audit-bovin-cloud-merged',async event=>{
  ...
  const noHotReload=new Set(['audit','visits','documents','building','analysis','animals','feeding','quicknotes','questionnaires','herddata','metabolic','parasitism','waterlab','labimports']);
...
function activeVisit() { const visits=Array.isArray(db?.visits)?db.visits:[]; return visits.find(v => v.id === activeVisitId) || null; }
...
function render() {
  ...
  const renderers = { dashboard: renderDashboard, farms: renderFarms, journal: renderJournalSuivi, documents: renderFarmDocuments, visits: renderVisits, questionnaires: renderFarmerQuestionnaires, prepprint: renderPreparationPrints, animals: renderAnimals, quicknotes: renderQuickFieldNotes, analysis: renderAnalysis, assistant: renderAssistantGDS, review: renderPeerReview, feeding: renderFeeding, nutrition: renderNutritionAnalysis, labimports: renderLabImports, reproduction: renderReproduction, building: renderBuilding, audit: renderAuditGlobal, planches: renderPlanches, photos: renderPhotos, herddata: renderHerdData, metabolic: renderMetabolic, parasitism: renderParasitism, waterlab: renderWaterLab, references: renderReferenceSettings, followup: renderFollowup, checkout: renderEndVisitCheckup, study: renderStudyTracking, economy: renderEconomicProgress, pilotage: renderPilotageActions, reports: renderReports, backup: renderBackup };
  app.innerHTML = '';
  try {
...
function farmDossierPayload(farmId){
  const farm=db.farms.find(f=>f.id===farmId);
  if(!farm)return null;
  const clone=x=>JSON.parse(JSON.stringify(x));
  return {
    format:'audit-bovin-farm-dossier',
    schemaVersion:1,
    appVersion:'14.6.21.123',
    exportedAt:new Date().toISOString(),
    farm:clone(farm),
    visits:clone(db.visits.filter(v=>v.farmId===farmId)),
    herdImports:clone((db.herdImports||[]).filter(x=>x.farmId===farmId))
  };
}
function safeArchiveName(value='document'){
  return String(value||'document').normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-zA-Z0-9._-]+/g,'-').replace(/^-+|-+$/g,'').slice(0,120)||'document';
}
function archiveWordBlob(title,body,styles=''){
  const html=`<!doctype html><html lang="fr"><head><meta charset="utf-8"><title>${escapeHtml(title)}</title><style>${styles||fullReportStyles()}</style></head><body>${body}</body></html>`;
  return new Blob(['\ufeff',html],{type:'application/msword'});
}
function archiveStructuredTextFromHtml(html=''){
  const holder=document.createElement('div');
  holder.setAttribute('aria-hidden','true');
  holder.style.cssText='position:fixed;left:-100000px;top:0;width:900px;visibility:hidden;white-space:normal';
  holder.innerHTML=String(html||'');
  document.body.appendChild(holder);
  let text='';
  try{text=holder.innerText||holder.textContent||'';}finally{holder.remove();}
  return String(text||'').replace(/\t+/g,' | ').replace(/[ \u00a0]+\n/g,'\n').replace(/\n{3,}/g,'\n\n').trim();
}
function archivePdfWinAnsi(value=''){
  ...
  let out='';
  for(const ch of String(value||'')){const cp=ch.codePointAt(0);let b=cp<=255?cp:map[ch];if(b===undefined)b=63;out+=String.fromCharCode(b);}
  return out;
}
function archivePdfEscape(value=''){return archivePdfWinAnsi(value).replace(/\\/g,'\\\\').replace(/\(/g,'\\(').replace(/\)/g,'\\)').replace(/[\r\n]+/g,' ');}
function archiveWrapText(text,maxChars=104){
  const out=[];
  String(text||'').split(/\n/).forEach(raw=>{
    const line=raw.trimEnd();if(!line.trim()){out.push('');return;}
    const words=line.replace(/\s+/g,' ').trim().split(' ');let cur='';
    for(const word of words){if(!cur){cur=word;continue;}if((cur+' '+word).length<=maxChars){cur+=' '+word;continue;}out.push(cur);cur=word;while(cur.length>maxChars){out.push(cur.slice(0,maxChars));cur=cur.slice(maxChars);}}
    if(cur)out.push(cur);
  });
  return out;
}
function archiveTextPdfBlob(title,html){
  const raw=[title,'',archiveStructuredTextFromHtml(html)].join('\n'),lines=archiveWrapText(raw,104),linesPerPage=60,pages=[];
  for(let i=0;i<lines.length;i+=linesPerPage)pages.push(lines.slice(i,i+linesPerPage));if(!pages.length)pages.push([title]);
  const objects=[],pageIds=pages.map((_,i)=>4+i*2),contentIds=pages.map((_,i)=>5+i*2);
  objects[1]='<< /Type /Catalog /Pages 2 0 R >>';objects[2]=`<< /Type /Pages /Kids [${pageIds.map(id=>`${id} 0 R`).join(' ')}] /Count ${pages.length} >>`;objects[3]='<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica /Encoding /WinAnsiEncoding >>';
  pages.forEach((page,idx)=>{const pid=pageIds[idx],cid=contentIds[idx];objects[pid]=`<< /Type /Page /Parent 2 0 R /MediaBox [0 0 595 842] /Resources << /Font << /F1 3 0 R >> >> /Contents ${cid} 0 R >>`;let stream='BT\n/F1 9 Tf\n',y=804;for(const line of page){stream+=`1 0 0 1 38 ${y} Tm (${archivePdfEscape(line)}) Tj\n`;y-=12;}stream+='ET\n';const bin=archivePdfWinAnsi(stream);objects[cid]=`<< /Length ${bin.length} >>\nstream\n${bin}endstream`;});
  let pdf='%PDF-1.4\n%\xE2\xE3\xCF\xD3\n',offsets=[0];
  for(let i=1;i<objects.length;i++){if(objects[i]===undefined)continue;offsets[i]=pdf.length;pdf+=`${i} 0 obj\n${objects[i]}\nendobj\n`;}
  const xref=pdf.length,maxObj=objects.length-1;pdf+=`xref\n0 ${maxObj+1}\n0000000000 65535 f \n`;for(let i=1;i<=maxObj;i++)pdf+=`${String(offsets[i]||0).padStart(10,'0')} 00000 n \n`;pdf+=`trailer\n<< /Size ${maxObj+1} /Root 1 0 R >>\nstartxref\n${xref}\n%%EOF`;
...
function archiveCompleteReportOptions(){return {rawMeasurements:true,observations:true,audit:true,reproduction:true,reproductionDetails:true,feeding:true,building:true,herddata:true,metabolic:true,parasitism:true,analysis:true,reasoning:true,photos:true};}
function capturePreparationBodyForArchive(visit){
  if(!visit)return '';const previousVisitId=activeVisitId,previousPaperWindow=paperWindow;let captured='';
  try{activeVisitId=visit.id;paperWindow=(title,body)=>{captured=body||'';};printReproMortalityPrep();}
  catch(err){console.warn('Génération bilan préparatoire pour archive',err);}
  finally{paperWindow=previousPaperWindow;activeVisitId=previousVisitId;}
  return captured||`<h1>Bilan technique préparatoire</h1>${activeFarmNameForPrint(visit)}<p>Bilan non disponible pour cette visite.</p>`;
}
function archiveReportFolderName(visit,index=0){const date=String(visit?.date||'sans-date').replace(/[^0-9-]/g,'-');return `rapports/${date}-${safeArchiveName(visit?.id||String(index+1))}`;}
function addSystematicVisitReportsToZip(zip,visit,index=0){
  const folder=archiveReportFolderName(visit,index),farm=safeArchiveName(farmName(visit.farmId)||'exploitation'),prepBody=capturePreparationBodyForArchive(visit),prepTitle=`Bilan technique préparatoire — ${farmName(visit.farmId)} — ${visit.date||''}`;
  const prepStyles=printBaseStyles()+'.print-metrics{display:grid;grid-template-columns:repeat(4,1fr);gap:8px}.print-metric{border:1px solid #ddd;border-radius:8px;padding:8px;text-align:center}.print-metric strong{display:block;font-size:18px;color:#b53670}.herd-chart-grid{display:grid;grid-template-columns:1fr;gap:12px}.herd-chart-card{border:1px solid #cfdad3;border-radius:10px;padding:10px;margin:7px 0}.herd-svg-chart{width:100%;height:auto}.herd-chart-legend{display:flex;gap:10px;flex-wrap:wrap}.notes{border:1px solid #bbb;min-height:90px;padding:8px}';
  zip.file(`${folder}/bilan-preparatoire-${farm}.doc`,archiveWordBlob(prepTitle,prepBody,prepStyles));zip.file(`${folder}/bilan-preparatoire-${farm}.pdf`,archiveTextPdfBlob(prepTitle,prepBody));
  const reportBody=reportDocumentHtml(visit,'expert',archiveCompleteReportOptions()),reportTitle=`Rapport complet — ${farmName(visit.farmId)} — ${visit.date||''}`;
  zip.file(`${folder}/rapport-complet-${farm}.doc`,archiveWordBlob(reportTitle,reportBody,fullReportStyles()));zip.file(`${folder}/rapport-complet-${farm}.pdf`,archiveTextPdfBlob(reportTitle,reportBody));
}

async function exportFarmDossier(farmId){
  const payload=farmDossierPayload(farmId),farm=payload?.farm;
  if(!payload||!farm)return showToast('Exploitation introuvable.');
  const Zip=window.JSZip;
  if(!Zip){downloadJson(`dossier-${slugify(farm.name||'exploitation')}.json`,payload);return showToast('Dossier JSON exporté (ZIP indisponible).');}
  const zip=new Zip(),docs=Array.isArray(payload.farm.documents)?payload.farm.documents:[];
  let included=0,missing=0;
  for(const doc of docs){
    let blob=null;
    try{if(doc.blobKey)blob=await getFarmDocumentBlob(doc.blobKey);if(!blob&&doc.dataUrl)blob=dataUrlToBlob(doc.dataUrl);}catch(e){console.warn('Lecture pièce jointe pour export',e);}
    if(blob){
      const ext=String(doc.fileName||'').match(/(\.[a-z0-9]{1,8})$/i)?.[1]||'';
      const base=safeArchiveName(String(doc.fileName||doc.title||'document').replace(/\.[a-z0-9]{1,8}$/i,''));
      const archivePath=`documents/${safeArchiveName(doc.id)}-${base}${ext}`;
      zip.file(archivePath,blob);
      doc.archivePath=archivePath;doc.archiveMissing=false;included++;
    }else{doc.archiveMissing=true;missing++;}
  }
  payload.visits.forEach((visit,index)=>addSystematicVisitReportsToZip(zip,visit,index));
  zip.file('dossier.json',JSON.stringify(payload,null,2));
  zip.file('A_LIRE.txt',`Dossier Audit Bovin — ${farm.name||''}\nExport : ${new Date().toLocaleString('fr-FR')}\nVisites : ${payload.visits.length}\nPièces jointes incluses : ${included}\nPièces jointes absentes de cet appareil : ${missing}\n\nLe dossier « rapports » contient systématiquement, pour chaque visite :\n- le bilan technique préparatoire en PDF + Word ;\n- un rapport complet en PDF + Word, avec toutes les rubriques disponibles.\n\nLe PDF automatique d’archive privilégie la conservation de toutes les informations textuelles. Le Word conserve la mise en forme riche, les tableaux, graphiques et photos intégrées.\n\nPour restaurer ce dossier, utilisez « Importer un dossier éleveur » dans l’onglet Exploitations.`);
  const blob=await zip.generateAsync({type:'blob',compression:'DEFLATE',compressionOptions:{level:6}});
  const url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`dossier-eleveur-${slugify(farm.name||'exploitation')}-${new Date().toISOString().slice(0,10)}.zip`;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),5000);
  showToast(`Dossier exporté : ${payload.visits.length} visite(s), bilan préparatoire + rapport complet PDF/Word inclus, ${included} pièce(s) jointe(s)${missing?`, ${missing} absente(s) de cet appareil`:''}.`);
}
function findExistingFarmForDossier(farm){
  if(!farm)return null;
  const number=normalizeHerdNumber(farm.farmNumber||farm.herdNumber||farm.ede||'');
  return db.farms.find(f=>f.id===farm.id)
    ||(number?db.farms.find(f=>normalizeHerdNumber(f.farmNumber||f.herdNumber||f.ede||'')===number):null)
    ||db.farms.find(f=>normalizeSearchText(f.name||'')===normalizeSearchText(farm.name||''));
}
async function importFarmDossierPayload(payload,zip=null){
  if(!payload||payload.format!=='audit-bovin-farm-dossier'||!payload.farm)throw new Error('Dossier éleveur non reconnu');
  const incoming=JSON.parse(JSON.stringify(payload.farm));
  const existing=findExistingFarmForDossier(incoming);
  if(existing&&!confirm(`Un dossier existe déjà pour « ${existing.name} ».\n\nLe remplacer par le dossier importé ? Les autres exploitations ne seront pas modifiées.`))return false;
  let targetFarmId=existing?.id||incoming.id||uid('farm');
  if(!existing&&db.farms.some(f=>f.id===targetFarmId))targetFarmId=uid('farm');
  const oldDocs=existing?(existing.documents||[]):[];
  if(existing){
    const i=db.farms.findIndex(f=>f.id===existing.id);
    db.farms[i]={...existing,...incoming,id:targetFarmId,updatedAt:new Date().toISOString()};
    db.visits=db.visits.filter(v=>v.farmId!==targetFarmId);
    db.herdImports=(db.herdImports||[]).filter(x=>x.farmId!==targetFarmId);
  }else db.farms.push({...incoming,id:targetFarmId,updatedAt:new Date().toISOString()});
  const usedVisitIds=new Set(db.visits.map(v=>v.id));
  const importedVisitIds=[];
  for(const raw of Array.isArray(payload.visits)?payload.visits:[]){
    const v=JSON.parse(JSON.stringify(raw));
    if(!v.id||usedVisitIds.has(v.id))v.id=uid('visit');
    usedVisitIds.add(v.id);v.farmId=targetFarmId;v.subjects=Array.isArray(v.subjects)?v.subjects:[];db.visits.push(v);importedVisitIds.push(v.id);
  }
  const usedImportIds=new Set((db.herdImports||[]).map(x=>x.id));
  for(const raw of Array.isArray(payload.herdImports)?payload.herdImports:[]){
    const x=JSON.parse(JSON.stringify(raw));if(!x.id||usedImportIds.has(x.id))x.id=uid('herdimport');usedImportIds.add(x.id);x.farmId=targetFarmId;db.herdImports.push(x);
  }
  const farm=db.farms.find(f=>f.id===targetFarmId);farm.documents=Array.isArray(farm.documents)?farm.documents:[];
  let restored=0,missing=0;
  if(zip){
    for(const doc of farm.documents){
      const archivePath=doc.archivePath,entry=archivePath?zip.file(archivePath):null;
      const newBlobKey=`${targetFarmId}:${doc.id||uid('document')}`;
      if(entry){try{const blob=await entry.async('blob');await putFarmDocumentBlob(newBlobKey,blob);doc.blobKey=newBlobKey;delete doc.archiveMissing;restored++;}catch(e){console.warn('Restauration pièce jointe',e);missing++;}}
      else if(doc.archiveMissing){missing++;doc.blobKey='';}
      delete doc.archivePath;
    }
  }else{
    farm.documents.forEach(doc=>{delete doc.archivePath;});
  }
  if(existing){
    const newKeys=new Set(farm.documents.map(d=>d.blobKey).filter(Boolean));
    for(const doc of oldDocs){if(doc?.blobKey&&!newKeys.has(doc.blobKey))await deleteFarmDocumentBlob(doc.blobKey);}
  }
  db.updatedAt=new Date().toISOString();
  await saveDatabase(db);
  if(importedVisitIds.length)setActiveVisit(importedVisitIds[0]);
  showToast(`Dossier « ${farm.name} » importé : ${importedVisitIds.length} visite(s)${zip?`, ${restored} pièce(s) jointe(s) restaurée(s)`:''}${missing?`, ${missing} manquante(s)`:''}.`);
  return true;
}
async function importFarmDossierFile(file){
  const isZip=/\.zip$/i.test(file.name)||/zip/i.test(file.type||'');
  if(isZip){
    const Zip=window.JSZip;if(!Zip)throw new Error('Lecture ZIP indisponible');
    const zip=await Zip.loadAsync(file),entry=zip.file('dossier.json');if(!entry)throw new Error('dossier.json absent du ZIP');
    const payload=JSON.parse(await entry.async('text'));return importFarmDossierPayload(payload,zip);
  }
  const parsed=JSON.parse(await file.text());return importFarmDossierPayload(parsed,null);
}
...
function renderFarms() {
  ...
  app.innerHTML = `
    <div class="section-title"><div><h2>Exploitations</h2><span class="muted">${db.farms.length} exploitation(s)</span></div><div class="actions"><button class="btn secondary" id="import-farm-dossier">📥 Importer un dossier éleveur</button></div></div>
    <section class="grid cols-2">
      ...
      <section class="card">
        ...
        ${db.farms.length ? `<div class="table-wrap"><table><thead><tr><th>Exploitation</th><th>Commune</th><th>Visites</th><th></th></tr></thead><tbody>${db.farms.map(f => `<tr><td><strong>${escapeHtml(f.name)}</strong><br><span class="muted">${escapeHtml(f.farmer || '')}${f.farmNumber?` · EDE ${escapeHtml(f.farmNumber)}`:''}</span></td><td>${escapeHtml(f.commune || '—')}</td><td>${db.visits.filter(v => v.farmId === f.id).length}</td><td><div class="actions"><button class="btn small" data-open-farm-journal="${f.id}">Journal</button><button class="btn small" data-open-farm-documents="${f.id}">Documents</button><button class="btn small" data-set-farm-number="${f.id}">N° EDE</button><button class="btn small secondary" data-export-farm-dossier="${f.id}" title="Inclut données + bilan préparatoire + rapport complet PDF/Word">💾 Exporter dossier</button><button class="btn small danger" data-delete-farm="${f.id}">Supprimer</button></div></td></tr>`).join('')}</tbody></table></div>` : '<div class="empty">Aucune exploitation.</div>'}
      </section>
    </section>`;
  ...
  document.getElementById('import-farm-dossier')?.addEventListener('click',()=>{fileInput.accept='.zip,.json,application/zip,application/json';fileInput.dataset.importMode='farm-dossier';fileInput.click();});
  app.querySelectorAll('[data-export-farm-dossier]').forEach(button=>button.onclick=()=>exportFarmDossier(button.dataset.exportFarmDossier));
...
// Les pièces jointes ne doivent pas être placées dans localStorage : un PDF de quelques Mo
// dépasse très vite son quota (le base64 ajoute ~33 %). On conserve donc les octets dans
// IndexedDB et seulement les métadonnées dans la base principale / synchronisée.
const FARM_DOC_BLOB_DB='audit-bovin-document-blobs-v1',FARM_DOC_BLOB_STORE='files';
function openFarmDocumentBlobDb(){return new Promise((resolve,reject)=>{const req=indexedDB.open(FARM_DOC_BLOB_DB,1);req.onupgradeneeded=()=>{const d=req.result;if(!d.objectStoreNames.contains(FARM_DOC_BLOB_STORE))d.createObjectStore(FARM_DOC_BLOB_STORE);};req.onsuccess=()=>resolve(req.result);req.onerror=()=>reject(req.error);});}
async function putFarmDocumentBlob(key,blob){const d=await openFarmDocumentBlobDb();return new Promise((resolve,reject)=>{const tx=d.transaction(FARM_DOC_BLOB_STORE,'readwrite');tx.objectStore(FARM_DOC_BLOB_STORE).put(blob,key);tx.oncomplete=()=>{d.close();resolve();};tx.onerror=()=>{const e=tx.error;d.close();reject(e);};});}
async function getFarmDocumentBlob(key){const d=await openFarmDocumentBlobDb();return new Promise((resolve,reject)=>{const tx=d.transaction(FARM_DOC_BLOB_STORE,'readonly'),r=tx.objectStore(FARM_DOC_BLOB_STORE).get(key);r.onsuccess=()=>{d.close();resolve(r.result||null);};r.onerror=()=>{const e=r.error;d.close();reject(e);};});}
async function deleteFarmDocumentBlob(key){if(!key)return;try{const d=await openFarmDocumentBlobDb();await new Promise((resolve,reject)=>{const tx=d.transaction(FARM_DOC_BLOB_STORE,'readwrite');tx.objectStore(FARM_DOC_BLOB_STORE).delete(key);tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error);});d.close();}catch(e){console.warn('Suppression pièce jointe locale',e);}}
async function downloadStoredDocument(docu){
  if(!docu)return;
  try{let blob=null;if(docu.blobKey)blob=await getFarmDocumentBlob(docu.blobKey);if(!blob&&docu.dataUrl)blob=dataUrlToBlob(docu.dataUrl);if(!blob)return showToast('La pièce jointe n’est pas disponible sur cet appareil. Les métadonnées du document sont bien enregistrées.');const url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=docu.fileName||docu.title||'document';a.target='_blank';document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),10000);}catch(err){console.error(err);showToast('Impossible d’ouvrir ce document. Réimportez-le depuis le fichier d’origine.');}
}
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.6.21.123</span></div>
...
function renderVisits() {
  ...
  app.innerHTML = `
    <div class="section-title"><div><h2>Visites</h2><div class="muted">Toutes les visites enregistrées restent accessibles, y compris les plus anciennes.</div></div><span class="badge autosave">v14.6.21.123</span></div>
...
function buildFarmerQuestionnaireSchema(visit,type,selectedIds,technicianNote=''){
  ...
  auditGlobalSections.forEach(sec=>{if(!wanted.has(sec.id))return;let qs=sec.questions.map(farmerAuditQuestionSchema);if(remaining)qs=qs.filter(q=>!answerAlreadyKnown(visit,q));if(qs.length)sections.push({id:sec.id,title:`${sec.icon} ${sec.title}`,questions:qs})});
...
function renderFarmerQuestionnaires(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📨 Questionnaires éleveur</h2><div class="muted">Préparer la visite puis récupérer uniquement les réponses encore manquantes.</div></div><span class="badge autosave">v14.6.21.123</span></div>${activeVisitBanner(visit)}
...
function paperWindow(title,body){
  ...
  w.document.write(`<!doctype html><html lang="fr"><head><meta charset="utf-8"><title>${escapeHtml(title)}</title><style>@page{size:A4 portrait;margin:12mm 10mm 14mm 10mm}body{font-family:Arial,sans-serif;color:#222;padding:0;max-width:none;margin:0;font-size:11.7pt;line-height:1.42}h1{color:#b53670;break-after:avoid-page}h2{border-bottom:2px solid #ddd;padding-bottom:5px;margin-top:22px;break-after:avoid-page}h3,h4,h5{break-after:avoid-page}p,li{orphans:3;widows:3}.toolbar{position:sticky;top:0;background:#fff;padding:8px 0;border-bottom:1px solid #ddd}.toolbar button{margin-right:8px}.q{padding:8px 0;border-bottom:1px solid #eee}.known{background:#eef8f0;padding:4px 7px;border-radius:6px}.line{height:26px;border-bottom:1px solid #aaa}.source{font-size:11px;font-weight:bold;padding:2px 6px;border-radius:10px;margin-right:5px}.ask{background:#e8f1ff}.measure{background:#fff0d5}.later{background:#f2e7ff}.calc{background:#e5f7e8}.print-metrics{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin:10px 0 16px}.print-metric{border:1px solid #ddd;border-radius:8px;padding:8px;text-align:center;break-inside:avoid}.print-metric strong{display:block;font-size:18px;color:#b53670}.print-metric span{display:block;font-weight:bold;font-size:9pt;margin-top:3px}.print-metric small{display:block;color:#666;margin-top:2px}.period-alert{border-left:4px solid #d36b2c;background:#fff7ed;padding:8px 10px;margin:7px 0;break-inside:avoid}.notes{border:1px solid #bbb;min-height:110px;padding:8px;margin-top:12px}.herd-chart-grid{display:grid;grid-template-columns:1fr;gap:12px;margin:14px 0}.herd-chart-card{border:1px solid #cfdad3;border-radius:10px;padding:12px;margin:8px 0;background:#fffafd;break-inside:avoid}.herd-svg-chart{width:100%;height:auto;display:block}.herd-chart-legend{display:flex;gap:12px;flex-wrap:wrap;margin:6px 0 10px}.herd-chart-legend span{display:inline-flex;align-items:center;gap:6px;font-size:9.8pt;color:#425047}.legend-square{font-size:15pt;line-height:1}.herd-tech-origin{background:#f6faf7;border:1px solid #d7e4dc;border-radius:8px;padding:9px 11px;margin:10px 0 14px}.herd-tech-origin p{margin:3px 0}.muted{color:#66756c}.card{break-inside:auto}table{width:100%;border-collapse:collapse;font-size:10.3pt}th,td{border:1px solid #ccc;padding:5px;text-align:left}.annual-history-table{table-layout:fixed;font-size:8.5pt}.annual-history-table th,.annual-history-table td{padding:3px;text-align:center}.annual-history-table th:first-child{width:18%;text-align:left}.annual-history-table td.annual-trend-good{color:#15803d!important;font-weight:800!important;background:#eefbf2!important;-webkit-print-color-adjust:exact;print-color-adjust:exact}.annual-history-table td.annual-trend-bad{color:#b91c1c!important;font-weight:800!important;background:#fff0f0!important;-webkit-print-color-adjust:exact;print-color-adjust:exact}.print-annual-history{break-inside:auto}.print-annual-history h5{margin:10px 0 4px}.repro-age-donut-svg{width:100%;max-width:285px!important;height:auto;overflow:visible}.repro-age-overview.grouped{grid-template-columns:minmax(210px,285px) 1fr!important;gap:12px!important}.herd-chart-card{padding:10px!important;margin:7px 0!important;width:100%!important;box-sizing:border-box}.herd-svg-chart{width:100%!important;max-width:none!important;height:auto!important;display:block;margin:2px 0 0}.herd-chart-legend{font-size:9.5pt;gap:10px!important;margin:5px 0 7px!important}.registry-10y-charts{width:100%!important;max-width:none!important;margin-left:0!important;margin-right:0!important}.registry-10y-charts .herd-chart-card{width:100%!important;max-width:none!important}.registry-10y-charts .herd-svg-chart{width:100%!important;max-width:none!important}.decision-support{margin:12px 0;padding:12px;border:1px solid #ead5df;border-radius:10px}.decision-theme-grid{display:grid;grid-template-columns:repeat(5,1fr);gap:5px}.decision-theme{border:1px solid #ccc;border-left-width:4px;border-radius:6px;padding:6px;break-inside:avoid}.decision-theme.good{border-left-color:#2f8a50;background:#eff9f2}.decision-theme.warn{border-left-color:#d59620;background:#fff8e7}.decision-theme.bad{border-left-color:#c44848;background:#fff0f0}.decision-theme strong,.decision-theme span,.decision-theme small{display:block}.decision-alert{padding:6px;border-bottom:1px solid #ddd}.decision-alert small{display:block}.decision-comparison{margin:10px 0}.decision-trend-good{color:#176238;background:#e5f5ea;font-weight:800}.decision-trend-bad{color:#a01818;background:#fde4e1;font-weight:800}.decision-trend-neutral{color:#5d6570}.decision-onepage{border:2px solid #ead5df;border-radius:8px;padding:8px;margin:10px 0;break-inside:auto}.decision-onepage .grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}.decision-topic-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px}.decision-topic-grid article{border:1px solid #ddd;padding:7px;border-radius:6px}.decision-hypotheses article{break-inside:avoid}.decision-followup,.decision-movement-history{margin-top:12px}.decision-support.for-print>h2{margin-top:0}@media print{.toolbar{display:none}.smart-page-before{break-before:page!important;page-break-before:always!important}.q{break-inside:avoid}h1,h2,h3,h4,h5{break-after:avoid-page!important;page-break-after:avoid!important;margin-bottom:8px}h1+*,h2+*,h3+*,h4+*,h5+*{break-before:avoid-page!important;page-break-before:avoid!important}p{orphans:4;widows:4}li{orphans:3;widows:3;break-inside:avoid-page;page-break-inside:avoid}thead{display:table-header-group}tr{break-inside:avoid-page;page-break-inside:avoid}.annual-history-table td.annual-trend-good{color:#15803d!important;background:#eefbf2!important}.annual-history-table td.annual-trend-bad{color:#b91c1c!important;background:#fff0f0!important}}</style></head><body><div class="toolbar"><button onclick="window.print()">🖨️ Imprimer / PDF</button><button onclick="window.close()">↩ Fermer / retour audit</button></div>${body}${smartPrintPaginationScript()}</body></html>`);w.document.close();try{w.focus()}catch(e){};
}
...
function printReproMortalityPrep(){
  ...
  const f=db.farms.find(x=>x.id===v.farmId),x=reproPreparationSnapshot(v,f);
  if(!x){paperWindow('Bilan technique préparatoire',`<h1>Bilan technique préparatoire</h1>${activeFarmNameForPrint(v)}<div class="notice warning">Aucun registre bovins avec mouvements n’est disponible. Le bilan technique préparatoire repose désormais sur ce registre unique : importez le fichier contenant tous les animaux pour générer les synthèses, ratios, graphiques et listes individuelles.</div>`);return;}
  ...
  const structure=`<h2>1. Structure du troupeau à la date de visite</h2><div class="print-metrics">${metric('Animaux présents',x.present.length)}${metric('Femelles >24 mois',x.females24.length)}${metric('Femelles >36 mois',x.females36.length)}${metric('Mâles >24 mois',x.males24.length)}${metric('Mâles >36 mois',x.males36.length)}${metric('Femelles >36 mois sans vêlage',x.heifers36.length,'alerte prioritaire')}${metric('Naissances sur 12 mois',x.births.length,`${formatDate(x.startIso)} → ${formatDate(x.date)}`)}</div>`;
  const ageStructure=`<h2>2. Répartition des femelles par âge</h2>${reproAgeDistributionHtml(x.females24,x.date,{interactive:false,wrap:false,showBucketLegend:false,detailTargetId:'print-prep-age-year-detail'})}`;
  const rolling=reproRolling12Stats(v,f,x),annualNative=reproMortalityAnnualHistory(v,f,10);
  const decisionSupport=prepDecisionSupportHtml(v,f,x,annualNative,rolling,{forPrint:true});
  const repro=`<h2>3. Reproduction — 12 derniers mois</h2><p class="muted"><i>Période : ${formatDate(rolling.start)} → ${formatDate(rolling.end)}. Les IVV sont rattachés à la date du vêlage qui termine l’intervalle.</i></p><div class="print-metrics">${metric('Vaches ayant vêlé',rolling.dams)}${metric(`IVV moyen sur ${rolling.ivvCowCount??rolling.intervals.length} femelle(s)`,rolling.ivvMean==null?'—':rolling.ivvMean+' j')}${metric('IVV ≤400 j',rolling.ivvLe400)}${metric('IVV 401–450 j',rolling.ivv401to450)}${metric('IVV 451–500 j',rolling.ivv451to500)}${metric('IVV >500 j',rolling.ivvOver500)}${metric('Âge moyen au 1er vêlage',rolling.firstCalvingMean==null?'—':rolling.firstCalvingMean+' mois')}${metric('Vaches à problème actuelles',x.cowProblems.length)}</div>`;
  const currentAnnual=annualNative.at(-1),currentYear=currentAnnual?.year||Number(x.date.slice(0,4));
  const mortality=`<h2>4. Mortalité — année civile ${currentYear}${currentAnnual?.truncated?' (tronquée)':''}</h2><p class="muted"><i>Période : 01/01/${currentYear} → ${formatDate(currentAnnual?.periodEnd||x.date)}.${currentAnnual?.truncated?' Année en cours : résultats provisoires/tronqués.':''} Le taux veaux &lt;6 mois rapporte les décès avant 6 mois aux veaux nés sur cette année civile ; les veaux les plus récents n’ont pas encore tous 6 mois de recul.</i></p><div class="print-metrics">${metric('Veaux nés / suivis',currentAnnual?.births??'—')}${metric('Veaux morts <6 mois',currentAnnual?.dead6??'—')}${metric('Mortalité veaux <6 mois',currentAnnual?.mortalityRate==null?'—':currentAnnual.mortalityRate+' %')}${metric('Décès tous âges',currentAnnual?.deathsTotal??'—','sur l’année civile')}</div>${annualHistoryTablesHtml(annualNative,{forPrint:true})}`;
  const technicalCharts=`<h2>5. Graphiques issus du registre bovins — historique 10 ans</h2><p class="muted"><b>Source unique :</b> fichier contenant tous les animaux et leurs mouvements. Les périodes sont des années civiles ; l’année en cours est conservée telle quelle et explicitement signalée comme tronquée.</p>${prepRegistryTenYearChartsHtml(v,f,annualNative,{forPrint:true})}`;
  const paternity=`<h2>6. Paternité — uniquement lorsqu’elle est renseignée</h2><p><b>Couverture de la paternité :</b> ${paternityAll==null?'—':paternityAll+' %'} sur l’historique complet · ${paternity12==null?'—':paternity12+' %'} sur les 12 derniers mois. L’absence de paternité n’est pas interprétée comme une anomalie.</p>${prepReproSireTableHtml(x.sires,'La part des mères attribuées n’est pas un taux de saillies : les saillies sans naissance ne sont pas connues dans le registre.')}<p><b>Taureaux/pères IA ou jamais présents dans le registre :</b> ${x.aiSireCount}. Ils sont comptés mais non détaillés.</p>`;
  const problemRegistry=(reproductionSourceForVisit(v,f)?.registry||[]),problemById=new Map(problemRegistry.map(a=>[normalizeAnimalId(a.id),a]));
  const problemMotherCounts=new Map(),problemFatherCounts=new Map();
  x.cowProblems.forEach(r=>{const m=normalizeAnimalId(r.cow?.motherId),p=normalizeAnimalId(r.cow?.fatherId);if(m)problemMotherCounts.set(m,(problemMotherCounts.get(m)||0)+1);if(p)problemFatherCounts.set(p,(problemFatherCounts.get(p)||0)+1);});
  const problemParentLabel=(id,counts)=>{const k=normalizeAnimalId(id);if(!k)return '—';const a=problemById.get(k),n=counts.get(k)||0,label=a?`${escapeHtml(a.workNumber||a.id)}${a.name?` · ${escapeHtml(a.name)}`:''}<br><small>${escapeHtml(a.id||k)}</small>`:escapeHtml(id);return n>1?`<strong>${label}</strong><br><small><b>×${n} dans la liste</b></small>`:label;};
  const problems=`<h2>7. Vaches à problèmes</h2><p class="muted"><i>Situation à la date de visite : ${formatDate(x.date)}. Jours depuis vêlage : orange &gt;400 j, rouge &gt;450 j. Les parents qui reviennent chez plusieurs vaches de cette liste sont mis en gras, avec le nombre d’occurrences.</i></p><table><thead><tr><th>N° travail / animal</th><th>Score</th><th>Pourquoi ce score ?</th><th>Âge</th><th>Dernier vêlage</th><th>Jours depuis vêlage</th><th>Rang de vêlage</th><th>Mère</th><th>Père</th><th>IVV moyen</th><th>IVV complet</th><th>1er vêlage</th><th>Veaux morts &lt;6m</th></tr></thead><tbody>${x.cowProblems.map(r=>{const sd=reproductionScoreDetails(r),why=sd.lines.filter(z=>z.delta<0).map(z=>`${escapeHtml(z.label)} (${z.delta} pt${z.delta<-1?'s':''})`).join(' · ')||'Aucune pénalité calculée',flags=reproductionPenaltyFlags(r),scoreClass=sd.score>=75?'report-score-good':sd.score>=60?'report-score-warning':'report-score-danger',scoreStyle=sd.score>=75?'color:#176238;font-weight:800;background:#dff3e6':sd.score>=60?'color:#9a6700;font-weight:800;background:#fff0c7':'color:#a01818;font-weight:800;background:#ffd9d6',penaltyStyle=(warn,severe)=>warn?(severe?'color:#a01818;font-weight:800;background:#ffd9d6':'color:#9a6700;font-weight:800;background:#fff1c7'):'',daysStyle=(r.daysSinceLast||0)>450?'color:#a01818;font-weight:800;background:#ffd9d6':(r.daysSinceLast||0)>400?'color:#9a6700;font-weight:800;background:#fff0c7':'';return `<tr><td><b>${escapeHtml(r.cow.workNumber||r.cow.id)}</b>${r.cow.name?` · ${escapeHtml(r.cow.name)}`:''}<br><small>${escapeHtml(r.cow.id||'')}</small></td><td class="${scoreClass}" style="${scoreStyle}">${sd.score}/100</td><td>${why}</td><td>${escapeHtml(ageLabelAt(r.cow.birthDate,x.date)||'—')}</td><td>${r.lastCalvingDate?formatDate(r.lastCalvingDate):'—'}</td><td style="${daysStyle}">${r.daysSinceLast??'—'}</td><td><b>${(r.calves||[]).length}</b></td><td>${problemParentLabel(r.cow.motherId,problemMotherCounts)}</td><td>${problemParentLabel(r.cow.fatherId,problemFatherCounts)}</td><td class="${flags.meanIVVWarn?'report-penalty-hit':''} ${flags.meanIVVSevere?'severe':''}" style="${penaltyStyle(flags.meanIVVWarn,flags.meanIVVSevere)}">${r.meanIVV??'—'}</td><td class="${flags.maxIVVWarn?'report-penalty-hit':''} ${flags.maxIVVSevere?'severe':''}" style="${penaltyStyle(flags.maxIVVWarn,flags.maxIVVSevere)}">${r.intervals?.length?r.intervals.join(' / ')+' j':'—'}</td><td class="${flags.firstCalvingWarn?'report-penalty-hit':''} ${flags.firstCalvingSevere?'severe':''}" style="${penaltyStyle(flags.firstCalvingWarn,flags.firstCalvingSevere)}">${r.firstCalvingAgeMonths??'—'}${r.firstCalvingAgeMonths!=null?' mois':''}</td><td class="${flags.deadWarn?'report-penalty-hit':''} ${flags.deadSevere?'severe':''}" style="${penaltyStyle(flags.deadWarn,flags.deadSevere)}">${(r.deadBefore6||[]).length}</td></tr>`}).join('')||'<tr><td colspan="13">Aucune alerte selon les critères actuels.</td></tr>'}</tbody></table><p class="muted"><i>Les indicateurs qui ont réellement fait perdre des points sont en gras et colorés.</i></p>`;
  const heiferRegistry=(reproductionSourceForVisit(v,f)?.registry||[]),heiferById=new Map(heiferRegistry.map(a=>[normalizeAnimalId(a.id),a])),heiferCohortCounts=new Map();heiferRegistry.filter(a=>a.sex==='F'&&a.birthDate&&a.birthDate<=x.date).forEach(a=>{const y=Number(a.birthDate.slice(0,4));heiferCohortCounts.set(y,(heiferCohortCounts.get(y)||0)+1)});
  const parentLabel=id=>{const k=normalizeAnimalId(id);if(!k)return '—';const a=heiferById.get(k);return a?`${escapeHtml(a.workNumber||a.id)}${a.name?` · ${escapeHtml(a.name)}`:''}<br><small>${escapeHtml(a.id||k)}</small>`:escapeHtml(id)};
  const heifers=`<h2>8. Femelles &gt;36 mois sans vêlage</h2><p class="muted"><i>Situation à la date de visite : ${formatDate(x.date)}. La taille de cohorte correspond au nombre de femelles nées la même année dans le registre. Mère et père sont affichés lorsqu’ils sont renseignés afin de repérer une éventuelle concentration familiale.</i></p>${x.heifers36.length?`<table><thead><tr><th>N° travail / animal</th><th>Date naissance</th><th>Âge</th><th>Cohorte de naissance</th><th>Race</th><th>Mère</th><th>Père</th><th>Date entrée</th></tr></thead><tbody>${x.heifers36.slice().sort(sortByWorkNumber).map(a=>{const y=a.birthDate?Number(a.birthDate.slice(0,4)):null,cnt=y?heiferCohortCounts.get(y):null;return `<tr><td><b>${escapeHtml(a.workNumber||a.id)}</b>${a.name?` · ${escapeHtml(a.name)}`:''}<br><small>${escapeHtml(a.id||'')}</small></td><td>${a.birthDate?formatDate(a.birthDate):'—'}</td><td>${escapeHtml(ageLabelAt(a.birthDate,x.date)||'—')}</td><td>${y?`<b>${y}</b><br><small>${cnt??'—'} femelle(s) dans la cohorte</small>`:'—'}</td><td>${escapeHtml(a.breed||'—')}</td><td>${parentLabel(a.motherId)}</td><td>${parentLabel(a.fatherId)}</td><td>${a.entryDate?formatDate(a.entryDate):'Née ici / non renseigné'}</td></tr>`}).join('')}</tbody></table>`:'<p>Aucune femelle &gt;36 mois sans vêlage détectée.</p>'}`;
  const chronoData=x.chrono||{buckets:[],problems:[],conceptionEpisodes:[]};
  const chronoInsights=reproChronologyInsights(chronoData);
  const questions=[...new Set(chronoInsights.flatMap(i=>i.questions||[]))];
  if(x.heifers36.length)questions.push(`${x.heifers36.length} femelle(s) de plus de 36 mois sans vêlage : vérifier mise à la reproduction, fertilité, lots et conduite.`);
  if(x.cowProblems.length)questions.push(`${x.cowProblems.length} vache(s) ressortent dans la liste à problème : reprendre les motifs individuels et chercher une cause commune si plusieurs anomalies se concentrent sur la même période.`);
  const chrono=`<h2>9. Chronologie des performances — analyse croisée</h2><p>Seules les dégradations suffisamment nettes sont remontées. Les fluctuations mineures ne sont pas imprimées.</p>${reproChronologyInsightHtml(chronoData,{print:true})}`;
  const ez=prepEconomicSnapshot(v,f);
  const euroPrint=n=>Math.round(Number(n)||0).toLocaleString('fr-FR')+' €';
  const economy=ez?`<h2>10. Estimation économique pré-visite</h2><p><b>Ordre de grandeur minimum calculé :</b> ${euroPrint(ez.total)} de manque à gagner estimé.</p><table><thead><tr><th>Poste</th><th>Calcul retenu</th><th>Estimation</th></tr></thead><tbody><tr><td>Mortalité veaux au-dessus du repère</td><td>${ez.births} naissances · ${ez.dead} morts · repère ${String(ez.target).replace('.',',')} % · ${ez.excessDead.toFixed(1).replace('.',',')} décès excédentaire(s) × ${euroPrint(ez.calfValue)}</td><td><b>${euroPrint(ez.calfLoss)}</b></td></tr><tr><td>Mortalité adulte</td><td>${ez.adultDeaths.length} décès ≥24 mois × ${euroPrint(ez.e.adultDeathNetCost)}</td><td><b>${euroPrint(ez.adultLoss)}</b></td></tr><tr><td>Décalage des IVV</td><td>${ez.excessIvvDays} jours au-delà de ${ez.e.ivvTarget} j = ${ez.missingCalfEq.toFixed(2).replace('.',',')} veau(x) théorique(s) × ${euroPrint(ez.calfValue)}</td><td><b>${euroPrint(ez.ivvLoss)}</b></td></tr></tbody><tfoot><tr><th colspan="2">Total minimum estimé</th><th>${euroPrint(ez.total)}</th></tr></tfoot></table><p><small><b>Hypothèses :</b> race ${escapeHtml(ez.e.breed||'—')} · valeur veau/broutard ${euroPrint(ez.calfValue)} · repère mortalité ${String(ez.target).replace('.',',')} % · IVV cible ${ez.e.ivvTarget} j. Ce chiffrage n’inclut pas les coûts alimentaires supplémentaires, frais vétérinaires, traitements, analyses, main-d’œuvre, baisse de croissance ni autres coûts indirects.</small></p>`:'';
  const toClarify=`<h2>${ez?'11':'10'}. Points à éclaircir pendant l’audit</h2>${questions.length?`<ul>${[...new Set(questions)].map(q=>`<li>${escapeHtml(q)}</li>`).join('')}</ul>`:'<p>Aucun point automatique supplémentaire détecté.</p>'}<div class="notes"><b>Notes technicien :</b><br><br><br><br><br></div>`;
  const body=`<h1>Bilan technique préparatoire</h1>${activeFarmNameForPrint(v)}<p><b>Objectif :</b> regrouper au même endroit les données techniques connues avant la visite : structure du troupeau, reproduction, mortalité et introductions/mouvements à partir du registre bovins, afin de repérer les évolutions et les points à éclaircir avec l’éleveur.</p>${targetedAuditHtml(v,{forPrint:true})}${decisionSupport}${structure}${ageStructure}${repro}${mortality}${technicalCharts}${paternity}${problems}${heifers}${chrono}${economy}${toClarify}`;
  paperWindow('Bilan technique préparatoire',body)
}
...
window.addEventListener('audit-bovin-local-quota',()=>{
  showToast('⚠️ Stockage local de la tablette saturé : la saisie reste active. Synchronisez avant de fermer l’application.');
});
...
const supplementalBloodDefaultParams=[
  {key:'ureaGL',label:'Urée',unit:'g/L'},
  {key:'glucoseGL',label:'Glucose',unit:'g/L'},
  {key:'hctPct',label:'Hématocrite (Hct)',unit:'%'},
  {key:'ph',label:'pH',unit:''},
  {key:'na',label:'Sodium (Na)',unit:'mmol/L'},
  {key:'k',label:'Potassium (K)',unit:'mmol/L'},
  {key:'cl',label:'Chlorures (Cl)',unit:'mmol/L'},
  {key:'ica',label:'Calcium ionisé (iCa)',unit:'mmol/L'},
  {key:'lactate',label:'Lactate',unit:'mmol/L'}
];
function ensureSupplementalBlood(visit){
  if(!visit)return {enabled:[],customParams:[],source:'',date:'',comment:''};
  visit.supplementalBlood=visit.supplementalBlood&&typeof visit.supplementalBlood==='object'?visit.supplementalBlood:{};
  const cfg=visit.supplementalBlood;
  cfg.enabled=Array.isArray(cfg.enabled)?cfg.enabled:[];
  cfg.customParams=Array.isArray(cfg.customParams)?cfg.customParams:[];
  cfg.source=String(cfg.source||''); cfg.date=String(cfg.date||visit.date||''); cfg.comment=String(cfg.comment||'');
  (Array.isArray(visit.subjects)?visit.subjects:[]).forEach(subject=>{
    subject.measurements=subject.measurements&&typeof subject.measurements==='object'?subject.measurements:{};
    subject.measurements.supplementalBlood=subject.measurements.supplementalBlood&&typeof subject.measurements.supplementalBlood==='object'?subject.measurements.supplementalBlood:{};
  });
  return cfg;
}
function supplementalBloodParams(visit){
  const cfg=ensureSupplementalBlood(visit);
  const fixed=supplementalBloodDefaultParams.filter(p=>cfg.enabled.includes(p.key));
  const custom=cfg.customParams.filter(p=>p&&p.id&&p.label).map(p=>({key:'custom:'+p.id,label:p.label,unit:p.unit||'',custom:true,id:p.id}));
  return [...fixed,...custom];
}
function glucoseConversions(value,unit){
  const n=numericValue(value); if(n===null)return null;
  if(unit==='g/L')return {gl:n,mmol:n*5.55075,mgdl:n*100};
  if(unit==='mmol/L')return {gl:n/5.55075,mmol:n,mgdl:n*18.0156};
  return {gl:n/100,mmol:n/18.0156,mgdl:n};
}
function formatGlucoseValue(v,d=2){return Number.isFinite(v)?v.toLocaleString('fr-FR',{maximumFractionDigits:d}):'—';}
function supplementalBloodValue(subject,param){
  const bag=subject?.measurements?.supplementalBlood||{};
  return param.custom?(bag.custom?.[param.id]??''):(bag[param.key]??'');
}
function setSupplementalBloodValue(subject,param,value){
  subject.measurements=subject.measurements&&typeof subject.measurements==='object'?subject.measurements:{};
  const bag=subject.measurements.supplementalBlood=subject.measurements.supplementalBlood&&typeof subject.measurements.supplementalBlood==='object'?subject.measurements.supplementalBlood:{};
  if(param.custom){bag.custom=bag.custom&&typeof bag.custom==='object'?bag.custom:{};bag.custom[param.id]=value;}else bag[param.key]=value;
}
function renderSupplementalBloodSection(visit){
  const cfg=ensureSupplementalBlood(visit),params=supplementalBloodParams(visit),subjects=Array.isArray(visit.subjects)?visit.subjects:[];
  const minWidth=170+params.length*125+190;
  return `<section class="card supplemental-blood-card" style="margin-top:16px"><div class="section-title"><div><h3>🧪 Analyses sanguines complémentaires</h3><div class="muted">Résultats d'un analyseur vétérinaire ou de laboratoire, séparés des mesures terrain.</div></div></div>
  <details ${cfg.enabled.length||cfg.customParams.length?'open':''}><summary><strong>+ Choisir les analyses à ajouter</strong></summary><div class="chip-options supplemental-blood-choices">${supplementalBloodDefaultParams.map(p=>`<label class="choice-chip ${cfg.enabled.includes(p.key)?'selected':''}"><input type="checkbox" data-supp-blood-enable="${p.key}" ${cfg.enabled.includes(p.key)?'checked':''}>${escapeHtml(p.label)}${p.unit?` <small>(${escapeHtml(p.unit)})</small>`:''}</label>`).join('')}</div><div class="actions" style="margin-top:10px"><button class="btn secondary small" id="supp-blood-add-custom">+ Autre analyse</button></div>${cfg.customParams.length?`<div class="custom-blood-params">${cfg.customParams.map(p=>`<div class="row" data-custom-blood-param="${p.id}"><div class="field"><label>Nom</label><input data-custom-blood-name="${p.id}" value="${escapeHtml(p.label||'')}"></div><div class="field"><label>Unité</label><input data-custom-blood-unit="${p.id}" value="${escapeHtml(p.unit||'')}"></div><button class="btn danger small" data-custom-blood-delete="${p.id}">Supprimer</button></div>`).join('')}</div>`:''}</details>
  <div class="grid cols-3" style="margin-top:12px"><div class="field"><label>Source / appareil / vétérinaire</label><input id="supp-blood-source" value="${escapeHtml(cfg.source)}" placeholder="Ex. analyseur du cabinet vétérinaire"></div><div class="field"><label>Date d'analyse</label><input id="supp-blood-date" type="date" value="${escapeHtml(cfg.date)}"></div><div class="field"><label>Commentaire</label><input id="supp-blood-comment" value="${escapeHtml(cfg.comment)}" placeholder="Lot, conditions, remarque…"></div></div>
  ${params.length?`<div class="table-wrap" style="margin-top:12px"><table class="analysis-table matrix-table supplemental-blood-table" style="min-width:${minWidth}px"><thead><tr><th style="min-width:150px">Sujet</th>${params.map(p=>`<th style="min-width:120px">${escapeHtml(p.label)}${p.unit?`<br><small>${escapeHtml(p.unit)}</small>`:''}</th>`).join('')}<th style="min-width:180px">Commentaire</th></tr></thead><tbody>${subjects.map(subject=>`<tr><td><strong>${escapeHtml(subject.tag||subject.name||'Sujet')}</strong></td>${params.map(p=>`<td><input class="decimal-input" inputmode="decimal" data-supp-blood-value data-subject-id="${subject.id}" data-param-key="${escapeHtml(p.key)}" value="${escapeHtml(supplementalBloodValue(subject,p))}" placeholder="—"></td>`).join('')}<td><input data-supp-blood-subject-comment data-subject-id="${subject.id}" value="${escapeHtml(subject.measurements?.supplementalBlood?.comment||'')}"></td></tr>`).join('')}</tbody></table></div>`:'<div class="empty" style="margin-top:12px">Cochez uniquement les paramètres disponibles pour cette visite.</div>'}
  <div class="glucose-converter"><h4>Convertisseur glycémie</h4><div class="row"><div class="field"><label>Valeur</label><input id="glucose-convert-value" class="decimal-input" inputmode="decimal" placeholder="Ex. 0,60"></div><div class="field"><label>Unité saisie</label><select id="glucose-convert-unit"><option>g/L</option><option>mmol/L</option><option>mg/dL</option></select></div></div><div id="glucose-convert-result" class="notice"><strong>Entrez une valeur</strong> pour afficher les trois unités.</div></div></section>`;
}
function bindSupplementalBloodEvents(visit){
  if(!visit)return; const cfg=ensureSupplementalBlood(visit); const save=()=>{visit.updatedAt=new Date().toISOString();saveDatabase(db);};
  app.querySelectorAll('[data-supp-blood-enable]').forEach(el=>el.onchange=()=>{const k=el.dataset.suppBloodEnable;cfg.enabled=el.checked?[...new Set([...cfg.enabled,k])]:cfg.enabled.filter(x=>x!==k);save();renderAnalysis();});
  document.getElementById('supp-blood-add-custom')?.addEventListener('click',()=>{cfg.customParams.push({id:uid('bloodparam'),label:'Nouvelle analyse',unit:''});save();renderAnalysis();});
  app.querySelectorAll('[data-custom-blood-name]').forEach(el=>el.onchange=()=>{const p=cfg.customParams.find(x=>x.id===el.dataset.customBloodName);if(p){p.label=el.value.trim()||'Analyse';save();renderAnalysis();}});
  app.querySelectorAll('[data-custom-blood-unit]').forEach(el=>el.onchange=()=>{const p=cfg.customParams.find(x=>x.id===el.dataset.customBloodUnit);if(p){p.unit=el.value.trim();save();renderAnalysis();}});
  app.querySelectorAll('[data-custom-blood-delete]').forEach(el=>el.onclick=()=>{cfg.customParams=cfg.customParams.filter(x=>x.id!==el.dataset.customBloodDelete);(Array.isArray(visit.subjects)?visit.subjects:[]).forEach(s=>{if(s.measurements?.supplementalBlood?.custom)delete s.measurements.supplementalBlood.custom[el.dataset.customBloodDelete];});save();renderAnalysis();});
  const source=document.getElementById('supp-blood-source'),date=document.getElementById('supp-blood-date'),comment=document.getElementById('supp-blood-comment');
  if(source)source.oninput=()=>{cfg.source=source.value;save();}; if(date)date.onchange=()=>{cfg.date=date.value;save();}; if(comment)comment.oninput=()=>{cfg.comment=comment.value;save();};
  const pmap=new Map(supplementalBloodParams(visit).map(p=>[p.key,p]));
  app.querySelectorAll('[data-supp-blood-value]').forEach(el=>{const fn=()=>{const subject=(Array.isArray(visit.subjects)?visit.subjects:[]).find(x=>x.id===el.dataset.subjectId),param=pmap.get(el.dataset.paramKey);if(!subject||!param)return;setSupplementalBloodValue(subject,param,el.value);save();};el.oninput=fn;el.onchange=fn;});
  app.querySelectorAll('[data-supp-blood-subject-comment]').forEach(el=>el.oninput=()=>{const subject=(Array.isArray(visit.subjects)?visit.subjects:[]).find(x=>x.id===el.dataset.subjectId);if(!subject)return;subject.measurements=subject.measurements||{};subject.measurements.supplementalBlood=subject.measurements.supplementalBlood||{};subject.measurements.supplementalBlood.comment=el.value;save();});
  const gv=document.getElementById('glucose-convert-value'),gu=document.getElementById('glucose-convert-unit'),gr=document.getElementById('glucose-convert-result');
  const update=()=>{if(!gv||!gu||!gr)return;const c=glucoseConversions(gv.value,gu.value);gr.innerHTML=c?`<strong>${formatGlucoseValue(c.gl,3)} g/L</strong> · <strong>${formatGlucoseValue(c.mmol,2)} mmol/L</strong> · <strong>${formatGlucoseValue(c.mgdl,1)} mg/dL</strong>`:'<strong>Entrez une valeur</strong> pour afficher les trois unités.';};
  if(gv)gv.oninput=update;if(gu)gu.onchange=update;
}
...
function renderNumericSection(visit) {
  ...
  const subjects = Array.isArray(visit?.subjects) ? visit.subjects : [];
  const rows = subjects.map(subject => `<tr data-analysis-subject-row="${subject.id}" class="${focusedAnalysisSubjectId===subject.id?'focused-subject-row':''}">
  ...
  </section>${activeAnalysisFamily==='Sang'?renderSupplementalBloodSection(visit):''}`;
}
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.6.21.123</span></div>
...
function renderAnalysis() {
  ...
  ${activeVisitBanner(visit)}
  ${!visit?'<div class="empty" style="margin-top:16px">Choisissez une visite dans l’onglet Visites.</div>':!visit.subjects?.length?'<div class="empty" style="margin-top:16px">Ajoutez des sujets dans l’onglet Animaux.</div>':`<section class="card analysis-utilities"><div class="actions"><button class="btn primary" id="analysis-export-excel">📊 Exporter les mesures Excel</button><button class="btn" id="analysis-demo">Jeu d’essai</button><button class="btn secondary" id="analysis-clear">Effacer l’analyse</button></div></section><nav class="analysis-tabs">${tabs.map(([k,l])=>`<button class="analysis-tab ${activeAnalysisSection===k?'active':''}" data-analysis-section="${k}">${l}</button>`).join('')}</nav><section class="analysis-content">${activeAnalysisSection==='numeric'?renderNumericSection(visit):activeAnalysisSection==='observations'?renderObservationsSection(visit):activeAnalysisSection==='general'?renderGeneralSection(visit):activeAnalysisSection==='reasoning'?renderReasoningSection(visit):activeAnalysisSection==='summary'?renderSynthesisSection(visit):renderConclusionSection(visit)}</section>`}`;
  enhanceNumericEntry(app);
  ...
  if(visit&&activeAnalysisSection==='numeric'&&activeAnalysisFamily==='Sang')bindSupplementalBloodEvents(visit);
...
function bindAnalysisEvents(visit) {
  if(!visit)return;
  document.getElementById('analysis-export-excel')?.addEventListener('click',async()=>{const btn=document.getElementById('analysis-export-excel');if(btn){btn.disabled=true;const old=btn.textContent;btn.textContent='Génération Excel…';try{await downloadMeasurementsExcel(visit);}catch(err){console.error(err);alert('Impossible de générer le fichier Excel des mesures.');}finally{btn.disabled=false;btn.textContent=old;}}});
  enhanceNumericEntry(app);
...
const DEFAULT_MACRO_TARGETS={ca:0.60,p:0.35,mg:0.20,na:0.10};
const MACRO_LABELS={ca:'Calcium (Ca)',p:'Phosphore (P)',mg:'Magnésium (Mg)',na:'Sodium (Na)'};
function ensureFeedProducts(visit){
  visit.feeding=visit.feeding&&typeof visit.feeding==='object'?visit.feeding:{rations:[],settings:{},history:[]};
  visit.feeding.feedProducts=Array.isArray(visit.feeding.feedProducts)?visit.feeding.feedProducts:[];
  return visit.feeding.feedProducts;
}
function newFeedProduct(){return {id:uid('feedprod'),name:'Nouvel aliment',manufacturer:'',category:'Vaches allaitantes',dose:'',doseUnit:'kg/j',compositionText:'',ms:'',mat:'',fat:'',cellulose:'',ash:'',starch:'',sugar:'',ufl:'',pdi:'',ca:'',p:'',na:'',mg:'',cu:'',zn:'',mn:'',i:'',co:'',se:'',vitA:'',vitD3:'',vitE:'',vitB1:'',notes:''};}
function savedFeedProducts(){return ensureReferenceSettings().savedFeedProducts;}
function feedProductTemplateCopy(p){const x={...newFeedProduct(),...(p||{})};x.id=uid('feedprod');x.dose='';x.doseUnit=p?.doseUnit||'kg/j';return x;}
function feedProductDailyContribution(product){
  const out={}; if(!product)return out; const dose=nValue(product.dose); if(dose===null||dose<0)return out;
  let kg=dose; if(product.doseUnit==='g/j')kg=dose/1000;
  for(const k of ['ca','p','na','mg']){const c=nValue(product[k]); if(c!==null)out[k]=c*kg*10;}
  for(const k of ['cu','zn','mn','i','co','se']){const c=nValue(product[k]); if(c!==null)out[k]=c*kg;}
  for(const k of ['vitA','vitD3','vitE','vitB1']){const c=nValue(product[k]); if(c!==null)out[k]=c*kg;}
  for(const k of ['mat','fat','cellulose','ash','starch','sugar']){const c=nValue(product[k]); if(c!==null)out[k]=c*kg*10;}
  const ms=nValue(product.ms); if(ms!==null)out.dm=kg*ms/100;
  const ufl=nValue(product.ufl); if(ufl!==null)out.ufl=ufl*kg;
  const pdi=nValue(product.pdi); if(pdi!==null)out.pdi=pdi*kg;
  return out;
}
function feedProductsContribution(visit,category){const total={};for(const p of ensureFeedProducts(visit).filter(x=>!category||x.category===category)){const c=feedProductDailyContribution(p);for(const [k,v] of Object.entries(c))total[k]=(total[k]||0)+(Number(v)||0);}return total;}
function macroTarget(key){const v=nValue(ensureReferenceSettings().macroMineralNeedTargets?.[key]);return v===null?DEFAULT_MACRO_TARGETS[key]:v;}
function rationEstimatedMineralsExtended(visit,category){
  const totals=rationEstimatedMinerals(visit,category); totals.ca=0;totals.p=0;totals.mg=0;totals.na=0;
  for(const r of (visit.feeding?.rations||[]).filter(x=>!category||x.category===category)){
    const ref=feedReference(r.feedRefId); if(!ref)continue; const q=nValue(r.quantity); if(q===null)continue;
    let kgBrut=q;if(r.unit==='kg MS/j')kgBrut=q/(ref.ms/100);else if(r.unit==='g/j')kgBrut=q/1000;else if(r.unit!=='kg brut/j')continue;
    const dm=kgBrut*(ref.ms/100);
    for(const k of ['ca','p','mg','na']){const v=nValue(ref[k]);if(v!==null)totals[k]+=v*dm*10;}
  }
  return totals;
}
function mineralCoverageRows(visit){
  const sup=ensureSupplementation(visit),prod=supplementProducts().find(x=>x.id===sup.productId),fromSupplement=supplementDailyContribution(prod,sup),fromRation=rationEstimatedMineralsExtended(visit,sup.category),fromFeedProducts=feedProductsContribution(visit,sup.category),dm=nValue(sup.dmIntake)??((fromRation.dm||0)+(fromFeedProducts.dm||0)||null);
  const macro=['ca','p','mg','na'].map(k=>{const target=macroTarget(k),need=dm!==null?target*dm*10:null,feed=fromRation[k]||0,food=fromFeedProducts[k]||0,total=feed+food,coverage=need&&need>0?total/need*100:null;return {k,label:MACRO_LABELS[k],target,targetUnit:'% MS',need,feed,food,add:0,total,coverage,unit:'g/j'};});
  const trace=['co','cu','i','mn','se','zn'].map(k=>{const target=mineralTarget(k),need=dm!==null?target*dm:null,feed=fromRation[k]||0,food=fromFeedProducts[k]||0,add=fromSupplement[k]||0,total=(k==='i'?food+add:feed+food+add),coverage=need&&need>0?total/need*100:null;return {k,label:MINERAL_LABELS[k],target,targetUnit:'mg/kg MS',need,feed,food,add,total,coverage,unit:'mg/j'};});
  return [...macro,...trace];
}
function feedProductCardHtml(visit){
  const products=ensureFeedProducts(visit);
  return `<section class="card" style="margin-top:16px"><div class="section-title"><div><h3>🥣 Aliments / concentrés</h3><div class="muted">Saisissez l’étiquette et la quantité réellement distribuée. Les minéraux et oligo-éléments sont ajoutés automatiquement au bilan de couverture. Un aliment peut être mémorisé et réutilisé lors d’une prochaine visite.</div></div><div class="actions"><select id="saved-feed-product-select"><option value="">Aliment enregistré…</option>${savedFeedProducts().map(x=>`<option value="${x.id}">${escapeHtml(x.name||'Aliment')} — ${escapeHtml(x.manufacturer||'')}</option>`).join('')}</select><button type="button" class="btn secondary" id="use-saved-feed-product">Réutiliser</button><button type="button" class="btn primary" id="add-feed-product">+ Ajouter un aliment</button></div></div>
  ${products.length?products.map(p=>{const c=feedProductDailyContribution(p);return `<article class="card compact" data-feed-product="${p.id}"><div class="section-title"><h4>${escapeHtml(p.name||'Aliment')}</h4><div class="actions"><button type="button" class="btn small secondary" data-save-feed-product="${p.id}">💾 Enregistrer pour les prochaines visites</button><button type="button" class="btn small danger" data-delete-feed-product="${p.id}">Supprimer</button></div></div>
    <div class="grid cols-4"><div class="field"><label>Nom de l’aliment</label><input data-feed-product-field="name" data-id="${p.id}" value="${escapeHtml(p.name||'')}"></div><div class="field"><label>Fabricant</label><input data-feed-product-field="manufacturer" data-id="${p.id}" value="${escapeHtml(p.manufacturer||'')}"></div><div class="field"><label>Catégorie</label><select data-feed-product-field="category" data-id="${p.id}">${feedingCategories.map(v=>`<option ${p.category===v?'selected':''}>${v}</option>`).join('')}</select></div><div class="field"><label>Quantité / animal / jour</label><div class="supplement-dose-row"><input inputmode="decimal" data-feed-product-field="dose" data-id="${p.id}" value="${escapeHtml(p.dose||'')}" placeholder="Dose"><select data-feed-product-field="doseUnit" data-id="${p.id}">${['kg/j','g/j'].map(v=>`<option ${p.doseUnit===v?'selected':''}>${v}</option>`).join('')}</select></div></div></div>
    <div class="field"><label>Composition / ingrédients de l’étiquette</label><textarea data-feed-product-field="compositionText" data-id="${p.id}" placeholder="Ex. coques de soja, drêches, maïs, son de blé…">${escapeHtml(p.compositionText||'')}</textarea></div>
    <h4>Composition analytique</h4><div class="grid cols-4">${[['ms','MS (%)'],['mat','Protéines brutes / MAT (%)'],['fat','Matières grasses (%)'],['cellulose','Cellulose brute (%)'],['ash','Cendres brutes (%)'],['starch','Amidon (%)'],['sugar','Sucres (%)'],['ufl','UFL/kg produit'],['pdi','PDI (g/kg produit)']].map(([k,l])=>`<div class="field"><label>${l}</label><input inputmode="decimal" data-feed-product-field="${k}" data-id="${p.id}" value="${escapeHtml(p[k]||'')}"></div>`).join('')}</div>
    <h4>Minéraux (% du produit)</h4><div class="grid cols-4">${[['ca','Calcium'],['p','Phosphore'],['na','Sodium'],['mg','Magnésium']].map(([k,l])=>`<div class="field"><label>${l} (%)</label><input inputmode="decimal" data-feed-product-field="${k}" data-id="${p.id}" value="${escapeHtml(p[k]||'')}"></div>`).join('')}</div>
    <h4>Oligo-éléments (mg/kg de produit)</h4><div class="grid cols-4">${[['cu','Cuivre'],['zn','Zinc'],['mn','Manganèse'],['i','Iode'],['co','Cobalt'],['se','Sélénium'],['fe','Fer']].map(([k,l])=>`<div class="field"><label>${l} (mg/kg)</label><input inputmode="decimal" data-feed-product-field="${k}" data-id="${p.id}" value="${escapeHtml(p[k]||'')}"></div>`).join('')}</div>
    <div class="field"><label>Autres constituants / compléments</label><textarea data-feed-product-field="otherComposition" data-id="${p.id}" placeholder="Ex. molybdène, soufre, biotine, additifs, levures…">${escapeHtml(p.otherComposition||'')}</textarea></div>
    <h4>Vitamines (par kg de produit)</h4><div class="grid cols-4">${[['vitA','Vitamine A (UI/kg)'],['vitD3','Vitamine D3 (UI/kg)'],['vitE','Vitamine E (UI/kg)'],['vitB1','Vitamine B1 (mg/kg)']].map(([k,l])=>`<div class="field"><label>${l}</label><input inputmode="decimal" data-feed-product-field="${k}" data-id="${p.id}" value="${escapeHtml(p[k]||'')}"></div>`).join('')}</div>
    <div class="notice"><strong>Apport journalier calculé${p.dose?' pour '+escapeHtml(p.dose)+' '+escapeHtml(p.doseUnit):''}</strong><br>${[['ca','Ca','g'],['p','P','g'],['mg','Mg','g'],['na','Na','g'],['cu','Cu','mg'],['zn','Zn','mg'],['mn','Mn','mg'],['i','I','mg'],['co','Co','mg'],['se','Se','mg']].map(([k,l,u])=>c[k]!=null?`${l} : ${Number(c[k]).toFixed(1)} ${u}/j`:null).filter(Boolean).join(' · ')||'Renseignez la dose et la composition pour calculer les apports.'}</div>
    <div class="field"><label>Commentaire</label><textarea data-feed-product-field="notes" data-id="${p.id}">${escapeHtml(p.notes||'')}</textarea></div></article>`}).join(''):'<div class="empty">Aucun aliment complémentaire enregistré pour cette visite.</div>'}
  <p class="muted small-text">Les teneurs de l’étiquette sont calculées sur la quantité de produit distribuée. Les champs UFL/PDI sont facultatifs si le fabricant ne les fournit pas. Les repères de couverture restent indicatifs et doivent être interprétés selon le lot et le stade physiologique.</p></section>`;
}
...
const forageLabFieldGroups=[
  {title:'Analyse minérale — éléments majeurs',fields:[
    ['ca_gkg','Ca — Calcium','g/kg MS'],['cu_mgkg','Cu — Cuivre','mg/kg MS'],['k_gkg','K — Potassium','g/kg MS'],['p_gkg','P — Phosphore','g/kg MS'],['mg_gkg','Mg — Magnésium','g/kg MS'],['mn_mgkg','Mn — Manganèse','mg/kg MS'],['fe_mgkg','Fe — Fer','mg/kg MS'],['na_gkg','Na — Sodium','g/kg MS'],['zn_mgkg','Zn — Zinc','mg/kg MS']
  ]},
  {title:'Constituants principaux',fields:[
    ['ms_pct_mb','MS — Matière sèche','% MB'],['ph','pH','unité pH'],['mm_gkg','MM — Matière minérale','g/kg MS'],['mat_gkg','MAT — Matières azotées totales','g/kg MS'],['cb_gkg','CB — Cellulose brute','g/kg MS'],['dcs_pct','DCS — Digestibilité pepsine-cellulase','%'],['sugars_pct','Sucres résiduels','%'],['starch_gkg','AMI — Amidon','g/kg MS'],['adf_gkg','ADF — Lignocellulose','g/kg MS'],['adl_gkg','ADL — Lignine','g/kg MS'],['ndf_gkg','NDF — Parois totales','g/kg MS'],['mgt_gkg','MGT — Matières grasses totales','g/kg MS'],['mo_gkg','MO — Matière organique','g/kg MS']
  ]},
  {title:'Valeur alimentaire calculée',fields:[
    ['dmo_pct','dMO — Digestibilité de la matière organique','%'],['dt6n_pct','DT6_N — Dégradabilité théorique de l’azote','%'],['eb_kcal','EB — Énergie brute','kcal/kg MO'],['ueb','UEB — Unités d’encombrement bovin','UEB/kg MS'],['uel','UEL — Unités d’encombrement lait','UEL/kg MS'],['ufl','UFL — Unités fourragères lait','UFL/kg MS'],['ufv','UFV — Unité fourragère viande','UFV/kg MS'],['pdi_gkg','PDI — Protéines digestibles dans l’intestin','g/kg MS'],['bpr_gkg','BPR — Balance protéique du rumen','g/kg MS'],['pdia_gkg','PDIA — Protéines digestibles dans l’intestin d’origine alimentaire','g/kg MS'],['niref','NIREF — Niveau d’ingestion de référence',''],['uem','UEM — Unités d’encombrement mouton','UEM/kg MS'],['dndf_pct','DNDF — Digestibilité des parois végétales','%'],['pabs_gkg','P_ABS — Phosphore absorbable','g/kg MS'],['caabs_gkg','CA_ABS — Calcium absorbable','g/kg MS'],['baca_meq','BACA — Bilan cations-anions','mEq/kg MS'],['be_meq','BE — Bilan électrolytique','mEq/kg MS']
  ]}
];
function ensureForageLab(a){a.lab=a.lab&&typeof a.lab==='object'?a.lab:{};return a.lab;}
function renderForageLabDetails(a){
  const lab=ensureForageLab(a);
  const has=Object.values(lab).some(v=>String(v??'').trim()!=='');
  return `<details class="forage-lab-details" ${has?'open':''}><summary><strong>🧪 Saisie détaillée du bulletin laboratoire</strong> <span class="muted">— mêmes unités que le compte rendu du labo</span></summary>
    <div class="notice" style="margin-top:10px"><strong>Astuce :</strong> saisissez ici les valeurs exactement comme sur le rapport (g/kg MS, mg/kg MS, %, UFL…). Le bouton ci-dessous convertit ensuite les paramètres principaux vers les champs utilisés par l’interprétation de la ration.</div>
    ${forageLabFieldGroups.map(g=>`<div class="forage-lab-group"><h4>${g.title}</h4><div class="grid cols-4 nutrition-fields">${g.fields.map(([k,label,unit])=>`<div class="field"><label>${label}${unit?` <span class="muted">(${unit})</span>`:''}</label><input inputmode="decimal" data-forage-lab-field="${k}" data-id="${a.id}" value="${escapeHtml(lab[k]||'')}" placeholder="—"></div>`).join('')}</div></div>`).join('')}
    <div class="field"><label>Autres paramètres / remarques du laboratoire</label><textarea data-forage-lab-field="other" data-id="${a.id}" placeholder="Ex. méthode, paramètre supplémentaire, valeur non prévue…">${escapeHtml(lab.other||'')}</textarea></div>
    <div class="actions"><button type="button" class="btn secondary" data-forage-lab-sync="${a.id}">↪ Reporter / convertir les valeurs principales</button></div>
  </details>`;
}
function syncForageLabToMain(a){
  const l=ensureForageLab(a),num=v=>{const n=nValue(v);return n===null?null:n},set=(k,v,d=3)=>{if(v!==null&&Number.isFinite(v))a[k]=String(Number(v.toFixed(d)));};
  set('ms',num(l.ms_pct_mb),2); set('mat',num(l.mat_gkg)!==null?num(l.mat_gkg)/10:null,3); set('ndf',num(l.ndf_gkg)!==null?num(l.ndf_gkg)/10:null,3); set('adf',num(l.adf_gkg)!==null?num(l.adf_gkg)/10:null,3); set('starch',num(l.starch_gkg)!==null?num(l.starch_gkg)/10:null,3); set('sugar',num(l.sugars_pct),3); set('dmo',num(l.dmo_pct)!==null?num(l.dmo_pct):num(l.dcs_pct),3); set('ph',num(l.ph),3); set('ca',num(l.ca_gkg)!==null?num(l.ca_gkg)/10:null,4); set('p',num(l.p_gkg)!==null?num(l.p_gkg)/10:null,4); set('na',num(l.na_gkg)!==null?num(l.na_gkg)/10:null,4); set('k',num(l.k_gkg)!==null?num(l.k_gkg)/10:null,4);
}

function ensureNutritionContextAnalyses(visit){
  visit.feeding=visit.feeding&&typeof visit.feeding==='object'?visit.feeding:{rations:[],settings:{},history:[]};
  visit.feeding.nutrition=visit.feeding.nutrition&&typeof visit.feeding.nutrition==='object'?visit.feeding.nutrition:{};
  const n=visit.feeding.nutrition;
  n.contextAnalyses=Array.isArray(n.contextAnalyses)?n.contextAnalyses:[];
  n.contextAnalyses.forEach(a=>{a.rows=Array.isArray(a.rows)?a.rows:[];});
  return n.contextAnalyses;
}
function nutritionContextAnalysisHtml(a){
  const label=a.kind==='SOL'?'🌱 Analyse de sol':a.kind==='VEGETAL'?'🌿 Analyse de végétaux':'🧾 Analyse complémentaire';
  return `<article class="nutrition-analysis-card context-analysis-card" data-context-analysis="${a.id}">
    <div class="section-title"><div><h4>${label} — ${escapeHtml(a.name||'Sans nom')}</h4><div class="muted">${escapeHtml(a.date||'')}${a.source?` · ${escapeHtml(a.source)}`:''}</div></div><button class="btn small danger" data-delete-context-analysis="${a.id}">Supprimer</button></div>
    <div class="grid cols-3">
      <div class="field"><label>Nom / parcelle / échantillon</label><input data-context-field="name" value="${escapeHtml(a.name||'')}"></div>
      <div class="field"><label>Type</label><select data-context-field="kind"><option value="SOL" ${a.kind==='SOL'?'selected':''}>Analyse de sol</option><option value="VEGETAL" ${a.kind==='VEGETAL'?'selected':''}>Analyse de végétaux</option></select></div>
      <div class="field"><label>Date</label><input type="date" data-context-field="date" value="${escapeHtml(a.date||'')}"></div>
      <div class="field"><label>Source / laboratoire</label><input data-context-field="source" value="${escapeHtml(a.source||'')}"></div>
      <div class="field" style="grid-column:span 2"><label>Commentaire</label><input data-context-field="comment" value="${escapeHtml(a.comment||'')}"></div>
    </div>
    <div class="table-wrap"><table class="compact-table"><thead><tr><th>Paramètre</th><th>Valeur</th><th>Unité</th><th>Commentaire</th><th></th></tr></thead><tbody>
      ${(a.rows||[]).map(r=>`<tr data-context-row="${r.id}"><td><input data-context-row-field="parameter" value="${escapeHtml(r.parameter||'')}"></td><td><input data-context-row-field="value" inputmode="decimal" value="${escapeHtml(r.value||'')}"></td><td><input data-context-row-field="unit" value="${escapeHtml(r.unit||'')}"></td><td><input data-context-row-field="comment" value="${escapeHtml(r.comment||'')}"></td><td><button class="btn danger small" data-delete-context-row="${r.id}">×</button></td></tr>`).join('')}
    </tbody></table></div>
    <button class="btn secondary small" data-add-context-row="${a.id}">+ Paramètre</button>
  </article>`;
}
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.6.21.123</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
  visit.feeding=visit.feeding&&typeof visit.feeding==='object'?visit.feeding:{rations:[],settings:{},history:[]};
  visit.feeding.nutrition=visit.feeding.nutrition&&typeof visit.feeding.nutrition==='object'?visit.feeding.nutrition:{};
  const nutrition=visit.feeding.nutrition;
  const contextAnalyses=ensureNutritionContextAnalyses(visit);
  ...
  const fields=(a)=>`<div class="grid cols-4 nutrition-fields">
    ...
    <div class="field"><label>Ca — Calcium (% MS)</label><input inputmode="decimal" data-forage-field="ca" data-id="${a.id}" value="${escapeHtml(a.ca||'')}"></div>
    <div class="field"><label>P — Phosphore (% MS)</label><input inputmode="decimal" data-forage-field="p" data-id="${a.id}" value="${escapeHtml(a.p||'')}"></div>
    <div class="field"><label>Na — Sodium (% MS)</label><input inputmode="decimal" data-forage-field="na" data-id="${a.id}" value="${escapeHtml(a.na||'')}"></div>
    <div class="field"><label>K — Potassium (% MS)</label><input inputmode="decimal" data-forage-field="k" data-id="${a.id}" value="${escapeHtml(a.k||'')}"></div>
    <div class="field"><label>Cl — Chlorures (% MS)</label><input inputmode="decimal" data-forage-field="cl" data-id="${a.id}" value="${escapeHtml(a.cl||'')}"></div>
    <div class="field"><label>S — Soufre (% MS)</label><input inputmode="decimal" data-forage-field="s" data-id="${a.id}" value="${escapeHtml(a.s||'')}"></div>
  </div>`;
  ...
  ${nutrition.forageAnalyses.length?nutrition.forageAnalyses.map(a=>`<article class="nutrition-analysis-card"><div class="section-title"><h4>${escapeHtml(a.name||a.type||'Analyse de fourrage')} ${a.estimated?'<span class="badge warning">📚 estimé</span>':'<span class="badge positive">🧪 analysé/saisi</span>'}</h4><button class="btn small danger" data-delete-forage="${a.id}">Supprimer</button></div>${fields(a)}${renderForageLabDetails(a)}<div class="analysis-interpretations">${interpretForage(a).map(x=>nutritionFlag(...x)).join('')}</div><div class="field"><label>Commentaire du technicien</label><textarea data-forage-field="comment" data-id="${a.id}">${escapeHtml(a.comment||'')}</textarea></div></article>`).join(''):'<div class="empty">Aucune analyse renseignée.</div>'}</section>
  <section class="card" style="margin-top:16px"><div class="section-title"><div><h3>🌱 Sol & végétaux</h3><div class="muted">Analyses de terre et de végétaux associées à la parcelle/fourrage, utiles pour relier fertilité, disponibilité minérale et qualité du fourrage.</div></div><div class="actions"><button class="btn secondary" id="add-soil-analysis">+ Analyse de sol</button><button class="btn secondary" id="add-plant-analysis">+ Analyse de végétaux</button></div></div>${contextAnalyses.length?contextAnalyses.map(nutritionContextAnalysisHtml).join(''):'<div class="empty">Aucune analyse de sol ou de végétaux enregistrée.</div>'}</section>
  <section class="card" style="margin-top:16px"><h3>🥣 Analyse de la ration complète</h3><div class="grid cols-4 nutrition-fields">
    ...
    <div class="field"><label>Ca — Calcium (% MS)</label><input inputmode="decimal" data-ration-field="ca" value="${escapeHtml(r.ca||'')}"></div>
    <div class="field"><label>P — Phosphore (% MS)</label><input inputmode="decimal" data-ration-field="p" value="${escapeHtml(r.p||'')}"></div>
  ...
  app.querySelectorAll('[data-forage-lab-field]').forEach(el=>{const save=()=>{const a=nutrition.forageAnalyses.find(x=>x.id===el.dataset.id);if(!a)return;const lab=ensureForageLab(a);lab[el.dataset.forageLabField]=el.value;visit.updatedAt=new Date().toISOString();saveDatabase(db);};el.onchange=save;el.onblur=save;});
  app.querySelectorAll('[data-forage-lab-sync]').forEach(b=>b.onclick=()=>{const a=nutrition.forageAnalyses.find(x=>x.id===b.dataset.forageLabSync);if(!a)return;syncForageLabToMain(a);visit.updatedAt=new Date().toISOString();saveDatabase(db);showToast?.('Valeurs principales converties depuis le bulletin labo.');renderNutritionAnalysis();});
  ...
  document.getElementById('add-soil-analysis')?.addEventListener('click',()=>{contextAnalyses.push({id:uid('ctx'),kind:'SOL',name:'Nouvelle analyse de sol',date:'',source:'',comment:'',rows:[]});saveDatabase(db);renderNutritionAnalysis();});
  document.getElementById('add-plant-analysis')?.addEventListener('click',()=>{contextAnalyses.push({id:uid('ctx'),kind:'VEGETAL',name:'Nouvelle analyse de végétaux',date:'',source:'',comment:'',rows:[]});saveDatabase(db);renderNutritionAnalysis();});
  app.querySelectorAll('[data-context-analysis]').forEach(card=>{
    const a=contextAnalyses.find(x=>x.id===card.dataset.contextAnalysis); if(!a)return;
    card.querySelectorAll('[data-context-field]').forEach(el=>{const save=()=>{a[el.dataset.contextField]=el.value;visit.updatedAt=new Date().toISOString();saveDatabase(db);};el.oninput=save;el.onchange=save;});
    card.querySelectorAll('[data-context-row]').forEach(tr=>{const r=a.rows.find(x=>x.id===tr.dataset.contextRow);if(!r)return;tr.querySelectorAll('[data-context-row-field]').forEach(el=>{const save=()=>{r[el.dataset.contextRowField]=el.value;visit.updatedAt=new Date().toISOString();saveDatabase(db);};el.oninput=save;el.onchange=save;});});
  });
  app.querySelectorAll('[data-add-context-row]').forEach(b=>b.onclick=()=>{const a=contextAnalyses.find(x=>x.id===b.dataset.addContextRow);if(a){a.rows.push({id:uid('ctxrow'),parameter:'',value:'',unit:'',comment:''});saveDatabase(db);renderNutritionAnalysis();}});
  app.querySelectorAll('[data-delete-context-row]').forEach(b=>b.onclick=()=>{const card=b.closest('[data-context-analysis]'),a=contextAnalyses.find(x=>x.id===card?.dataset.contextAnalysis);if(a){a.rows=a.rows.filter(x=>x.id!==b.dataset.deleteContextRow);saveDatabase(db);renderNutritionAnalysis();}});
  app.querySelectorAll('[data-delete-context-analysis]').forEach(b=>b.onclick=()=>{if(!confirm('Supprimer cette analyse ?'))return;nutrition.contextAnalyses=contextAnalyses.filter(x=>x.id!==b.dataset.deleteContextAnalysis);saveDatabase(db);renderNutritionAnalysis();});
...
function supplementationCardHtml(visit){const sup=ensureSupplementation(visit),products=supplementProducts(),prod=products.find(x=>x.id===sup.productId),rows=mineralCoverageRows(visit);return `<section class="card" style="margin-top:16px"><div class="section-title"><div><h3>🧂 Couverture minérale globale</h3><div class="muted">Fourrages / ration + aliments / concentrés + minéral ou bolus. Les apports sont additionnés avant comparaison aux repères internes.</div></div><button class="btn secondary" id="open-supp-settings">⚙️ Gérer les produits & besoins</button></div><div class="grid supplement-entry-grid"><div class="field"><label>Catégorie</label><select data-sup-field="category">${feedingCategories.map(v=>`<option ${sup.category===v?'selected':''}>${v}</option>`).join('')}</select></div><div class="field"><label>Ingestion MS (kg/j)</label><input inputmode="decimal" data-sup-field="dmIntake" value="${escapeHtml(sup.dmIntake||'')}" placeholder="Laisser vide pour calcul auto"><small class="muted">Si vide, calculée à partir des aliments disposant d’une MS ou d’une valeur type.</small></div><div class="field"><label>Minéral / bolus</label><select data-sup-field="productId"><option value="">Aucun produit sélectionné</option>${products.map(x=>`<option value="${x.id}" ${sup.productId===x.id?'selected':''}>${escapeHtml(x.name)}</option>`).join('')}</select></div><div class="field"><label>Dose réelle / animal</label><div class="supplement-dose-row"><input inputmode="decimal" data-sup-field="dose" value="${escapeHtml(sup.dose||'')}" placeholder="Dose"><select data-sup-field="doseUnit">${['g/j','kg/j','bolus'].map(v=>`<option ${sup.doseUnit===v?'selected':''}>${v}</option>`).join('')}</select></div></div></div>${prod?`<div class="notice"><strong>${escapeHtml(prod.name)}</strong> — ${escapeHtml(prod.type||'Complément')} · ${prod.type==='Bolus'?`libération renseignée : ${escapeHtml(prod.releaseDays||'—')} jours`:'composition exprimée en mg/kg de produit'}.</div>`:'<div class="notice">Aucun minéral/bolus sélectionné. Les apports des fourrages et des aliments complémentaires restent néanmoins calculés.</div>'}<div class="table-wrap"><table class="compact-table"><thead><tr><th>Élément</th><th>Repère</th><th>Besoin estimé</th><th>Fourrages / ration</th><th>Aliments / concentrés</th><th>Minéral / bolus</th><th>Total</th><th>Couverture</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${escapeHtml(r.label)}</td><td>${r.target} ${r.targetUnit}</td><td>${r.need===null?'—':r.need.toFixed(1)+' '+r.unit}</td><td>${r.k==='i'?'—':r.feed.toFixed(1)+' '+r.unit}</td><td>${r.food.toFixed(1)} ${r.unit}</td><td>${r.add.toFixed(1)} ${r.unit}</td><td>${r.total.toFixed(1)} ${r.unit}</td><td>${r.coverage===null?'—':`<span class="badge ${r.coverage<80?'danger':r.coverage<95?'warning':r.coverage>180?'warning':'positive'}">${Math.round(r.coverage)} %</span>`}</td></tr>`).join('')}</tbody></table></div><p class="muted small-text">⚠️ Calcul indicatif. Les valeurs types, les repères de besoins, la biodisponibilité et les antagonismes peuvent modifier la couverture biologique. Le calcium, phosphore, magnésium et sodium sont comparés à des repères de concentration de ration en % MS ; les oligo-éléments à des mg/kg MS.</p><div class="field"><label>Commentaire complémentation</label><textarea data-sup-field="notes">${escapeHtml(sup.notes||'')}</textarea></div></section>`;}



function ensureQuickFieldNotes(visit){
  visit.fieldNotes = visit.fieldNotes && typeof visit.fieldNotes==='object' ? visit.fieldNotes : {raw:'',items:[],integratedAt:''};
  visit.fieldNotes.items = Array.isArray(visit.fieldNotes.items) ? visit.fieldNotes.items : [];
  return visit.fieldNotes;
}
function classifyQuickNote(text=''){
  const t=normalizeSearchText(text);
  const rules=[
    ['Alimentation / ration',/(ration|aliment|ensilage|foin|paille|concentre|mineral|sel|melangeuse|distribution|fourrage|paturage|prairie|fumure|chaulage|complement)/g],
    ['Eau / bâtiment',/(eau|abreuv|pipette|palette|bac|reseau|batiment|ventilation|litiere|aire|couchage|logette|temperature|humidite|courant d air|fumiere)/g],
    ['Reproduction',/(repro|velage|ivv|chaleur|saillie|insemin|avort|gestation|taureau|genisse.*vel)/g],
    ['Mortalité / sanitaire',/(mort|mortal|maladie|diarrhee|pneum|boiter|mammit|sanitaire|traitement|veto|vaccin|tique|mouche|deltamil|parasite)/g],
    ['Animaux / observations',/(vache|veau|genisse|animal|troupeau|etat corporel|nec|bouse|urine|poil|lot|cohorte)/g],
    ['Organisation / économie',/(travail|main d oeuvre|organisation|achat|vente|cout|prix|temps|objectif|projet|facture|analyse.*facture)/g]
  ];
  let best='À classer',score=0;
  for(const [cat,rx] of rules){const m=t.match(rx);const n=m?m.length:0;if(n>score){score=n;best=cat;}}
  return best;
}
function quickNoteHeadingCategory(text=''){
  const t=normalizeSearchText(String(text).replace(/[:–—-]+$/,'').trim());
  if(!t || t.length>80)return '';
  const headings=[
    ['Alimentation / ration',/(^| )(alimentation|ration|fourrage|paturage|prairie|complementation|mineralisation)( |$)/],
    ['Eau / bâtiment',/(^| )(eau|abreuvement|abreuvage|batiment|logement|litiere|ambiance|fumier|fumiere)( |$)/],
    ['Reproduction',/(^| )(reproduction|repro|velage|fertilite|taureau|saillie)( |$)/],
    ['Mortalité / sanitaire',/(^| )(sanitaire|sante|mortalite|pathologie|traitement|parasites|parasitisme)( |$)/],
    ['Animaux / observations',/(^| )(animaux|troupeau|observations|lots|genisses|veaux|vaches)( |$)/],
    ['Organisation / économie',/(^| )(organisation|economie|economique|travail|projet|documents|analyses a recuperer)( |$)/]
  ];
  return headings.find(([,rx])=>rx.test(t))?.[0]||'';
}
function groupQuickNotes(rawText=''){
  const raw=String(rawText||'').replace(/\r/g,'').trim(); if(!raw)return [];
  const groups=new Map(),order=['Alimentation / ration','Eau / bâtiment','Reproduction','Mortalité / sanitaire','Animaux / observations','Organisation / économie','À classer'];
  const add=(cat,text)=>{const clean=String(text||'').replace(/^[•\-–]\s*/,'').replace(/\s+/g,' ').trim();if(!clean)return;const key=cat||'À classer';if(!groups.has(key))groups.set(key,[]);const arr=groups.get(key);if(!arr.some(x=>normalizeSearchText(x)===normalizeSearchText(clean)))arr.push(clean);};
  let explicit='';
  for(const source of raw.split(/\n+/).map(x=>x.trim()).filter(Boolean)){
    const heading=quickNoteHeadingCategory(source),looksHeading=heading&&(source.endsWith(':')||source.length<48||/^#{1,3}\s/.test(source));
    if(looksHeading){explicit=heading;continue;}
    let parts=source.split(/(?<=[.!?;])\s+|\s+[–—]\s+/).map(x=>x.trim()).filter(Boolean);
    if(parts.length===1&&source.length>350)parts=source.split(/\s+(?=(?:au niveau de|concernant|pour ce qui est de|côté|en reproduction|en alimentation|pour l'eau|pour les vaches|pour les génisses|sanitaire|au pâturage|dans le bâtiment)\b)/i).map(x=>x.trim()).filter(Boolean);
    for(const part of parts)add(explicit||classifyQuickNote(part),part);
  }
  return order.filter(k=>groups.has(k)).map(category=>({category,text:groups.get(category).map(x=>`• ${x}`).join('\n'),use:true}));
}
function appendSectionNote(target,text){
  const add=(old,label)=>{const block=`${label ? '['+label+'] ' : ''}${text}`;return String(old||'').trim()?`${String(old).trim()}\n${block}`:block;};
  const visit=activeVisit(); if(!visit)return;
  const ag=ensureAuditGlobal(visit);
  const addChapter=(id,label)=>{
    ag.chapterSummaries[id]=ag.chapterSummaries[id]||{};
    ag.chapterSummaries[id].comments=add(ag.chapterSummaries[id].comments,label||'Notes terrain');
  };
  if(target==='Alimentation / ration'){
    visit.feeding=visit.feeding&&typeof visit.feeding==='object'?visit.feeding:{rations:[],settings:{},history:[]};
    visit.feeding.settings=visit.feeding.settings&&typeof visit.feeding.settings==='object'?visit.feeding.settings:{};
    visit.feeding.settings.generalComment=add(visit.feeding.settings.generalComment,'Notes terrain');
    addChapter('fourrages','Alimentation / ration');
  }else if(target==='Eau / bâtiment'){
    addChapter('pratiques','Eau / bâtiment');
  }else if(target==='Reproduction'){
    addChapter('reproduction','Reproduction');
  }else if(target==='Mortalité / sanitaire'){
    addChapter('sanitaire','Mortalité / sanitaire');
  }else if(target==='Animaux / observations'){
    addChapter('pratiques','Animaux / observations');
  }else if(target==='Organisation / économie'){
    addChapter('organisation','Organisation / économie');
  }
  ag.notes=add(ag.notes,target);
}
function exportQuickFieldNotes(visit,n){
  const farm=db.farms?.find?.(f=>f.id===visit.farmId);
  const title=`NOTES TERRAIN — ${farm?.name||'Exploitation'}`;
  const grouped=(n.items||[]).filter(x=>x?.text?.trim()).map(x=>`## ${x.category||'À classer'}\n${x.text.trim()}`).join('\n\n');
  const body=[title,`EDE : ${farm?.farmNumber||farm?.ede||'—'}`,`Visite : ${visit.date?formatDate(visit.date):'—'}`,`Export : ${new Date().toLocaleString('fr-FR')}`,'','=== NOTES BRUTES ===',String(n.raw||'').trim()||'(aucune note)','',grouped?'=== CLASSEMENT PROPOSÉ ===\n'+grouped:''].join('\n');
  const blob=new Blob(['\ufeff'+body],{type:'text/plain;charset=utf-8'}),url=URL.createObjectURL(blob),a=document.createElement('a');
  a.href=url;a.download=`notes-terrain-${slugify(farm?.name||'exploitation')}-${visit.date||new Date().toISOString().slice(0,10)}.txt`;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),3000);
}
const __quickFieldNotesUiState=new Map();
function rememberQuickFieldNotesUi(visitId,raw){
  if(!visitId||!raw)return;
  __quickFieldNotesUiState.set(visitId,{scrollTop:raw.scrollTop,selectionStart:raw.selectionStart,selectionEnd:raw.selectionEnd,pageY:window.scrollY});
}
function restoreQuickFieldNotesUi(visitId,raw){
  const st=__quickFieldNotesUiState.get(visitId);if(!st||!raw)return;
  requestAnimationFrame(()=>{
    raw.scrollTop=st.scrollTop||0;
    try{raw.setSelectionRange(st.selectionStart??0,st.selectionEnd??st.selectionStart??0);}catch(_){ }
    if(Number.isFinite(st.pageY))window.scrollTo({top:st.pageY,left:window.scrollX,behavior:'instant'});
  });
}
let __fieldNotesRecognition=null;
function renderQuickFieldNotes(){
  const visit=activeVisit();
  if(!visit){renderNoActiveVisit('Notes terrain');return;}
  const n=ensureQuickFieldNotes(visit);
  app.innerHTML=`<div class="section-title"><div><h2>📝 Notes terrain</h2><div class="muted">Zone libre pour noter les informations recueillies pendant la visite et les retrouver ensuite pour compléter l’audit.</div></div><span class="badge autosave">Sauvegarde automatique</span></div>
  ${activeVisitBanner(visit)}
  <section class="card quick-field-notes">
    <div class="notice"><strong>Notes libres :</strong> saisissez ou collez ici tout ce que vous souhaitez conserver. Les notes restent liées à cette visite.</div>
    <div class="field"><label>Notes prises pendant la visite</label><textarea id="quick-field-notes-raw" rows="28" style="min-height:620px;resize:vertical" placeholder="Saisissez vos notes ici…">${escapeHtml(n.raw||'')}</textarea></div>
    <div class="actions quick-notes-actions">
      <button class="btn secondary" id="quick-notes-export">⬇️ Exporter les notes</button>
    </div>
  </section>`;
  const raw=document.getElementById('quick-field-notes-raw');
  restoreQuickFieldNotesUi(visit.id,raw);
  const saveRaw=()=>{rememberQuickFieldNotesUi(visit.id,raw);n.raw=raw.value;n.updatedAt=new Date().toISOString();visit.updatedAt=n.updatedAt;saveDatabase(db);};
  raw.addEventListener('scroll',()=>rememberQuickFieldNotesUi(visit.id,raw),{passive:true});
  raw.oninput=saveRaw;raw.onblur=saveRaw;raw.onkeyup=()=>rememberQuickFieldNotesUi(visit.id,raw);raw.onclick=()=>rememberQuickFieldNotesUi(visit.id,raw);
  document.getElementById('quick-notes-export').onclick=()=>{saveRaw();exportQuickFieldNotes(visit,n);showToast('Notes exportées en fichier texte.');};
}
...
function renderFeeding() {
  ...
  document.getElementById('add-feed-product')?.addEventListener('click',()=>{ensureFeedProducts(visit).push(newFeedProduct());visit.updatedAt=new Date().toISOString();saveDatabase(db);renderFeeding();});
  document.getElementById('use-saved-feed-product')?.addEventListener('click',()=>{const id=document.getElementById('saved-feed-product-select')?.value,src=savedFeedProducts().find(x=>x.id===id);if(!src){showToast('Choisissez un aliment enregistré.');return;}ensureFeedProducts(visit).push(feedProductTemplateCopy(src));visit.updatedAt=new Date().toISOString();saveDatabase(db);renderFeeding();showToast('Aliment repris dans cette visite.');});
  app.querySelectorAll('[data-save-feed-product]').forEach(b=>b.onclick=()=>{const src=ensureFeedProducts(visit).find(x=>x.id===b.dataset.saveFeedProduct);if(!src)return;const lib=savedFeedProducts(),key=`${String(src.name||'').trim().toLowerCase()}|${String(src.manufacturer||'').trim().toLowerCase()}`,existing=lib.find(x=>`${String(x.name||'').trim().toLowerCase()}|${String(x.manufacturer||'').trim().toLowerCase()}`===key);const copy=JSON.parse(JSON.stringify(src));delete copy.id;delete copy.dose;copy.doseUnit=src.doseUnit||'kg/j';if(existing){Object.assign(existing,copy);showToast('Aliment enregistré mis à jour.');}else{lib.push({id:uid('savedfeed'),...copy});showToast('Aliment enregistré pour les prochaines visites.');}saveDatabase(db);renderFeeding();});
  app.querySelectorAll('[data-feed-product-field]').forEach(el=>{const save=()=>{const p=ensureFeedProducts(visit).find(x=>x.id===el.dataset.id);if(!p)return;p[el.dataset.feedProductField]=el.value;visit.updatedAt=new Date().toISOString();saveDatabase(db);};el.oninput=save;el.onblur=save;el.onchange=()=>{save();renderFeeding();};});
  app.querySelectorAll('[data-delete-feed-product]').forEach(b=>b.onclick=()=>{if(!confirm('Supprimer cet aliment complémentaire ?'))return;visit.feeding.feedProducts=ensureFeedProducts(visit).filter(x=>x.id!==b.dataset.deleteFeedProduct);visit.updatedAt=new Date().toISOString();saveDatabase(db);renderFeeding();});
...
function printBaseStyles(){return `@page{size:A4 portrait;margin:12mm 10mm 14mm 10mm}html{font-size:12pt}body{font-family:Arial,sans-serif;color:#172033;margin:0;font-size:11.7pt;line-height:1.42}h1{color:#b53670;break-after:avoid-page;page-break-after:avoid}h2{color:#8e2e5b;border-bottom:2px solid #efd5e1;padding-bottom:4px;break-after:avoid-page;page-break-after:avoid}h3,h4,h5{break-after:avoid-page;page-break-after:avoid}p,li{orphans:3;widows:3}.print-note,.known,.box,.decision-alert,.print-metric{break-inside:avoid-page;page-break-inside:avoid}.print-note{background:#fff6fb;border:1px solid #e7bfd2;padding:8px;border-radius:7px;margin:8px 0}.source{display:inline-block;border-radius:10px;padding:2px 6px;font-size:8.8pt;margin-right:5px}.source.ask{background:#e8f1ff}.source.measure{background:#fff1d9}.source.later{background:#f1e7ff}.source.calc{background:#e5f7e8}.qid{font-size:8.3pt;color:#6d7280;font-family:monospace}.known{background:#eef9f0;border-left:4px solid #3c9b59;padding:5px 7px;margin:4px 0}.paper-choice{display:inline-block;margin:1px 8px 2px 0;white-space:nowrap}.paper-box{font-size:13pt;vertical-align:-1px}.write-line{min-height:20px;border-bottom:1px solid #9aa1ab;margin-top:4px}.comment-lines{height:34px;background:repeating-linear-gradient(to bottom,transparent 0,transparent 16px,#c5c8ce 17px)}table{width:100%;border-collapse:collapse;margin-bottom:18px}tr{page-break-inside:avoid}th,td{border:1px solid #aab5ad;padding:6px;vertical-align:top;font-size:10.3pt;line-height:1.35}th{background:#fbeaf2}.meta,.summary-print{display:grid;grid-template-columns:repeat(3,1fr);gap:9px;margin:15px 0}.box{border:1px solid #aab5ad;padding:10px;min-height:44px}.blank-line{height:24px}.writing{min-height:40px}.checks{line-height:1.6}.landscape{font-size:9pt}.prelev-table td{height:24px}.prelev-head{background:#eef5ff}.prepared-tag{color:#267642;font-weight:bold}.page-break{page-break-before:always}.repro-age-overview.grouped{display:grid;grid-template-columns:minmax(250px,340px) 1fr;gap:18px;align-items:start}.repro-age-chart-panel{display:flex;flex-direction:column;gap:8px;align-items:center}.repro-age-donut-svg{width:100%;max-width:430px;height:auto;overflow:visible}.repro-age-donut-total{font-size:24px;font-weight:800;fill:#222}.repro-age-donut-sub{font-size:11px;fill:#69747a}.repro-age-donut-label line{stroke:#9a8a93;stroke-width:1}.repro-age-donut-label text{font-size:10px;fill:#394248}.repro-age-donut-label .bucket{font-weight:800}.repro-age-donut-label .count{fill:#6d7580}.repro-age-bucket-legend{display:flex;flex-wrap:wrap;gap:6px;justify-content:center;margin:8px 0 10px}.repro-age-bucket-legend span{display:inline-flex;align-items:center;gap:5px;padding:4px 8px;border:1px solid #ead4de;border-radius:999px;background:#fff6fa;font-size:9pt}.repro-age-bucket-legend i,.repro-age-legend-item i{width:11px;height:11px;border-radius:50%;display:inline-block;flex:0 0 auto}.repro-age-year-title{font-weight:800;margin-bottom:7px;color:#8e2e5b}.repro-age-legend{display:grid;grid-template-columns:repeat(auto-fit,minmax(170px,1fr));gap:6px}.repro-age-legend-item{border:1px solid #e3d8dd;background:#fff;border-radius:9px;padding:7px 9px;display:flex;align-items:center;gap:7px;text-align:left}.repro-penalty-chip{display:inline-block;padding:3px 6px;border-radius:7px;background:#fff0c9;border:1px solid #efd38a;color:#6f5200;font-weight:700}.repro-penalty-chip.severe,.report-penalty-hit.severe{background:#ffd9d6!important;border-color:#f0a29c!important;color:#8e2121}.report-penalty-hit{background:#fff1c7!important;color:#9a6700!important;font-weight:800!important;-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}.report-score-good{background:#dff3e6!important;color:#176238!important;font-weight:800!important}.report-score-warning{background:#fff0c7!important;color:#775800!important;font-weight:800!important}.report-score-danger{background:#ffd9d6!important;color:#8e2121!important;font-weight:800!important}.score-ok{color:#176238!important;background:#dff3e6!important;font-weight:800!important}.score-warn{color:#9a6700!important;background:#fff0c7!important;font-weight:800!important}.score-bad{color:#a01818!important;background:#ffd9d6!important;font-weight:800!important}.repro-score-hit-row td{background:#fff4d5}.decision-support.for-print{border:1px solid #d9d9d9;padding:8px;margin:8px 0 12px}.decision-theme-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:5px}.decision-theme{border:1px solid #bbb;padding:5px}.decision-theme.good{background:#e5f5ea}.decision-theme.warn{background:#fff1c7}.decision-theme.bad{background:#ffd9d6}.decision-theme.pending{background:#f3f5f7;border-left:4px solid #8a94a3}.decision-theme.pending span{font-weight:800;color:#596574}.decision-theme strong,.decision-theme span,.decision-theme small{display:block}.decision-alert{border:1px solid #ccc;padding:5px;margin:4px 0}.decision-alert.sev-3{border-left:5px solid #a01818}.decision-alert.sev-2{border-left:5px solid #9a6700}.decision-alert small{display:block}.decision-trend-good{color:#176238!important;background:#e5f5ea!important;font-weight:800!important}.decision-trend-bad{color:#a01818!important;background:#fde4e1!important;font-weight:800!important}.decision-support *{-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}@media print{button{display:none}.smart-page-before{break-before:page!important;page-break-before:always!important}.landscape-page{page:landscape}h1,h2,h3,h4,h5{break-after:avoid-page!important;page-break-after:avoid!important;margin-bottom:8px}h1+*,h2+*,h3+*,h4+*,h5+*{break-before:avoid-page!important;page-break-before:avoid!important}.report-section,.decision-support,.decision-onepage{break-before:auto;page-break-before:auto}p{orphans:4;widows:4}li{orphans:3;widows:3;break-inside:avoid-page;page-break-inside:avoid}table{break-inside:auto;page-break-inside:auto}thead{display:table-header-group}tfoot{display:table-footer-group}tr{break-inside:avoid-page;page-break-inside:avoid}.print-note,.known,.box,.decision-alert,.print-metric,.herd-chart-card,.targeted-audit-print,figure{break-inside:avoid-page!important;page-break-inside:avoid!important}}`}
...
function fullVisitPaperHtml(visit,filled){return (filled?targetedAuditHtml(visit,{forPrint:true}):'')+samplingPrintHtml(visit,filled)+feedingPrintHtml(visit,filled)+buildingPrintHtml(visit,filled)+auditPrintHtml(visit,filled)}
function printAuditDocument(visit,mode){
  const filled=mode==='audit-filled'||mode==='full-prepared';let title,content;
  if(mode==='full-blank'){title='Support papier complet vierge';content=fullVisitPaperHtml(visit,false)}
  else if(mode==='full-prepared'){title='Support papier complet préparé';content=fullVisitPaperHtml(visit,true)}
  else if(mode==='analysis-blank'){title='Grilles analyses vierges';content=samplingPrintHtml(visit,false)}
  else if(mode==='audit-blank'){title='Audit vierge';content=auditPrintHtml(visit,false)}
  else{title='Audit renseigné';content=auditPrintHtml(visit,true)}
  const w=window.open('','_blank');if(!w){showToast('Autorisez les fenêtres surgissantes.');return}
  w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>${escapeHtml(title)}</title><style>${printBaseStyles()} .print-toolbar{position:sticky;top:0;z-index:20;background:#fff;padding:9px 0 10px;border-bottom:1px solid #ddd;margin-bottom:12px}.print-toolbar button{margin-right:8px}@media print{.print-toolbar{display:none!important}}</style></head><body><div class="print-toolbar"><button onclick="window.print()">🖨️ Imprimer / Enregistrer en PDF</button><button onclick="if(window.opener){window.close()}else if(history.length>1){history.back()}">↩ Fermer / retour audit</button></div><h1>Audit Bovin GDS 32-65 — ${escapeHtml(title)}</h1><div class="meta"><div class="box"><b>Exploitation</b><br>${mode.includes('blank')?'':escapeHtml(farmName(visit.farmId))}</div><div class="box"><b>Date</b><br>${mode.includes('blank')?'':escapeHtml(formatDate(visit.date))}</div><div class="box"><b>Technicien</b><br>${mode.includes('blank')?'':escapeHtml(visit.technician||'')}</div></div>${content}</body></html>`);
...
function renderFollowup(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Historique & évolution</h2><div class="muted">Comparer deux visites d’une même exploitation : reproduction, mortalité, mesures, alimentation, bâtiment et plans d’action.</div></div><span class="badge autosave">v14.6.21.123</span></div>
...
const TRACE_ROLE_GROUPS=[
  {id:'reproduction',label:'Reproduction / fertilité',icon:'🐄',elements:['cuivre','manganese','selenium','zinc','iode','cobalt'],why:'Fertilité, cyclicité, développement embryonnaire, qualité des gamètes, immunité et vigueur des veaux.'},
  {id:'immunite',label:'Immunité / infections',icon:'🛡️',elements:['cuivre','zinc','selenium','cobalt'],why:'Défenses immunitaires, réponse antioxydante, cicatrisation et résistance aux infections.'},
  {id:'croissance',label:'Croissance / veaux / squelette',icon:'🐮',elements:['manganese','cuivre','zinc','selenium','cobalt'],why:'Croissance, ossification, métabolisme énergétique et vigueur des jeunes.'},
  {id:'peau',label:'Peau / poils / corne / onglons',icon:'🦶',elements:['zinc','cuivre','selenium'],why:'Kératinisation, cicatrisation, pigmentation, qualité de peau et de corne.'},
  {id:'anemie',label:'Anémie / baisse d’état',icon:'🩸',elements:['fer','cuivre','cobalt'],why:'Transport de l’oxygène, utilisation du fer et synthèse de vitamine B12.'},
  {id:'antioxydant',label:'Stress oxydatif / délivrance / muscle',icon:'⚡',elements:['selenium','zinc','cuivre'],why:'Défense antioxydante, immunité, muscle et récupération autour du vêlage.'}
];
const TRACE_ANTAGONISMS={
  cuivre:[
    {key:'molybdene',label:'Molybdène (Mo)',detail:'Avec le soufre, peut former des thiomolybdates dans le rumen et diminuer fortement la disponibilité du cuivre.'},
    {key:'soufre',label:'Soufre / sulfates (S)',detail:'Un apport élevé, surtout associé au molybdène, peut réduire l’utilisation du cuivre.'},
    {key:'fer',label:'Fer (Fe)',detail:'Des apports élevés peuvent réduire le statut cuivre et aussi pénaliser le manganèse.'},
    {key:'zinc',label:'Zinc (Zn)',detail:'Un excès de zinc peut concurrencer le cuivre au niveau de l’absorption.'}
  ],
  manganese:[
    {key:'fer',label:'Fer (Fe)',detail:'Un apport élevé en fer peut diminuer l’absorption du manganèse.'},
    {key:'calcium',label:'Calcium (Ca)',detail:'Des apports élevés en calcium peuvent réduire la disponibilité du manganèse.'},
    {key:'phosphore',label:'Phosphore (P)',detail:'Des apports élevés en phosphore peuvent réduire la disponibilité du manganèse.'}
  ],
  zinc:[
    {key:'calcium',label:'Calcium (Ca)',detail:'Des apports élevés en calcium peuvent diminuer la disponibilité du zinc.'},
    {key:'cuivre',label:'Cuivre (Cu)',detail:'Cuivre et zinc partagent certaines voies d’absorption : l’équilibre global de la ration est à considérer.'}
  ],
  selenium:[
    {key:'soufre',label:'Soufre / sulfates (S)',detail:'Le statut soufre et les conditions sol–plante peuvent modifier la disponibilité du sélénium ; à raisonner sur l’ensemble de la chaîne alimentaire.'}
  ],
  magnesium:[
    {key:'potassium',label:'Potassium (K)',detail:'Un potassium élevé, notamment sur herbe jeune, peut diminuer l’absorption du magnésium.'}
  ],
  fer:[
    {key:'cuivre',label:'Cuivre (Cu)',detail:'Le métabolisme du fer dépend en partie du statut cuivre ; une anomalie de fer ne doit pas être interprétée isolément.'}
  ],
  iode:[],cobalt:[]
};
function traceKeyFromText(name=''){
  const n=normalizeSearchText(name).replace(/[^a-z0-9à-ÿ]+/g,' ').trim();
  const tests=[
    ['molybdene',/(^| )molybdene( |$)|(^| )mo( |$)/],['soufre',/(^| )(soufre|sulfate|sulfates)( |$)|(^| )s( |$)/],
    ['cuivre',/(^| )cuivre( |$)|(^| )cu( |$)/],['selenium',/(^| )selenium( |$)|(^| )se( |$)/],['zinc',/(^| )zinc( |$)|(^| )zn( |$)/],
    ['iode',/(^| )iode( |$)|(^| )iodine( |$)/],['cobalt',/(^| )cobalt( |$)|(^| )co( |$)/],['manganese',/(^| )manganese( |$)|(^| )mn( |$)/],
    ['fer',/(^| )fer( |$)|(^| )iron( |$)|(^| )fe( |$)/],['calcium',/(^| )calcium( |$)|(^| )ca( |$)/],['magnesium',/(^| )magnesium( |$)|(^| )mg( |$)/],
    ['phosphore',/(^| )phosphore( |$)|(^| )phosphorus( |$)|(^| )p( |$)/],['potassium',/(^| )potassium( |$)|(^| )k( |$)/],['sodium',/(^| )sodium( |$)|(^| )na( |$)/]
  ];
  for(const [k,rx] of tests)if(rx.test(n))return k;
  return '';
}
function traceForageFieldForKey(key){return {cuivre:'cu_mgkg',zinc:'zn_mgkg',manganese:'mn_mgkg',fer:'fe_mgkg',calcium:'ca_gkg',magnesium:'mg_gkg',phosphore:'p_gkg',potassium:'k_gkg',sodium:'na_gkg'}[key]||'';}
function traceDataPoints(visit,key){
  const out=[],push=(layer,label,value,unit='',status='',detail='')=>{if(value===undefined||value===null||String(value).trim()==='')return;out.push({layer,label,value:String(value),unit:unit||'',status:status||'',detail:detail||''});};
  const m=ensureMetabolic(visit);
  (m.rows||[]).forEach(r=>{if(traceKeyFromText(r.analyte)===key)push('animal',`${m.sampleType||'Animal'} · ${r.animal||m.category||'lot'}`,r.value,r.unit,metabolicStatus(r),m.lab||'');});
  const nutrition=visit.feeding?.nutrition||{};
  (nutrition.forageAnalyses||[]).forEach(a=>{const f=traceForageFieldForKey(key),lab=a.lab||{};if(f&&lab[f]!=null)push('fourrage',a.name||'Fourrage',lab[f],f.includes('mgkg')?'mg/kg MS':'g/kg MS','',a.source||'');const other=String(lab.other||'');if(other&&traceKeyFromText(other)===key)push('fourrage',a.name||'Fourrage','présent dans remarques','','',other);});
  (nutrition.contextAnalyses||[]).forEach(a=>(a.rows||[]).forEach(r=>{if(traceKeyFromText(r.parameter)===key)push(a.kind==='SOL'?'sol':'vegetal',a.name||a.kind,r.value,r.unit,'',r.comment||a.comment||'');}));
  const w=ensureWaterLab(visit);(w.points||[]).forEach(pt=>(pt.rows||[]).forEach(r=>{if(traceKeyFromText(r.parameter)===key)push('eau',pt.name||'Eau',r.value,r.unit,waterStatus(r),r.comment||'');}));
  (ensureFeedProducts(visit)||[]).forEach(p=>{const field={cuivre:'cu',zinc:'zn',manganese:'mn',selenium:'se',iode:'i',cobalt:'co',fer:'fe',calcium:'ca',magnesium:'mg',phosphore:'p'}[key];if(field&&p[field]!=null&&String(p[field]).trim()!=='')push('ration',p.name||'Aliment',p[field],['ca','mg','p'].includes(field)?'% produit':'mg/kg produit','','étiquette');});
  if(['cuivre','zinc','manganese','selenium','iode','cobalt','calcium','magnesium','phosphore'].includes(key)){
    const ck={cuivre:'cu',zinc:'zn',manganese:'mn',selenium:'se',iode:'i',cobalt:'co',calcium:'ca',magnesium:'mg',phosphore:'p'}[key];
    const cov=mineralCoverageRows(visit).find(x=>x.k===ck);if(cov&&cov.coverage!=null)push('ration','Couverture ration calculée',Math.round(cov.coverage),'% du besoin',cov.coverage<80?'bas':cov.coverage>180?'haut':'normal',`${Math.round(cov.total*10)/10} ${cov.unit}`);
  }
  return out;
}
function traceElementAssessment(visit,key){
  const k=METABOLIC_KNOWLEDGE[key]||{label:key},pts=traceDataPoints(visit,key),animal=pts.filter(x=>x.layer==='animal'),ration=pts.filter(x=>x.layer==='ration');
  let level='neutral',headline='Pas de dosage animal classable';
  if(animal.some(x=>x.status==='bas')){level='danger';headline='Valeur animale basse / déficit possible';}
  else if(animal.some(x=>x.status==='haut')){level='warning';headline='Valeur animale haute / déséquilibre possible';}
  else if(animal.some(x=>x.status==='normal')){level='positive';headline='Valeur animale dans le repère utilisé';}
  const cov=ration.find(x=>x.label==='Couverture ration calculée');if(cov&&cov.status==='bas'&&level!=='danger'){level='warning';headline='Couverture ration calculée insuffisante';}
  const ant=(TRACE_ANTAGONISMS[key]||[]).map(a=>({def:a,points:traceDataPoints(visit,a.key)})).filter(x=>x.points.length);
  return {key,label:k.label||key,level,headline,points:pts,antagonists:ant};
}
function traceRoleDetailHtml(visit,roleId){
  const role=TRACE_ROLE_GROUPS.find(x=>x.id===roleId)||TRACE_ROLE_GROUPS[0],items=role.elements.map(k=>traceElementAssessment(visit,k));
  return `<article class="trace-role-detail"><div class="section-title"><div><h4>${role.icon} ${escapeHtml(role.label)}</h4><div class="muted">${escapeHtml(role.why)}</div></div></div><div class="trace-element-grid">${items.map(it=>`<div class="trace-element-card ${it.level}"><div><strong>${escapeHtml(it.label)}</strong><span class="badge ${it.level==='danger'?'danger':it.level==='warning'?'warning':it.level==='positive'?'complete':''}">${escapeHtml(it.headline)}</span></div>${it.points.length?`<ul class="small-text">${it.points.slice(0,6).map(p=>`<li><b>${escapeHtml(p.layer)}</b> — ${escapeHtml(p.label)} : ${escapeHtml(p.value)}${p.unit?' '+escapeHtml(p.unit):''}${p.status&&p.status!=='non-classe'?` <em>(${escapeHtml(p.status)})</em>`:''}</li>`).join('')}</ul>`:'<p class="muted small-text">Aucune donnée disponible dans les analyses/ration.</p>'}${it.antagonists.length?`<div class="trace-antagonists"><strong>Antagonistes / interactions à vérifier</strong>${it.antagonists.map(a=>`<p><b>${escapeHtml(a.def.label)}</b> — ${escapeHtml(a.def.detail)}<br><small>Donnée(s) retrouvée(s) : ${a.points.slice(0,3).map(p=>`${escapeHtml(p.label)} ${escapeHtml(p.value)}${p.unit?' '+escapeHtml(p.unit):''}`).join(' · ')}</small></p>`).join('')}</div>`:''}</div>`).join('')}</div><p class="muted small-text">⚠️ Une teneur mesurée ne prouve pas à elle seule un défaut d’assimilation. Les interactions proposées servent à orienter le raisonnement et doivent être replacées dans la ration totale, la matrice analysée, le stade physiologique et le contexte clinique.</p></article>`;
}
function traceFunctionalGuideHtml(visit){
  return `<section class="card trace-guide"><div class="section-title"><div><h3>🧭 Lecture fonctionnelle des oligo-éléments</h3><div class="muted">Partir d’un problème observé, voir quels éléments explorer, puis croiser les dosages et les antagonismes disponibles.</div></div></div><div class="field"><label>Problématique à explorer</label><select id="trace-role-select">${TRACE_ROLE_GROUPS.map(r=>`<option value="${r.id}">${r.icon} ${escapeHtml(r.label)}</option>`).join('')}</select></div><div id="trace-role-detail">${traceRoleDetailHtml(visit,'reproduction')}</div><details><summary><strong>Voir tous les rôles par élément</strong></summary><div class="table-wrap"><table class="compact-table"><thead><tr><th>Élément</th><th>Rôles principaux</th><th>Interactions importantes</th></tr></thead><tbody>${['cuivre','zinc','manganese','selenium','cobalt','iode','fer'].map(k=>{const x=METABOLIC_KNOWLEDGE[k],ants=TRACE_ANTAGONISMS[k]||[];return `<tr><td><strong>${escapeHtml(x.label)}</strong></td><td>${escapeHtml(x.impacts.join(' · '))}</td><td>${ants.length?ants.map(a=>escapeHtml(a.label)).join(' · '):'Pas d’antagonisme majeur intégré'}</td></tr>`}).join('')}</tbody></table></div></details></section>`;
}
function compactUniqueNotes(values,limit=3){
  const seen=new Set(),out=[];
  const feed=(value)=>{
    String(value||'').split(/\s*·\s*|\n+/).map(x=>x.trim()).filter(Boolean).forEach(x=>{
      const key=normalizeSearchText(x).replace(/\s+/g,' ').trim();
      if(!key||seen.has(key))return;
      seen.add(key);out.push(x);
    });
  };
  (Array.isArray(values)?values:[values]).forEach(feed);
  return out.slice(0,limit).join(' · ');
}
function appendUniqueNote(existing,note){
  return compactUniqueNotes([existing,note],50);
}
function cycleStageData(visit){
  const n=visit.feeding?.nutrition||{},ctx=n.contextAnalyses||[],sol=ctx.filter(a=>a.kind==='SOL'),veg=ctx.filter(a=>a.kind==='VEGETAL'),forages=n.forageAnalyses||[],w=ensureWaterLab(visit),m=ensureMetabolic(visit),coverage=mineralCoverageRows(visit),rationWarn=coverage.filter(r=>r.coverage!=null&&(r.coverage<80||r.coverage>180)),forageWarn=forages.flatMap(a=>interpretForage(a).filter(x=>x[0]!=='good').map(x=>x[1])),waterBad=(w.points||[]).flatMap(p=>(p.rows||[]).filter(r=>waterStatus(r)==='hors repère').map(r=>`${p.name||'Eau'} : ${r.parameter}`)),met=metabolicSummary(visit),animalBad=met.filter(x=>x.low||x.high);
  const solObs=compactUniqueNotes(sol.flatMap(a=>[a.comment,...(a.rows||[]).map(r=>r.comment)]),2);
  const forageObs=compactUniqueNotes([...forages.map(a=>a.comment),...veg.flatMap(a=>[a.comment,...(a.rows||[]).map(r=>r.comment)])],2);
  const waterObs=compactUniqueNotes([w.notes,...(w.points||[]).flatMap(p=>(p.rows||[]).map(r=>r.comment))],2);
  const rationObs=compactUniqueNotes([ensureSupplementation(visit).notes,...(visit.feeding?.rations||[]).map(r=>r.comment)],2);
  const animalObs=compactUniqueNotes([m.notes,...(m.rows||[]).map(r=>r.comment)],2);
  const joinWatch=(a,b)=>[a,b].filter(Boolean).join(' · ');
  return [
    {id:'sol',icon:'🌱',label:'Sol',present:sol.length>0,summary:sol.length?`${sol.length} analyse(s) de sol disponible(s)`:'Aucune analyse de sol',watch:joinWatch(sol.length?'Comparer disponibilité P/K/Mg/oligos avec végétaux et fourrages.':'',solObs)},
    {id:'fourrage',icon:'🌿',label:'Végétaux / fourrages',present:veg.length+forages.length>0,summary:`${veg.length} analyse(s) végétale(s) · ${forages.length} analyse(s) de fourrage`,watch:joinWatch(forageWarn.slice(0,3).join(' · '),forageObs)},
    {id:'eau',icon:'💧',label:'Eau',present:(w.points||[]).length>0,summary:(w.points||[]).length?`${w.points.length} point(s) analysé(s)`:'Aucune analyse d’eau',watch:joinWatch(waterBad.slice(0,3).join(' · '),waterObs)},
    {id:'ration',icon:'🥣',label:'Ration / complémentation',present:(visit.feeding?.rations||[]).length>0||coverage.some(x=>x.coverage!=null),summary:(visit.feeding?.rations||[]).length?`${visit.feeding.rations.length} ligne(s) de ration renseignée(s)`:'Couverture minérale partielle / non calculable',watch:joinWatch(rationWarn.slice(0,4).map(r=>`${r.label} ${Math.round(r.coverage)} %`).join(' · '),rationObs)},
    {id:'animal',icon:'🐄',label:'Animal',present:(m.rows||[]).length>0,summary:(m.rows||[]).length?`${m.rows.length} dosage(s) métabolique(s) / poils / sang`:'Aucun profil animal',watch:joinWatch(animalBad.slice(0,4).map(x=>`${x.label}: ${x.low?'bas':''}${x.low&&x.high?'/':''}${x.high?'haut':''}`).join(' · '),animalObs)}
  ];
}
function cycleCrossWarnings(visit){
  const out=[],cu=traceElementAssessment(visit,'cuivre');
  if(cu.level==='danger'&&cu.antagonists.length)out.push(`Cuivre bas : vérifier en priorité les antagonistes retrouvés (${cu.antagonists.map(x=>x.def.label).join(', ')}), avant d’interpréter comme un simple manque d’apport.`);
  const mn=traceElementAssessment(visit,'manganese');if(mn.level==='danger'&&mn.antagonists.length)out.push(`Manganèse bas : croiser avec ${mn.antagonists.map(x=>x.def.label).join(', ')} et la ration totale.`);
  const mg=traceElementAssessment(visit,'magnesium');if(mg.level==='danger'&&mg.antagonists.length)out.push(`Magnésium bas : potassium retrouvé dans les données ; vérifier l’équilibre K/Mg et le contexte pâturage.`);
  const cov=mineralCoverageRows(visit).filter(r=>r.coverage!=null&&r.coverage<80);if(cov.length)out.push(`Couverture ration calculée <80 % pour : ${cov.map(x=>x.label).join(', ')}.`);
  const animal=metabolicSummary(visit).filter(x=>x.low);if(animal.length)out.push(`Profil animal : valeur(s) basse(s) pour ${animal.map(x=>x.label).join(', ')}.`);
  return out;
}
function cycleSummaryHtml(visit,{report=false}={}){
  const stages=cycleStageData(visit),warnings=cycleCrossWarnings(visit),cls=report?'report-cycle':'cycle-summary';
  return `<div class="${cls}"><div class="cycle-flow">${stages.map((s,i)=>`${i?'<span class="cycle-arrow">→</span>':''}<article class="cycle-stage ${s.present?'has-data':'missing'}"><strong>${s.icon} ${escapeHtml(s.label)}</strong><span>${escapeHtml(s.summary)}</span>${s.watch?`<small><b>Vigilance :</b> ${escapeHtml(s.watch)}</small>`:''}</article>`).join('')}</div>${warnings.length?`<div class="notice warning"><strong>Points de vigilance transversaux</strong><ul>${warnings.map(x=>`<li>${escapeHtml(x)}</li>`).join('')}</ul></div>`:'<div class="notice"><strong>Lecture transversale :</strong> aucune alerte croisée automatique suffisamment documentée. Les données restent à interpréter ensemble.</div>'}<p class="muted small-text">Cette lecture relie les données disponibles ; elle ne remplace ni un calcul complet de ration ni l’interprétation vétérinaire/laboratoire des matrices biologiques.</p></div>`;
}
...
function reportReproductionHtml(visit,includeProblemCows=false){
  const farm=db.farms.find(f=>f.id===visit.farmId),source=reproductionSourceForVisit(visit,farm),registry=source.registry||[],prep=reproPreparationSnapshot(visit,farm);
  ...
  let html=`<p><strong>Période analysée :</strong> ${periodStart?formatDate(periodStart):'—'} → ${periodEnd?formatDate(periodEnd):'—'}${source.meta?.fileName?` · Source : ${escapeHtml(source.meta.fileName)}`:''}</p><div class="report-kpis"><div><span>Taux vêlage &gt;24 mois</span><strong>${rate24??'—'}%</strong></div><div><span>Taux vêlage &gt;36 mois</span><strong>${rate36??'—'}%</strong></div><div><span>IVV moyen</span><strong>${mean??'—'} j</strong></div><div><span>IVV mini</span><strong>${min??'—'} j</strong></div><div><span>IVV maxi</span><strong>${max??'—'} j</strong></div><div><span>Âge moyen 1er vêlage</span><strong>${firstMean??'—'} mois</strong></div><div><span>Mortalité veaux &lt;6 mois</span><strong>${calves.length?Math.round(dead.length/calves.length*1000)/10:'—'}%</strong></div><div><span>Vaches à surveiller</span><strong>${problemCows.length}</strong></div></div>${reproductionScoreLegendHtml(true)}${prep?.females24?.length?reproAgeDistributionHtml(prep.females24,analysisDate,{interactive:false,wrap:false,showBucketLegend:false,detailTargetId:'report-repro-age-year-detail'}):''}${prep?`<h3>Paternité / taureaux identifiés dans le registre</h3><p class="report-note"><strong>Couverture de la paternité :</strong> ${prep.allBirths.length?Math.round(prep.fatherKnownAll/prep.allBirths.length*1000)/10+' %':'—'} sur l’historique complet · ${prep.births.length?Math.round(prep.fatherKnown/prep.births.length*1000)/10+' %':'—'} sur les 12 derniers mois.</p>${prepReproSireTableHtml(prep.sires)}<p class="report-note"><strong>${prep.aiSireCount}</strong> père(s) d’IA / extérieur(s) détecté(s), comptés mais non détaillés.</p>`:''}`;
  if(includeProblemCows&&prep){
    const rolling=reproRolling12Stats(visit,farm,prep),hist=reproMortalityAnnualHistory(visit,farm,10);
    html=`${prepDecisionSupportHtml(visit,farm,prep,hist,rolling,{forPrint:true})}<h3>Historique reproduction, mortalité et mouvements — 10 ans</h3><p class="report-note">Les historiques sont calculés par années civiles à partir du registre complet. L’année en cours est signalée comme tronquée. La reproduction récente reste analysée sur les 12 derniers mois.</p>${prepRegistryTenYearChartsHtml(visit,farm,hist,{forPrint:true})}`+html;
  }
  if(includeProblemCows){
    const byId=new Map(registry.map(a=>[normalizeAnimalId(a.id),a])),motherCounts=new Map(),fatherCounts=new Map();
    problemCows.forEach(r=>{const m=normalizeAnimalId(r.cow?.motherId),p=normalizeAnimalId(r.cow?.fatherId);if(m)motherCounts.set(m,(motherCounts.get(m)||0)+1);if(p)fatherCounts.set(p,(fatherCounts.get(p)||0)+1);});
    const parentHtml=(id,counts)=>{const k=normalizeAnimalId(id);if(!k)return '—';const a=byId.get(k),n=counts.get(k)||0,label=a?`${escapeHtml(a.workNumber||a.id)}${a.name?` · ${escapeHtml(a.name)}`:''}<br><small>${escapeHtml(a.id||k)}</small>`:escapeHtml(id);return n>1?`<strong>${label}</strong><br><small><b>×${n} dans la liste</b></small>`:label;};
    html+=problemCows.length?`<h3>Détail des vaches à problème / à surveiller</h3><p class="report-note">Jours depuis vêlage : orange &gt;400 j, rouge &gt;450 j. Les mères et surtout les pères qui reviennent chez plusieurs vaches à problème sont mis en gras.</p><table class="report-repro-problem-table"><thead><tr><th>N° + nom</th><th>Âge</th><th>1er V</th><th>Dernier V</th><th>Jours depuis vêlage</th><th>Rang de vêlage</th><th>Mère</th><th>Père</th><th>IVV moy.</th><th>IVV min/max</th><th>Veaux morts &lt;6 m</th><th>Score</th></tr></thead><tbody>${problemCows.map(r=>{const d=reproductionScoreDetails(r),flags=reproductionPenaltyFlags(r),daysClass=(r.daysSinceLast||0)>450?'report-days-danger':(r.daysSinceLast||0)>400?'report-days-warning':'';return `<tr><td><strong>${escapeHtml(r.cow.workNumber||r.cow.id)}</strong>${r.cow.name?`<br><small>${escapeHtml(r.cow.name)}</small>`:''}<br><small>${escapeHtml(r.cow.id)}</small></td><td>${escapeHtml(ageLabelAt(r.cow.birthDate,analysisDate)||'—')}</td><td class="${flags.firstCalvingWarn?'report-penalty-hit':''} ${flags.firstCalvingSevere?'severe':''}">${r.firstCalvingDate?formatDate(r.firstCalvingDate):'—'}${r.firstCalvingAgeMonths!=null?`<br><small>${r.firstCalvingAgeMonths} mois</small>`:''}</td><td>${r.lastCalvingDate?formatDate(r.lastCalvingDate):'—'}</td><td class="${daysClass}">${r.daysSinceLast??'—'}</td><td><strong>${(r.calves||[]).length}</strong></td><td>${parentHtml(r.cow.motherId,motherCounts)}</td><td>${parentHtml(r.cow.fatherId,fatherCounts)}</td><td class="${flags.meanIVVWarn?'report-penalty-hit':''} ${flags.meanIVVSevere?'severe':''}">${r.meanIVV??'—'}</td><td class="${flags.maxIVVWarn?'report-penalty-hit':''} ${flags.maxIVVSevere?'severe':''}">${r.minIVV??'—'} / ${r.maxIVV??'—'}</td><td class="${flags.deadWarn?'report-penalty-hit':''} ${flags.deadSevere?'severe':''}">${r.deadBefore6.length}</td><td class="score-${reproductionScore(r)<60?'bad':reproductionScore(r)<75?'warn':'ok'}"><strong>${reproductionScore(r)}</strong></td></tr>`}).join('')}</tbody></table>`:'<p>Aucune vache ne répond aux critères d’alerte retenus.</p>';
  }
  return html;
}
function reportDocumentHtml(visit,type,options={}){
  ...
  if(type!=='farmer'){
    ...
    if(options.herddata!==false)body+=`<section class="report-section page-break"><h2>Données technico-économiques</h2>${reportHerdDataHtml(visit,{includeLegacyCharts:type!=='expert'})}</section>`;
    ...
    if(type==='expert')body+=`<section class="report-section page-break"><h2>Lecture transversale — cycle sol → fourrage → eau → ration → animal</h2><p class="report-note">Condensé des données collectées pendant ou après la visite, avec les points de vigilance issus des analyses et de la couverture de ration.</p>${cycleSummaryHtml(visit,{report:true})}</section>`;
...
function fullReportStyles(){return `${printBaseStyles()} body{max-width:none;margin:0;padding:0;background:#fff;color:#16231c;font-size:11.7pt;line-height:1.42}.report-cover{padding:24px;border:2px solid #b53670;border-radius:16px;margin-bottom:24px}.report-brand{display:flex;gap:18px;align-items:center}.report-logo{width:72px;height:72px;border-radius:18px;background:#b53670;color:white;display:grid;place-items:center;text-align:center;font-weight:800}.report-cover h1{margin:0}.report-cover-grid,.report-kpis{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-top:20px}.report-cover-grid>div,.report-kpis>div{padding:10px;background:#fff4f8;border-radius:8px}.report-cover-grid span,.report-kpis span{display:block;color:#66756c;font-size:10.2pt}.report-cover-grid strong,.report-kpis strong{display:block;margin-top:3px}.report-section{margin:22px 0;break-inside:auto}.report-summary-grid,.report-columns,.herd-chart-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:12px}.report-box,.report-reason,.herd-chart-card{border:1px solid #cfdad3;border-radius:10px;padding:12px;margin:10px 0;break-inside:avoid-page}.report-subsection{border:1px solid #cfdad3;border-radius:10px;padding:12px;margin:10px 0;break-inside:auto}.report-box.positive{border-left:6px solid #b53670}.report-box.warning{border-left:6px solid #e0a326}.report-empty{color:#6b746e;font-style:italic}.report-footer{margin-top:30px;border-top:2px solid #d5dfd8;padding-top:16px}.signature-grid{display:grid;grid-template-columns:1fr 1fr;gap:30px;margin-top:25px}.signature-grid>div{min-height:90px;border:1px solid #aab5ad;padding:12px}.report-photo-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:14px}.report-photo-grid figure{margin:0;break-inside:avoid;border:1px solid #cfdad3;border-radius:10px;padding:8px}.report-photo-grid img{width:100%;max-height:360px;object-fit:contain}.report-photo-grid figcaption{padding:7px 2px;font-size:10.7pt}.herd-chart-grid{grid-template-columns:1fr}.herd-chart-card{break-inside:avoid;background:#fffafd;padding:10px;width:100%;box-sizing:border-box}.herd-svg-chart{width:100%!important;max-width:none!important;height:auto;display:block;margin:2px 0 0}.registry-10y-charts{width:100%;max-width:none;margin-left:0;margin-right:0}.registry-10y-charts .herd-chart-card{width:100%;max-width:none}.repro-age-donut-svg{width:100%;max-width:285px;height:auto}.annual-history-table td.annual-trend-good{color:#15803d!important;font-weight:800!important;background:#eefbf2!important;-webkit-print-color-adjust:exact;print-color-adjust:exact}.annual-history-table td.annual-trend-bad{color:#b91c1c!important;font-weight:800!important;background:#fff0f0!important;-webkit-print-color-adjust:exact;print-color-adjust:exact}.herd-chart-legend{display:flex;gap:12px;flex-wrap:wrap;margin:6px 0 10px}.herd-chart-legend span{display:inline-flex;align-items:center;gap:6px;font-size:10.5pt;color:#425047}.herd-chart-legend i{width:12px;height:12px;border-radius:3px;display:inline-block}.legend-square{font-size:15pt;line-height:1}.report-color-legend{display:flex;gap:8px;flex-wrap:wrap;margin:8px 0 12px}.report-color-legend span{padding:5px 9px;border:1px solid #b8c4bd;border-radius:6px;font-weight:700}.report-value-green{background:#dff3e7!important;color:#174d2e!important}.report-value-yellow{background:#fff0b8!important;color:#6b5300!important}.report-value-red{background:#ffd5d2!important;color:#7d1b17!important}.report-wide-table{overflow:visible}.report-wide-table table{font-size:9.3pt}.abnormal-subjects{margin:10px 0;padding:9px 11px;border-left:4px solid #d14d49;background:#fff5f4}.abnormal-subjects>div{display:flex;gap:8px;flex-wrap:wrap;margin-top:4px}.abnormal-subjects span{color:#5e4a4a}.page-break{page-break-before:always}.report-penalty-hit{background:#fff1c7!important;border-radius:6px;font-weight:800!important}.report-penalty-hit.severe{background:#ffd8d5!important;color:#a01818!important;-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}.report-days-warning{background:#fff0c7!important;color:#9a6700!important;font-weight:800!important}.report-days-danger{background:#ffd9d6!important;color:#a01818!important;font-weight:800!important}.score-ok{color:#176238!important;background:#dff3e6!important;font-weight:800!important}.score-warn{color:#9a6700!important;background:#fff0c7!important;font-weight:800!important}.score-bad{color:#a01818!important;background:#ffd9d6!important;font-weight:800!important}.repro-score-hit-row td{background:#fff4d5}*{-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}@media print{.no-print{display:none!important}.smart-page-before{break-before:page!important;page-break-before:always!important}h1,h2,h3,h4,h5{break-after:avoid-page!important;page-break-after:avoid!important;margin-bottom:8px}h1+*,h2+*,h3+*,h4+*,h5+*{break-before:avoid-page!important;page-break-before:avoid!important}p,li{orphans:3;widows:3}thead{display:table-header-group}tr{break-inside:avoid-page;page-break-inside:avoid}.report-box,.report-reason,.herd-chart-card,.decision-alert,.decision-theme{break-inside:avoid-page;page-break-inside:avoid}.report-subsection{break-inside:auto;page-break-inside:auto}.herd-chart-legend .legend-square{display:inline!important}.report-value-green,.report-value-yellow,.report-value-red{box-shadow:inset 0 0 0 1000px currentColor 0!important}}@media(max-width:700px){.report-cover-grid,.report-kpis,.report-summary-grid,.report-columns,.repro-age-overview.grouped{grid-template-columns:1fr}}`}
function smartPrintPaginationScript(){return `<script>(function(){
function paginate(){
  var mm=96/25.4, pageH=(297-12-14)*mm, minAfter=150;
  var selectors='h1,h2,h3,h4,h5,.report-box,.report-reason,.herd-chart-card,.print-metric,.decision-alert,.decision-theme,.targeted-audit-print,figure,blockquote';
  document.querySelectorAll(selectors).forEach(function(el){el.classList.remove('smart-page-before')});
  for(var pass=0;pass<3;pass++){
    document.querySelectorAll('h1,h2,h3,h4,h5').forEach(function(h){
      var r=h.getBoundingClientRect(), top=r.top+window.scrollY, inPage=top%pageH;
      var next=h.nextElementSibling, need=r.height+minAfter;
      if(next){need=Math.min(320,r.height+Math.min(next.getBoundingClientRect().height,230)+18)}
      if(pageH-inPage<need)h.classList.add('smart-page-before');
    });
    document.querySelectorAll('.report-box,.report-reason,.herd-chart-card,.print-metric,.decision-alert,.decision-theme,.targeted-audit-print,figure').forEach(function(el){
      var r=el.getBoundingClientRect(); if(r.height>pageH*.82)return;
      var top=r.top+window.scrollY, inPage=top%pageH;
      if(pageH-inPage<r.height+18)el.classList.add('smart-page-before');
    });
  }
}
window.addEventListener('load',function(){setTimeout(paginate,80)});
window.addEventListener('beforeprint',paginate);
})();<\/script>`}
function reportStandaloneHtml(visit,type,options={}){return `<!doctype html><html lang="fr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Rapport ${type}</title><style>${fullReportStyles()}</style></head><body><div class="no-print" style="position:sticky;top:0;background:white;padding:8px;border-bottom:1px solid #ddd;z-index:5"><button onclick="window.print()">Imprimer / Enregistrer en PDF</button></div>${reportDocumentHtml(visit,type,options)}${smartPrintPaginationScript()}</body></html>`;}
...
function excelColumnName(index){let n=index+1,out='';while(n>0){n--;out=String.fromCharCode(65+(n%26))+out;n=Math.floor(n/26);}return out;}
function excelSheetSafeName(name){return String(name||'Feuille').replace(/[\\\/?*\[\]:]/g,' ').trim().slice(0,31)||'Feuille';}
function excelCellXml(value,row,col,style=0){const ref=`${excelColumnName(col)}${row}`;if(value===null||value===undefined||value==='')return `<c r="${ref}" s="${style}"/>`;if(typeof value==='number'&&Number.isFinite(value))return `<c r="${ref}" s="${style}"><v>${value}</v></c>`;if(typeof value==='boolean')return `<c r="${ref}" t="b" s="${style}"><v>${value?1:0}</v></c>`;return `<c r="${ref}" t="inlineStr" s="${style}"><is><t xml:space="preserve">${partnerExcelEscapeXml(String(value))}</t></is></c>`;}
function excelWorksheetXml(headers,rows){
  const all=[headers,...rows],lastCol=excelColumnName(Math.max(0,headers.length-1)),lastRow=Math.max(1,all.length);
  const widths=headers.map((h,i)=>{let m=String(h||'').length;for(const r of rows.slice(0,250))m=Math.max(m,String(r?.[i]??'').length);return Math.max(9,Math.min(34,m+2));});
  const data=all.map((r,ri)=>`<row r="${ri+1}"${ri===0?' ht="24" customHeight="1"':''}>${headers.map((_,ci)=>excelCellXml(r?.[ci],ri+1,ci,ri===0?1:2)).join('')}</row>`).join('');
  return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><dimension ref="A1:${lastCol}${lastRow}"/><sheetViews><sheetView workbookViewId="0"><pane ySplit="1" topLeftCell="A2" activePane="bottomLeft" state="frozen"/></sheetView></sheetViews><sheetFormatPr defaultRowHeight="18"/><cols>${widths.map((w,i)=>`<col min="${i+1}" max="${i+1}" width="${w}" customWidth="1"/>`).join('')}</cols><sheetData>${data}</sheetData><autoFilter ref="A1:${lastCol}${lastRow}"/><pageMargins left="0.3" right="0.3" top="0.5" bottom="0.5" header="0.2" footer="0.2"/></worksheet>`;
}
function measurementExportSheets(visit){
  const subjectBase=s=>[s.tag||s.workNumber||s.id||'',s.name||'',s.category||'Non classé',s.location||''];
  const measureHeaders=['Sujet','Nom','Catégorie','Localisation',...analysisParameters.map(p=>p.label),...['Physique','Urines','Sang','Bouses','Lait','Colostrum'].map(x=>`Commentaire ${x}`)];
  const measureRows=(visit.subjects||[]).map(s=>{const a=s.measurements?.analysis||{},c=s.measurements?.comments||{};return [...subjectBase(s),...analysisParameters.map(p=>{const n=numericValue(a[p.key]);return n===null?(a[p.key]??''):n;}),...['Physique','Urines','Sang','Bouses','Lait','Colostrum'].map(x=>c[x]||'')];});
  const obsHeaders=['Sujet','Nom','Catégorie','Localisation',...observationFields.map(f=>f.label)];
  const obsRows=(visit.subjects||[]).map(s=>{const o=s.measurements?.observations||{};return [...subjectBase(s),...observationFields.map(f=>Array.isArray(o[f.key])?o[f.key].join(', '):(o[f.key]??''))];});
  const sheets=[{name:'Mesures individuelles',headers:measureHeaders,rows:measureRows},{name:'Observations',headers:obsHeaders,rows:obsRows}];
  for(const [kind,cfg] of Object.entries(generalConfigs)){
    const headers=['Type',...cfg.fields.map(f=>f[1])];
    const rows=(visit.analysisGeneral?.[kind]||[]).map(r=>[cfg.title,...cfg.fields.map(([key,,type])=>{const v=r[key];if(Array.isArray(v))return v.join(', ');if(type==='number'){const n=numericValue(v);return n===null?(v??''):n;}return v??'';})]);
    sheets.push({name:excelSheetSafeName(cfg.title),headers,rows});
  }
  const sb=ensureSupplementalBlood(visit),sbParams=supplementalBloodParams(visit);
  if(sbParams.length){
    const headers=['Sujet','Nom','Catégorie','Localisation',...sbParams.map(p=>`${p.label}${p.unit?' ('+p.unit+')':''}`),'Commentaire','Source / appareil / vétérinaire','Date analyse','Commentaire général'];
    const rows=(Array.isArray(visit.subjects)?visit.subjects:[]).map(subject=>[...subjectBase(subject),...sbParams.map(p=>{const v=supplementalBloodValue(subject,p),n=numericValue(v);return n===null?v:n;}),subject.measurements?.supplementalBlood?.comment||'',sb.source||'',sb.date||'',sb.comment||'']);
    sheets.push({name:'Analyses complémentaires',headers,rows});
  }
  return sheets;
}
async function downloadMeasurementsExcel(visit){
  if(typeof JSZip==='undefined'){showToast('Module Excel indisponible.');return;}
  // Force l'enregistrement de la cellule en cours avant l'export.
  if(document.activeElement&&typeof document.activeElement.blur==='function')document.activeElement.blur();
  await new Promise(r=>setTimeout(r,80));
  const sheets=measurementExportSheets(visit),zip=new JSZip();
  ...
  zip.folder('xl').file('workbook.xml',`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets>${sheets.map((sh,i)=>`<sheet name="${partnerExcelEscapeXml(excelSheetSafeName(sh.name))}" sheetId="${i+1}" r:id="rId${i+1}"/>`).join('')}</sheets></workbook>`);
  ...
  zip.folder('xl').file('styles.xml',`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><fonts count="2"><font><sz val="11"/><name val="Calibri"/><family val="2"/></font><font><b/><color rgb="FFFFFFFF"/><sz val="11"/><name val="Calibri"/><family val="2"/></font></fonts><fills count="3"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="gray125"/></fill><fill><patternFill patternType="solid"><fgColor rgb="FFB53670"/><bgColor indexed="64"/></patternFill></fill></fills><borders count="2"><border><left/><right/><top/><bottom/><diagonal/></border><border><left style="thin"><color rgb="FFD9D9D9"/></left><right style="thin"><color rgb="FFD9D9D9"/></right><top style="thin"><color rgb="FFD9D9D9"/></top><bottom style="thin"><color rgb="FFD9D9D9"/></bottom><diagonal/></border></borders><cellStyleXfs count="1"><xf numFmtId="0" fontId="0" fillId="0" borderId="0"/></cellStyleXfs><cellXfs count="3"><xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0"/><xf numFmtId="0" fontId="1" fillId="2" borderId="1" xfId="0" applyFont="1" applyFill="1" applyBorder="1" applyAlignment="1"><alignment horizontal="center" vertical="center" wrapText="1"/></xf><xf numFmtId="0" fontId="0" fillId="0" borderId="1" xfId="0" applyBorder="1" applyAlignment="1"><alignment vertical="top" wrapText="1"/></xf></cellXfs><cellStyles count="1"><cellStyle name="Normal" xfId="0" builtinId="0"/></cellStyles></styleSheet>`);
  const ws=zip.folder('xl').folder('worksheets');sheets.forEach((sh,i)=>ws.file(`sheet${i+1}.xml`,excelWorksheetXml(sh.headers,sh.rows)));
  ...
  const farm=db.farms.find(f=>f.id===visit.farmId);a.href=url;a.download=`Mesures-${slugify(farm?.name||'exploitation')}-${visit.date||'visite'}.xlsx`;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1500);
  visit.generatedReports=Array.isArray(visit.generatedReports)?visit.generatedReports:[];visit.generatedReports.unshift({id:uid('report'),type:'Mesures de visite',format:'Excel',createdAt:new Date().toISOString()});saveDatabase(db);showToast('Mesures exportées en Excel.');
}

function reportOptionsFromUi(type){
  if(type==='farmer')return {};
  const prefix=type==='expert'?'expert':'technical',visit=activeVisit(),saved=visit?.reportOptions?.[prefix]||{};
  const checked=(key,def=true)=>{const el=document.getElementById(`report-${prefix}-${key}`);return el?el.checked:(saved[key]??def);};
...
function renderReports(){
  ...
  document.getElementById('export-measures-excel-report')?.addEventListener('click',async()=>{const btn=document.getElementById('export-measures-excel-report');btn.disabled=true;const old=btn.textContent;btn.textContent='Génération Excel…';try{await downloadMeasurementsExcel(visit);}catch(err){console.error(err);alert('Impossible de générer le fichier Excel des mesures.');}finally{btn.disabled=false;btn.textContent=old;}});
...
function herdLineChartSvg(labels,values,{color='#2F6F73',height=300}={}){
  ...
  const width=900,pad={top:34,right:28,bottom:58,left:62};
  ...
  const grid=Array.from({length:4},(_,i)=>{const gv=min+range*(i/3);const gy=y(gv);return `<line x1="${pad.left}" y1="${gy.toFixed(1)}" x2="${width-pad.right}" y2="${gy.toFixed(1)}" stroke="#d9e4dd" stroke-width="1"/><text x="${pad.left-6}" y="${(gy+4).toFixed(1)}" text-anchor="end" font-size="15" fill="#66756c">${Math.round(gv)}</text>`;}).join('');
  const points=vals.map((v,i)=>v===null?'':`<circle cx="${x(i).toFixed(1)}" cy="${y(v).toFixed(1)}" r="6" fill="${color}"/><text x="${x(i).toFixed(1)}" y="${(y(v)-10).toFixed(1)}" text-anchor="middle" font-size="16" font-weight="700" fill="#22312a">${String(v).replace('.',',')}</text>`).join('');
  const xLabels=labels.map((lbl,i)=>`<text x="${x(i).toFixed(1)}" y="${height-14}" text-anchor="middle" font-size="16" fill="#4a5750">${escapeHtml(lbl)}</text>`).join('');
...
function herdGroupedBarChartSvg(labels,series,{height=300}={}){
  ...
  const width=900,pad={top:38,right:28,bottom:60,left:62};
  ...
  const grid=Array.from({length:4},(_,i)=>{const gv=max*(i/3),gy=y(gv);return `<line x1="${pad.left}" y1="${gy.toFixed(1)}" x2="${width-pad.right}" y2="${gy.toFixed(1)}" stroke="#d9e4dd" stroke-width="1"/><text x="${pad.left-6}" y="${(gy+4).toFixed(1)}" text-anchor="end" font-size="14" fill="#6b746e">${Math.round(gv)}</text>`;}).join('');
  let bars='';
  labels.forEach((lbl,i)=>{const baseX=pad.left+i*groupW+8;series.forEach((s,j)=>{const v=chartValue(s.values[i]);if(v===null)return;const bh=Math.max(0,(v/max)*plotH);const bx=baseX+j*barW,by=pad.top+plotH-bh;bars+=`<rect x="${bx.toFixed(1)}" y="${by.toFixed(1)}" width="${(barW-4).toFixed(1)}" height="${bh.toFixed(1)}" rx="5" fill="${s.color}"/><text x="${(bx+(barW-4)/2).toFixed(1)}" y="${(by-6).toFixed(1)}" text-anchor="middle" font-size="14" fill="#22312a">${String(v).replace('.',',')}</text>`;});bars+=`<text x="${(pad.left+i*groupW+groupW/2).toFixed(1)}" y="${height-14}" text-anchor="middle" font-size="15" fill="#4a5750">${escapeHtml(lbl)}</text>`;});
...
function herdStackedBarChartSvg(labels,series,{height=330}={}){
  if(!series.length)return herdChartEmpty();
  const width=900,pad={top:38,right:28,bottom:60,left:62};
  ...
  const y=v=>pad.top+(1-v/max)*plotH;
  const pctLabel=v=>{const n=chartValue(v);if(n===null)return '';const r=Math.round(n*10)/10;return `${String(r).replace('.',',')}%`;};
  const grid=Array.from({length:4},(_,i)=>{const gv=max*(i/3),gy=y(gv);return `<line x1="${pad.left}" y1="${gy.toFixed(1)}" x2="${width-pad.right}" y2="${gy.toFixed(1)}" stroke="#d9e4dd" stroke-width="1"/><text x="${pad.left-6}" y="${(gy+4).toFixed(1)}" text-anchor="end" font-size="14" fill="#6b746e">${Math.round(gv)}</text>`;}).join('');
  let bars='';
  labels.forEach((lbl,i)=>{const x=pad.left+i*(plotW/Math.max(1,labels.length))+(plotW/Math.max(1,labels.length)-barW)/2;let cumulative=0;series.forEach(s=>{const v=chartValue(s.values[i])||0;if(v<=0)return;const y0=y(cumulative+v),y1=y(cumulative),h=y1-y0,label=pctLabel(s.labelValues?.[i]);bars+=`<rect x="${x.toFixed(1)}" y="${y0.toFixed(1)}" width="${barW.toFixed(1)}" height="${h.toFixed(1)}" fill="${s.color}"/>${label&&h>=11?`<text x="${(x+barW/2).toFixed(1)}" y="${(y0+h/2+3.5).toFixed(1)}" text-anchor="middle" font-size="12" font-weight="700" fill="#fff" stroke="#26332d" stroke-width="0.35" paint-order="stroke">${label}</text>`:''}`;cumulative+=v;});bars+=`<text x="${(x+barW/2).toFixed(1)}" y="${(y(cumulative)-8).toFixed(1)}" text-anchor="middle" font-size="14" font-weight="700" fill="#22312a">${cumulative||''}</text><text x="${(x+barW/2).toFixed(1)}" y="${height-14}" text-anchor="middle" font-size="15" fill="#4a5750">${escapeHtml(lbl)}</text>`;});
  return `${herdChartLegend(series)}<svg viewBox="0 0 ${width} ${height}" class="herd-svg-chart" aria-hidden="true"><rect x="0" y="0" width="${width}" height="${height}" rx="12" fill="#fffafd"/><g>${grid}<line x1="${pad.left}" y1="${height-pad.bottom}" x2="${width-pad.right}" y2="${height-pad.bottom}" stroke="#b9c9bf" stroke-width="1.2"/>${bars}</g></svg>`;
}
...
function herdChartsHtml(item,{forReport=false}={}){
  ...
  return `<div class="herd-chart-grid${forReport?' report-mode':''}">
    ...
    <article class="card herd-chart-card"><h4>Introductions et mouvements du troupeau</h4><p class="muted">Naissances, introductions / achats et mortalités enregistrées.</p>${herdGroupedBarChartSvg(labels,movements)}</article>
  </div>`;
}
...
function reproPreparationSnapshot(visit,farm){
  ...
  const sires=[...residentSireMap.values()].map(r=>{const births=r.birthDates.slice().sort(),conceptions=r.conceptionDates.slice().sort(),presentNow=isRegistryAnimalPresent(r.male,date),exitDate=r.male.exitDate||'',currentAge=r.male.birthDate?ageLabelAt(r.male.birthDate,date):'',ageAtExit=r.male.birthDate&&exitDate?ageLabelAt(r.male.birthDate,exitDate):'',firstServiceDate=conceptions[0]||'',lastServiceDate=conceptions.at(-1)||'';return {fatherId:r.fatherId,calves:r.calves,mothersCount:r.mothers.size,pctFemales:knownResidentDams.size?Math.round(r.mothers.size/knownResidentDams.size*1000)/10:null,present:presentNow,workNumber:r.male.workNumber||'',name:r.male.name||'',age:presentNow?currentAge:(ageAtExit||currentAge),birthDate:r.male.birthDate||'',entryDate:r.male.entryDate||'',exitDate,originCode:r.male.entryDate?'A':'N',firstBirth:births[0]||'',lastBirth:births.at(-1)||'',firstServiceDate,lastServiceDate,firstServiceAge:r.male.birthDate&&firstServiceDate?ageLabelAt(r.male.birthDate,firstServiceDate):'',lastServiceAge:r.male.birthDate&&lastServiceDate?ageLabelAt(r.male.birthDate,lastServiceDate):''};}).sort((a,b)=>b.calves-a.calves);
...
function reproRolling12Stats(visit,farm,x=null){
  x=x||reproPreparationSnapshot(visit,farm);if(!x)return null;
  const source=reproductionSourceForVisit(visit,farm),registry=source.registry||[],end=x.date,start=x.startIso;
  const births=registry.filter(a=>a.motherId&&a.birthDate&&a.birthDate>=start&&a.birthDate<=end);
  const dams=new Set(births.map(a=>normalizeAnimalId(a.motherId)).filter(Boolean));
  const byDam=new Map();registry.filter(a=>a.motherId&&a.birthDate&&a.birthDate<=end).forEach(c=>{const k=normalizeAnimalId(c.motherId);if(!k)return;if(!byDam.has(k))byDam.set(k,[]);byDam.get(k).push(c.birthDate);});
  const intervals=[],intervalDetails=[],ivvDams=new Set();byDam.forEach((ds,k)=>{const dates=[...new Set(ds)].sort();for(let i=1;i<dates.length;i++){if(dates[i]>=start&&dates[i]<=end){const d=daysBetweenDates(dates[i-1],dates[i]);if(d!=null){intervals.push(d);ivvDams.add(k);const cow=registry.find(a=>normalizeAnimalId(a.id)===k)||null;intervalDetails.push({damId:k,cow,previousCalvingDate:dates[i-1],calvingDate:dates[i],days:d});}}}});
  const firstAges=[];byDam.forEach((ds,k)=>{const first=[...new Set(ds)].sort()[0];if(!first||first<start||first>end)return;const cow=registry.find(a=>normalizeAnimalId(a.id)===k);const m=cow?.birthDate?monthsBetweenDates(cow.birthDate,first):null;if(m!=null)firstAges.push(m);});
  const dead6=births.filter(c=>c.exitCause==='M'&&c.exitDate&&c.exitDate<=end&&daysBetweenDates(c.birthDate,c.exitDate)!=null&&daysBetweenDates(c.birthDate,c.exitDate)<183);
  const mean=a=>a.length?Math.round(a.reduce((u,v)=>u+v,0)/a.length):null,mean1=a=>a.length?Math.round(a.reduce((u,v)=>u+v,0)/a.length*10)/10:null;
  const firstCalvingLateCount=firstAges.filter(m=>m>36).length;
  const firstCalvingLateRate=firstAges.length?Math.round(firstCalvingLateCount/firstAges.length*1000)/10:null;
  return {start,end,births,dams:dams.size,intervals,intervalDetails,ivvCowCount:ivvDams.size,ivvMean:mean(intervals),ivvLe400:intervals.filter(d=>d<=400).length,ivv401to450:intervals.filter(d=>d>=401&&d<=450).length,ivv451to500:intervals.filter(d=>d>=451&&d<=500).length,ivvOver500:intervals.filter(d=>d>500).length,firstCalvingCount:firstAges.length,firstCalvingAges:firstAges,firstCalvingMean:mean1(firstAges),firstCalvingLateCount,firstCalvingLateRate,dead6,mortalityRate:births.length?Math.round(dead6.length/births.length*1000)/10:null};
}
function reproMortalityAnnualHistory(visit,farm,yearsCount=10){
  const source=reproductionSourceForVisit(visit,farm),registry=source.registry||[],endIso=visit?.date||new Date().toISOString().slice(0,10);if(!registry.length)return [];
  const endYear=Number(endIso.slice(0,4)),firstYear=endYear-yearsCount+1;
  const cowsById=new Map(registry.filter(a=>a.sex==='F').map(a=>[normalizeAnimalId(a.id),a]));
  const calves=registry.filter(a=>a.motherId&&a.birthDate&&a.birthDate<=endIso);
  const byDam=new Map();calves.forEach(c=>{const k=normalizeAnimalId(c.motherId);if(!k)return;if(!byDam.has(k))byDam.set(k,[]);byDam.get(k).push(c.birthDate);});
  const annualIntervals=new Map(),annualFirstAges=new Map();
  byDam.forEach((ds,k)=>{const dates=[...new Set(ds)].sort();for(let i=1;i<dates.length;i++){const y=Number(dates[i].slice(0,4));if(y<firstYear||y>endYear)continue;const d=daysBetweenDates(dates[i-1],dates[i]);if(d!=null){if(!annualIntervals.has(y))annualIntervals.set(y,[]);annualIntervals.get(y).push(d);}}const first=dates[0],fy=first?Number(first.slice(0,4)):0;if(fy>=firstYear&&fy<=endYear){const cow=cowsById.get(k),m=cow?.birthDate?monthsBetweenDates(cow.birthDate,first):null;if(m!=null){if(!annualFirstAges.has(fy))annualFirstAges.set(fy,[]);annualFirstAges.get(fy).push(m);}}});
  const mean=a=>a.length?Math.round(a.reduce((u,v)=>u+v,0)/a.length):null,mean1=a=>a.length?Math.round(a.reduce((u,v)=>u+v,0)/a.length*10)/10:null;
  const out=[];for(let y=firstYear;y<=endYear;y++){
    const yEnd=y===endYear?endIso:`${y}-12-31`,births=calves.filter(c=>Number(c.birthDate.slice(0,4))===y&&c.birthDate<=yEnd),dams=new Set(births.map(c=>normalizeAnimalId(c.motherId)).filter(Boolean)),ivvs=annualIntervals.get(y)||[],ages=annualFirstAges.get(y)||[],dead6=births.filter(c=>c.exitCause==='M'&&c.exitDate&&c.exitDate<=endIso&&daysBetweenDates(c.birthDate,c.exitDate)!=null&&daysBetweenDates(c.birthDate,c.exitDate)<183);
    const deaths=registry.filter(a=>a.exitCause==='M'&&a.exitDate&&Number(String(a.exitDate).slice(0,4))===y&&a.exitDate<=yEnd&&a.birthDate);
    const deathAgeDays=a=>daysBetweenDates(a.birthDate,a.exitDate);
    // Classes mutuellement exclusives pour que les bâtons correspondent exactement au nombre total de décès.
    const d0_2=deaths.filter(a=>{const d=deathAgeDays(a);return d!=null&&d<2}).length;
    const d2_30=deaths.filter(a=>{const d=deathAgeDays(a);return d!=null&&d>=2&&d<31}).length;
    const d1_6=deaths.filter(a=>{const d=deathAgeDays(a);return d!=null&&d>=31&&d<183}).length;
    const d6_12=deaths.filter(a=>{const d=deathAgeDays(a);return d!=null&&d>=183&&d<365}).length;
    const d12_24=deaths.filter(a=>{const d=deathAgeDays(a);return d!=null&&d>=365&&d<730}).length;
    const dOver24=deaths.filter(a=>{const d=deathAgeDays(a);return d!=null&&d>=730}).length;
    const dp=n=>deaths.length?Math.round(n/deaths.length*1000)/10:null;
    out.push({year:y,truncated:y===endYear,periodStart:`${y}-01-01`,periodEnd:yEnd,births:births.length,cowsCalved:dams.size,ivvCount:ivvs.length,ivvMean:mean(ivvs),ivvLe400:ivvs.filter(d=>d<=400).length,ivv401to450:ivvs.filter(d=>d>=401&&d<=450).length,ivv451to500:ivvs.filter(d=>d>=451&&d<=500).length,ivvOver500:ivvs.filter(d=>d>500).length,firstCalvingCount:ages.length,firstCalvingMean:mean1(ages),dead6:dead6.length,mortalityRate:births.length?Math.round(dead6.length/births.length*1000)/10:null,deathsTotal:deaths.length,deaths0_2:d0_2,deaths2_30:d2_30,deaths1_6:d1_6,deaths6_12:d6_12,deaths12_24:d12_24,deathsOver24:dOver24,deaths0_2Pct:dp(d0_2),deaths2_30Pct:dp(d2_30),deaths1_6Pct:dp(d1_6),deaths6_12Pct:dp(d6_12),deaths12_24Pct:dp(d12_24),deathsOver24Pct:dp(dOver24)});
  }return out;
}
function annualHistoryTablesHtml(rows,{forPrint=false}={}){
  if(!rows?.length)return '<div class="empty">Historique annuel indisponible.</div>';
  const years=rows.map(r=>r.year),cell=(v,s='')=>v==null?'—':`${String(v).replace('.',',')}${s}`;
  const trendClass=(cur,prev,direction)=>{
    if(direction==='neutral'||cur==null||prev==null||Number(cur)===Number(prev))return '';
    const improved=direction==='up'?Number(cur)>Number(prev):Number(cur)<Number(prev);
    return improved?'annual-trend-good':'annual-trend-bad';
  };
  const tr=(label,key,suffix='',direction='neutral')=>`<tr><th>${label}</th>${rows.map((r,i)=>{const v=r[key],prev=i?rows[i-1][key]:null,cls=trendClass(v,prev,direction);return `<td class="${cls}">${cell(v,suffix)}</td>`;}).join('')}</tr>`;
  return `<div class="annual-history-block ${forPrint?'print-annual-history':''}"><h4>Évolution sur les 10 dernières années</h4><p class="muted small-text">Années civiles. Pour l’année en cours, données arrêtées à la date de visite. Mortalité : veaux nés dans l’année et morts avant 6 mois ; la dernière année peut être incomplète pour les veaux n’ayant pas encore 6 mois de recul. <strong>Vert</strong> = amélioration par rapport à l’année précédente ; <strong>rouge</strong> = dégradation ; sans couleur = stable, non comparable ou indicateur de volume.</p><h5>Reproduction</h5><div class="table-wrap"><table class="annual-history-table"><thead><tr><th>Indicateur</th>${years.map(y=>`<th>${y}</th>`).join('')}</tr></thead><tbody>${tr('Vaches ayant vêlé','cowsCalved')}${tr('IVV calculables','ivvCount')}${tr('IVV moyen','ivvMean',' j','down')}${tr('IVV ≤400 j','ivvLe400','','up')}${tr('IVV 401–450 j','ivv401to450','','down')}${tr('IVV 451–500 j','ivv451to500','','down')}${tr('IVV >500 j','ivvOver500','','down')}${tr('Âge moyen 1er vêlage','firstCalvingMean',' m','down')}</tbody></table></div><h5>Mortalité veaux</h5><div class="table-wrap"><table class="annual-history-table"><thead><tr><th>Indicateur</th>${years.map(y=>`<th>${y}</th>`).join('')}</tr></thead><tbody>${tr('Veaux nés / suivis','births')}${tr('Morts <6 mois','dead6','','down')}${tr('Mortalité <6 mois','mortalityRate',' %','down')}</tbody></table></div></div>`;
}



function prepAnnualSaleExitHistory(visit,farm,years=10){
  const reg=(reproductionSourceForVisit(visit,farm)?.registry||[]),end=visit?.date||new Date().toISOString().slice(0,10),endYear=Number(end.slice(0,4)),out=[];
  const bands=[['<8 mois',0,8],['8–16 mois',8,16],['16–36 mois',16,36],['>36 mois',36,Infinity]];
  for(let y=endYear-years+1;y<=endYear;y++){
    const yEnd=y===endYear?end:`${y}-12-31`,inYear=d=>d&&Number(String(d).slice(0,4))===y&&d<=yEnd;
    const sold=reg.filter(a=>inYear(a.exitDate)&&['E','H','B'].includes(String(a.exitCause||'').toUpperCase())&&a.birthDate);
    const row={year:y};
    for(const [label,min,max] of bands){
      const key=label===' <8 mois'?'lt8':label.startsWith('<8')?'lt8':label.startsWith('8')?'m8_16':label.startsWith('16')?'m16_36':'gt36';
      for(const sex of ['F','M'])for(const cause of ['EH','B'])row[`${key}_${sex}_${cause}`]=0;
    }
    sold.forEach(a=>{
      const age=monthsBetweenDates(a.birthDate,a.exitDate);if(age==null)return;
      const key=age<8?'lt8':age<16?'m8_16':age<=36?'m16_36':'gt36';
      const sex=a.sex==='M'?'M':'F',cause=String(a.exitCause||'').toUpperCase()==='B'?'B':'EH';
      row[`${key}_${sex}_${cause}`]=(row[`${key}_${sex}_${cause}`]||0)+1;
    });
    out.push(row);
  }
  return out;
}
function herdGroupedBarLineChartSvg(labels,barSeries,lineSeries,{height=340}={}){
  const width=900,pad={top:42,right:32,bottom:62,left:62},all=[...barSeries.flatMap(s=>s.values),...(lineSeries?.values||[])].map(chartValue).filter(v=>v!==null);
  if(!all.length)return herdChartEmpty();
  const max=Math.max(...all,1),plotW=width-pad.left-pad.right,plotH=height-pad.top-pad.bottom,groupW=plotW/Math.max(1,labels.length),barW=Math.min(24,(groupW-16)/Math.max(1,barSeries.length)),y=v=>pad.top+(1-v/max)*plotH;
  const grid=Array.from({length:5},(_,i)=>{const gv=max*(i/4),gy=y(gv);return `<line x1="${pad.left}" y1="${gy.toFixed(1)}" x2="${width-pad.right}" y2="${gy.toFixed(1)}" stroke="#d9e4dd" stroke-width="1"/><text x="${pad.left-6}" y="${(gy+4).toFixed(1)}" text-anchor="end" font-size="14" fill="#6b746e">${Math.round(gv)}</text>`;}).join('');
  let bars='';const pts=[];
  labels.forEach((lbl,i)=>{const center=pad.left+i*groupW+groupW/2,baseX=center-(barSeries.length*barW)/2;barSeries.forEach((bs,j)=>{const v=chartValue(bs.values[i]);if(v===null)return;const bh=(v/max)*plotH,bx=baseX+j*barW,by=pad.top+plotH-bh,bw=Math.max(5,barW-4),tx=bx+bw/2,inside=bh>=22,ty=inside?by+14:Math.max(pad.top+11,by-4);bars+=`<rect x="${bx.toFixed(1)}" y="${by.toFixed(1)}" width="${bw.toFixed(1)}" height="${bh.toFixed(1)}" rx="4" fill="${bs.color}"/>${v>0?`<text x="${tx.toFixed(1)}" y="${ty.toFixed(1)}" text-anchor="middle" font-size="11.5" font-weight="800" fill="${inside?'#fff':'#253129'}" stroke="${inside?'rgba(0,0,0,.22)':'none'}" stroke-width="0.35" paint-order="stroke">${v}</text>`:''}`;});const lv=chartValue(lineSeries?.values?.[i]);if(lv!==null)pts.push({x:center,y:y(lv),v:lv});bars+=`<text x="${center.toFixed(1)}" y="${height-14}" text-anchor="middle" font-size="15" fill="#4a5750">${escapeHtml(lbl)}</text>`;});
  const path=pts.map((p,i)=>`${i?'L':'M'}${p.x.toFixed(1)},${p.y.toFixed(1)}`).join(' '),line=pts.map(p=>`<circle cx="${p.x.toFixed(1)}" cy="${p.y.toFixed(1)}" r="5" fill="${lineSeries.color}"/><text x="${p.x.toFixed(1)}" y="${(p.y-9).toFixed(1)}" text-anchor="middle" font-size="13" font-weight="700">${p.v}</text>`).join('');
  const legend=herdChartLegend([...barSeries,{label:lineSeries.label,color:lineSeries.color}]);
  return `${legend}<svg viewBox="0 0 ${width} ${height}" class="herd-svg-chart" aria-hidden="true"><rect width="${width}" height="${height}" rx="12" fill="#fffafd"/><g>${grid}<line x1="${pad.left}" y1="${height-pad.bottom}" x2="${width-pad.right}" y2="${height-pad.bottom}" stroke="#b9c9bf" stroke-width="1.2"/>${bars}<path d="${path}" fill="none" stroke="${lineSeries.color}" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/>${line}</g></svg>`;
}
function herdSexGroupedStackedBarChartSvg(labels,femaleSeries,maleSeries,{height=320}={}){
  const width=900,pad={top:40,right:28,bottom:64,left:58},all=[...femaleSeries,...maleSeries].flatMap(s=>s.values).map(chartValue).filter(v=>v!==null),totals=labels.map((_,i)=>Math.max(femaleSeries.reduce((a,s)=>a+(chartValue(s.values[i])||0),0),maleSeries.reduce((a,s)=>a+(chartValue(s.values[i])||0),0)));
  if(!all.length||!totals.some(v=>v>0))return herdChartEmpty();
  const max=Math.max(...totals,1),plotW=width-pad.left-pad.right,plotH=height-pad.top-pad.bottom,groupW=plotW/Math.max(1,labels.length),barW=Math.min(30,(groupW-12)/2.5),gap=5,y=v=>pad.top+(1-v/max)*plotH;
  const grid=Array.from({length:5},(_,i)=>{const gv=max*(i/4),gy=y(gv);return `<line x1="${pad.left}" y1="${gy.toFixed(1)}" x2="${width-pad.right}" y2="${gy.toFixed(1)}" stroke="#d9e4dd" stroke-width="1"/><text x="${pad.left-6}" y="${(gy+4).toFixed(1)}" text-anchor="end" font-size="14" fill="#6b746e">${Math.round(gv)}</text>`;}).join('');
  let bars='';
  const drawStack=(series,x)=>{let cum=0;series.forEach(s=>{const v=chartValue(s.values[currentIndex])||0;if(v<=0)return;const y0=y(cum+v),y1=y(cum),h=y1-y0;bars+=`<rect x="${x.toFixed(1)}" y="${y0.toFixed(1)}" width="${barW.toFixed(1)}" height="${h.toFixed(1)}" fill="${s.color}"/>`;cum+=v;});if(cum>0)bars+=`<text x="${(x+barW/2).toFixed(1)}" y="${(y(cum)-6).toFixed(1)}" text-anchor="middle" font-size="12" font-weight="800" fill="#26332d">${cum}</text>`;};
  let currentIndex=0;
  labels.forEach((lbl,i)=>{currentIndex=i;const center=pad.left+i*groupW+groupW/2,fx=center-barW-gap/2,mx=center+gap/2;drawStack(femaleSeries,fx);drawStack(maleSeries,mx);bars+=`<text x="${(fx+barW/2).toFixed(1)}" y="${height-pad.bottom+16}" text-anchor="middle" font-size="10.5" font-weight="700" fill="#7b5364">F</text><text x="${(mx+barW/2).toFixed(1)}" y="${height-pad.bottom+16}" text-anchor="middle" font-size="10.5" font-weight="700" fill="#3d6478">M</text><text x="${center.toFixed(1)}" y="${height-12}" text-anchor="middle" font-size="14" fill="#4a5750">${escapeHtml(lbl)}</text>`;});
  const legend=herdChartLegend(femaleSeries.map(s=>({label:s.label,color:s.color})));
  return `${legend}<div class="sale-sex-hint"><span><b>F</b> Femelles</span><span><b>M</b> Mâles</span></div><svg viewBox="0 0 ${width} ${height}" class="herd-svg-chart" aria-hidden="true"><rect width="${width}" height="${height}" rx="12" fill="#fffafd"/><g>${grid}<line x1="${pad.left}" y1="${height-pad.bottom}" x2="${width-pad.right}" y2="${height-pad.bottom}" stroke="#b9c9bf" stroke-width="1.2"/>${bars}</g></svg>`;
}
function prepSaleExitChartHtml(visit,farm,labels){
  const rows=prepAnnualSaleExitHistory(visit,farm,10),colors={lt8:'#2F5D8A',m8_16:'#5C8A4D',m16_36:'#C99324',gt36:'#8A5E8F'},defs=[['lt8','<8 mois'],['m8_16','8–16 mois'],['m16_36','16–36 mois'],['gt36','>36 mois']];
  const any=rows.some(r=>Object.keys(r).some(k=>k!=='year'&&r[k]>0));if(!any)return '<div class="empty">Aucune sortie E/H/B exploitable sur la période.</div>';
  const causePanel=(cause,title)=>{const mk=sex=>defs.map(([k,l])=>({label:l,color:colors[k],values:rows.map(r=>r[`${k}_${sex}_${cause}`]||0)}));return `<div class="sale-exit-panel cause-panel"><h5>${title}</h5><p class="muted small-text">Deux barres par année : femelles (F) et mâles (M), empilées par âge à la sortie.</p>${herdSexGroupedStackedBarChartSvg(labels,mk('F'),mk('M'),{height:330})}</div>`;};
  return `<div class="sale-exit-chart"><p class="muted">Sorties de vente uniquement. Les sorties cause <b>M</b> sont exclues car elles sont déjà analysées dans la mortalité. Les causes <b>E/H</b> et <b>B</b> sont volontairement séparées pour rendre les débouchés plus lisibles.</p><div class="sale-exit-panels separated-causes">${causePanel('EH','Sorties E/H — 10 ans')}${causePanel('B','Sorties B — 10 ans')}</div></div>`;
}
function prepRegistryTenYearChartsHtml(visit,farm,annualRows=null,{forPrint=false}={}){
  const hist=annualRows||reproMortalityAnnualHistory(visit,farm,10),moves=prepAnnualMovementHistory(visit,farm,10);if(!hist?.length)return '<div class="empty">Historique 10 ans indisponible dans le registre bovins.</div>';
  const currentYear=Number((visit?.date||new Date().toISOString().slice(0,10)).slice(0,4));
  const labels=hist.map(r=>r.year===currentYear?`${r.year}*`:String(r.year));
  const ivv=hist.map(r=>r.ivvMean);
  const mortalitySeries=[
    {label:'0–2 j',color:'#2F5D8A',values:hist.map(r=>r.deaths0_2),labelValues:hist.map(r=>r.deaths0_2Pct)},
    {label:'2 j–1 mois',color:'#B85C38',values:hist.map(r=>r.deaths2_30),labelValues:hist.map(r=>r.deaths2_30Pct)},
    {label:'1–6 mois',color:'#6B7C72',values:hist.map(r=>r.deaths1_6),labelValues:hist.map(r=>r.deaths1_6Pct)},
    {label:'6–12 mois',color:'#C99324',values:hist.map(r=>r.deaths6_12),labelValues:hist.map(r=>r.deaths6_12Pct)},
    {label:'12–24 mois',color:'#587FA8',values:hist.map(r=>r.deaths12_24),labelValues:hist.map(r=>r.deaths12_24Pct)},
    {label:'≥24 mois',color:'#3E7655',values:hist.map(r=>r.deathsOver24),labelValues:hist.map(r=>r.deathsOver24Pct)}
  ];
  const movementByYear=new Map((moves||[]).map(r=>[r.year,r]));
  const movementSeries=[
    {label:'Naissances',color:'#2F6F73',values:hist.map(r=>movementByYear.get(r.year)?.births)},
    {label:'Introductions ACHAT/PENSION',color:'#5C8A4D',values:hist.map(r=>movementByYear.get(r.year)?.entries)},
    {label:'Sorties totales',color:'#8A5E8F',values:hist.map(r=>movementByYear.get(r.year)?.exits)},
    {label:'Mortalités',color:'#B85C38',values:hist.map(r=>movementByYear.get(r.year)?.adultDeaths)}
  ];
  const productionFemales={label:'Femelles >36 mois présentes',color:'#B53670',values:hist.map(r=>movementByYear.get(r.year)?.femalesOver36)};
  const startYear=hist[0].year,endRow=hist[hist.length-1],periodText=`${startYear} → ${endRow.year}${endRow.truncated?` (année ${endRow.year} tronquée au ${formatDate(endRow.periodEnd)})`:''}`;
  return `<div class="herd-chart-grid registry-10y-charts ${forPrint?'report-mode':''}">
    <article class="card herd-chart-card"><h4>Évolution IVV — 10 ans</h4><p class="muted">Années civiles ${periodText}. L’IVV est rattaché à l’année du vêlage qui termine l’intervalle.</p>${herdLineChartSvg(labels,ivv,{color:'#2F6F73'})}</article>
    <article class="card herd-chart-card"><h4>Mortalité par classe d’âge — 10 ans</h4><p class="muted">Années civiles ${periodText}. La hauteur de chaque barre correspond au <b>nombre total de décès enregistrés dans l’année</b>. À l’intérieur de chaque segment figure la part (%) de cette classe d’âge parmi les décès de l’année. Les couleurs de la légende sont exactement celles du graphique.</p>${herdStackedBarChartSvg(labels,mortalitySeries)}</article>
    <article class="card herd-chart-card"><h4>Introductions / mouvements — 10 ans</h4><p class="muted">Années civiles ${periodText}. Barres : naissances, entrées ACHAT/PENSION, sorties et mortalité adulte. La courbe indique le nombre de <b>femelles de plus de 36 mois présentes</b> à la fin de chaque année (à la date de visite pour l’année en cours), afin de comparer les flux à la taille du troupeau vraisemblablement en production.</p>${herdGroupedBarLineChartSvg(labels,movementSeries,productionFemales)}</article>
    <article class="card herd-chart-card sale-exit-card"><h4>Répartition des sorties vendues — 10 ans</h4>${prepSaleExitChartHtml(visit,farm,labels)}</article>
    ${endRow.truncated?`<p class="muted"><b>* ${endRow.year} :</b> année en cours tronquée, du 01/01/${endRow.year} au ${formatDate(endRow.periodEnd)}.</p>`:''}
  </div>`;
}


// v14.6.21.99 — aide à la décision transversale : priorités, tendances et références internes.
function prepAnalysisReliability(n){n=Number(n)||0;return n>=30?{label:'forte',cls:'high'}:n>=10?{label:'moyenne',cls:'medium'}:{label:'prudence',cls:'low'};}
function prepAnalysisMean(vals){const a=vals.map(Number).filter(Number.isFinite);return a.length?Math.round(a.reduce((x,y)=>x+y,0)/a.length*10)/10:null;}
function prepAnalysisBest(vals,direction='down'){const a=vals.map(Number).filter(Number.isFinite);return a.length?(direction==='up'?Math.max(...a):Math.min(...a)):null;}
function prepAnalysisComparisonRows(hist=[]){
  if(!hist.length)return [];
  const cur=hist[hist.length-1]||{},prev=hist[hist.length-2]||{},completed=hist.slice(0,-1),ref5=completed.slice(-5),ref10=completed.slice(-10);
  const defs=[
    ['IVV moyen','ivvMean',' j','down'],
    ['IVV >500 j','ivvOver500Rate',' %','down'],
    ['Âge moyen 1er vêlage','firstCalvingMean',' mois','down'],
    ['Mortalité veaux <6 mois','mortalityRate',' %','down']
  ];
  const enrich=r=>({...r,ivvOver500Rate:r?.ivvCount?Math.round((r.ivvOver500||0)/r.ivvCount*1000)/10:null});
  const C=enrich(cur),P=enrich(prev),R5=ref5.map(enrich),R10=ref10.map(enrich);
  const isBetter=(a,b,direction)=>a!=null&&b!=null&&(direction==='down'?Number(a)<Number(b):Number(a)>Number(b));
  const isWorse=(a,b,direction)=>a!=null&&b!=null&&(direction==='down'?Number(a)>Number(b):Number(a)<Number(b));
  return defs.map(([label,key,suffix,direction])=>{
    const c=C[key],pr=P[key],avg=prepAnalysisMean(R5.map(r=>r[key])),best=prepAnalysisBest(R10.map(r=>r[key]),direction);
    let trend='—',cls='neutral',detail='';
    if(c!=null&&pr!=null){
      const currentBetterAvg=avg!=null&&isBetter(c,avg,direction), previousBetterAvg=avg!=null&&isBetter(pr,avg,direction);
      const currentWorseAvg=avg!=null&&isWorse(c,avg,direction);
      if(Number(c)===Number(pr)){
        trend=currentBetterAvg?'✓ stable à un niveau favorable':'→ stable';cls=currentBetterAvg?'good':'neutral';
      }else if(isBetter(c,pr,direction)){
        trend='↗ amélioration';cls='good';
        if(currentBetterAvg)detail='N est aussi meilleur que la moyenne 5 ans.';
      }else if(currentBetterAvg&&previousBetterAvg){
        // Un recul ponctuel n'est pas qualifié de dégradation si N et N-1 restent tous les deux meilleurs que la tendance de fond.
        trend='✓ reste favorable';cls='good';detail='Recul vs N-1, mais N et N-1 restent meilleurs que la moyenne des 5 années précédentes.';
      }else if(currentWorseAvg){
        trend='↘ dégradation';cls='bad';detail='N se dégrade vs N-1 et se situe du mauvais côté de la moyenne 5 ans.';
      }else{
        trend='→ variation à surveiller';cls='neutral';detail='Variation vs N-1 sans dégradation confirmée par la tendance de fond.';
      }
    }
    return {label,key,suffix,direction,current:c,previous:pr,avg5:avg,best10:best,trend,cls,detail};
  });
}
function prepAnalysisComparisonHtml(hist=[],{forPrint=false}={}){
  const rows=prepAnalysisComparisonRows(hist);if(!rows.length)return '';
  const fmt=(v,s)=>v==null?'—':`${String(v).replace('.',',')}${s}`;
  return `<div class="decision-comparison ${forPrint?'for-print':''}"><h4>Repères internes — N / N-1 / moyenne 5 ans / meilleur résultat 10 ans</h4><p class="muted small-text">N correspond à l’année de la visite et peut être incomplète. Les références 5 et 10 ans privilégient les années civiles terminées.</p><div class="table-wrap"><table><thead><tr><th>Indicateur</th><th>N</th><th>N-1</th><th>Moy. 5 ans</th><th>Meilleur 10 ans</th><th>Tendance</th></tr></thead><tbody>${rows.map(r=>`<tr><th>${escapeHtml(r.label)}</th><td>${fmt(r.current,r.suffix)}</td><td>${fmt(r.previous,r.suffix)}</td><td>${fmt(r.avg5,r.suffix)}</td><td>${fmt(r.best10,r.suffix)}</td><td class="decision-trend-${r.cls}"><strong>${escapeHtml(r.trend)}</strong>${r.detail?`<br><small>${escapeHtml(r.detail)}</small>`:''}</td></tr>`).join('')}</tbody></table></div></div>`;
}

// v14.6.21.99 — cohérence pré-visite, tableau global mouvements/renouvellement et âges lisibles.
function prepAnnualMovementHistory(visit,farm,years=10){
  const reg=(reproductionSourceForVisit(visit,farm)?.registry||[]),end=visit?.date||new Date().toISOString().slice(0,10),endYear=Number(end.slice(0,4)),out=[];
  for(let y=endYear-years+1;y<=endYear;y++){
    const yEnd=y===endYear?end:`${y}-12-31`,inYear=d=>d&&Number(String(d).slice(0,4))===y&&d<=yEnd;
    const births=reg.filter(a=>inYear(a.birthDate)),entries=reg.filter(a=>inYear(a.entryDate)&&reproIsPurchaseIntroduction(a)),exits=reg.filter(a=>inYear(a.exitDate));
    const adultDeaths=exits.filter(a=>a.exitCause==='M'&&a.birthDate&&monthsBetweenDates(a.birthDate,a.exitDate)>=24);
    const adultFemaleOutputs=exits.filter(a=>a.sex==='F'&&a.exitCause!=='M'&&a.birthDate&&monthsBetweenDates(a.birthDate,a.exitDate)>=24);
    const femaleBirths=births.filter(a=>a.sex==='F');
    const retained24=femaleBirths.filter(a=>{if(!a.birthDate)return false;const d24=dateShiftDays(a.birthDate,730);return d24<=end&&(!a.exitDate||a.exitDate>=d24);});
    const entryAges=entries.map(a=>a.birthDate&&a.entryDate?monthsBetweenDates(a.birthDate,a.entryDate):null).filter(v=>v!=null&&Number.isFinite(v));
    const femalesOver36=reg.filter(a=>a.sex==='F'&&isRegistryAnimalPresent(a,yEnd)&&a.birthDate&&monthsBetweenDates(a.birthDate,yEnd)>36).length;
    out.push({year:y,births:births.length,entries:entries.length,exits:exits.length,adultDeaths:adultDeaths.length,adultFemaleOutputs:adultFemaleOutputs.length,femaleBirths:femaleBirths.length,retained24:retained24.length,retained24Rate:femaleBirths.length?Math.round(retained24.length/femaleBirths.length*1000)/10:null,entryAgeMean:entryAges.length?Math.round(entryAges.reduce((a,b)=>a+b,0)/entryAges.length*10)/10:null,femalesOver36});
  }
  return out;
}
function prepAgeYearsMonths(value){
  const m=Number(value);if(!Number.isFinite(m))return '—';
  const total=Math.max(0,Math.round(m)),y=Math.floor(total/12),mo=total%12;
  if(y&&mo)return `${y} an${y>1?'s':''} ${mo} mois`;
  if(y)return `${y} an${y>1?'s':''}`;
  return `${mo} mois`;
}
function prepMovementHistoryHtml(rows,{forPrint=false}={}){
  if(!rows?.length)return '';
  const years=rows.map(r=>r.year),cell=v=>v==null?'—':String(v).replace('.',',');
  const row=(label,key,suf='')=>`<tr><th>${escapeHtml(label)}</th>${rows.map(r=>`<td>${cell(r[key])}${r[key]!=null?suf:''}</td>`).join('')}</tr>`;
  return `<div class="decision-movement-history ${forPrint?'for-print':''}"><h4>Mouvements et renouvellement — historique 10 ans</h4><p class="muted small-text">Données issues du registre bovins. « Introductions » = entrées identifiées ACHAT/PENSION. « Réformes » = sorties de femelles adultes hors mortalité (indicateur approché à confirmer avec l’éleveur).</p><div class="table-wrap"><table class="annual-history-table"><thead><tr><th>Indicateur</th>${years.map(y=>`<th>${y}</th>`).join('')}</tr></thead><tbody>${row('Naissances','births')}${row('Introductions ACHAT/PENSION','entries')}${row('Sorties totales','exits')}${row('Réformes femelles adultes*','adultFemaleOutputs')}${row('Mortalité adulte','adultDeaths')}${row('Femelles nées','femaleBirths')}${row('Femelles conservées ≥24 m','retained24')}${row('Part conservée ≥24 m','retained24Rate',' %')}${row('Âge moyen à l’introduction','entryAgeMean',' mois')}</tbody></table></div></div>`;
}

// v14.6.21.99 — « fabrique du troupeau » : naissance femelles → conservation jusqu’à la reproduction → 1er vêlage → réforme.
function prepRenewalFactoryData(visit,farm,years=10){
  const reg=(reproductionSourceForVisit(visit,farm)?.registry||[]),end=visit?.date||new Date().toISOString().slice(0,10),endYear=Number(end.slice(0,4));
  const a=ensureAuditGlobal(visit),configured=Number(a.renewal?.reproAgeMonths),reproAgeMonths=Number.isFinite(configured)&&configured>0?configured:24;
  const femaleReg=reg.filter(x=>x.sex==='F'&&x.birthDate&&x.birthDate<=end),firstCalvingByDam=new Map();
  reg.filter(c=>c.motherId&&c.birthDate&&c.birthDate<=end).forEach(c=>{const k=normalizeAnimalId(c.motherId);if(!k)return;const old=firstCalvingByDam.get(k);if(!old||c.birthDate<old)firstCalvingByDam.set(k,c.birthDate);});
  const rows=[];
  for(let y=endYear-years+1;y<=endYear;y++){
    const cohort=femaleReg.filter(f=>Number(String(f.birthDate).slice(0,4))===y),evaluable=cohort.filter(f=>dateShiftDays(f.birthDate,Math.round(reproAgeMonths*30.44))<=end);
    const reached=evaluable.filter(f=>!f.exitDate||f.exitDate>=dateShiftDays(f.birthDate,Math.round(reproAgeMonths*30.44)));
    const lostBefore=evaluable.filter(f=>f.exitDate&&f.exitDate<dateShiftDays(f.birthDate,Math.round(reproAgeMonths*30.44)));
    const calvingEligible=reached.filter(f=>dateShiftDays(f.birthDate,Math.round(reproAgeMonths*30.44)+310)<=end);
    const calved=calvingEligible.filter(f=>{const d=firstCalvingByDam.get(normalizeAnimalId(f.id));return d&&d<=end;});
    const calvingAges=calved.map(f=>monthsBetweenDates(f.birthDate,firstCalvingByDam.get(normalizeAnimalId(f.id)))).filter(Number.isFinite);
    const reformEligible=calved.filter(f=>{const fc=firstCalvingByDam.get(normalizeAnimalId(f.id));if(!fc)return false;const observedExit=f.exitDate&&f.exitDate>=fc&&f.exitDate<=end;return observedExit||dateShiftDays(fc,365)<=end;});
    const reformed=reformEligible.filter(f=>{const fc=firstCalvingByDam.get(normalizeAnimalId(f.id));return fc&&f.exitDate&&f.exitDate>=fc&&f.exitDate<=end&&f.exitCause!=='M';});
    const reformAges=reformed.map(f=>monthsBetweenDates(f.birthDate,f.exitDate)).filter(Number.isFinite);
    rows.push({year:y,femaleBirths:cohort.length,evaluable:evaluable.length,reachedRepro:reached.length,lostBeforeRepro:lostBefore.length,reachedReproRate:evaluable.length?Math.round(reached.length/evaluable.length*1000)/10:null,calvingEligible:calvingEligible.length,firstCalved:calved.length,firstCalvingRate:calvingEligible.length?Math.round(calved.length/calvingEligible.length*1000)/10:null,firstCalvingAgeMean:calvingAges.length?Math.round(calvingAges.reduce((x,z)=>x+z,0)/calvingAges.length*10)/10:null,reformEligible:reformEligible.length,reformed:reformed.length,reformAgeMean:reformAges.length?Math.round(reformAges.reduce((x,z)=>x+z,0)/reformAges.length*10)/10:null});
  }
  const usable=rows.filter(r=>r.evaluable>0),lastRepro=usable.at(-1)||null,previous=usable.at(-2)||null;
  const lastCalving=rows.filter(r=>r.calvingEligible>0).at(-1)||null;
  const lastReform=rows.filter(r=>r.reformEligible>0).at(-1)||null;
  return {reproAgeMonths,rows,last:lastRepro,lastRepro,lastCalving,lastReform,previous};
}
function prepRenewalFactoryHtml(data,{forPrint=false}={}){
  if(!data?.rows?.length)return '';
  const rows=data.rows,years=rows.map(r=>r.year),cell=(v,s='')=>v==null?'—':`${String(v).replace('.',',')}${s}`;
  const tr=(label,key,s='')=>`<tr><th>${escapeHtml(label)}</th>${rows.map(r=>`<td>${cell(r[key],s)}</td>`).join('')}</tr>`;
  const LR=data.lastRepro||data.last||{},LC=data.lastCalving||{},LF=data.lastReform||{};
  const summary=[
    ['Femelles nées',LR.femaleBirths??'—',LR.year?`cohorte ${LR.year} · dernière cohorte évaluable à ${data.reproAgeMonths} mois`:'recul insuffisant'],
    [`Conservées jusqu’à ${data.reproAgeMonths} mois`,LR.reachedRepro??'—',LR.year?(LR.reachedReproRate!=null?`cohorte ${LR.year} · ${LR.reachedReproRate}% des femelles évaluables`:`cohorte ${LR.year}`):'recul insuffisant'],
    ['Ayant atteint le 1er vêlage',LC.calvingEligible>0?LC.firstCalved:'—',LC.year?`cohorte ${LC.year} · ${LC.firstCalved}/${LC.calvingEligible} avec recul suffisant${LC.firstCalvingRate!=null?` (${LC.firstCalvingRate}%)`:''}`:'recul insuffisant'],
    ['Réformées après 1er vêlage',LF.reformEligible>0?LF.reformed:'—',LF.year?`cohorte ${LF.year} · ${LF.reformed}/${LF.reformEligible} avec recul de suivi${LF.reformAgeMean!=null?` · âge moyen ${prepAgeYearsMonths(LF.reformAgeMean)}`:''}`:'recul insuffisant']
  ];
  return `<div class="renewal-factory ${forPrint?'for-print':''}"><h4>🐄 Fabrique du troupeau — naissance → reproduction → 1er vêlage → réforme</h4><p class="muted small-text">Le seuil de « conservation jusqu’à la reproduction » est actuellement fixé à <strong>${data.reproAgeMonths} mois</strong> (modifiable dans Structure du troupeau et renouvellement). <strong>Chaque tuile indique désormais la cohorte réellement utilisée</strong> : une cohorte trop jeune n’est plus affichée artificiellement à 0.</p><div class="renewal-funnel">${summary.map(([l,v,n])=>`<article><strong>${escapeHtml(String(v))}</strong><span>${escapeHtml(l)}</span><small>${escapeHtml(n||'')}</small></article>`).join('')}</div><div class="table-wrap"><table class="annual-history-table"><thead><tr><th>Indicateur / cohorte de naissance</th>${years.map(y=>`<th>${y}</th>`).join('')}</tr></thead><tbody>${tr('Femelles nées','femaleBirths')}${tr('Cohorte évaluable à l’âge repro','evaluable')}${tr(`Conservées jusqu’à ${data.reproAgeMonths} mois`,'reachedRepro')}${tr('Pertes / sorties avant mise à la repro','lostBeforeRepro')}${tr('Taux conservées jusqu’à la repro','reachedReproRate',' %')}${tr('Femelles avec recul suffisant pour 1er vêlage','calvingEligible')}${tr('Ayant atteint le 1er vêlage','firstCalved')}${tr('Taux atteignant le 1er vêlage','firstCalvingRate',' %')}${tr('Âge moyen au 1er vêlage','firstCalvingAgeMean',' mois')}${tr('Femelles avec recul de suivi après 1er vêlage','reformEligible')}${tr('Réformées / sorties après 1er vêlage*','reformed')}${tr('Âge moyen à la réforme / sortie*','reformAgeMean',' mois')}</tbody></table></div><p class="muted small-text">* Réforme approchée par les sorties de femelles après leur premier vêlage, hors mortalité. Pour éviter les faux « 0 », la synthèse réforme utilise uniquement une cohorte disposant d’un recul de suivi (sortie déjà observée ou au moins 12 mois depuis le 1er vêlage).</p></div>`;
}

function prepMovementRenewalGlobalHtml(movements,renewalFactory,{forPrint=false}={}){
  if(!movements?.length||!renewalFactory?.rows?.length)return '';
  const years=movements.map(r=>r.year),fByYear=new Map(renewalFactory.rows.map(r=>[r.year,r]));
  const cell=v=>v==null?'—':String(v).replace('.',',');
  const mvRow=(label,key,suf='')=>`<tr><th>${escapeHtml(label)}</th>${movements.map(r=>`<td>${r[key]==null?'—':cell(r[key])+suf}</td>`).join('')}</tr>`;
  const fRow=(label,key,suf='',formatter=null)=>`<tr><th>${escapeHtml(label)}</th>${years.map(y=>{const r=fByYear.get(y)||{},v=r[key];return `<td>${v==null?'—':(formatter?formatter(v):cell(v)+suf)}</td>`}).join('')}</tr>`;
  const LR=renewalFactory.lastRepro||renewalFactory.last||{},LC=renewalFactory.lastCalving||{},LF=renewalFactory.lastReform||{};
  const summary=[
    ['Femelles nées',LR.femaleBirths??'—',LR.year?`cohorte ${LR.year} · dernière cohorte évaluable`:'recul insuffisant'],
    [`Conservées jusqu’à ${renewalFactory.reproAgeMonths} mois`,LR.reachedRepro??'—',LR.year?`cohorte ${LR.year}${LR.reachedReproRate!=null?` · ${LR.reachedReproRate}% des femelles évaluables`:''}`:'recul insuffisant'],
    ['Ayant atteint le 1er vêlage',LC.calvingEligible>0?LC.firstCalved:'—',LC.year?`cohorte ${LC.year} · ${LC.firstCalved}/${LC.calvingEligible} avec recul suffisant`:'recul insuffisant'],
    ['Réformées après 1er vêlage',LF.reformEligible>0?LF.reformed:'—',LF.year?`cohorte ${LF.year} · ${LF.reformed}/${LF.reformEligible} avec recul de suivi${LF.reformAgeMean!=null?` · âge moyen ${prepAgeYearsMonths(LF.reformAgeMean)}`:''}`:'recul insuffisant']
  ];
  return `<div class="decision-movement-history renewal-factory ${forPrint?'for-print':''}"><h4>🐄 Mouvements, renouvellement et fabrique du troupeau — historique 10 ans</h4><p class="muted small-text">Tableau unique de référence pour éviter les doublons. Les tuiles et les lignes « fabrique du troupeau » reposent exactement sur les mêmes cohortes et les mêmes règles de calcul. « Introductions » = entrées ACHAT/PENSION.</p><div class="renewal-funnel">${summary.map(([l,v,n])=>`<article><strong>${escapeHtml(String(v))}</strong><span>${escapeHtml(l)}</span><small>${escapeHtml(n||'')}</small></article>`).join('')}</div><div class="table-wrap"><table class="annual-history-table"><thead><tr><th>Indicateur / cohorte</th>${years.map(y=>`<th>${y}</th>`).join('')}</tr></thead><tbody>${mvRow('Naissances totales','births')}${mvRow('Introductions ACHAT/PENSION','entries')}${mvRow('Sorties totales','exits')}${mvRow('Mortalité adulte','adultDeaths')}${fRow('Femelles nées','femaleBirths')}${fRow('Cohorte évaluable à l’âge repro','evaluable')}${fRow(`Conservées jusqu’à ${renewalFactory.reproAgeMonths} mois`,'reachedRepro')}${fRow('Pertes / sorties avant mise à la repro','lostBeforeRepro')}${fRow('Taux conservées jusqu’à la repro','reachedReproRate',' %')}${fRow('Femelles avec recul suffisant pour 1er vêlage','calvingEligible')}${fRow('Ayant atteint le 1er vêlage','firstCalved')}${fRow('Taux atteignant le 1er vêlage','firstCalvingRate',' %')}${fRow('Âge moyen au 1er vêlage','firstCalvingAgeMean',' mois')}${fRow('Femelles avec recul de suivi après 1er vêlage','reformEligible')}${fRow('Réformées / sorties après 1er vêlage*','reformed')}${fRow('Âge moyen à la réforme / sortie*','reformAgeMean','',prepAgeYearsMonths)}${mvRow('Âge moyen à l’introduction','entryAgeMean',' mois')}</tbody></table></div><p class="muted small-text">* Réforme approchée par les sorties de femelles après leur premier vêlage, hors mortalité. À confirmer avec le motif réel de sortie lorsqu’il est disponible. Les cohortes trop récentes restent à « — ».</p></div>`;
}

function prepFeedingRiskData(visit){
  let rows=[];try{rows=mineralCoverageRows(visit)||[]}catch(_){rows=[]}
  const low=rows.filter(r=>r.coverage!=null&&r.coverage<80),border=rows.filter(r=>r.coverage!=null&&r.coverage>=80&&r.coverage<95),high=rows.filter(r=>r.coverage!=null&&r.coverage>180);
  const rations=visit?.feeding?.rations||[],foods=visit?.feeding?.feedProducts||visit?.feeding?.products||[];
  return {low,border,high,rations,foods,hasData:!!(rations.length||rows.some(r=>r.coverage!=null))};
}
function prepPreviousVisit(visit){
  return (db.visits||[]).filter(v=>v.farmId===visit.farmId&&v.id!==visit.id&&v.date&&visit.date&&v.date<visit.date).sort((a,b)=>(b.date||'').localeCompare(a.date||''))[0]||null;
}
function prepPreviousRecommendations(visit){
  const prev=prepPreviousVisit(visit);if(!prev)return {prev:null,items:[]};
  const a=ensureAuditGlobal(visit);a.recommendationFollowUp=a.recommendationFollowUp&&typeof a.recommendationFollowUp==='object'?a.recommendationFollowUp:{};
  const src=[];
  (prev.visitConclusion?.priorities||[]).filter(x=>x?.text).forEach((x,i)=>src.push({key:`priority-${i}-${normalizeSearchText(x.text).slice(0,32)}`,text:x.text,source:'Priorité de conclusion'}));
  Object.entries(prev.auditGlobal?.chapterSummaries||{}).forEach(([section,v])=>{
    const txt=[v?.watch,v?.comments].filter(Boolean).join('\n');txt.split(/\n|•|;/).map(x=>x.trim()).filter(x=>x.length>5).slice(0,3).forEach((t,i)=>src.push({key:`${section}-${i}-${normalizeSearchText(t).slice(0,28)}`,text:t,source:'Synthèse '+section}));
  });
  const seen=new Set(),items=src.filter(x=>{const k=normalizeSearchText(x.text);if(seen.has(k))return false;seen.add(k);return true;}).slice(0,8).map(x=>({...x,status:a.recommendationFollowUp[x.key]||'À vérifier'}));
  return {prev,items};
}
function prepCrossHypotheses(visit,farm,x,hist,rolling,decision){
  const a=ensureAuditGlobal(visit),domains=Object.fromEntries(domainIndicators(visit).map(d=>[d.label,d.value])),feed=prepFeedingRiskData(visit),h=[];
  const build=buildingRecords(visit),hasWaterData=build.drinkers.length>0,hasBuildingData=Object.keys(visit.buildingAudits||{}).length>0;
  const sanSec=auditGlobalSections.find(q=>q.id==='sanitaire'),hasSanitaryData=sanSec?auditSectionProgress(sanSec,a).done>0:false;
  const add=(title,signals,checks)=>h.push({title,signals,checks});
  if((rolling?.mortalityRate??0)>=10&&((rolling?.firstCalvingMean??0)>32||feed.low.length))add('Veaux / renouvellement : plusieurs signaux convergent',[`mortalité <6 mois ${rolling.mortalityRate}%`,rolling.firstCalvingMean?`1er vêlage ${rolling.firstCalvingMean} mois`:null,feed.low.length?`${feed.low.length} couverture(s) nutritionnelle(s) basse(s)`:null].filter(Boolean),['colostrum (délai, quantité, qualité)','désinfection du nombril','logement/hygiène des veaux','alimentation des génisses et vaches en fin de gestation']);
  if((rolling?.ivvMean??0)>450&&feed.low.length)add('Reproduction + alimentation à croiser',[`IVV ${rolling.ivvMean} j`,`${feed.low.length} élément(s) de ration sous 80% de couverture`],['état corporel','ingestion réelle et disponibilité des fourrages','énergie/protéines si valeurs disponibles','minéraux et transitions alimentaires']);
  const intro=prepAnnualMovementHistory(visit,farm,10).slice(-1)[0];
  if((intro?.entries||0)>0&&hasSanitaryData&&((domains['Santé / analyses']??100)<70||auditAttentionCount(visit,'sanitaire')>0))add('Introductions + sanitaire : vérifier la chronologie',[`${intro.entries} introduction(s) sur N`,`signal sanitaire/audit présent`],['origine et statut sanitaire','quarantaine et délai avant mélange','date d’apparition des problèmes par rapport aux entrées']);
  if(hasBuildingData&&hasSanitaryData&&(domains['Bâtiment']??100)<60&&auditAttentionCount(visit,'sanitaire')>0)add('Bâtiment / ambiance + pathologies : rechercher une cause environnementale',[`indice bâtiment ${domains['Bâtiment']}/100`,'points sanitaires à surveiller'],['ventilation et ammoniac','litière et zones humides','densité/couchage','lien avec respiratoire, mammites ou boiteries']);
  if(hasWaterData&&(domains['Eau']??100)<60)add('Abreuvement : impact transversal possible',[`indice eau ${domains['Eau']}/100`],['débit réel aux heures de pointe','nombre de points d’eau et concurrence','propreté/accessibilité','impact sur ingestion, production et chaleur']);
  return h.slice(0,6);
}
function prepSanitarySummary(visit){
  const a=ensureAuditGlobal(visit),cur=auditAttentionCount(visit,'sanitaire'),prev=prepPreviousVisit(visit),prevCount=prev?auditAttentionCount(prev,'sanitaire'):null;
  const status=prevCount==null?'Première comparaison disponible':cur<prevCount?'Amélioration depuis la dernière visite':cur>prevCount?'Davantage de points à surveiller qu’à la dernière visite':'Stable depuis la dernière visite';
  return {cur,prevCount,status};
}
function prepExtendedDecisionHtml(visit,farm,x,hist,rolling,d,{forPrint=false}={}){
  const movements=prepAnnualMovementHistory(visit,farm,10),renewalFactory=prepRenewalFactoryData(visit,farm,10),hyp=prepCrossHypotheses(visit,farm,x,hist,rolling,d),follow=prepPreviousRecommendations(visit);
  const excluded=new Set(['Sanitaire','Biosécurité','Alimentation','Eau / abreuvement','Logement / litière']);
  const strengths=d.themes.filter(t=>!excluded.has(t.label)&&Number.isFinite(t.score)&&t.score>=75).sort((a,b)=>b.score-a.score).slice(0,5);
  const followHtml=follow.prev?`<div class="decision-followup"><h4>Suivi depuis la visite du ${formatDate(follow.prev.date)}</h4>${follow.items.length?`<table><thead><tr><th>Recommandation / point de vigilance</th><th>État à cette visite</th></tr></thead><tbody>${follow.items.map(i=>`<tr><td>${escapeHtml(i.text)}</td><td>${forPrint?`<strong>${escapeHtml(i.status)}</strong>`:`<select data-recommend-follow="${escapeHtml(i.key)}"><option ${i.status==='À vérifier'?'selected':''}>À vérifier</option><option ${i.status==='Fait'?'selected':''}>Fait</option><option ${i.status==='Partiellement fait'?'selected':''}>Partiellement fait</option><option ${i.status==='Non fait'?'selected':''}>Non fait</option></select>`}</td></tr>`).join('')}</tbody></table>`:'<p class="muted">Aucune recommandation structurée retrouvée dans la visite précédente.</p>'}</div>`:'';
  return `<div class="decision-extended ${forPrint?'for-print':''}"><div class="decision-onepage"><h3>Repères complémentaires</h3><div><h4>Points forts</h4>${strengths.length?`<ul>${strengths.map(t=>`<li><strong>${escapeHtml(t.label)}</strong> — ${escapeHtml(t.why)}</li>`).join('')}</ul>`:'<p class="muted">Pas assez de données pour dégager des points forts fiables sur les indicateurs documentaires.</p>'}</div>
  ${hyp.length?`<div class="decision-hypotheses"><h4>Hypothèses à vérifier — croisements automatiques</h4>${hyp.map(q=>`<article class="notice warning"><strong>${escapeHtml(q.title)}</strong><div><b>Signaux :</b> ${escapeHtml(q.signals.join(' · '))}</div><div><b>À vérifier :</b> ${escapeHtml(q.checks.join(' · '))}</div><small>Hypothèse de travail uniquement : la convergence d’indicateurs ne prouve pas un lien de causalité.</small></article>`).join('')}</div>`:''}
  ${prepMovementRenewalGlobalHtml(movements,renewalFactory,{forPrint})}${followHtml}</div>`;
}
document.addEventListener('change',e=>{
  const el=e.target;if(!el?.matches?.('[data-recommend-follow]'))return;
  const visit=activeVisit();if(!visit)return;const a=ensureAuditGlobal(visit);a.recommendationFollowUp=a.recommendationFollowUp&&typeof a.recommendationFollowUp==='object'?a.recommendationFollowUp:{};a.recommendationFollowUp[el.dataset.recommendFollow]=el.value;saveAuditGlobal(visit);
});

function prepDecisionSupportData(visit,farm,x,hist,rolling){
  const a=ensureAuditGlobal(visit),domainList=domainIndicators(visit),domains=Object.fromEntries(domainList.map(d=>[d.label,d.value])),alerts=[];
  const sectionDone=id=>{const sec=auditGlobalSections.find(q=>q.id===id);return sec?auditSectionProgress(sec,a).done:0;};
  const buildData=buildingRecords(visit),hasWaterData=buildData.drinkers.length>0,hasBuildingData=Object.keys(visit.buildingAudits||{}).length>0,hasYoungData=sectionDone('jeunes')>0,hasSanitaryData=sectionDone('sanitaire')>0;
  const add=(severity,theme,title,detail,key='',n=null)=>alerts.push({severity,theme,title,detail,key,n});
  if(rolling?.mortalityRate!=null){const n=rolling.births.length,rel=prepAnalysisReliability(n);if(rolling.mortalityRate>=15)add(3,'Mortalité / veaux',`Mortalité <6 mois élevée : ${rolling.mortalityRate}%`,`${rolling.dead6.length}/${n} veaux sur 12 mois · fiabilité ${rel.label}`,'dead6',n);else if(rolling.mortalityRate>=8)add(2,'Mortalité / veaux',`Mortalité <6 mois à surveiller : ${rolling.mortalityRate}%`,`${rolling.dead6.length}/${n} veaux sur 12 mois · fiabilité ${rel.label}`,'dead6',n);}
  if(rolling?.ivvMean!=null){const n=rolling.intervals.length,rel=prepAnalysisReliability(n);if(rolling.ivvMean>500)add(3,'Reproduction',`IVV moyen très long : ${rolling.ivvMean} j`,`${n} IVV calculables · fiabilité ${rel.label}`,'ivvover',n);else if(rolling.ivvMean>450)add(2,'Reproduction',`IVV moyen à approfondir : ${rolling.ivvMean} j`,`${n} IVV calculables · fiabilité ${rel.label}`,'problems',n);}
  if(rolling?.firstCalvingMean!=null&&rolling.firstCalvingMean>36)add(3,'Renouvellement',`1er vêlage tardif : ${rolling.firstCalvingMean} mois`,`${rolling.firstCalvingLateCount||0}/${rolling.firstCalvingCount||0} femelle(s) ont vêlé après 36 mois sur les 12 derniers mois${rolling.firstCalvingLateRate!=null?` (${rolling.firstCalvingLateRate}%)`:''}`,'problems',rolling.firstCalvingCount||null);
  else if(rolling?.firstCalvingMean!=null&&rolling.firstCalvingMean>30)add(2,'Renouvellement',`Âge au 1er vêlage à surveiller : ${rolling.firstCalvingMean} mois`,`${rolling.firstCalvingLateCount||0}/${rolling.firstCalvingCount||0} femelle(s) ont vêlé après 36 mois sur les 12 derniers mois${rolling.firstCalvingLateRate!=null?` (${rolling.firstCalvingLateRate}%)`:''}`,'problems',rolling.firstCalvingCount||null);
  if(x?.heifers36?.length)add(x.heifers36.length>=3?3:2,'Renouvellement',`${x.heifers36.length} femelle(s) >36 mois sans vêlage`,'À identifier et expliquer pendant la visite','h36',x.heifers36.length);
  if(x?.cowProblems?.length){const p=x.cows?.length?Math.round(x.cowProblems.length/x.cows.length*100):null;if(p!=null&&p>=40)add(3,'Reproduction',`${x.cowProblems.length} vache(s) à problème (${p}%)`,'Concentrer la revue sur les animaux les plus pénalisés','problems',x.cowProblems.length);}
  const mv=prepAnnualMovementHistory(visit,farm,10),currentMv=mv[mv.length-1]||{},base=Math.max(1,x?.present?.length||0),introPct=Math.round((currentMv.entries||0)/base*100);
  if((currentMv.entries||0)>0&&introPct>=15)add(2,'Mouvements / introductions',`${currentMv.entries} introduction(s) sur N`,`≈ ${introPct}% de l’effectif présent : vérifier origine, quarantaine, statut sanitaire et devenir des animaux`,'',currentMv.entries);
  const feed=prepFeedingRiskData(visit);
  if(feed.low.length)add(feed.low.length>=3?3:2,'Alimentation',`${feed.low.length} couverture(s) calculée(s) sous 80%`,feed.low.map(r=>`${r.label} ${Math.round(r.coverage)}%`).join(' · '));
  if(feed.high.length)add(2,'Alimentation',`${feed.high.length} surapport(s) potentiel(s) >180%`,feed.high.map(r=>`${r.label} ${Math.round(r.coverage)}%`).join(' · '));
  const waterScore=domains['Eau']??40,buildingScore=domains['Bâtiment']??40,feedScore=domains['Alimentation']??40,sanScore=domains['Santé / analyses']??40;
  if(hasWaterData&&waterScore<60)add(2,'Eau / abreuvement',`Abreuvement à approfondir (indice ${waterScore}/100)`,'Les données existent déjà : vérifier surtout débit, accessibilité, propreté et concurrence aux points d’eau.');
  if(hasBuildingData&&buildingScore<60)add(2,'Logement / litière',`Bâtiment à approfondir (indice ${buildingScore}/100)`,'Les données existent déjà : cibler ventilation, litière, densité, zones humides, couchage et circulation.');
  const youngAttention=auditAttentionCount(visit,'jeunes');if(youngAttention>0)add(youngAttention>=4?3:2,'Veaux / colostrum',`${youngAttention} point(s) de conduite des veaux à revoir`,'Croiser colostrum, nombril, logement, nettoyage, eau/aliment solide, diarrhées et respiratoire.');
  const sanitaryAttention=auditAttentionCount(visit,'sanitaire');if(sanitaryAttention>0)add(sanitaryAttention>=5?3:2,'Sanitaire',`${sanitaryAttention} point(s) sanitaires à approfondir`,'Rechercher les pathologies récurrentes, vaccinations, traitements, parasitisme et chronologie des épisodes.');
  const bioQuestions=['Gestion des introductions et quarantaine','Statut sanitaire des animaux achetés','Gestion des animaux malades et possibilité d’isolement','Accès visiteurs maîtrisé'];
  const bioAnswered=bioQuestions.filter(q=>{const z=a.answers?.[q]||{};return !!(z.answer||(z.values||[]).length||z.comment);}).length;
  const bioIssues=bioQuestions.filter(q=>{const z=a.answers?.[q]||{};const t=normalizeSearchText([z.answer,(z.values||[]).join(' '),z.comment].join(' '));return /non|jamais|rare|insuff|corriger|surveill|absence|pas de/.test(t);}).length;
  if(bioAnswered&&bioIssues)add(bioIssues>=2?3:2,'Biosécurité',`${bioIssues} point(s) de biosécurité à revoir`,'Introductions, quarantaine, isolement et maîtrise des accès doivent être recroisés avec les mouvements et le sanitaire.');
  alerts.sort((u,v)=>v.severity-u.severity);
  const mortScore=rolling?.mortalityRate==null?40:rolling.mortalityRate<5?90:rolling.mortalityRate<10?72:rolling.mortalityRate<15?55:30;
  const reproScore=rolling?.ivvMean==null?40:rolling.ivvMean<=400?92:rolling.ivvMean<=450?75:rolling.ivvMean<=500?55:30;
  const renewalRaw=renewalRate(a.renewal),renewalN=Number(renewalRaw),renewalScore=Number.isFinite(renewalN)?(renewalN>=15&&renewalN<=30?85:renewalN>=10&&renewalN<=35?65:45):((x?.heifers36?.length||0)===0?70:45);
  const movementScore=(currentMv.entries||0)===0?85:introPct>15?50:70;
  const calfScore=Math.max(20,90-youngAttention*12-(rolling?.mortalityRate>=10?20:0));
  const sanitaryThemeScore=Math.max(20,Math.min(100,sanScore-sanitaryAttention*5));
  const biosafetyScore=Math.max(20,85-bioIssues*25-(introPct>=15?15:0));
  const themes=[
    {label:'Reproduction',score:reproScore,why:rolling?.ivvMean!=null?`IVV 12 mois : ${rolling.ivvMean} j`:'Données insuffisantes'},
    {label:'Mortalité / veaux',score:mortScore,why:rolling?.mortalityRate!=null?`${rolling.mortalityRate}% sur ${rolling.births.length} veaux`:'Données insuffisantes'},
    {label:'Renouvellement',score:renewalScore,why:Number.isFinite(renewalN)?`Taux renseigné : ${String(renewalN).replace('.',',')}%`:`${x?.heifers36?.length||0} femelle(s) >36 m sans vêlage`},
    {label:'Veaux / colostrum',score:hasYoungData?calfScore:null,why:hasYoungData?(youngAttention?`${youngAttention} point(s) d’attention dans la conduite des veaux`:'Données terrain renseignées : pas de point majeur signalé'):'À évaluer pendant la visite'},
    {label:'Mouvements / introductions',score:movementScore,why:`${currentMv.entries||0} introduction(s) sur N · ${introPct}% de l’effectif présent`},
    {label:'Sanitaire',score:hasSanitaryData?sanitaryThemeScore:null,why:hasSanitaryData?(sanitaryAttention?`${sanitaryAttention} point(s) à approfondir`:'Données renseignées : pas de point majeur signalé'):'À évaluer / confirmer pendant la visite'},
    {label:'Biosécurité',score:bioAnswered?biosafetyScore:null,why:bioAnswered?(bioIssues?`${bioIssues} point(s) à revoir`:'Données terrain renseignées : pas de point majeur signalé'):'À évaluer pendant la visite'},
    {label:'Alimentation',score:feed.hasData?feedScore:null,why:feed.hasData?(feed.low.length?`${feed.low.length} déficit(s) potentiel(s) calculé(s)`:'Ration renseignée / couverture calculable'):'À évaluer pendant la visite'},
    {label:'Eau / abreuvement',score:hasWaterData?waterScore:null,why:hasWaterData?'Débit, accès, propreté et disponibilité renseignés':'À évaluer pendant la visite'},
    {label:'Logement / litière',score:hasBuildingData?buildingScore:null,why:hasBuildingData?'Ventilation, litière, densité et ambiance renseignées':'À évaluer pendant la visite'}
  ];
  return {alerts:alerts.slice(0,12),themes,movements:mv};
}
function prepDecisionSupportHtml(visit,farm,x,hist,rolling,{forPrint=false}={}){
  const d=prepDecisionSupportData(visit,farm,x,hist,rolling),status=s=>s==null?'pending':s>=75?'good':s>=55?'warn':'bad';
  const hiddenThemes=new Set(['Sanitaire','Biosécurité','Alimentation','Eau / abreuvement','Logement / litière']);
  const visibleThemes=d.themes.filter(t=>!hiddenThemes.has(t.label));
  const priorityAlerts=d.alerts.filter(a=>!hiddenThemes.has(a.theme)).slice(0,5);
  const alertHtml=priorityAlerts.length?priorityAlerts.map(a=>`<div class="decision-alert sev-${a.severity}"><div><strong>${escapeHtml(a.theme)} — ${escapeHtml(a.title)}</strong><small>${escapeHtml(a.detail)}</small></div>${!forPrint&&a.key?`<button type="button" class="btn small secondary" data-prep-repro-list="${a.key}">Voir les animaux</button>`:''}</div>`).join(''):'<div class="notice success">Aucun point prioritaire fort détecté avec les données disponibles.</div>';
  return `<section class="decision-support ${forPrint?'for-print':''}"><h2>${forPrint?'0. Synthèse prioritaire avant visite':'🎯 Synthèse avant visite'}</h2><p class="muted">Les couleurs servent de repère visuel ; les notes chiffrées /100 ont été retirées. Les domaines à observer de toute façon pendant la visite (sanitaire, biosécurité, alimentation, eau, logement) ne sont pas répétés ici.</p><div class="decision-theme-grid">${visibleThemes.map(t=>`<article class="decision-theme ${status(t.score)}"><strong>${escapeHtml(t.label)}</strong><small>${escapeHtml(t.why)}</small></article>`).join('')}</div><h3>À creuser en priorité</h3><div class="decision-alert-list">${alertHtml}</div>${prepAnalysisComparisonHtml(hist,{forPrint})}${prepExtendedDecisionHtml(visit,farm,x,hist,rolling,{...d,themes:visibleThemes},{forPrint})}</section>`;
}
...
function maleOriginCode(animal){return animal?.entryDate?'A':'N';}
function maleOriginLabel(code){return code==='A'?'Achat (A)':'Né sur l’exploitation (N)';}
function makeSireIndex(sires=[]){return new Map((sires||[]).map(s=>[normalizeAnimalId(s.fatherId),s]));}
function prepMaleRow(a,date,sireIndex){const s=sireIndex?.get(normalizeAnimalId(a.id));return `<tr><td><strong>${escapeHtml(a.workNumber||a.id)}</strong>${a.name?`<br>${escapeHtml(a.name)}`:''}<br><small>${escapeHtml(a.id||'')}</small></td><td>${a.birthDate?formatDate(a.birthDate):'—'}</td><td><strong>${maleOriginCode(a)}</strong><br><small>${maleOriginLabel(maleOriginCode(a))}</small></td><td>${escapeHtml(ageLabelAt(a.birthDate,date)||'—')}</td><td>${a.entryDate?formatDate(a.entryDate):'Né ici / non renseigné'}</td><td>${s?.firstServiceAge?escapeHtml(s.firstServiceAge):'—'}</td><td>${s?.lastServiceAge?escapeHtml(s.lastServiceAge):'—'}</td></tr>`;}
function prepReproNativeMaleTable(rows,date,sires=[]){const sireIndex=makeSireIndex(sires);return `<div class="table-wrap prep-native-detail-table"><table><thead><tr><th>Mâle</th><th>Naissance</th><th>Origine</th><th>Âge actuel</th><th>Entrée</th><th>Âge 1re saillie*</th><th>Âge dernière saillie*</th></tr></thead><tbody>${(rows||[]).map(a=>prepMaleRow(a,date,sireIndex)).join('')||'<tr><td colspan="7">Aucun mâle.</td></tr>'}</tbody></table><p class="muted small-text">* Âges déduits des premières/dernières naissances attribuées (saillie estimée ≈ 283 jours avant naissance).</p></div>`;}
function reproductionPenaltyFlags(r){return {firstCalvingWarn:(r.firstCalvingAgeMonths??0)>28,firstCalvingSevere:(r.firstCalvingAgeMonths??0)>36,meanIVVWarn:(r.meanIVV??0)>400,meanIVVSevere:(r.meanIVV??0)>450,maxIVVWarn:(r.maxIVV??0)>500,maxIVVSevere:(r.maxIVV??0)>730,deadWarn:(r.deadBefore6||[]).length>0,deadSevere:(r.deadBefore6||[]).length>=2};}
function reproPenaltyCell(content,active=false,severe=false){return active?`<span class="repro-penalty-chip ${severe?'severe':''}">${content}</span>`:content;}
function prepReproSireTableHtml(sires=[],extraNote=''){return sires.length?`<div class="table-wrap"><table><thead><tr><th>Taureau ayant été dans l’exploitation</th><th>Naissance</th><th>Origine</th><th>Présence / sortie</th><th>Âge actuel / âge au départ</th><th>Veaux attribués</th><th>Mères distinctes</th><th>Part des mères attribuées</th><th>1re / dernière naissance</th><th>Âge à la 1re saillie*</th><th>Âge à la dernière saillie*</th></tr></thead><tbody>${sires.map(s=>`<tr><td><strong>${escapeHtml(s.name||s.workNumber||s.fatherId)}</strong><br><small>${s.workNumber?`N° travail ${escapeHtml(s.workNumber)} · `:''}${escapeHtml(s.fatherId||'')}</small></td><td>${s.birthDate?formatDate(s.birthDate):'—'}</td><td><strong>${escapeHtml(s.originCode||'—')}</strong><br><small>${escapeHtml(maleOriginLabel(s.originCode))}</small></td><td>${s.present?'✅ oui':`Non actuellement${s.exitDate?`<br><small>Sorti le ${formatDate(s.exitDate)}</small>`:''}`}</td><td>${s.age?escapeHtml(s.age):'—'}</td><td>${s.calves}</td><td>${s.mothersCount}</td><td>${s.pctFemales!=null?s.pctFemales+'%':'—'}</td><td>${s.firstBirth?formatDate(s.firstBirth):'—'} / ${s.lastBirth?formatDate(s.lastBirth):'—'}</td><td>${s.firstServiceAge?escapeHtml(s.firstServiceAge):'—'}</td><td>${s.lastServiceAge?escapeHtml(s.lastServiceAge):'—'}</td></tr>`).join('')}</tbody></table><p class="muted small-text">* Âges de saillie estimés à partir des dates probables de conception des naissances attribuées.${extraNote?` ${escapeHtml(extraNote)}`:''}</p></div>`:'<div class="empty">Aucun père correspondant à un mâle ayant figuré dans le registre de l’exploitation.</div>';}
function prepReproNativeCowTable(rows,date){return `<div class="table-wrap prep-native-detail-table"><table><thead><tr><th>Vache</th><th>Âge</th><th>Dernier vêlage</th><th>IVV moyen</th><th>IVV complet</th><th>Score</th><th>Pourquoi ?</th></tr></thead><tbody>${(rows||[]).map(z=>{const sd=reproductionScoreDetails(z),why=sd.lines.filter(q=>q.delta<0).map(q=>`<strong>${escapeHtml(q.label)}</strong>`).join(' · ')||'Pas de pénalité',flags=reproductionPenaltyFlags(z);return `<tr><td><strong>${escapeHtml(z.cow.workNumber||z.cow.id)}</strong>${z.cow.name?`<br>${escapeHtml(z.cow.name)}`:''}<br><small>${escapeHtml(z.cow.id)}</small></td><td>${escapeHtml(ageLabelAt(z.cow.birthDate,date)||'—')}</td><td>${reproPenaltyCell(`${z.lastCalvingDate?formatDate(z.lastCalvingDate):'—'}${z.firstCalvingAgeMonths!=null?`<br><small>1er vêlage ${z.firstCalvingAgeMonths} mois</small>`:''}`,flags.firstCalvingWarn,flags.firstCalvingSevere)}</td><td>${reproPenaltyCell(String(z.meanIVV??'—'),flags.meanIVVWarn,flags.meanIVVSevere)}</td><td>${reproPenaltyCell(z.intervals.length?z.intervals.join(' / ')+' j':'—',flags.maxIVVWarn,flags.maxIVVSevere)}</td><td>${sd.score}/100</td><td>${why}</td></tr>`}).join('')||'<tr><td colspan="7">Aucune vache.</td></tr>'}</tbody></table></div>`;}
...
function prepReproNativeIvvTable(rows,atDate){if(!rows?.length)return '<div class="empty">Aucun animal concerné.</div>';return `<div class="table-wrap"><table><thead><tr><th>Vache</th><th>Naissance</th><th>Vêlage précédent</th><th>Vêlage suivant</th><th>IVV</th></tr></thead><tbody>${rows.map(r=>{const c=r.cow||{};return `<tr><td><strong>${escapeHtml(c.workNumber||c.name||c.id||r.damId||'—')}</strong>${c.name&&c.workNumber?`<br><small>${escapeHtml(c.name)}</small>`:''}${c.id?`<br><small>${escapeHtml(c.id)}</small>`:''}</td><td>${c.birthDate?formatDate(c.birthDate):'—'}</td><td>${formatDate(r.previousCalvingDate)}</td><td>${formatDate(r.calvingDate)}</td><td><strong>${r.days} j</strong></td></tr>`}).join('')}</tbody></table></div>`;}
function reproPreparationHtml(visit,farm){
  ...
  <h4>Structure actuelle</h4><div class="repro-kpi-grid compact">${prepReproNativeMetric('Femelles >24 mois',x.females24.length,'',prepReproNativeAnimalTable(x.females24,x.date))}${prepReproNativeMetric('Femelles >36 mois',x.females36.length,'',prepReproNativeAnimalTable(x.females36,x.date))}${prepReproNativeMetric('Mâles >24 mois',x.males24.length,'',prepReproNativeMaleTable(x.males24,x.date,x.sires))}${prepReproNativeMetric('Mâles >36 mois',x.males36.length,'',prepReproNativeMaleTable(x.males36,x.date,x.sires))}${prepReproNativeMetric('Femelles >36 m sans vêlage',x.heifers36.length,'',prepReproNativeAnimalTable(x.heifers36,x.date))}${prepReproNativeMetric('Naissances 12 mois',x.births.length,`${formatDate(x.startIso)} → ${formatDate(x.date)}`,prepReproNativeAnimalTable(x.births,x.date))}</div>
  <div class="prep-age-distribution-wrap">${reproAgeDistributionHtml(x.females24,x.date,{interactive:false,wrap:false,detailTargetId:'prep-repro-age-year-detail'})}</div>${(()=>{const r12=reproRolling12Stats(visit,farm,x),hist=reproMortalityAnnualHistory(visit,farm,10);return `<h4>Reproduction et mortalité — 12 derniers mois</h4><p class="muted small-text">${formatDate(r12.start)} → ${formatDate(r12.end)}. Les tuiles donnent la situation récente ; le tableau dessous permet de voir l’évolution sur 10 ans.</p><div class="repro-kpi-grid compact">${prepReproNativeMetric('Vaches ayant vêlé',r12.dams,'12 derniers mois',prepReproNativeCowTable(x.cows.filter(z=>(z.calves||[]).some(c=>c.birthDate>=r12.start&&c.birthDate<=r12.end)),x.date))}${prepReproNativeMetric(`IVV moyen sur ${r12.ivvCowCount} femelle(s)`,r12.ivvMean==null?'—':r12.ivvMean+' j','12 derniers mois',prepReproNativeIvvTable(r12.intervalDetails,x.date))}${prepReproNativeMetric('IVV ≤400 j',r12.ivvLe400,'12 derniers mois',prepReproNativeIvvTable(r12.intervalDetails.filter(r=>r.days<=400),x.date))}${prepReproNativeMetric('IVV 401–450 j',r12.ivv401to450,'12 derniers mois',prepReproNativeIvvTable(r12.intervalDetails.filter(r=>r.days>=401&&r.days<=450),x.date))}${prepReproNativeMetric('IVV 451–500 j',r12.ivv451to500,'12 derniers mois',prepReproNativeIvvTable(r12.intervalDetails.filter(r=>r.days>=451&&r.days<=500),x.date))}${prepReproNativeMetric('IVV >500 j',r12.ivvOver500,'12 derniers mois',prepReproNativeIvvTable(r12.intervalDetails.filter(r=>r.days>500),x.date))}${prepReproNativeMetric('Âge moyen 1er vêlage',r12.firstCalvingMean==null?'—':r12.firstCalvingMean+' mois',r12.firstCalvingCount?`${r12.firstCalvingCount} premier(s) vêlage(s) sur 12 mois`:'12 derniers mois','')}${prepReproNativeMetric('1er vêlage >36 mois',r12.firstCalvingLateCount??0,r12.firstCalvingCount?`${r12.firstCalvingLateCount}/${r12.firstCalvingCount} femelle(s) · ${r12.firstCalvingLateRate??0}%`:'Aucun 1er vêlage calculable','')}${prepReproNativeMetric('Veaux morts <6 mois',r12.dead6.length,r12.mortalityRate!=null?`${r12.mortalityRate}% des veaux nés sur la période`:'',prepReproNativeAnimalTable(r12.dead6,x.date))}${prepReproNativeMetric('Vaches à problème actuelles',x.cowProblems.length,'Alerte actuelle',prepReproNativeCowTable(x.cowProblems,x.date))}</div>${annualHistoryTablesHtml(hist)}${prepDecisionSupportHtml(visit,farm,x,hist,r12)}`;})()}
  ...
  ${prepReproSireTableHtml(x.sires,'La « part des mères » est calculée parmi les mères ayant au moins un veau attribué à un taureau de l’exploitation ; ce n’est pas un taux exact de saillies.')}
  <p class="muted small-text"><strong>${x.aiSireCount}</strong> père(s) d’IA / père(s) jamais présents dans le registre détecté(s). Ils sont comptés mais volontairement non détaillés.</p>
...
function bindPrepReproClicks(visit,farm){
  ...
  const reproRows=r=>`<div class="table-wrap"><table><thead><tr><th>Vache</th><th>Âge</th><th>Dernier vêlage</th><th>IVV moyen</th><th>IVV complet</th><th>Score</th><th>Pourquoi ?</th></tr></thead><tbody>${r.map(z=>{const sd=reproductionScoreDetails(z),why=sd.lines.filter(q=>q.delta<0).map(q=>`<strong>${escapeHtml(q.label)}</strong>`).join(' · ')||'Pas de pénalité',flags=reproductionPenaltyFlags(z);return `<tr><td><strong>${escapeHtml(z.cow.workNumber||z.cow.id)}</strong>${z.cow.name?`<br>${escapeHtml(z.cow.name)}`:''}<br><small>${escapeHtml(z.cow.id)}</small></td><td>${escapeHtml(ageLabelAt(z.cow.birthDate,x.date)||'—')}</td><td>${reproPenaltyCell(`${z.lastCalvingDate?formatDate(z.lastCalvingDate):'—'}${z.firstCalvingAgeMonths!=null?`<br><small>1er vêlage ${z.firstCalvingAgeMonths} mois</small>`:''}`,flags.firstCalvingWarn,flags.firstCalvingSevere)}</td><td>${reproPenaltyCell(String(z.meanIVV??'—'),flags.meanIVVWarn,flags.meanIVVSevere)}</td><td>${reproPenaltyCell(z.intervals.length?z.intervals.join(' / ')+' j':'—',flags.maxIVVWarn,flags.maxIVVSevere)}</td><td>${sd.score}/100</td><td>${why}</td></tr>`}).join('')||'<tr><td colspan="7">Aucune vache.</td></tr>'}</tbody></table></div>`;
  const openTile=(b)=>{
...
// v14.6.21.99 — estimation économique pré-visite, calculée uniquement sur les données déjà disponibles.
...
const CHATGPT_IMPORT_COLUMNS=['TYPE','ELEMENT','PARAMETRE','VALEUR','UNITE','DATE','SOURCE','COMMENTAIRE'];
function chatgptImportCell(row,...names){const l=rowLookup(row);return l.exact(...names)||''}
function chatgptNormParam(x=''){return normalizeSearchText(x).replace(/[^a-z0-9]+/g,' ').trim()}
function chatgptForageKey(param=''){
  const p=chatgptNormParam(param), pairs=[
    ['ca_gkg',['calcium','ca']],['cu_mgkg',['cuivre','cu']],['k_gkg',['potassium','k']],['p_gkg',['phosphore','p']],['mg_gkg',['magnesium','mg']],['mn_mgkg',['manganese','mn']],['fe_mgkg',['fer','fe']],['na_gkg',['sodium','na']],['zn_mgkg',['zinc','zn']],
    ['ms_pct_mb',['matiere seche','matieres seches','ms']],['ph',['ph']],['mm_gkg',['matiere minerale','mm']],['mat_gkg',['matieres azotees totales','matiere azotee totale','mat']],['cb_gkg',['cellulose brute','cb']],['dcs_pct',['digestibilite pepsine cellulose','dcs']],['sugars_pct',['sucres residuels','sucres']],['starch_gkg',['amidon','ami']],['adf_gkg',['lignocellulose','adf']],['adl_gkg',['lignine','adl']],['ndf_gkg',['parois totales','ndf']],['mgt_gkg',['matieres grasses totales','mgt']],['mo_gkg',['matiere organique','mo']],
    ['dmo_pct',['digestibilite de la matiere organique','dmo']],['dt6n_pct',['degradabilite theorique de l azote','dt6 n','dt6n']],['eb_kcal',['energie brute','eb']],['ueb',['unites d encombrement bovin','ueb']],['uel',['unites d encombrement lait','uel']],['ufl',['unites fourrageres lait','ufl']],['ufv',['unite fourragere viande','ufv']],['pdi_gkg',['proteines digestibles dans l intestin','pdi']],['bpr_gkg',['balance proteique du rumen','bpr']],['pdia_gkg',['proteines digestibles dans l intestin d origine alimentaire','pdia']],['niref',['niveau d ingestion de reference','niref']],['uem',['unites d encombrement mouton','uem']],['dndf_pct',['digestibilite des parois vegetales','dndf']],['pabs_gkg',['phosphore absorbable','p abs']],['caabs_gkg',['calcium absorbable','ca abs']],['baca_meq',['bilan cations anions','baca']],['be_meq',['bilan electrolytique','be']]
  ];
  // Correspondance STRICTE : ne jamais utiliser includes() ici.
  // Les anciens tests partiels faisaient notamment correspondre BACA→Ca, MO→MAT,
  // DNDF→NDF, UEB→EB et PDIA→PDI.
  for(const [key,aliases] of pairs){
    if(aliases.some(a=>p===chatgptNormParam(a)))return key;
  }
  return '';
}
function chatgptFeedKey(param=''){
  const p=chatgptNormParam(param), map=[['ms',['matiere seche','ms']],['mat',['matieres azotees totales','mat']],['ndf',['ndf','parois totales']],['starch',['amidon','ami']],['ca',['calcium','ca']],['p',['phosphore','p']],['cu',['cuivre','cu']],['zn',['zinc','zn']],['mn',['manganese','mn']],['se',['selenium','se']],['co',['cobalt','co']],['fe',['fer','fe']],['other',['autres','autre','additifs','composition autre']]];
  for(const [key,aliases] of map)if(aliases.some(a=>p===chatgptNormParam(a)))return key; return '';
}
function removeChatgptImportSource(visit,fileName){
  const tag=`ChatGPT:${fileName}`;
  if(visit.feeding?.nutrition?.forageAnalyses)visit.feeding.nutrition.forageAnalyses=visit.feeding.nutrition.forageAnalyses.filter(a=>a.importSource!==tag);
  const m=ensureMetabolic(visit);m.rows=m.rows.filter(r=>r.importSource!==tag);
  const p=ensureParasitism(visit);p.rows=p.rows.filter(r=>r.importSource!==tag);p.pepsinogen=p.pepsinogen.filter(r=>r.importSource!==tag);
  const w=ensureWaterLab(visit);w.points.forEach(pt=>pt.rows=(pt.rows||[]).filter(r=>r.importSource!==tag));w.points=w.points.filter(pt=>pt.importSource!==tag || (pt.rows||[]).length);
  const settings=ensureReferenceSettings();settings.customFeedReferences=settings.customFeedReferences.filter(r=>r.importSource!==tag);
  if(visit.feeding?.nutrition?.contextAnalyses)visit.feeding.nutrition.contextAnalyses=visit.feeding.nutrition.contextAnalyses.filter(a=>a.importSource!==tag);
}
function importChatgptRows(visit,rows,fileName){
  const tag=`ChatGPT:${fileName}`; removeChatgptImportSource(visit,fileName);
  visit.feeding=visit.feeding&&typeof visit.feeding==='object'?visit.feeding:{rations:[],settings:{},history:[]};
  visit.feeding.nutrition=visit.feeding.nutrition&&typeof visit.feeding.nutrition==='object'?visit.feeding.nutrition:{};
  const nutrition=visit.feeding.nutrition;nutrition.forageAnalyses=Array.isArray(nutrition.forageAnalyses)?nutrition.forageAnalyses:[];nutrition.contextAnalyses=Array.isArray(nutrition.contextAnalyses)?nutrition.contextAnalyses:[];
  const settings=ensureReferenceSettings();let count=0,unknown=[];
  for(const row of rows){
    const type=normalizeSearchText(chatgptImportCell(row,'TYPE','Type')),element=chatgptImportCell(row,'ELEMENT','ÉLÉMENT','Element','Lot','Animal','Point','Nom').trim(),param=chatgptImportCell(row,'PARAMETRE','PARAMÈTRE','Parametre','Analyse').trim(),value=chatgptImportCell(row,'VALEUR','Valeur','Résultat','Resultat').trim(),unit=chatgptImportCell(row,'UNITE','UNITÉ','Unite').trim(),date=chatgptImportCell(row,'DATE','Date').trim(),source=chatgptImportCell(row,'SOURCE','Source','Laboratoire').trim(),comment=chatgptImportCell(row,'COMMENTAIRE','Commentaire','Notes').trim();
    if(!type||!param||value==='')continue;
    if(type.includes('fourrage')){
      const name=element||'Analyse de fourrage importée', key=chatgptForageKey(param);let a=nutrition.forageAnalyses.find(x=>x.importSource===tag&&x.name===name);
      if(!a){a={id:uid('forage'),name,type:'Autre',source:source||'Import ChatGPT',estimated:false,importSource:tag,lab:{}};nutrition.forageAnalyses.push(a)}
      a.lab=ensureForageLab(a); if(key){a.lab[key]=value;count++}else{a.lab.other=[a.lab.other,`${param}: ${value}${unit?' '+unit:''}`].filter(Boolean).join('\n');unknown.push(`Fourrage · ${param}`)} if(comment)a.comment=appendUniqueNote(a.comment,comment); syncForageLabToMain(a);
    }else if(type.includes('sol')||type.includes('terre')){
      const name=element||'Analyse de sol importée';let a=nutrition.contextAnalyses.find(x=>x.importSource===tag&&x.kind==='SOL'&&x.name===name);
      if(!a){a={id:uid('ctx'),kind:'SOL',name,date,source:source||'Import ChatGPT',comment:'',importSource:tag,rows:[]};nutrition.contextAnalyses.push(a)}
      if(date)a.date=date;if(source)a.source=source;if(comment)a.comment=appendUniqueNote(a.comment,comment);a.rows.push({id:uid('ctxrow'),parameter:param,value,unit,comment,importSource:tag});count++;
    }else if(type.includes('vegetal')||type.includes('plante')||type.includes('feuille')){
      const name=element||'Analyse de végétaux importée';let a=nutrition.contextAnalyses.find(x=>x.importSource===tag&&x.kind==='VEGETAL'&&x.name===name);
      if(!a){a={id:uid('ctx'),kind:'VEGETAL',name,date,source:source||'Import ChatGPT',comment:'',importSource:tag,rows:[]};nutrition.contextAnalyses.push(a)}
      if(date)a.date=date;if(source)a.source=source;if(comment)a.comment=appendUniqueNote(a.comment,comment);a.rows.push({id:uid('ctxrow'),parameter:param,value,unit,comment,importSource:tag});count++;
    }else if(type.includes('poil')||type.includes('hair')){
      const m=ensureMetabolic(visit);if(date)m.date=date;if(source)m.lab=source;m.sampleType='Poils';if(element)m.category=element;
      m.rows.push({id:uid('met'),animal:element,analyte:param,value,unit,refMin:'',refMax:'',labStatus:'auto',comment,importSource:tag});count++;
    }else if(type.includes('eau')){
      const w=ensureWaterLab(visit);if(date)w.date=date;if(source)w.lab=source;const point=element||'Point d’eau';let pt=w.points.find(x=>x.importSource===tag&&x.name===point);if(!pt){pt={id:uid('wpt'),name:point,location:'',linkedDrinkerId:'',importSource:tag,rows:[]};w.points.push(pt)}pt.rows.push({id:uid('wrow'),parameter:param,value,unit,comment,importSource:tag});count++;
    }else if(type.includes('paras')){
      const p=ensureParasitism(visit);if(date)p.date=date;if(source)p.lab=source;if(chatgptNormParam(param).includes('pepsinogene'))p.pepsinogen.push({id:uid('pep'),animal:element,value,unit:unit||'mUT',comment,importSource:tag});else p.rows.push({id:uid('par'),animal:element,parasite:param,value,unit,labStatus:'auto',comment,importSource:tag});count++;
    }else if(type.includes('sang')||type.includes('metabol')||type.includes('profil')){
      const m=ensureMetabolic(visit);if(date)m.date=date;if(source)m.lab=source;m.rows.push({id:uid('met'),animal:element,analyte:param,value,unit,refMin:'',refMax:'',labStatus:'auto',comment,importSource:tag});count++;
    }else if(type.includes('aliment')||type.includes('etiquette')){
      const name=element||'Aliment importé',key=chatgptFeedKey(param);let r=settings.customFeedReferences.find(x=>x.importSource===tag&&x.label===name);if(!r){r={id:uid('feedref'),label:name,source:source||'Étiquette / import ChatGPT',ms:'',mat:'',ndf:'',starch:'',ca:'',p:'',cu:'',zn:'',mn:'',se:'',co:'',fe:'',other:'',importSource:tag};settings.customFeedReferences.push(r)}if(key){r[key]=key==='other'?[r[key],`${param}: ${value}${unit?' '+unit:''}`].filter(Boolean).join(' · '):(parseFrenchNumber(value)??value);count++}else{r.other=[r.other,`${param}: ${value}${unit?' '+unit:''}`].filter(Boolean).join(' · ');unknown.push(`Aliment · ${param}`)}if(comment)r.other=[r.other,comment].filter(Boolean).join(' · ');
    }else unknown.push(`Type inconnu · ${type}`);
  }
  visit.chatgptImports=Array.isArray(visit.chatgptImports)?visit.chatgptImports:[];visit.chatgptImports=visit.chatgptImports.filter(x=>x.fileName!==fileName);visit.chatgptImports.unshift({id:uid('cgimp'),fileName,createdAt:new Date().toISOString(),count,unknown});visit.updatedAt=new Date().toISOString();saveDatabase(db);return {count,unknown};
}
function chatgptImportCardHtml(visit){const items=(visit.chatgptImports||[]).slice(0,8);return `<section class="card import-type-card"><div class="section-title"><div><h3>Import des analyses & étiquettes préparé par ChatGPT</h3><p class="muted">Après la visite — ou dès que vous récupérez un résultat — envoyez-moi la photo/PDF. Le CSV généré ici peut alimenter les modules correspondants sans ressaisie manuelle.</p></div><a class="btn secondary" href="./MODELE_IMPORT_CHATGPT.csv" download>⬇️ Modèle CSV</a></div><div class="notice"><strong>Colonnes :</strong> TYPE ; ELEMENT ; PARAMETRE ; VALEUR ; UNITE ; DATE ; SOURCE ; COMMENTAIRE.<br><strong>Types acceptés :</strong> FOURRAGE, SOL, VEGETAL, EAU, PARASITISME, SANG, POILS ou ALIMENT.</div><div class="field"><label>Fichier CSV préparé par ChatGPT</label><input id="chatgpt-analysis-import" type="file" accept=".csv,text/csv"></div>${items.length?`<div class="small-text muted">Derniers imports : ${items.map(x=>`${escapeHtml(x.fileName)} (${x.count} valeur(s))`).join(' · ')}</div>`:''}</section>`}
function renderLabImports(){
  const visit=activeVisit();if(!visit){renderNoActiveVisit('Imports analyses');return;}
  app.innerHTML=`<div class="section-title"><div><h2>📥 Imports analyses & étiquettes</h2><div class="muted">Résultats collectés le jour de la visite ou reçus ensuite : fourrages, sol, végétaux, eau, parasitisme, sang/poils et aliments.</div></div><span class="badge autosave">v14.6.21.123</span></div>${activeVisitBanner(visit)}${chatgptImportCardHtml(visit)}<section class="card notice"><strong>Destination automatique :</strong> fourrages + sol + végétaux → Analyse nutritionnelle ; eau → Analyse d’eau ; sang/poils → Profil métabolique ; parasitisme → Parasitisme ; aliments → référentiel des aliments.</section>`;
  document.getElementById('chatgpt-analysis-import')?.addEventListener('change',async e=>{const file=e.target.files?.[0];if(!file)return;try{const rows=parseCsvText(await file.text());if(!rows.length)throw new Error('Aucune ligne exploitable');const res=importChatgptRows(visit,rows,file.name);showToast(`${res.count} valeur(s) importée(s) depuis ${file.name}${res.unknown.length?` · ${res.unknown.length} paramètre(s) conservé(s) en remarque / non reconnus`:''}.`);renderLabImports()}catch(err){console.error(err);showToast('Import ChatGPT impossible : '+err.message)}});
}
function renderHerdData(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Données technico-économiques</h2><span class="muted">Visite du ${formatDate(visit.date)} — ${escapeHtml(farm?.name||'Exploitation')}</span></div><span class="badge autosave">v14.6.21.123</span></div><section class="notice"><strong>Ordre conseillé :</strong> créez d’abord la visite, importez les données préparatoires, puis définissez ici l’orientation de l’audit. Le mode ciblé automatique utilise les éléments calculés ci-dessous et le motif éventuellement signalé par l’éleveur.</section>${targetedAuditHtml(visit)}${registryImportHtml}${reproPreparationHtml(visit,farm)}${prepEconomicHtml(visit,farm)}${currentHtml}<div class="actions"><button class="btn secondary" id="toggle-herd-library">${hasCurrentCsv?'Changer de CSV':'Choisir un CSV déjà importé'}</button></div>${libraryHtml}${importHtml}`;
  bindTargetedAudit(visit);
  ...
  document.getElementById('chatgpt-analysis-import')?.addEventListener('change',async e=>{const file=e.target.files?.[0];if(!file)return;try{const rows=parseCsvText(await file.text());if(!rows.length)throw new Error('Aucune ligne exploitable');const res=importChatgptRows(visit,rows,file.name);showToast(`${res.count} valeur(s) importée(s) depuis ${file.name}${res.unknown.length?` · ${res.unknown.length} paramètre(s) conservé(s) en remarque / non reconnus`:''}.`);renderHerdData()}catch(err){console.error(err);showToast('Import ChatGPT impossible : '+err.message)}});
...
function renderReferenceSettings(){const settings=ensureReferenceSettings(),labs=metabolicLabProfiles(),rows=[];Object.entries(THRESHOLDS||{}).forEach(([stage,rules])=>Object.entries(rules||{}).forEach(([key,rule])=>rows.push({stage,key,rule,ov:settings.referenceThresholdOverrides?.[stage]?.[key]||{}})));const labCards=labs.map(l=>`<article class="card" data-lab-card="${l.id}"><div class="section-title"><div><h3>${escapeHtml(l.name)}</h3><span class="muted">Mis à jour : ${escapeHtml(l.updatedAt?formatDate(l.updatedAt):'non renseigné')}</span></div><div class="actions"><button class="btn secondary small" data-add-lab-ref="${l.id}">+ Paramètre</button><button class="btn danger small" data-delete-lab="${l.id}">Supprimer labo</button></div></div><div class="grid cols-3"><div class="field"><label>Nom</label><input data-lab-name="${l.id}" value="${escapeHtml(l.name||'')}"></div><div class="field"><label>Date de mise à jour</label><input type="date" data-lab-date="${l.id}" value="${escapeHtml((l.updatedAt||'').slice(0,10))}"></div><div class="field"><label>Source / commentaire</label><input data-lab-notes="${l.id}" value="${escapeHtml(l.notes||'')}"></div></div><div class="table-wrap"><table class="compact-table"><thead><tr><th>Paramètre</th><th>Matrice</th><th>Unité</th><th>Seuil bas</th><th>Seuil haut</th><th></th></tr></thead><tbody>${(l.references||[]).map(r=>`<tr data-lab-ref-row="${r.id}"><td><input list="met-analytes-settings" data-lab-ref-field="analyte" value="${escapeHtml(r.analyte||'')}"></td><td><select data-lab-ref-field="sampleType">${['','Sérum','Plasma','Sang total','Foie','Autre'].map(x=>`<option value="${x}" ${r.sampleType===x?'selected':''}>${x||'Toutes'}</option>`).join('')}</select></td><td><input data-lab-ref-field="unit" value="${escapeHtml(r.unit||'')}"></td><td><input inputmode="decimal" data-lab-ref-field="refMin" value="${escapeHtml(r.refMin??'')}"></td><td><input inputmode="decimal" data-lab-ref-field="refMax" value="${escapeHtml(r.refMax??'')}"></td><td><button class="btn danger small" data-delete-lab-ref="${r.id}" data-lab-id="${l.id}">×</button></td></tr>`).join('')}</tbody></table></div></article>`).join('');app.innerHTML=`<div class="section-title"><div><h2>⚙️ Paramètres & seuils</h2><div class="muted">Référentiel central modifiable pour les analyses et les laboratoires.</div></div><span class="badge autosave">v14.6.21.123</span></div><section class="card notice"><strong>Historique protégé</strong><br>Une modification d’un laboratoire change les références proposées pour les <b>nouvelles saisies</b>. Les analyses déjà enregistrées gardent les unités et seuils utilisés au moment du résultat, sauf si vous appuyez volontairement sur « Appliquer les seuils du labo » dans la visite.</section><section class="card"><div class="section-title"><div><h3>🧬 Laboratoires — oligo-éléments & vitamines</h3><span class="muted">Nom, matrice, unité et bornes propres à chaque laboratoire.</span></div><button class="btn primary" id="add-met-lab">+ Ajouter un laboratoire</button></div><datalist id="met-analytes-settings">${Object.values(METABOLIC_KNOWLEDGE).map(x=>`<option value="${escapeHtml(x.label)}">`).join('')}</datalist>${labCards||'<div class="empty">Aucun laboratoire enregistré.</div>'}</section><section class="card"><div class="section-title"><div><h3>📏 Seuils internes des différentes analyses</h3><span class="muted">Valeur vide = seuil d’origine de l’application.</span></div><button class="btn secondary" id="reset-threshold-overrides">Réinitialiser les personnalisations</button></div><div class="table-wrap"><table class="compact-table"><thead><tr><th>Stade</th><th>Analyse</th><th>Rouge bas</th><th>Vert bas</th><th>Vert haut</th><th>Rouge haut</th></tr></thead><tbody>${rows.map(({stage,key,rule,ov})=>`<tr data-th-stage="${escapeHtml(stage)}" data-th-key="${escapeHtml(key)}"><td>${escapeHtml(stage)}</td><td>${escapeHtml(key)}</td>${['redLow','greenLow','greenHigh','redHigh'].map(k=>`<td><input inputmode="decimal" data-th-field="${k}" placeholder="${escapeHtml(rule?.[k]??'—')}" value="${escapeHtml(ov?.[k]??'')}"></td>`).join('')}</tr>`).join('')}</tbody></table></div><p class="muted small-text">Les règles contextuelles non numériques (par exemple certains raisonnements reproduction, bâtiment ou parasitisme) restent configurées dans leur module ; cet écran couvre tous les seuils numériques exposés par le moteur d’analyse.</p></section>${supplementSettingsHtml(settings)}${feedReferenceSettingsHtml(settings)}`;document.getElementById('add-met-lab')?.addEventListener('click',()=>{labs.push({id:uid('lab'),name:'Nouveau laboratoire',updatedAt:new Date().toISOString().slice(0,10),notes:'',references:[]});saveDatabase(db);renderReferenceSettings()});app.querySelectorAll('[data-lab-name]').forEach(el=>el.oninput=()=>{const l=metabolicLabById(el.dataset.labName);if(l){l.name=el.value;saveDatabase(db)}});app.querySelectorAll('[data-lab-date]').forEach(el=>el.onchange=()=>{const l=metabolicLabById(el.dataset.labDate);if(l){l.updatedAt=el.value;saveDatabase(db)}});app.querySelectorAll('[data-lab-notes]').forEach(el=>el.oninput=()=>{const l=metabolicLabById(el.dataset.labNotes);if(l){l.notes=el.value;saveDatabase(db)}});app.querySelectorAll('[data-add-lab-ref]').forEach(b=>b.onclick=()=>{const l=metabolicLabById(b.dataset.addLabRef);if(l){l.references=Array.isArray(l.references)?l.references:[];l.references.push({id:uid('labref'),analyte:'Cuivre (Cu)',sampleType:'Sérum',unit:'',refMin:'',refMax:''});saveDatabase(db);renderReferenceSettings()}});app.querySelectorAll('[data-delete-lab]').forEach(b=>b.onclick=()=>{if(!confirm('Supprimer ce laboratoire ? Les anciennes analyses gardent leurs seuils enregistrés.'))return;settings.metabolicLabs=settings.metabolicLabs.filter(l=>l.id!==b.dataset.deleteLab);saveDatabase(db);renderReferenceSettings()});app.querySelectorAll('[data-lab-ref-field]').forEach(el=>{const save=()=>{const tr=el.closest('[data-lab-ref-row]'),card=el.closest('[data-lab-card]'),l=metabolicLabById(card?.dataset.labCard),r=l?.references?.find(x=>x.id===tr?.dataset.labRefRow);if(r){r[el.dataset.labRefField]=el.value;l.updatedAt=new Date().toISOString().slice(0,10);saveDatabase(db)}};el.oninput=save;el.onchange=save});app.querySelectorAll('[data-delete-lab-ref]').forEach(b=>b.onclick=()=>{const l=metabolicLabById(b.dataset.labId);if(l){l.references=l.references.filter(r=>r.id!==b.dataset.deleteLabRef);saveDatabase(db);renderReferenceSettings()}});app.querySelectorAll('[data-th-field]').forEach(el=>{const save=()=>{const tr=el.closest('tr'),stage=tr.dataset.thStage,key=tr.dataset.thKey;settings.referenceThresholdOverrides[stage]=settings.referenceThresholdOverrides[stage]||{};settings.referenceThresholdOverrides[stage][key]=settings.referenceThresholdOverrides[stage][key]||{};const v=parseFrenchNumber(el.value);if(v===null)delete settings.referenceThresholdOverrides[stage][key][el.dataset.thField];else settings.referenceThresholdOverrides[stage][key][el.dataset.thField]=v;if(!Object.keys(settings.referenceThresholdOverrides[stage][key]).length)delete settings.referenceThresholdOverrides[stage][key];saveDatabase(db)};el.oninput=save;el.onchange=save});document.getElementById('reset-threshold-overrides')?.addEventListener('click',()=>{if(!confirm('Revenir à tous les seuils internes d’origine ?'))return;settings.referenceThresholdOverrides={};saveDatabase(db);renderReferenceSettings();showToast('Seuils internes réinitialisés.')});
 ...
 app.querySelectorAll('[data-macro-min-target]').forEach(el=>{const save=()=>{settings.macroMineralNeedTargets=settings.macroMineralNeedTargets||{};const v=parseFrenchNumber(el.value);if(v===null)delete settings.macroMineralNeedTargets[el.dataset.macroMinTarget];else settings.macroMineralNeedTargets[el.dataset.macroMinTarget]=v;saveDatabase(db)};el.oninput=save;el.onchange=save});
 ...
 document.getElementById('add-custom-feed-ref')?.addEventListener('click',()=>{settings.customFeedReferences.push({id:uid('feedref'),label:'Nouvel aliment',source:'Tables INRAE-CIRAD-AFZ',ms:'',mat:'',ndf:'',starch:'',ca:'',p:'',cu:'',zn:'',mn:'',se:'',co:'',fe:'',other:''});saveDatabase(db);renderReferenceSettings();});document.getElementById('open-inrae-feedtables')?.addEventListener('click',()=>window.open('https://www.feedtables.com/fr','_blank','noopener'));app.querySelectorAll('[data-custom-feed-field]').forEach(el=>{const save=()=>{const card=el.closest('[data-custom-feed-id]'),r=settings.customFeedReferences.find(x=>x.id===card?.dataset.customFeedId);if(!r)return;const k=el.dataset.customFeedField;r[k]=['label','source','other'].includes(k)?el.value:(parseFrenchNumber(el.value)??'');saveDatabase(db);};el.oninput=save;el.onchange=save;});app.querySelectorAll('[data-delete-custom-feed]').forEach(b=>b.onclick=()=>{const id=String(b.dataset.deleteCustomFeed||'');if(!id)return;if(!confirm('Supprimer cet aliment du référentiel ?'))return;db.deletedCustomFeedReferenceIds=Array.isArray(db.deletedCustomFeedReferenceIds)?db.deletedCustomFeedReferenceIds:[];db.deletedCustomFeedReferenceIds=[...new Set([...db.deletedCustomFeedReferenceIds,id])];settings.customFeedReferences=(settings.customFeedReferences||[]).filter(x=>String(x?.id)!==id);db.updatedAt=new Date().toISOString();saveDatabase(db);renderReferenceSettings();showToast('Aliment supprimé du référentiel.');});document.getElementById('reset-feed-refs')?.addEventListener('click',()=>{if(!confirm('Réinitialiser toutes les valeurs types des aliments ?'))return;settings.feedReferenceOverrides={};saveDatabase(db);renderReferenceSettings();showToast('Valeurs types réinitialisées.')});} 
...
function renderBackup() {
  app.innerHTML = `
    ...
    <section class="grid cols-2">
      ...
      <article class="card"><h3>Dossier d’un éleveur</h3><p class="muted">Pour sauvegarder ou transférer une seule exploitation, « Exporter dossier » inclut toutes ses visites, les pièces jointes locales disponibles, ainsi que pour chaque visite le bilan préparatoire et le rapport complet en PDF + Word.</p><button class="btn" id="backup-open-farms">Ouvrir les exploitations</button></article>
  ...
  document.getElementById('backup-open-farms')?.addEventListener('click',()=>setView('farms'));
  document.getElementById('import-db').onclick = () => {fileInput.accept='application/json,.json';fileInput.dataset.importMode='full-backup';fileInput.click();};
  document.getElementById('reset-db').onclick = async () => {
    if (confirm('Effacer définitivement toutes les données de cet appareil ?')) { db = await replaceDatabase({ farms: [], visits: [] }); ensureCoreDatabaseShape(); clearDraft(); setActiveVisit(''); showToast('Base locale effacée.'); renderBackup(); }
  };
}
...
fileInput.addEventListener('change', async () => {
  ...
  if (!file) return;
  const mode=fileInput.dataset.importMode||'';
  try {
    if(mode==='farm-dossier'||/\.zip$/i.test(file.name)){
      const done=await importFarmDossierFile(file);if(done)render();return;
    }
    const parsed = JSON.parse(await file.text());
    if(parsed?.format==='audit-bovin-farm-dossier'){
      const done=await importFarmDossierPayload(parsed,null);if(done)render();return;
    }
    if (parsed.farm && parsed.visit) {
      ...
      db = await replaceDatabase(parsed);
      ensureCoreDatabaseShape();
      migrateDatabase();
    ...
    console.error(error);
    alert(mode==='farm-dossier'||/\.zip$/i.test(file.name)?'Ce fichier ne correspond pas à un dossier éleveur Audit Bovin valide.':'Ce fichier ne correspond pas à une sauvegarde Audit Bovin valide.');
  } finally {
    fileInput.value = '';
    fileInput.dataset.importMode='';
    fileInput.accept='application/json,.json';
  }
});

let __lastVisibleErrorMessage='';
let __lastVisibleErrorAt=0;
function showGlobalAppError(message){
  const text=String(message||'Erreur inconnue').trim();
  const now=Date.now();
  if(text===__lastVisibleErrorMessage&&now-__lastVisibleErrorAt<30000)return;
  __lastVisibleErrorMessage=text;__lastVisibleErrorAt=now;
  let errorBox=document.getElementById('global-runtime-warning');
  if(!errorBox){errorBox=document.createElement('div');errorBox.id='global-runtime-warning';errorBox.className='card notice warning';app.prepend(errorBox);}
  errorBox.innerHTML=`<strong>Incident technique détecté.</strong><br><span class="muted">${escapeHtml(text)}</span><div class="muted small-text" style="margin-top:6px">Si l’écran reste utilisable, vous pouvez continuer. Après une mise à jour, fermez puis rouvrez complètement l’application afin d’éviter un mélange d’anciens fichiers en cache.</div>`;
}
window.addEventListener('error',event=>{console.error(event.error||event.message);showGlobalAppError(event.message||'Erreur inconnue');});
window.addEventListener('unhandledrejection',event=>{console.error(event.reason);showGlobalAppError(event.reason?.message||String(event.reason||'Erreur asynchrone inconnue'));});
...
function buildAuditSearchIndex(){
  const navigation=[
    ...
    {view:'labimports',icon:'📥',title:'Imports analyses',keywords:'import chatgpt analyses fourrage sol vegetal eau parasitisme sang poils aliment etiquette'},
...
function showReproScoreModal(reproFarm,cowId,mode='score'){
  ...
  if(mode==='history'){
    ...
    overlay.innerHTML=`<div class="repro-detail-dialog"><div class="section-title"><div><h3>Détail du score · ${escapeHtml(r.cow.workNumber||r.cow.id)}${r.cow.name?` · ${escapeHtml(r.cow.name)}`:''}</h3><div class="muted">Score actuel : <strong>${reproductionScore(r)}/100</strong></div></div><button class="btn secondary" id="close-repro-detail">✕ Fermer</button></div><div class="table-wrap"><table><thead><tr><th>Critère</th><th>Valeur</th><th>Pénalité</th></tr></thead><tbody>${d.lines.map(x=>`<tr class="${x.delta<0?'repro-score-hit-row':''}"><td>${escapeHtml(x.label)}</td><td>${escapeHtml(x.value)}</td><td><strong>${x.delta}</strong></td></tr>`).join('')}</tbody></table></div><p class="muted small-text">Les lignes surlignées correspondent aux éléments qui retirent effectivement des points.</p>${reproductionScoreLegendHtml()}</div>`;
  }
...
function reproAgeBuckets(){return [
  {key:'2',label:'2 ans',min:2,max:2,color:'#F2A6C2'},
  {key:'3',label:'3 ans',min:3,max:3,color:'#D9688A'},
  {key:'4-6',label:'4–6 ans',min:4,max:6,color:'#B44E77'},
  {key:'7-9',label:'7–9 ans',min:7,max:9,color:'#8B5FA8'},
  {key:'10-12',label:'10–12 ans',min:10,max:12,color:'#5C86B1'},
  {key:'13+',label:'> 12 ans',min:13,max:99,color:'#70A17A'}
];}
function reproAgeYearColor(index){const hue=(index*41+334)%360;return `hsl(${hue} 56% 62%)`;}
function reproDonutLabelAnchor(x,cx){return Math.abs(x-cx)<8?'middle':x<cx?'end':'start';}
function reproAgeDonutSvg(bucketRows,total){
  const cx=220,cy=165,r=92,stroke=50,circ=2*Math.PI*r;
  const base=`<circle cx="${cx}" cy="${cy}" r="${r}" fill="none" stroke="#f3e4ea" stroke-width="${stroke}"></circle>`;
  let offset=0;
  const segments=bucketRows.map(row=>{
    const portion=row.count/total;if(!portion)return '';
    const dash=Math.max(0.0001,portion*circ),gap=Math.max(0,circ-dash),html=`<circle cx="${cx}" cy="${cy}" r="${r}" fill="none" stroke="${row.color}" stroke-width="${stroke}" stroke-dasharray="${dash} ${gap}" stroke-dashoffset="${-offset}" stroke-linecap="butt"></circle>`;offset+=dash;return html;
  }).join('');
  let startAngle=-Math.PI/2;
  const labels=bucketRows.map(row=>{
    if(!row.count)return '';
    const frac=row.count/total,mid=startAngle+frac*Math.PI,labelAngle=mid,innerX=cx+Math.cos(labelAngle)*(r+stroke/2-3),innerY=cy+Math.sin(labelAngle)*(r+stroke/2-3),outerX=cx+Math.cos(labelAngle)*(r+stroke/2+24),outerY=cy+Math.sin(labelAngle)*(r+stroke/2+24),textX=cx+Math.cos(labelAngle)*(r+stroke/2+70),textY=cy+Math.sin(labelAngle)*(r+stroke/2+48),pct=Math.round(row.count/total*1000)/10;startAngle+=frac*2*Math.PI;return `<g class="repro-age-donut-label"><line x1="${innerX.toFixed(1)}" y1="${innerY.toFixed(1)}" x2="${outerX.toFixed(1)}" y2="${outerY.toFixed(1)}"></line><text x="${textX.toFixed(1)}" y="${(textY-2).toFixed(1)}" text-anchor="${reproDonutLabelAnchor(textX,cx)}"><tspan class="bucket">${escapeHtml(row.label)}</tspan><tspan x="${textX.toFixed(1)}" dy="1.15em" class="count">${row.count} · ${pct}%</tspan></text></g>`;
  }).join('');
  return `<svg class="repro-age-donut-svg" viewBox="-35 -20 515 365" role="img" aria-label="Répartition des femelles par tranche d'âge"><g transform="rotate(-90 ${cx} ${cy})">${base}${segments}</g><circle cx="${cx}" cy="${cy}" r="58" fill="var(--card,#fff)"></circle><text x="${cx}" y="${cy-6}" text-anchor="middle" class="repro-age-donut-total">${total}</text><text x="${cx}" y="${cy+22}" text-anchor="middle" class="repro-age-donut-sub">femelles</text>${labels}</svg>`;
}
function reproAgeDistributionHtml(animals=[],analysisDate='',opts={}){
  const options=opts&&typeof opts==='object'?opts:{};
  const interactive=options.interactive!==false,wrap=options.wrap!==false,showBucketLegend=options.showBucketLegend!==false,detailTargetId=options.detailTargetId===undefined?'repro-age-year-detail':String(options.detailTargetId||'');
  const vals=animals.map(a=>({animal:a,months:monthsBetweenDates(a.birthDate,analysisDate)})).filter(x=>x.months!=null&&x.months>=24);
  if(!vals.length){
    const empty=`<div class="empty">Âges non disponibles dans le registre.</div>`;
    return wrap?`<section class="card"><h3>🥧 Répartition des femelles par âge</h3>${empty}</section>`:empty;
  }
  const years=new Map();for(const x of vals){const y=Math.max(2,Math.floor(x.months/12));years.set(y,(years.get(y)||0)+1);}
  const yearEntries=[...years.entries()].sort((a,b)=>a[0]-b[0]),total=vals.length,buckets=reproAgeBuckets().map(b=>({...b,count:yearEntries.filter(([y])=>y>=b.min&&y<=b.max).reduce((acc,[,n])=>acc+n,0)})).filter(b=>b.count>0);
  const yearLegend=yearEntries.map(([y,n],i)=>{const pct=Math.round(n/total*1000)/10,color=reproAgeYearColor(i),content=`<i style="background:${color}"></i><span><strong>${y} an${y>1?'s':''}</strong> — ${n} (${pct}%)</span>`;return interactive?`<button type="button" class="repro-age-legend-item" data-age-year="${y}" ${detailTargetId?`data-age-detail-target="${escapeHtml(detailTargetId)}"`:''} title="Voir les femelles de ${y} ans">${content}</button>`:`<div class="repro-age-legend-item static">${content}</div>`;}).join('');
  const bucketLegend=showBucketLegend?buckets.map(b=>{const pct=Math.round(b.count/total*1000)/10;return `<span><i style="background:${b.color}"></i><strong>${escapeHtml(b.label)}</strong> — ${b.count} (${pct}%)</span>`;}).join(''):'';
  const avg=Math.round(vals.reduce((a,x)=>a+x.months,0)/vals.length/12*10)/10,max=Math.max(...vals.map(x=>x.months)),maxRounded=Math.round(max),oldest=vals.filter(x=>x.months===max).map(x=>x.animal);
  const block=`<div class="${wrap?'':'repro-age-standalone '}repro-age-distribution"><div class="section-title"><div><h3>🥧 Répartition des femelles par âge</h3><div class="muted">Femelles présentes de 24 mois et plus. Le détail reste listé année par année, mais le camembert regroupe les classes d’âge.</div></div><span class="badge">${total} femelle(s)</span></div><div class="repro-age-overview grouped"><div class="repro-age-chart-panel">${reproAgeDonutSvg(buckets,total)}${showBucketLegend?`<div class="repro-age-bucket-legend">${bucketLegend}</div>`:''}</div><div><div class="repro-age-year-title">Détail par année</div><div class="repro-age-legend">${yearLegend}</div></div></div><div class="repro-kpi-grid compact"><article class="card metric"><strong>${avg.toLocaleString('fr-FR',{minimumFractionDigits:1,maximumFractionDigits:1})} ans</strong><span>Âge moyen des femelles ≥24 mois</span></article><article class="card metric"><strong>${Math.floor(maxRounded/12)} a ${maxRounded%12} m</strong><span>Âge maximum</span><small>${oldest.map(a=>escapeHtml(a.name||a.workNumber||a.id)).join(', ')}</small></article></div>${detailTargetId?`<div id="${escapeHtml(detailTargetId)}"></div>`:''}</div>`;
  return wrap?`<section class="card repro-age-distribution-card">${block}</section>`:block;
}
...
if ('serviceWorker' in navigator){
  ...
  const AUDIT_SW=location.origin+AUDIT_PATH+'sw.js?v=14.6.21.123';
...
(function initWorkflowPhases(){const map={dashboard:'prepare',farms:'prepare',visits:'prepare',documents:'prepare',herddata:'prepare',questionnaires:'prepare',prepprint:'prepare',followup:'prepare',animals:'terrain',quicknotes:'terrain',analysis:'terrain',feeding:'terrain',building:'terrain',audit:'terrain',photos:'terrain',checkout:'terrain',nutrition:'report',labimports:'report',reproduction:'report',metabolic:'report',parasitism:'report',waterlab:'report',assistant:'report',pilotage:'report',reports:'report',review:'report',study:'follow',economy:'follow',journal:'follow'};function apply(phase){localStorage.setItem('audit-bovin-phase',phase);document.querySelectorAll('.phase-btn').forEach(b=>b.classList.toggle('active',b.dataset.phase===phase));document.querySelectorAll('.nav-btn[data-phase-item]').forEach(b=>b.classList.toggle('phase-hidden',b.dataset.phaseItem!==phase));}window.__auditApplyPhaseForView=view=>{const ph=map[view];if(ph)apply(ph)};document.querySelectorAll('.phase-btn').forEach(b=>b.addEventListener('click',()=>apply(b.dataset.phase)));apply(map[currentView]||localStorage.getItem('audit-bovin-phase')||'prepare');})();


// v14.6.21.99 — check-up, suivi d'étude, économie et conservation globale du défilement horizontal
...
// v14.6.21.99 — suivi du temps par intervenant + temps GDS préparation/terrain/compte rendu/suivi

// v14.6.21.99 — questionnaires persistants, repères de saisie, bilan sanitaire/éco partenaire, supports papier, check-up responsive.

window.addEventListener('DOMContentLoaded',()=>{initGlobalSearch();initAccordionMemory();});
// v14.6.21.99 — cohérence, imprimables préparation, animaux présents, recherche globale.
