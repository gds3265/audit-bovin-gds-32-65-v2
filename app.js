...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Repères visuels, fiches pratiques et carnet technique, disponibles hors ligne.</div></div><span class="badge autosave">v14.6.21.55</span></div>
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.6.21.55</span></div>
...
function renderVisits() {
  ...
  app.innerHTML = `
    <div class="section-title"><div><h2>Visites</h2><div class="muted">Toutes les visites enregistrées restent accessibles, y compris les plus anciennes.</div></div><span class="badge autosave">v14.6.21.55</span></div>
...
function renderFarmerQuestionnaires(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📨 Questionnaires éleveur</h2><div class="muted">Préparer la visite puis récupérer uniquement les réponses encore manquantes.</div></div><span class="badge autosave">v14.6.21.55</span></div>${activeVisitBanner(visit)}
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.6.21.55</span></div>
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.6.21.55</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
...
function renderFollowup(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Historique & évolution</h2><div class="muted">Comparer deux visites d’une même exploitation : reproduction, mortalité, mesures, alimentation, bâtiment et plans d’action.</div></div><span class="badge autosave">v14.6.21.55</span></div>
...
function reproWeatherCardHtml(inv,farm,periodStart,periodEnd){
  ...
  return `<section class="card repro-weather-card"><div class="section-title"><div><h3>🌦️ Historique Météo-France</h3><div class="muted">Données quotidiennes publiques Météo-France via data.gouv.fr : température et pluie, sans clé API.</div></div><span class="badge ${rec.length?'complete':''}">${rec.length?rec.length+' jour(s) chargé(s)':'Non chargé'}</span></div>${rec.length?`<div class="notice"><strong>Source enregistrée :</strong> Météo-France / data.gouv.fr · ${escapeHtml(w.stationName||w.stationId||'station')} · ${formatDate(from)} → ${formatDate(to)}. Les périodes météo sont utilisées comme pistes de contexte, pas comme preuve de causalité.</div>`:''}<details ${rec.length?'':'open'}><summary><strong>${rec.length?'Mettre à jour les données météo':'Charger l’historique Météo-France'}</strong></summary><div class="grid cols-2" style="margin-top:12px"><div class="field"><label>Département</label><input id="repro-weather-dept" inputmode="numeric" maxlength="3" value="${escapeHtml(dept)}" placeholder="32 ou 65"></div><div class="field"><label>Période recherchée</label><div class="muted">${periodStart&&periodEnd?formatDate(periodStart)+' → '+formatDate(periodEnd):'Période reproduction sélectionnée'}</div></div></div><div class="actions"><button class="btn secondary small" id="repro-weather-stations" type="button">1. Charger les stations du département</button><select id="repro-weather-station"><option value="${escapeHtml(w.stationId||'')}">${w.stationId?escapeHtml((w.stationName||w.stationId)+' — station enregistrée'):'Choisir une station…'}</option></select><button class="btn primary small" id="repro-weather-import" type="button">2. Intégrer la station choisie</button>${rec.length?'<button class="btn secondary small" id="repro-weather-export" type="button">Exporter le CSV météo chargé</button><button class="btn danger small" id="repro-weather-clear" type="button">Effacer météo</button>':''}</div><div id="repro-weather-status" class="muted" style="margin-top:8px">Aucune clé nécessaire. L’application récupère les fichiers quotidiens officiels Météo-France publiés par département sur data.gouv.fr.</div><div class="field" style="margin-top:12px"><label>Ou importer un fichier Météo-France</label><input id="repro-weather-file" type="file" accept=".csv,.gz,text/csv,application/gzip"><small class="muted">Accepte le CSV quotidien ou le fichier .csv.gz téléchargé depuis data.gouv.fr.</small></div></details></section>`;
}
...
async function reproWeatherUngzipResponse(resp,url=''){
  ...
  if(typeof DecompressionStream==='undefined')throw new Error('Ce navigateur ne sait pas décompresser le .gz. Téléchargez le CSV puis importez-le manuellement.');
...
async function reproWeatherDataGouvResources(dept,start,end){
  ...
  // Priorité aux fichiers RR-T (pluie/température) et aux périodes les plus récentes.
...
function renderReproduction(){
  ...
  if(visit){const saveInv=()=>{visit.updatedAt=new Date().toISOString();saveDatabase(db)};app.querySelectorAll('[data-repro-heat]').forEach(el=>{const f=el.dataset.reproHeat;const fn=()=>{inv.heat[f]=el.value;saveInv()};el.onchange=()=>{fn();renderReproduction()};el.oninput=fn});app.querySelectorAll('[data-repro-setting]').forEach(el=>{const f=el.dataset.reproSetting;const fn=()=>{inv.settings[f]=el.type==='number'?Number(el.value||0):el.value;saveInv()};el.onchange=()=>{fn();renderReproduction()};el.oninput=fn});app.querySelectorAll('[data-bull-field]').forEach(el=>{const row=el.closest('[data-repro-bull-row]'),i=Number(row?.dataset.reproBullRow),f=el.dataset.bullField;const fn=()=>{if(!inv.bulls[i])return;inv.bulls[i][f]=el.type==='number'?Number(el.value||0):el.value;saveInv()};el.oninput=fn;el.onchange=()=>{fn();renderReproduction()}});document.getElementById('add-repro-bull')?.addEventListener('click',()=>{inv.bulls.push({id:uid('bull'),name:'',ageClass:'adult',females:'',contactDays:'',fertility:''});saveInv();renderReproduction()});app.querySelectorAll('[data-delete-bull]').forEach(b=>b.onclick=()=>{inv.bulls.splice(Number(b.dataset.deleteBull),1);saveInv();renderReproduction()});document.getElementById('add-repro-event')?.addEventListener('click',()=>{const date=document.getElementById('repro-event-date')?.value,label=document.getElementById('repro-event-label')?.value.trim(),type=document.getElementById('repro-event-type')?.value;if(!date||!label){showToast('Renseignez la date et l’événement.');return;}inv.events.push({id:uid('reproevt'),date,type,label});saveInv();renderReproduction()});app.querySelectorAll('[data-delete-repro-event]').forEach(b=>b.onclick=()=>{inv.events=inv.events.filter(e=>e.id!==b.dataset.deleteReproEvent);saveInv();renderReproduction()});
    ...
    let weatherDeptRows=[];
    document.getElementById('repro-weather-stations')?.addEventListener('click',async()=>{const dept=document.getElementById('repro-weather-dept')?.value.trim();if(!dept||!periodStart||!periodEnd){weatherStatus('Renseignez le département et une période d’analyse.');return;}try{weatherStatus('Recherche des fichiers Météo-France sur data.gouv.fr…');const got=await reproWeatherLoadDepartment(dept,periodStart,periodEnd,weatherStatus);weatherDeptRows=got.rows;const stations=reproWeatherStationsFromRows(weatherDeptRows),sel=document.getElementById('repro-weather-station');if(!stations.length)throw new Error('Aucune station identifiable dans les fichiers téléchargés.');sel.innerHTML=stations.map(x=>`<option value="${escapeHtml(String(x.id))}" data-name="${escapeHtml(x.name||'')}">${escapeHtml(x.name||String(x.id))} — ${x.count} j (${formatDate(x.from)} → ${formatDate(x.to)})</option>`).join('');if(inv.weather.stationId&&[...sel.options].some(o=>o.value===String(inv.weather.stationId)))sel.value=String(inv.weather.stationId);weatherStatus(`${stations.length} station(s) trouvée(s) dans ${got.resourcesUsed} fichier(s). Choisissez la station la plus représentative de l’exploitation.`);}catch(e){weatherStatus(`Impossible de charger les stations : ${e.message}`)}});
    document.getElementById('repro-weather-import')?.addEventListener('click',async()=>{const sel=document.getElementById('repro-weather-station'),stationId=sel?.value,stationName=sel?.selectedOptions?.[0]?.dataset?.name||sel?.selectedOptions?.[0]?.textContent||stationId,dept=document.getElementById('repro-weather-dept')?.value.trim();if(!stationId||!periodStart||!periodEnd){weatherStatus('Chargez les stations puis choisissez-en une.');return;}try{if(!weatherDeptRows.length){weatherStatus('Rechargement des données du département…');weatherDeptRows=(await reproWeatherLoadDepartment(dept,periodStart,periodEnd,weatherStatus)).rows;}const selected=weatherDeptRows.filter(r=>String(r.stationId||r.stationName)===String(stationId)).map(({date,rr,tn,tx,tm})=>({date,rr,tn,tx,tm})).sort((a,b)=>a.date.localeCompare(b.date));if(!selected.length)throw new Error('Aucune donnée trouvée pour cette station.');inv.weather={...inv.weather,department:dept,stationId:String(stationId),stationName:String(stationName).replace(/\s+—.*$/,''),records:selected,source:'Météo-France — données climatologiques quotidiennes via data.gouv.fr',importedAt:new Date().toISOString(),from:selected[0].date,to:selected.at(-1).date};saveInv();showToast(`${selected.length} jour(s) Météo-France enregistrés.`);renderReproduction();}catch(e){weatherStatus(`Import Météo-France impossible : ${e.message}`)}});
    document.getElementById('repro-weather-file')?.addEventListener('change',async e=>{const file=e.target.files?.[0];if(!file)return;try{const detailed=reproWeatherParseDetailedCsv(await reproWeatherReadFile(file));if(!detailed.length)throw new Error('Aucune donnée pluie/température reconnue.');const stations=reproWeatherStationsFromRows(detailed);let selected=detailed,stationId='',stationName='';if(stations.length>1){const choices=stations.slice(0,50).map((x,i)=>`${i+1}. ${x.name} (${x.count} j)`).join('\n');const n=Number(prompt(`Ce fichier contient ${stations.length} stations. Numéro de la station à importer :\n\n${choices}`, '1'));if(!n||!stations[n-1])return;const st=stations[n-1];stationId=st.id;stationName=st.name;selected=detailed.filter(r=>String(r.stationId||r.stationName)===String(st.id));}else if(stations.length===1){stationId=stations[0].id;stationName=stations[0].name;}const records=selected.map(({date,rr,tn,tx,tm})=>({date,rr,tn,tx,tm})).sort((a,b)=>a.date.localeCompare(b.date));inv.weather={...inv.weather,department:document.getElementById('repro-weather-dept')?.value.trim()||inv.weather.department||'',stationId,stationName,records,source:'Fichier quotidien Météo-France',fileName:file.name,importedAt:new Date().toISOString(),from:records[0].date,to:records.at(-1).date};saveInv();showToast(`${records.length} jour(s) météo importés depuis ${file.name}.`);renderReproduction();}catch(err){weatherStatus(`Fichier météo non importé : ${err.message}`)}});
...
// v14.6.21.55 — check-up, suivi d'étude, économie et conservation globale du défilement horizontal
...
// v14.6.21.55 — suivi du temps par intervenant + temps GDS préparation/terrain/compte rendu/suivi

// v14.6.21.55 — questionnaires persistants, repères de saisie, bilan sanitaire/éco partenaire, supports papier, check-up responsive.

window.addEventListener('DOMContentLoaded',()=>{initGlobalSearch();initAccordionMemory();});
// v14.6.21.55 — cohérence, imprimables préparation, animaux présents, recherche globale.
