...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Quelques repères visuels utiles, intégrés à l’application et disponibles hors ligne.</div></div><span class="badge autosave">v14.3</span></div>
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.3</span></div>
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.3</span></div>
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.3</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
...
function renderReproduction(){
  ...
  const registry=farm?.herdRegistry||[]; const females=registry.filter(a=>a.sex==='F'); let rows=females.map(c=>reproductionForCow(farm,c.id)).filter(r=>r.calves.length||!r.cow.exitDate);
  const today=new Date().toISOString().slice(0,10);
  const firstAges=rows.map(r=>r.firstCalvingAgeMonths).filter(x=>x!=null),ivvs=rows.flatMap(r=>r.intervals);
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🐄 Reproduction</h2><div class="muted">Import du registre bovins, IVV, âge au premier vêlage, descendance et alertes.</div></div><span class="badge autosave">v14.4</span></div>
  <section class="card"><div class="row"><div class="field"><label>Exploitation</label><select id="repro-farm">${db.farms.map(f=>`<option value="${f.id}" ${f.id===selectedFarmId?'selected':''}>${escapeHtml(f.name)}${f.farmNumber?` · ${escapeHtml(f.farmNumber)}`:''}</option>`).join('')}</select></div><div class="field"><label>Importer le registre bovins CSV</label><input id="repro-file" type="file" accept=".csv,text/csv"></div></div><div class="muted">${registry.length?`${registry.length} bovin(s) importé(s) · mise à jour ${formatDateTime(farm.herdRegistryImportedAt)}`:'Aucun registre importé pour cette exploitation.'}</div></section>
