...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Repères visuels, fiches pratiques et carnet technique, disponibles hors ligne.</div></div><span class="badge autosave">v14.6.21.52</span></div>
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.6.21.52</span></div>
...
function renderVisits() {
  ...
  app.innerHTML = `
    <div class="section-title"><div><h2>Visites</h2><div class="muted">Toutes les visites enregistrées restent accessibles, y compris les plus anciennes.</div></div><span class="badge autosave">v14.6.21.52</span></div>
...
function renderFarmerQuestionnaires(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📨 Questionnaires éleveur</h2><div class="muted">Préparer la visite puis récupérer uniquement les réponses encore manquantes.</div></div><span class="badge autosave">v14.6.21.52</span></div>${activeVisitBanner(visit)}
...
function printReproMortalityPrep(){const v=activeVisit();if(!v)return;const f=db.farms.find(x=>x.id===v.farmId),src=reproductionSourceForVisit(v,f),reg=src.registry||[],rf={...f,herdRegistry:reg},date=v.date||new Date().toISOString().slice(0,10),cows=currentReproductionCows(rf,date),present=reg.filter(a=>isRegistryAnimalPresent(a,date));const calves=cows.flatMap(r=>r.calves||[]),dead=calves.filter(c=>c.exitCause==='M'&&c.exitDate&&daysBetweenDates(c.birthDate,c.exitDate)<183),problems=cows.filter(r=>(r.daysSinceLast||0)>400||(r.meanIVV||0)>450||(r.deadBefore6||[]).length>=2||(r.firstCalvingAgeMonths||0)>36).sort((a,b)=>sortByWorkNumber(a.cow,b.cow));const body=`<h1>Bilan préparatoire — reproduction, mortalité et vaches à problèmes</h1>${activeFarmNameForPrint(v)}<p><b>Animaux présents dans le registre :</b> ${present.length} &nbsp; <b>Vaches avec vêlage :</b> ${cows.length} &nbsp; <b>Mortalité veaux &lt; 6 mois :</b> ${calves.length?Math.round(dead.length/calves.length*1000)/10:'—'} % (${dead.length}/${calves.length})</p><p><b>Classement :</b> n° de travail croissant. Le score reproduction est affiché pour comprendre pourquoi une vache ressort dans les alertes.</p><h2>Vaches à problèmes</h2><table><thead><tr><th>N° travail / animal</th><th>Score</th><th>Âge</th><th>Dernier vêlage</th><th>IVV moyen</th><th>1er vêlage</th><th>Veaux morts &lt;6m</th></tr></thead><tbody>${problems.map(r=>`<tr><td><b>${escapeHtml(r.cow.workNumber||r.cow.id)}</b>${r.cow.name?` · ${escapeHtml(r.cow.name)}`:''}<br><small>${escapeHtml(r.cow.id||'')}</small></td><td><b>${reproductionScore(r)}/100</b></td><td>${escapeHtml(ageLabelAt(r.cow.birthDate,date)||'—')}</td><td>${r.lastCalvingDate?formatDate(r.lastCalvingDate):'—'}</td><td>${r.meanIVV??'—'}</td><td>${r.firstCalvingAgeMonths??'—'} mois</td><td>${(r.deadBefore6||[]).length}</td></tr>`).join('')||'<tr><td colspan="7">Aucune alerte selon les critères actuels.</td></tr>'}</tbody></table>`;paperWindow('Bilan reproduction et mortalité',body)}
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.6.21.52</span></div>
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.6.21.52</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
...
function renderFollowup(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Historique & évolution</h2><div class="muted">Comparer deux visites d’une même exploitation : reproduction, mortalité, mesures, alimentation, bâtiment et plans d’action.</div></div><span class="badge autosave">v14.6.21.52</span></div>
...
function reproPreparationSnapshot(visit,farm){
  ...
  const births=registry.filter(a=>a.birthDate&&a.birthDate>=startIso&&a.birthDate<=date);
  const eligibleFemales=females24.length;
  const sireMap=new Map();
  births.filter(c=>c.fatherId).forEach(c=>{
    const key=normalizeAnimalId(c.fatherId);if(!key)return;
    if(!sireMap.has(key))sireMap.set(key,{fatherId:c.fatherId,calves:0,mothers:new Set(),birthDates:[]});
    const r=sireMap.get(key);r.calves++;if(c.motherId)r.mothers.add(normalizeAnimalId(c.motherId));r.birthDates.push(c.birthDate);
  });
  const sires=[...sireMap.values()].map(r=>{const male=registry.find(a=>normalizeAnimalId(a.id)===normalizeAnimalId(r.fatherId));return {...r,mothersCount:r.mothers.size,pctFemales:eligibleFemales?Math.round(r.mothers.size/eligibleFemales*1000)/10:null,present:!!male&&isRegistryAnimalPresent(male,date),workNumber:male?.workNumber||'',name:male?.name||'',age:male?.birthDate?ageLabelAt(male.birthDate,date):'',firstBirth:r.birthDates.slice().sort()[0]||'',lastBirth:r.birthDates.slice().sort().at(-1)||''}}).sort((a,b)=>b.calves-a.calves);
  const fatherKnown=births.filter(c=>c.fatherId).length;
  ...
  const chrono=reproChronology(registry,cows,startIso,date,ensureReproInvestigation(visit));
  return {date,startIso,present,females24,females36,males24,males36,births,sires,fatherKnown,eligibleFemales,heifers36,chrono};
}
function reproPreparationHtml(visit,farm){
  ...
  const paternity=x.births.length?Math.round(x.fatherKnown/x.births.length*1000)/10:null;
  const alerts=[];
  ...
  if(x.males24.length===0&&x.eligibleFemales)alerts.push(`<li><strong>Aucun mâle &gt;24 mois présent</strong> dans le registre : vérifier IA, taureau extérieur ou dates de présence.</li>`);
  if(paternity!=null&&paternity<70)alerts.push(`<li>Paternité renseignée pour seulement <strong>${paternity}%</strong> des naissances des 12 derniers mois : interprétation par mâle incomplète.</li>`);
  if(x.sires.length===1&&x.eligibleFemales>=40)alerts.push(`<li>Un seul père identifié pour ${x.eligibleFemales} femelles &gt;24 mois : vérifier l’organisation réelle des lots et la durée de contact.</li>`);
  const problems=x.chrono?.problems||[];
  if(problems.length)alerts.push(`<li><strong>${problems.length} période(s) à investiguer</strong> repérée(s) dans la chronologie des performances ; commencer par la plus ancienne pour dater le début de la dégradation.</li>`);
  return `<section class="card repro-prep-analysis"><div class="section-title"><div><h3>🔎 Analyse préparatoire reproduction</h3><div class="muted">À regarder avant l’audit : structure du troupeau, naissances, paternités et premiers signaux à éclaircir ensuite avec l’éleveur.</div></div><span class="badge">12 derniers mois</span></div><div class="repro-kpi-grid compact"><article class="metric"><strong>${x.females24.length}</strong><span>Femelles &gt;24 mois</span></article><article class="metric"><strong>${x.females36.length}</strong><span>Femelles &gt;36 mois</span></article><article class="metric"><strong>${x.males24.length}</strong><span>Mâles &gt;24 mois</span></article><article class="metric"><strong>${x.males36.length}</strong><span>Mâles &gt;36 mois</span></article><article class="metric"><strong>${x.births.length}</strong><span>Naissances</span><small>${formatDate(x.startIso)} → ${formatDate(x.date)}</small></article><article class="metric"><strong>${paternity??'—'}${paternity!=null?'%':''}</strong><span>Paternité renseignée</span><small>${x.fatherKnown}/${x.births.length} naissance(s)</small></article></div>${alerts.length?`<div class="notice warning"><strong>Points à préparer avant les questions terrain</strong><ul>${alerts.join('')}</ul></div>`:`<div class="notice success"><strong>Aucune alerte structurelle évidente sur ces indicateurs.</strong> Les questions terrain restent guidées par les performances détaillées.</div>`}<h4>Répartition des naissances par père — 12 derniers mois</h4>${x.sires.length?`<div class="table-wrap"><table><thead><tr><th>Père</th><th>Présent</th><th>Veaux</th><th>Femelles distinctes concernées</th><th>% des femelles &gt;24 m</th><th>1re / dernière naissance</th></tr></thead><tbody>${x.sires.map(s=>`<tr><td><strong>${escapeHtml(s.workNumber||s.name||s.fatherId)}</strong><br><small>${escapeHtml(s.fatherId)}${s.age?` · ${escapeHtml(s.age)}`:''}</small></td><td>${s.present?'✅ oui':'— non / inconnu'}</td><td>${s.calves}</td><td>${s.mothersCount||'—'}</td><td>${s.pctFemales!=null?s.pctFemales+'%':'—'}</td><td>${s.firstBirth?formatDate(s.firstBirth):'—'} / ${s.lastBirth?formatDate(s.lastBirth):'—'}</td></tr>`).join('')}</tbody></table></div><p class="muted small-text">Le pourcentage correspond aux <strong>mères distinctes ayant eu un veau attribué à ce père</strong> parmi les femelles &gt;24 mois présentes. Ce n’est pas un nombre exact de saillies : il sert à repérer un déséquilibre à confirmer avec l’éleveur.</p>`:'<div class="empty">Aucune paternité exploitable sur les naissances des 12 derniers mois.</div>'}${problems.length?`<details open><summary><strong>🕰️ Début possible des problèmes</strong></summary><p class="muted">La chronologie replace les anomalies autour des périodes probables de conception. Ces périodes servent à préparer les questions sur taureaux, détection des chaleurs, lots, sanitaire, alimentation, introductions et météo.</p><ul>${problems.slice().sort((a,b)=>quarterSortValue(a.key)-quarterSortValue(b.key)).slice(0,6).map(p=>`<li><strong>${escapeHtml(p.key||'Période')}</strong>${p.reasons?.length?` — ${p.reasons.map(escapeHtml).join(' · ')}`:''}</li>`).join('')}</ul></details>`:''}</section>`;
}
...
async function reproWeatherFetch(path,params,authMode,credential){const u=reproWeatherApiUrl(path,params),headers={'accept':'*/*'};if(credential){if(authMode==='apikey')headers.apikey=credential;else headers.Authorization=`Bearer ${credential}`;}const r=await fetch(u,{headers});if(!r.ok&&r.status!==202&&r.status!==201&&r.status!==204){if(r.status===401)throw new Error(authMode==='apikey'?'Météo-France : clé API refusée (401). Vérifiez l’abonnement à Données Climatologiques et utilisez bien la clé générée en mode API Key.':'Météo-France : token OAuth2 refusé ou expiré (401). Régénérez un token temporaire.');throw new Error(`Météo-France : erreur ${r.status}`)}return r;}
...
// v14.6.21.52 — check-up, suivi d'étude, économie et conservation globale du défilement horizontal
...
// v14.6.21.52 — suivi du temps par intervenant + temps GDS préparation/terrain/compte rendu/suivi

// v14.6.21.52 — questionnaires persistants, repères de saisie, bilan sanitaire/éco partenaire, supports papier, check-up responsive.

window.addEventListener('DOMContentLoaded',()=>{initGlobalSearch();initAccordionMemory();});
// v14.6.21.52 — cohérence, imprimables préparation, animaux présents, recherche globale.
