...
function renderPlanches() {
  ...
  app.innerHTML = `<div class="section-title"><div><h2>Bibliothèque technique</h2><div class="muted">Repères visuels, fiches pratiques et carnet technique, disponibles hors ligne.</div></div><span class="badge autosave">v14.6.21.54</span></div>
...
function renderJournalSuivi(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📒 Journal & points de vigilance</h2><div class="muted">Noter les nouvelles reçues entre deux visites, les difficultés et les événements sanitaires.</div></div><span class="badge autosave">v14.6.21.54</span></div>
...
function renderVisits() {
  ...
  app.innerHTML = `
    <div class="section-title"><div><h2>Visites</h2><div class="muted">Toutes les visites enregistrées restent accessibles, y compris les plus anciennes.</div></div><span class="badge autosave">v14.6.21.54</span></div>
...
function renderFarmerQuestionnaires(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>📨 Questionnaires éleveur</h2><div class="muted">Préparer la visite puis récupérer uniquement les réponses encore manquantes.</div></div><span class="badge autosave">v14.6.21.54</span></div>${activeVisitBanner(visit)}
...
function printReproMortalityPrep(){
  ...
  const chronoProblems=x.chrono?.problems||[],chrono=`<h2>7. Chronologie des performances</h2><p>Les IVV sont replacés sur leur période probable de conception et croisés avec mortalité, introductions, événements historiques et météo lorsqu’elle est disponible.</p>${chronoProblems.length?chronoProblems.slice().sort((a,b)=>quarterSortValue(a.key)-quarterSortValue(b.key)).map(p=>`<div class="period-alert"><b>${escapeHtml(p.key||'Période à investiguer')}</b>${p.reasons?.length?`<br>${p.reasons.map(escapeHtml).join(' · ')}`:''}</div>`).join(''):'<p>Aucune période problématique détectée avec les critères actuels.</p>'}${x.chrono?.buckets?.length?`<table><thead><tr><th>Période</th><th>IVV moyen</th><th>IVV &gt;450 j</th><th>Naissances</th><th>Morts &lt;6m</th><th>Introductions</th><th>Météo</th><th>Événements</th></tr></thead><tbody>${x.chrono.buckets.map(b=>`<tr><td><b>${escapeHtml(b.key)}</b></td><td>${b.avg??'—'}${b.avg!=null?' j':''} (${b.ivvs?.length||0})</td><td>${b.ivvs?.length?b.badRate+' %':'—'}</td><td>${b.calves??0}</td><td>${b.mortRate==null?'—':b.mortRate+' %'}</td><td>${b.introductions?.length||0}</td><td>${b.weather?`${b.weather.rain==null?'—':b.weather.rain+' mm'} / ${b.weather.temp==null?'—':b.weather.temp+' °C'}${b.weather.tags?.length?` · ${escapeHtml(b.weather.tags.join(', '))}`:''}`:'—'}</td><td>${b.events?.length?b.events.map(e=>escapeHtml(e.label||e.type||'Événement')).join('<br>'):'—'}</td></tr>`).join('')}</tbody></table>`:''}`;
  const questions=[];
  if(x.heifers36.length)questions.push(`${x.heifers36.length} femelle(s) >36 mois sans vêlage : âge à la mise à la reproduction, chaleurs observées, saillie/IA, état corporel, lot.`);
  if(x.males24.length===0&&x.females24.length)questions.push('Aucun mâle >24 mois actuellement présent : préciser IA, taureau extérieur, rotation ou période de présence des taureaux.');
  if(x.ivvOver500.length||x.ivv451to500.length)questions.push('IVV longs : dater les lots/périodes concernés puis rechercher chaleurs, fertilité taureau, alimentation, sanitaire, estive et organisation des lots.');
  if(chronoProblems.length)questions.push('Reprendre les périodes à investiguer ci-dessus avec l’éleveur : changement alimentaire, maladie, introductions, météo, déplacement de lots, taureau absent ou changé.');
  const toClarify=`<h2>8. Points à éclaircir pendant l’audit</h2>${questions.length?`<ul>${questions.map(q=>`<li>${escapeHtml(q)}</li>`).join('')}</ul>`:'<p>Aucun point automatique supplémentaire détecté.</p>'}<div class="notes"><b>Notes technicien :</b><br><br><br><br><br></div>`;
  const body=`<h1>Bilan préparatoire — reproduction, mortalité & chronologie des performances</h1>${activeFarmNameForPrint(v)}<p><b>Objectif :</b> analyser les données disponibles avant la visite, repérer ce qui cloche et depuis quand, puis utiliser l’entretien éleveur pour expliquer les anomalies.</p>${structure}${repro}${mortality}${annual}${paternity}${problems}${heifers}${chrono}${toClarify}`;
...
function renderPilotageActions(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>🎯 Pilotage des actions</h2><div class="muted">Vue commune de toutes les actions décidées lors des visites.</div></div><span class="badge autosave">v14.6.21.54</span></div>
...
function renderNutritionAnalysis(){
  const visit=activeVisit();
  if(!visit){app.innerHTML=`<div class="section-title"><div><h2>Analyse nutritionnelle</h2><div class="muted">Interprétation simple des analyses de fourrage et de ration.</div></div><span class="badge autosave">v14.6.21.54</span></div><section class="empty">Choisissez une visite dans l’onglet Visites.</section>`;return;}
...
function renderFollowup(){
  ...
  app.innerHTML=`<div class="section-title"><div><h2>Historique & évolution</h2><div class="muted">Comparer deux visites d’une même exploitation : reproduction, mortalité, mesures, alimentation, bâtiment et plans d’action.</div></div><span class="badge autosave">v14.6.21.54</span></div>
...
function reproPreparationSnapshot(visit,farm){
  ...
  allBirths.filter(c=>c.fatherId).forEach(c=>{
    ...
    if(!male){aiSires.add(key);return;}
    if(!residentSireMap.has(key))residentSireMap.set(key,{fatherId:c.fatherId,male,calves:0,mothers:new Set(),birthDates:[]});
    const r=residentSireMap.get(key);r.calves++;if(c.motherId)r.mothers.add(normalizeAnimalId(c.motherId));r.birthDates.push(c.birthDate);
  });
...
function reproPreparationHtml(visit,farm){
  ...
  <h4>Structure actuelle</h4><div class="repro-kpi-grid compact">${prepReproMetric('Femelles >24 mois',x.females24.length,'f24')}${prepReproMetric('Femelles >36 mois',x.females36.length,'f36')}${prepReproMetric('Mâles >24 mois',x.males24.length,'m24')}${prepReproMetric('Mâles >36 mois',x.males36.length,'m36')}${prepReproMetric('Femelles >36 m sans vêlage',x.heifers36.length,'h36')}${prepReproMetric('Naissances 12 mois',x.births.length,'births',`${formatDate(x.startIso)} → ${formatDate(x.date)}`)}</div>
  <h4>IVV et performances</h4><div class="repro-kpi-grid compact">${prepReproMetric('Vaches avec IVV calculable',x.cows.filter(r=>r.meanIVV!=null).length,'ivvcalc',meanIVV?`IVV moyen ${meanIVV} j`:'')}${prepReproMetric('IVV ≤400 j',x.ivvLe400.length,'ivv400')}${prepReproMetric('IVV 401–450 j',x.ivv401to450.length,'ivv450')}${prepReproMetric('IVV 451–500 j',x.ivv451to500.length,'ivv500')}${prepReproMetric('IVV >500 j',x.ivvOver500.length,'ivvover')}${prepReproMetric('Vaches à problème',x.cowProblems.length,'problems')}${prepReproMetric('Veaux morts <6 mois',x.dead6.length,'dead6',x.calves.length?`${Math.round(x.dead6.length/x.calves.length*1000)/10}% des veaux`:'')}</div>
  ...
  ${x.chrono?.problems?.length?`<details open><summary><strong>🕰️ Chronologie des performances — périodes à soucis (${x.chrono.problems.length})</strong></summary><p class="muted">À analyser avant la visite pour dater le début d’une dégradation, puis préparer les questions ciblées.</p>${x.chrono.problems.slice().sort((a,b)=>quarterSortValue(a.key)-quarterSortValue(b.key)).map(p=>`<div class="notice warning"><strong>${escapeHtml(p.key||'Période')}</strong>${p.reasons?.length?`<br>${p.reasons.map(escapeHtml).join(' · ')}`:''}</div>`).join('')}</details>`:'<div class="notice success"><strong>Chronologie :</strong> aucune période problématique détectée avec les critères actuels.</div>'}</section>`;
}
function bindPrepReproClicks(visit,farm){const x=reproPreparationSnapshot(visit,farm);if(!x)return;const table=rows=>`<div class="table-wrap"><table><thead><tr><th>Animal</th><th>Sexe</th><th>Naissance</th><th>Âge</th><th>Entrée</th></tr></thead><tbody>${rows.map(a=>prepAnimalRow(a,x.date)).join('')||'<tr><td colspan="5">Aucun animal.</td></tr>'}</tbody></table></div>`;document.querySelectorAll('[data-prep-repro-list]').forEach(b=>b.onclick=()=>{const k=b.dataset.prepReproList;let title=b.querySelector('span')?.textContent||'Détail',html='';const reproRows=r=>`<div class="table-wrap"><table><thead><tr><th>Vache</th><th>Âge</th><th>Dernier vêlage</th><th>IVV moyen</th><th>IVV complet</th><th>Score</th></tr></thead><tbody>${r.map(z=>`<tr><td><strong>${escapeHtml(z.cow.workNumber||z.cow.id)}</strong><br><small>${escapeHtml(z.cow.id)}</small></td><td>${escapeHtml(ageLabelAt(z.cow.birthDate,x.date)||'—')}</td><td>${z.lastCalvingDate?formatDate(z.lastCalvingDate):'—'}</td><td>${z.meanIVV??'—'}</td><td>${z.intervals.length?z.intervals.join(' / ')+' j':'—'}</td><td>${reproductionScore(z)}/100</td></tr>`).join('')||'<tr><td colspan="6">Aucune vache.</td></tr>'}</tbody></table></div>`;
    if(k==='f24')html=table(x.females24);else if(k==='f36')html=table(x.females36);else if(k==='m24')html=table(x.males24);else if(k==='m36')html=table(x.males36);else if(k==='h36')html=table(x.heifers36);else if(k==='births')html=table(x.births);else if(k==='dead6')html=table(x.dead6);else if(k==='ivvcalc')html=reproRows(x.cows.filter(r=>r.meanIVV!=null));else if(k==='ivv400')html=reproRows(x.ivvLe400);else if(k==='ivv450')html=reproRows(x.ivv401to450);else if(k==='ivv500')html=reproRows(x.ivv451to500);else if(k==='ivvover')html=reproRows(x.ivvOver500);else if(k==='problems')html=reproRows(x.cowProblems);showPrepReproList(title,html);});}
...
function parseMeteoFranceDailyCsv(text){
  ...
  const ix={date:col('DATE','AAAAMMJJ','DATEOBS','DAT'),rr:col('RR','RR24','PRECIPITATION','PRECIPITATIONS','HAUTEURPRECIPITATIONS'),tn:col('TN','TMIN','TEMPERATUREMINIMALE'),tx:col('TX','TMAX','TEMPERATUREMAXIMALE'),tm:col('TM','TMK','TMOY','TEMPERATUREMOYENNE')};
  ...
  return rows.slice(1).map(r=>({date:dateValue(r[ix.date]),rr:ix.rr>=0?reproWeatherNum(r[ix.rr]):null,tn:ix.tn>=0?reproWeatherNum(r[ix.tn]):null,tx:ix.tx>=0?reproWeatherNum(r[ix.tx]):null,tm:ix.tm>=0?reproWeatherNum(r[ix.tm]):null})).filter(r=>r.date&&(r.rr!=null||r.tn!=null||r.tx!=null||r.tm!=null));
}
...
function reproWeatherStoredCredential(){try{return localStorage.getItem('auditBovin.meteofrance.credential')||''}catch{return ''}}
function reproWeatherStoreCredential(value){try{if(value)localStorage.setItem('auditBovin.meteofrance.credential',value);else localStorage.removeItem('auditBovin.meteofrance.credential')}catch{}}
function reproWeatherCardHtml(inv,farm,periodStart,periodEnd){
  const w=inv?.weather||{},dept=w.department||reproWeatherDepartment(farm)||'',rec=w.records||[],from=rec.length?rec[0].date:'',to=rec.length?rec.at(-1).date:'',savedCredential=reproWeatherStoredCredential();
  return `<section class="card repro-weather-card"><div class="section-title"><div><h3>🌦️ Historique Météo-France</h3><div class="muted">Températures et pluviométrie quotidiennes pour croiser les périodes de reproduction et de mortalité.</div></div><span class="badge ${rec.length?'complete':''}">${rec.length?rec.length+' jour(s) chargé(s)':'Non chargé'}</span></div>${rec.length?`<div class="notice"><strong>Source enregistrée :</strong> Météo-France · ${escapeHtml(w.stationName||w.stationId||'station')} · ${formatDate(from)} → ${formatDate(to)}. Les périodes très sèches/humides ou chaudes/froides sont repérées relativement à l’historique chargé, pas comme un diagnostic climatique officiel.</div>`:''}<details ${rec.length?'':'open'}><summary><strong>${rec.length?'Mettre à jour les données météo':'Configurer la récupération Météo-France'}</strong></summary><div class="grid cols-3" style="margin-top:12px"><div class="field"><label>Département</label><input id="repro-weather-dept" inputmode="numeric" maxlength="3" value="${escapeHtml(dept)}" placeholder="32 ou 65"></div><div class="field"><label>Clé API Météo-France</label><input id="repro-weather-token" type="password" autocomplete="off" value="${escapeHtml(savedCredential)}" placeholder="Clé générée sur le portail Météo-France"></div><div class="field"><label>Accès</label><select id="repro-weather-auth"><option value="apikey">Clé API</option><option value="bearer">Token OAuth2 temporaire</option></select></div></div><div class="actions"><button class="btn secondary small" id="repro-weather-stations" type="button">1. Rechercher les stations</button><select id="repro-weather-station"><option value="${escapeHtml(w.stationId||'')}">${w.stationId?escapeHtml((w.stationName||w.stationId)+' — station enregistrée'):'Choisir une station…'}</option></select><button class="btn primary small" id="repro-weather-import" type="button">2. Récupérer ${periodStart&&periodEnd?formatDate(periodStart)+' → '+formatDate(periodEnd):'la période'}</button>${rec.length?'<button class="btn danger small" id="repro-weather-clear" type="button">Effacer météo</button>':''}</div><div id="repro-weather-status" class="muted" style="margin-top:8px">La clé est mémorisée uniquement sur cet appareil dans ce navigateur ; elle n’est ni enregistrée dans la visite ni synchronisée dans le cloud.</div><div class="field" style="margin-top:12px"><label>Ou importer un CSV quotidien Météo-France</label><input id="repro-weather-file" type="file" accept=".csv,text/csv"><small class="muted">Solution de secours si l’API n’est pas accessible depuis le navigateur.</small></div></details></section>`;
}
function reproWeatherApiUrl(path,params){const u=new URL('https://public-api.meteofrance.fr/public/DPClim/v1/'+path);Object.entries(params||{}).forEach(([k,v])=>{if(v!==''&&v!=null)u.searchParams.set(k,v)});return u;}
async function reproWeatherFetch(path,params,authMode,credential){
  const ok=r=>r.ok||[201,202,204].includes(r.status);
  if(authMode!=='apikey'){const r=await fetch(reproWeatherApiUrl(path,params),{headers:{accept:'*/*',Authorization:`Bearer ${credential}`}});if(!ok(r)){if(r.status===401)throw new Error('Météo-France : token OAuth2 refusé ou expiré (401). Régénérez un token temporaire.');throw new Error(`Météo-France : erreur ${r.status}`)}return r;}
  // Le portail Météo-France a fait coexister plusieurs modes d'appel pour les API Keys.
  // On essaye d'abord le paramètre apikey (sans préflight CORS), puis les deux formes d'en-tête.
  const attempts=[
    ()=>fetch(reproWeatherApiUrl(path,{...(params||{}),apikey:credential}),{headers:{accept:'*/*'}}),
    ()=>fetch(reproWeatherApiUrl(path,params),{headers:{accept:'*/*',Authorization:`Bearer ${credential}`}}),
    ()=>fetch(reproWeatherApiUrl(path,params),{headers:{accept:'*/*',apikey:credential}})
  ];
  let last=null;for(const fn of attempts){try{const r=await fn();last=r;if(ok(r))return r;if(r.status!==401&&r.status!==403)break;}catch(e){last=e;}}
  if(last instanceof Response){if(last.status===401||last.status===403)throw new Error(`Météo-France : clé API refusée (${last.status}) après les 3 modes d’authentification. Testez un token OAuth2 temporaire pour confirmer l’accès.`);throw new Error(`Météo-France : erreur ${last.status}`)}
  throw new Error(`Météo-France inaccessible depuis ce navigateur${last?.message?' : '+last.message:''}`);
}
function reproWeatherChunks(start,end){const out=[];let d=new Date(start+'T00:00:00Z'),last=new Date(end+'T00:00:00Z');while(d<=last){const a=new Date(d),b=new Date(d);b.setUTCFullYear(b.getUTCFullYear()+1);b.setUTCDate(b.getUTCDate()-1);if(b>last)b=last;out.push([a.toISOString().slice(0,10),b.toISOString().slice(0,10)]);d=new Date(b);d.setUTCDate(d.getUTCDate()+1);}return out;}
async function reproWeatherDownloadDaily(stationId,start,end,authMode,credential,onProgress){const chunks=reproWeatherChunks(start,end),all=[];for(let i=0;i<chunks.length;i++){const [a,b]=chunks[i];onProgress?.(`Commande Météo-France ${i+1}/${chunks.length} : ${formatDate(a)} → ${formatDate(b)}…`);const order=await reproWeatherFetch('commande-station/quotidienne',{'id-station':stationId,'date-deb-periode':a+'T00:00:00Z','date-fin-periode':b+'T23:59:59Z'},authMode,credential);const j=await order.json();const id=j?.elaboreProduitAvecDemandeResponse?.return||j?.return;if(!id)throw new Error('Numéro de commande Météo-France non reçu.');let text='';for(let k=0;k<10;k++){await new Promise(r=>setTimeout(r,k?1600:1200));const f=await reproWeatherFetch('commande/fichier',{'id-cmde':id},authMode,credential);if(f.status===204)continue;text=await f.text();if(text.trim())break;}if(!text)throw new Error('Le fichier Météo-France n’est pas encore disponible. Réessayez dans quelques secondes.');all.push(...parseMeteoFranceDailyCsv(text));}
  const byDate=new Map(all.map(r=>[r.date,r]));return [...byDate.values()].sort((a,b)=>a.date.localeCompare(b.date));}
function reproChronology(registry,cows,periodStart,periodEnd,inv){
  const buckets=new Map();const get=k=>{if(!buckets.has(k))buckets.set(k,{key:k,ivvs:[],calves:0,deaths:0,introductions:[],events:[]});return buckets.get(k)};
  for(const r of cows){for(let i=0;i<r.intervals.length;i++){const ivv=r.intervals[i],nextCalving=r.calvingDates[i+1];const conception=dateShiftDays(nextCalving,-283),k=quarterKey(conception);if(k)get(k).ivvs.push({ivv,cow:r.cow,conception,nextCalving});}}
  for(const c of cows.flatMap(r=>r.calves||[])){if(!c.birthDate)continue;const k=quarterKey(c.birthDate);if(!k)continue;const b=get(k);b.calves++;if(c.exitCause==='M'&&c.exitDate&&daysBetweenDates(c.birthDate,c.exitDate)<183)b.deaths++;}
  for(const a of registry){if(!a.entryDate)continue;const k=quarterKey(a.entryDate);if(k)get(k).introductions.push(a);}
  for(const e of inv?.events||[]){if(!e.date)continue;const k=quarterKey(e.date);if(k)get(k).events.push(e);}
  let arr=[...buckets.values()].map(b=>{const vals=b.ivvs.map(x=>x.ivv),avg=vals.length?Math.round(vals.reduce((a,c)=>a+c,0)/vals.length):null,bad=vals.filter(v=>v>450).length,badRate=vals.length?Math.round(bad/vals.length*100):0,mortRate=b.calves?Math.round(b.deaths/b.calves*1000)/10:null;let severity=0,reasons=[];if(vals.length>=3&&avg>450){severity+=3;reasons.push(`IVV moyen ${avg} j`)}else if(vals.length>=3&&avg>420){severity+=2;reasons.push(`IVV moyen ${avg} j`)}if(vals.length>=3&&badRate>=30){severity+=2;reasons.push(`${badRate}% des IVV > 450 j`)}if(b.calves>=5&&mortRate>=10){severity+=3;reasons.push(`mortalité veaux ${mortRate}%`)}else if(b.calves>=5&&mortRate>=6){severity+=1;reasons.push(`mortalité veaux ${mortRate}%`)}return {...b,avg,badRate,mortRate,severity,reasons};});
  const weatherMap=reproWeatherQuarterSummary(inv?.weather?.records||[]);arr.forEach(b=>{b.weather=weatherMap.get(b.key)||null;if(b.severity>=2&&b.weather?.tags?.length)b.reasons.push(`coïncidence météo : ${b.weather.tags.join(', ')}`);});
  arr=arr.filter(b=>{const r=quarterDateRange(b.key);return (!periodStart||r.to>=periodStart)&&(!periodEnd||r.from<=periodEnd)}).sort((a,b)=>quarterSortValue(a.key)-quarterSortValue(b.key));
  const problems=arr.filter(b=>b.severity>=2).sort((a,b)=>b.severity-a.severity||quarterSortValue(b.key)-quarterSortValue(a.key));
  return {buckets:arr,problems};
}
function reproChronologyHtml(chrono){
  ...
  const problemHtml=chrono.problems.length?`<div class="repro-problem-periods">${chrono.problems.slice(0,8).map(b=>`<article class="notice ${b.severity>=4?'warning':''}"><strong>${b.severity>=4?'🔴':'🟠'} ${b.key} — période à investiguer</strong><div>${b.reasons.map(escapeHtml).join(' · ')}</div>${b.introductions.length?`<small>📥 ${b.introductions.length} introduction(s) dans la même période.</small>`:''}${b.events.length?`<small>📌 ${b.events.map(e=>escapeHtml(e.label||e.type||'Événement')).join(' · ')}</small>`:''}</article>`).join('')}</div>`:'<div class="notice"><strong>✓ Aucune rupture nette détectée sur les périodes suffisamment renseignées.</strong></div>';
  const timeline=`<div class="table-wrap"><table class="compact-table"><thead><tr><th>Période</th><th>IVV liés aux conceptions estimées</th><th>IVV &gt;450 j</th><th>Naissances</th><th>Morts &lt;6m</th><th>Introductions</th><th>Météo-France</th><th>Événements</th></tr></thead><tbody>${chrono.buckets.map(b=>`<tr class="${b.severity>=4?'repro-period-danger':b.severity>=2?'repro-period-warning':''}"><td><strong>${b.key}</strong></td><td>${b.avg??'—'} j <small>(${b.ivvs.length})</small></td><td>${b.ivvs.length?b.badRate+'%':'—'}</td><td>${b.calves}</td><td>${b.mortRate==null?'—':b.mortRate+'%'}</td><td>${b.introductions.length}</td><td>${b.weather?`${b.weather.rain==null?'—':b.weather.rain+' mm'}<br><small>${b.weather.temp==null?'—':b.weather.temp+' °C'}${b.weather.tags?.length?' · '+escapeHtml(b.weather.tags.join(', ')):''}</small>`:'—'}</td><td>${b.events.length?b.events.map(e=>escapeHtml(e.label||e.type||'Événement')).join('<br>'):'—'}</td></tr>`).join('')}</tbody></table></div>`;
  return problemHtml+timeline;
}
...
function renderReproduction(){
  ...
  const ageKey=`audit-bovin-repro-age-${visit?.id||selectedFarmId||'default'}`;let ageSaved={};try{ageSaved=JSON.parse(localStorage.getItem(ageKey)||'{}')||{}}catch(_){ageSaved={};}let ageMin=Math.max(18,Number(ageSaved.min??18)),ageMax=Math.min(180,Number(ageSaved.max??180));if(ageMin>ageMax)[ageMin,ageMax]=[ageMax,ageMin];
  ...
  if(visit){const saveInv=()=>{visit.updatedAt=new Date().toISOString();saveDatabase(db)};app.querySelectorAll('[data-repro-heat]').forEach(el=>{const f=el.dataset.reproHeat;const fn=()=>{inv.heat[f]=el.value;saveInv()};el.onchange=()=>{fn();renderReproduction()};el.oninput=fn});app.querySelectorAll('[data-repro-setting]').forEach(el=>{const f=el.dataset.reproSetting;const fn=()=>{inv.settings[f]=el.type==='number'?Number(el.value||0):el.value;saveInv()};el.onchange=()=>{fn();renderReproduction()};el.oninput=fn});app.querySelectorAll('[data-bull-field]').forEach(el=>{const row=el.closest('[data-repro-bull-row]'),i=Number(row?.dataset.reproBullRow),f=el.dataset.bullField;const fn=()=>{if(!inv.bulls[i])return;inv.bulls[i][f]=el.type==='number'?Number(el.value||0):el.value;saveInv()};el.oninput=fn;el.onchange=()=>{fn();renderReproduction()}});document.getElementById('add-repro-bull')?.addEventListener('click',()=>{inv.bulls.push({id:uid('bull'),name:'',ageClass:'adult',females:'',contactDays:'',fertility:''});saveInv();renderReproduction()});app.querySelectorAll('[data-delete-bull]').forEach(b=>b.onclick=()=>{inv.bulls.splice(Number(b.dataset.deleteBull),1);saveInv();renderReproduction()});document.getElementById('add-repro-event')?.addEventListener('click',()=>{const date=document.getElementById('repro-event-date')?.value,label=document.getElementById('repro-event-label')?.value.trim(),type=document.getElementById('repro-event-type')?.value;if(!date||!label){showToast('Renseignez la date et l’événement.');return;}inv.events.push({id:uid('reproevt'),date,type,label});saveInv();renderReproduction()});app.querySelectorAll('[data-delete-repro-event]').forEach(b=>b.onclick=()=>{inv.events=inv.events.filter(e=>e.id!==b.dataset.deleteReproEvent);saveInv();renderReproduction()});
    ...
    document.getElementById('repro-weather-stations')?.addEventListener('click',async()=>{const dept=document.getElementById('repro-weather-dept')?.value.trim(),cred=document.getElementById('repro-weather-token')?.value.trim(),mode=document.getElementById('repro-weather-auth')?.value||'apikey';if(!dept||!cred){weatherStatus('Renseignez le département et la clé/token Météo-France.');return;}reproWeatherStoreCredential(cred);try{weatherStatus('Recherche des stations Météo-France…');const r=await reproWeatherFetch('liste-stations/quotidienne',{'id-departement':dept},mode,cred),stations=await r.json(),sel=document.getElementById('repro-weather-station');if(!Array.isArray(stations)||!stations.length)throw new Error('Aucune station quotidienne trouvée.');sel.innerHTML=stations.filter(x=>x.posteOuvert!==false||x.postePublic!==false).map(x=>`<option value="${escapeHtml(String(x.id))}" data-name="${escapeHtml(x.nom||'')}">${escapeHtml(x.nom||String(x.id))}${x.posteOuvert===false?' (fermée)':''}</option>`).join('');if(inv.weather.stationId&&[...sel.options].some(o=>o.value===String(inv.weather.stationId)))sel.value=String(inv.weather.stationId);weatherStatus(`${stations.length} station(s) trouvée(s). Choisissez la station la plus représentative de l’exploitation.`);}catch(e){weatherStatus(`Impossible de charger les stations : ${e.message}`)}});
    document.getElementById('repro-weather-import')?.addEventListener('click',async()=>{const sel=document.getElementById('repro-weather-station'),stationId=sel?.value,stationName=sel?.selectedOptions?.[0]?.dataset?.name||sel?.selectedOptions?.[0]?.textContent||stationId,dept=document.getElementById('repro-weather-dept')?.value.trim(),cred=document.getElementById('repro-weather-token')?.value.trim(),mode=document.getElementById('repro-weather-auth')?.value||'apikey';if(!stationId||!cred||!periodStart||!periodEnd){weatherStatus('Choisissez une station, renseignez la clé/token et une période d’analyse.');return;}reproWeatherStoreCredential(cred);try{const today=new Date().toISOString().slice(0,10),end=periodEnd>today?today:periodEnd;const records=await reproWeatherDownloadDaily(stationId,periodStart,end,mode,cred,weatherStatus);if(!records.length)throw new Error('Aucune valeur quotidienne exploitable dans le fichier reçu.');inv.weather={...inv.weather,department:dept,stationId:String(stationId),stationName:String(stationName).replace(/\s+—.*$/,''),records,source:'Météo-France DPClim quotidienne',importedAt:new Date().toISOString(),from:records[0].date,to:records.at(-1).date};saveInv();showToast(`${records.length} jour(s) Météo-France enregistrés.`);renderReproduction();}catch(e){weatherStatus(`Import Météo-France impossible : ${e.message}`)}});
    document.getElementById('repro-weather-file')?.addEventListener('change',async e=>{const file=e.target.files?.[0];if(!file)return;try{const records=parseMeteoFranceDailyCsv(await file.text());if(!records.length)throw new Error('Aucune donnée pluie/température reconnue.');inv.weather={...inv.weather,department:document.getElementById('repro-weather-dept')?.value.trim()||inv.weather.department||'',records,source:'CSV quotidien Météo-France',fileName:file.name,importedAt:new Date().toISOString(),from:records[0].date,to:records.at(-1).date};saveInv();showToast(`${records.length} jour(s) météo importés depuis ${file.name}.`);renderReproduction();}catch(err){weatherStatus(`CSV météo non importé : ${err.message}`)}});
...
// v14.6.21.54 — check-up, suivi d'étude, économie et conservation globale du défilement horizontal
...
// v14.6.21.54 — suivi du temps par intervenant + temps GDS préparation/terrain/compte rendu/suivi

// v14.6.21.54 — questionnaires persistants, repères de saisie, bilan sanitaire/éco partenaire, supports papier, check-up responsive.

window.addEventListener('DOMContentLoaded',()=>{initGlobalSearch();initAccordionMemory();});
// v14.6.21.54 — cohérence, imprimables préparation, animaux présents, recherche globale.
